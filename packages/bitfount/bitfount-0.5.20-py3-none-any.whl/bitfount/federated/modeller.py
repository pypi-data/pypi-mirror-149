...
class _Modeller:
    ...
    async def _send_task_requests(
        ...
        # The bare minimum task request has as just a protocol and an algorithm.
        partial_protocol_details = partial(
            _ProtocolDetails,
            protocol=ProtocolType[self.protocol.class_name].value,
            algorithm=AlgorithmType[self.protocol.algorithm.class_name].value,
        )
        if isinstance(self.protocol.algorithm, _BaseModelAlgorithmFactory):
            ...
            if isinstance(self.protocol.algorithm.model, BitfountModelReference):
                ...
                if isinstance(model.model_ref, Path):
                    model_name = f"{model.username}.{model.model_ref.stem}"
                else:
                    model_name = f"{model.username}.{model.model_ref}"
            else:
                # If the model is a built-in model, we send the model name prefixed
                # with `bitfount.`
                model_name = f"bitfount.{self.protocol.algorithm.model.class_name}"
            ...
            if hasattr(self.protocol, "aggregator"):
                protocol_details = partial_protocol_details(
                    model=model_name,
                    aggregator=AggregatorType[self.protocol.aggregator.class_name].value,  # type: ignore[attr-defined] # Reason: hasattr check # noqa: B950
                )
