...
class _BaseModelReference:
    @classmethod
    def __init_subclass__(cls, **kwargs: Any):
        if not inspect.isabstract(cls) and cls.__name__ not in MAIN_MODEL_REGISTRY:
            logger.debug(f"Adding {cls.__name__}: {cls} to registry")
            MAIN_MODEL_REGISTRY[cls.__name__] = cls


class BitfountModelReference(_BaseModelReference, _BaseSerializableObjectMixIn):
