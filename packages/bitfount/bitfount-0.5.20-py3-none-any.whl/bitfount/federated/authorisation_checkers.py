...
class _AuthorisationChecker(Protocol[_M_co]):
    ...
    def verify_protocol(self, protocol: _BaseProtocolFactory) -> bool:
        ...
        try:
            if (
                ProtocolType[protocol.class_name].value
                != self.task_protocol_details.protocol
            ):
                logger.error(
                    f"Protocol {protocol.class_name} does not match "
                    f"{self.task_protocol_details.protocol}"
                )
                return False

            if (
                AlgorithmType[protocol.algorithm.class_name].value
                != self.task_protocol_details.algorithm
            ):
                logger.error(
                    f"Algorithm {protocol.algorithm.class_name} does not match "
            ...
            if (
                ...
                and AggregatorType[protocol.aggregator.class_name].value != self.task_protocol_details.aggregator  # type: ignore[attr-defined] # Reason: hasattr check # noqa: B950
            ):
                logger.error(
                    f"Aggregator {protocol.aggregator.class_name} does not match "  # type: ignore[attr-defined] # Reason: hasattr check # noqa: B950
            ...
            if isinstance(protocol.algorithm, _BaseModelAlgorithmFactory):
                ...
                if isinstance(protocol.algorithm.model, BitfountModelReference):
                    ...
                    # in the task request.
                    if (
                        f"bitfount.{protocol.algorithm.model.class_name}"
                        != self.task_protocol_details.model
                    ):
                        logger.error(
                            f"Model {protocol.algorithm.model.class_name} does not "
                            f"match {self.task_protocol_details.model}"
                        )
                        return False
