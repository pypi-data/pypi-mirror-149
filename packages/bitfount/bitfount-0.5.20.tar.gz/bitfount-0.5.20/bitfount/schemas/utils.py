...
def bf_dump(bf_obj: Any) -> Dict[str, Any]:
    ...
    if schema_cls is None:
        ...
        # Get the schema dump
        schema_dump = cast(Dict[str, Any], myschema.dump(bf_obj))
...
def _bf_combine_dict_dump(
    cls: Any, fields_dict: Dict[str, Any], nested_fields: Dict[str, Any]
...
def _get_marshmallow_schema(
    obj_class: Any,
    unnested_fields: Dict[str, Any],
    nested_schemas: Dict[str, Any],
    ...
    @post_load
    def recreate_factory(self: Any, data: Dict[str, Any], **_kwargs: Any) -> Any:
        data = data.copy()
        data.pop("class_name")
...
def _combine_dict_helper(
    cls: Any, item: Any, fields_dict: Dict[str, Any], nested_fields: Dict[str, Any]
) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    if hasattr(item, "fields_dict"):
        for k, v in item.fields_dict.items():
            if k not in fields_dict.keys():
                fields_dict[k] = v
        for k, v in item.nested_fields.items():
            if k not in nested_fields.keys():
                nested_fields[k] = v
...
def _dict_to_schema(
    dct: Dict[str, Any], registry: Mapping[str, type], depth: int = 0
) -> Any:
    """Converts a dictionary to a marshmallow schema."""
    if depth > 5:
        raise SchemaClassError("Cannot have more than depth 5 in serialisation")
    if dct:
        # Get the class by looking in the given registry for the class_name
        cls: Any = registry[dct["class_name"]]
...
def _bf_combine_dict_load(
    ...
    if issubclass(cls, CNNModelStructure):
        if "layers" not in cls.fields_dict.keys():
