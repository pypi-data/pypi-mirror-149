...
class TestPercentageSplitter:
    ...
    def test_get_split_query(self, split: Optional[DataSplit]) -> None:
        ...
        data_splitter = PercentageSplitter()
        mock_loader = MagicMock(spec=DatabaseLoader, query="SELECT * FROM table")
        mock_loader.__len__.return_value = 1000

        if split is None:
            with pytest.raises(ValueError, match="Split not recognised"):
                data_splitter.get_split_query(mock_loader, split)  # type: ignore[arg-type] # Reason: purpose of test # noqa: B950
        else:
            query = data_splitter.get_split_query(mock_loader, split)
    ...
    def test_get_split_query_time_series(self, caplog: LogCaptureFixture) -> None:
        ...
        mock_loader = MagicMock(spec=DatabaseLoader, query="SELECT * FROM table")
        mock_loader.__len__.return_value = 1000
        data_splitter.get_split_query(mock_loader, DataSplit.TRAIN)
