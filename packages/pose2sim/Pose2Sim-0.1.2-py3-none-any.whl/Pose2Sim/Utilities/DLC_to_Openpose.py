...
def DLC_to_OpenPose_func(*args):
    ...
    try:
        h5_file_path = args[0]['input'] # invoked with argparse
        if args[0]['output'] == None:
            json_folder_path = os.path.splitext(h5_file_path)[0]
        else:
            json_folder_path = args[0]['output']
    except:
        h5_file_path = args[0] # invoked as a function
        try:
            json_folder_path = args[1]
        except:
            json_folder_path = os.path.splitext(h5_file_path)[0]
  ...
  # json preparation
    ...
    kpt_nb = int(len(h5_file.columns)/2)
    
    # write each h5 line in json file
    for f, frame in enumerate(h5_file.index):
        h5_line = np.array([[h5_file.iloc[f, 2*k], h5_file.iloc[f, 2*k+1], 0.9] for k in range(kpt_nb)]).flatten().tolist()
        json_dict['people'][0]['pose_keypoints_2d'] = h5_line
        json_file = os.path.join(json_folder_path, os.path.splitext(os.path.basename(frame))[0]+'.json')
