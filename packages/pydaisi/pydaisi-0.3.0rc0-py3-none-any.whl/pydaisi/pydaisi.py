...
class DaisiBulkExecution:
    ...
    def _bulk_fetch_result(self):
        ...
        r = self.daisi.session.post(
            f"{self.daisi.base_url}/{self.daisi.id}/results_bulk",
            headers=self.daisi.headers,
            json={"executions": my_exec}
        )

        my_outputs = [{"id": x["id"], "outputs": x["outputs"]} for x in r.json()]
        actual_outputs = [x for x in my_outputs if x["outputs"]]

        if len(actual_outputs) < len(self.executions):
            logger.warning("Your Bulk Daisi Execution is still in Progress! These are PARTIAL RESULTS.")
        else:
            self._result = my_outputs
...
class DaisiExecution:
    ...
    @property
    def value_id(self):
        return trio.run(self.fetch_result, unpickle_binary=False)
    ...
    async def fetch_result(
        self, timeout: int = 180, unpickle_binary=True
...
class Daisi:
    ...
    def _bulk_run(
        ...
        # Call the specified Daisi compute
        r = self.session.post(
            f"{self.base_url}/{self.id}/executions_bulk",
            headers=self.headers,
            json={"executions": my_exec}
        )

        return daisi_bulk_execution
