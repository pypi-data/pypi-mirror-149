# -*- coding: utf-8 -*-
"""
The behave plugin configures command-line options for SeleniumBase tests.
*************************************************************************
Examples:
...
def get_configured_sb(context):
    ...
    for key in userdata.keys():
        ...
        if low_key == "var3":
            ...
            continue
        # Handle: -D variables="{'KEY':'VAL','KEY2':'VAL2'}"
        if low_key == "variables":
            ...
            if not variables.startswith("{") or not variables.endswith("}"):
                raise Exception(
                    ...
                    "\nEg. -D variables=\"{'KEY':'VAL','KEY2':'VAL2'}\""
                )
            sb.variables = ast.literal_eval(variables)
            continue
