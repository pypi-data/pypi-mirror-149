...
class AhrefsClient:
    ...
    def export_keyword_list(
        ...
        Args:
            ...
            trunc_dict: If set to a non None dict object, this dictionary maps a set of columns
                to the character lengths each column should be truncated into.
    ...
    def get_serp_data(self, country_abbr: str, list_id: int, volume_list: List[int],
                      keyword_no: int, batch_size: int = 1000):
        ...
        df = pd.DataFrame()
        for i in range(0, keyword_no, 1000):
            # Get indexes of keyword batch for which we will export data
            batch_start = i
            batch_end = min(keyword_no - 1, i + 1000)
