...
class Plugin:
    ...
    def pytest_addoption(parser):
        parser.addoption(
            ...
            action="store_const",
            const=True,
            help="Create accele allure HTML reports."
        )

    @staticmethod
    def _accele_reports(config):
        if config.getoption("--one-reports") and not config.getoption("allure_report_dir"):
            return True
        else:
            return False
    ...
    def pytest_configure(config):
        if Plugin._accele_reports(config):
            print("+" * 20)
    ...
    def pytest_sessionfinish(session):
        if Plugin._accele_reports(session.config):
