...
class EARSLinearTS(AbstractRanker):
    ...
    def get_ranking(self, batch_users, round=None):
        ...
        if self.shuffle_K == -1:
            # For every user
            gamma = 0.9
            E_c = Parallel(n_jobs=-1)(delayed(powerset_expectation_negation_partial)(P_clicks[u], gamma, 1) for u in range(n_users))
            # print(f'Expected clicks without shuffling top-K:', np.mean(E_c), np.var(E_c))
            K = 6
            E_c = Parallel(n_jobs=-1)(delayed(powerset_expectation_negation_partial)(P_clicks[u], gamma, K) for u in range(n_users))
            # print(f'Expected clicks after shuffling Top-{K}:', np.mean(E_c), np.var(E_c))

            K_s = Parallel(n_jobs=-1)(delayed(compute_user_K_prime)(P_clicks[u], gamma, epsilon=self.epsilon) for u in range(n_users))
    ...
    def update(self, batch_users, rankings, clicks, round=None):#, user_ids, recos , rewards, l_init=3):
        # rewards = 2 * rewards - 1
