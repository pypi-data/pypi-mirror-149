...
class Api:
    ...
    def __init__(self):
        try:
            ...
            self.aws_access_key = os.getenv('AWS_ACCESS_KEY_ID') or self.aws_config.get('default').get(
                'aws_access_key_id')
            self.aws_secret_key = os.getenv('AWS_SECRET_ACCESS_KEY') or self.aws_config.get('default').get(
                'aws_secret_access_key')
            self.aws_secretsmanager_secret_name = os.getenv('AWS_SM_EXACT_ONLINE_SECRET_NAME')
            self.aws_secretsmanager_region = os.getenv('AWS_DEFAULT_REGION') or self.aws_config.get('secretsmanager').\
                get('region')
