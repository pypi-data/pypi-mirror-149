...
graph_operation = {
    "run_model": {"forward": [model.run_model], "backward": None, "require": [wml.check_model_stored]},
    "get_metadata": {"forward": [model.run_model], "backward": None, "require": [return_false]},
    "export_facts": {"forward": [wkc.export_facts], "backward": None, "require": [wml.check_model_stored]},
    "store_model": {"forward": [ws.store_model], "backward": [ws.delete_model_from_project_by_model_names], "require": [wml.check_model_stored]},
    "register_model": {"forward": [wkc.register_model_from_project], "backward": [wkc.delete_model_from_project_inventory_by_model_names], "require": [wml.check_model_stored]},
    "promote_model": {"forward": [wml.promote_model], "backward": [wkc.delete_model_from_inventory_by_model_names, wml.delete_model_by_model_names], "require": [wml.check_model_promoted]},
    "deploy_model": {"forward": [wml.deploy_model], "backward": [wml.delete_model_deployment_by_model_deployment_names], "require": [wml.check_model_deployed]},
    "create_metric_provider": {"forward": [wos.create_custom_metric_provider], "backward": None, "require": None},
    "create_integrated_system": {"forward": [wos.create_integrated_system], "backward": None, "require": None},
    "create_metric_monitor": {"forward": [wos.create_custom_metric_monitor], "backward": None, "require": None},
    "verify_ml_provider": {"forward": None, "backward": None, "require": [return_false]},
    "subscribe_model": {"forward": [wos.subscribe_model], "backward": [wos.delete_subscription_by_subscription_names], "require": None},
    "store_payload": {"forward": [wos.store_payload], "backward": None, "require": None},
    "score_model": {"forward": [wml.score_model], "backward": None, "require": None},
    "create_monitor": {"forward": [wos.create_monitor], "backward": None, "require": None},
    "store_feedback": {"forward": [wos.store_feedback], "backward": None, "require": None},
    "evaluate": {"forward": [wos.evaluate], "backward": None, "require": None},
}
...
def get_execution_plan(source: str, target: str, include: list[str] = None) -> list[str]:
    """
    Get execution plan.
...
def get_forward_steps(source: str, target: str, include: list[str] = None) -> list[str]:
    """
    Get forward steps.
...
def get_backward_steps(source: str, target: str, include: list[str] = None) -> list[str]:
    """
    Get backward steps.
...
def get_check_require_steps(source: str, target: str, include: list[str] = None) -> list[str]:
    """
...
def run(direction: str, steps: list[str], args: dict) -> None:
    """
    Run steps.
...
def remove(config: dict, model_names: list[str], space_types: list[str], backward_steps: list[str], log_format: str):
    """
    Remove steps.
