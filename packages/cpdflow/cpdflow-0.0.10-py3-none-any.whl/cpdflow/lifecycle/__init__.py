...
class apply:
    ...
    @staticmethod
    def deploy(config: dict, model_names: list[str], space_type: str) -> None:
        deploy.apply(config=config, model_names=model_names, space_type=space_type)

    @staticmethod
    def validate(config: dict, model_names: list[str]) -> None:
        validate.apply(config=config, model_names=model_names)

    @staticmethod
    def operate(config: dict, model_names: list[str]) -> None:
