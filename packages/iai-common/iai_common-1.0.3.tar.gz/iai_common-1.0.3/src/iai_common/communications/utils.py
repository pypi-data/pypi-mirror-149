...
CARLA_WEATHERS = ['ClearNoon', 'SoftRainNoon', 'HardRainNoon','ClearSunset', 'WetCloudySunset', 'SoftRainSunset']
...
def build_initialize(builder, scenario, scenario_parameters=None,
    ...
    if world_parameters is not None:
        ...
        if 'weather' in world_parameters:
            weather = builder.CreateString(world_parameters['weather'])
        isp_WorldParameters.WorldParametersStart(builder)
        if 'carlatown' in world_parameters:
            isp_WorldParameters.WorldParametersAddCarlatown(builder, carlatown)
        if 'weather' in world_parameters:
            isp_WorldParameters.WorldParametersAddWeather(builder, weather)
...
def unpack_initialize(message_body):
    ...
    if message_body.WorldParameters() is not None:
        ...
        if message_body.WorldParameters().Weather() is not None:
            world_parameters['weather'] = message_body.WorldParameters().Weather().decode("utf-8")
