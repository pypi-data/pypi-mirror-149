    ...
    def process_answers(self, questions: List[Question]) -> dict:
        ...
        for question in questions:
            if isinstance(question, MultipleChoiceQuestion):
                configuration[question.raw_prompt] = [choice.internal_value for choice in question.answer]
            elif isinstance(question, MultipleAnswerQuestion):
                configuration[question.raw_prompt] = [answer.value for answer in question.answer]
            elif isinstance(question, YesNoQuestion):
                configuration[question.raw_prompt] = True if question.answer.enum == YesNo.YES else False
            else:
                configuration[question.raw_prompt] = question.answer.value
