...
def _build_dataset_locally_and_store_remotely(
    ...
    def dataset_upload_progress(cur_step: int, total_steps: int) -> None:
        tracker.update_derived_dataset_saving_progress(
            derived_dataset.name, cur_step=cur_step, total_steps=total_steps
        )
    ...
    client.data_catalog.store_dataset(
        ...
        data=result,
        build_id=build_uuid,
        progress_callback=dataset_upload_progress,
    )
