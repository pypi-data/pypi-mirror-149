...
from typing import Dict, Iterable, MutableMapping, NewType, Optional, Set, Tuple, Union
...
def extract_configuration(values: tuple) -> Tuple[Union[Url, Path], Optional[Union[Url, Path]], Optional[Union[Url, Path]]]:
    if len(values) == 3:
...
def process_configuration(app, tag_filename: Union[Url, Path], rootdir: Optional[Union[Url, Path]], pdf_filename: Optional[Union[Url, Path]]):
