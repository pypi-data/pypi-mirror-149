...
class HUBLogger:
    ...
    def _upload_model(self, epoch, is_best, weights):
        ...
        with open(weights, "rb") as last:
            ...
            if r.status_code != 200:
                LOGGER.warning(f"{PREFIX}Unable to upload checkpoint!")
    ...
    def on_train_end(self, *args, **kwargs):
        ...
        LOGGER.info(f"{PREFIX}Uploading model {self.model_id}")
        with open(best, "rb") as best:
            r = requests.post(self.api_url, data=payload, files={"best.pt": best})
            if r.status_code != 200:
                LOGGER.warning(f"{PREFIX}Unable to upload final model!")
