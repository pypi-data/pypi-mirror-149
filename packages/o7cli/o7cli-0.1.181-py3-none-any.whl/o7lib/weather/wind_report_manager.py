...
DB_CLIENTS = [
    { 'email' : 'philippe.gosselin@hec.ca', 'locations' : 'BDB01,BSM01,NEU01,STA01'}
]
...
class WindReportManager(): # pylint: disable=too-many-instance-attributes
        ...
        for client in DB_CLIENTS :
            ...
            for location in locations.split(','):
                if location in self.locations:
                    self.clients[email].append(location)

            logger.info(f'LoadClients: {email} with {len(self.clients[email])} locations')
    ...
    def ProcessClients(self) -> str:
        ...
        for email, locations in self.clients.items():

            objs = [ self.locations[location] for location in locations ]
            reportObj = o7lib.weather.wind_report_v2.WindReport(objs)
