...
class pure_cloud(object):
    ...
    '''
    def __init__(self,fac,freq,group=10,boxcox=1,print_comments=1,plt_plot=1,plotly_plot=1,
                 filename=f'{self.freq}天{self.group}组多空净值图',
                 comments_writer=None,nets_writer=None,sheet_name=None):
    ...
    def run(self,print_comments,plt_plot,plotly_plot,filename):
        '''对因子值分组并匹配'''
        if self.boxcox:
            self.fac=decap_industry(fac)
        self.fac=self.fac.T.apply(lambda x:pd.qcut(x,self.group,labels=False)).T
        ...
        self.group_nets=self.group_nets/self.group_nets.head(1)
        ...
        self.long_short_comments=comments_on_twins(self.long_short_net,self.long_short_ret)
        if print_comments:
