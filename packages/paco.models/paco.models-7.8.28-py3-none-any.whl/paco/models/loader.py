...
Caveat: You can not have an environment named 'applications'.
    ...
    def process_generated_service_config(self, service_name, service_config, key, netenv_ref, paco_ref_list):
        ...
        self.generated_service_config[service_name][paco_ref_path] = service_config['config']

    def process_service_config_callback(self, config_obj, key, parent_obj, netenv_ref, paco_ref_list):
        service_sub_str = 'paco.ref service.'
        if isinstance(config_obj[key], str):
            if config_obj[key].startswith(service_sub_str):
                service_name = config_obj[key].split('.')[2]
                self.process_generated_service_config(service_name, config_obj, key, netenv_ref, paco_ref_list)
        elif config_obj[key] == None or isinstance(config_obj[key], int):
            pass
        else:
            pass
    ...
    def process_service_config(self, netenv_name, env_name, env_region_name, config):
        netenv_ref = f'netenv.{netenv_name}.{env_name}.{env_region_name}'
        return iterate_nested_dict(config, None, self.process_service_config_callback, callback_data=netenv_ref)
        ...
        if 'environments' in config:
            for env_name, env_config in config['environments'].items():
                ...
                for env_reg_name, env_reg_config in config['environments'][env_name].items():
                    ...
                    # Applications
                    global_config = self.instantiate_applications(
                        env_region_config,
                        env_region,
                        config
                    )
                    if global_config != None:
                        if env_reg_name != 'default':
                            self.process_service_config(name, env_region.__parent__.name, env_reg_name, global_config)
                    # Secrets Managers
