...
class MensajeRecibir(Mensaje):
    ...
    def __init__(símismo, conex: socket.socket, variable: str, prec: int = None):
        símismo.variable = variable
        símismo.prec = prec
        super().__init__(conex)

    def _encabezado(símismo):
        return {'var': símismo.variable, 'prec': símismo.prec if símismo.prec else 1, **super()._encabezado()}
