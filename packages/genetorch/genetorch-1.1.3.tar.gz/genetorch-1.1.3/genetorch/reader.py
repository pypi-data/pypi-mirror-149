...
def get_impact(a):
    for i in range(len(a.taglist)):
        ...
        inds = search(n, 'type', 'splice_donor_variant&intron_variant')
        indd = search(n, 'type', 'splice_acceptor_variant&intron_variant')
        m = n.loc[ind, :]
        b = n.loc[indp, :]
        c = n.loc[inds, :]
        d = n.loc[indd, :]
        n = pd.concat([m, n]).drop_duplicates(keep=False)
        a.taglist[i] = pd.concat([n, b, c, d]).drop_duplicates(keep=False)
    return a.taglist
