...
class AliasesApi(object):
    ...
    def create_aliases(
        ...
        **kwargs
        ...
        Stores multiple values at once & returns their aliases.  The number of values sent should not exceed 20.  The JSON object for each value should not exceed 32MB. This limit is per value sent, which includes its classifiers, format, and storage.  Alternatively, this endpoint may be used to associate additional (i.e. secondary) aliases with the same underlying data as the reference alias specified in the request body.  **NOTE:** You cannot reference the same alias more than once in a single request.   # noqa: E501
