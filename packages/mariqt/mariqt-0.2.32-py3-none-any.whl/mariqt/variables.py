...
ifdo_item_core_fields = {
	...
	'image-latitude':{'comment': 'Decimal degrees: D.DDDDDDD (use at least seven significant digits that is ca. 1cm resolution)', 'dataType': dataTypes.float},
	'image-longitude':{'comment': 'Decimal degrees: D.DDDDDDD (use at least seven significant digits that is ca. 1cm resolution)', 'dataType': dataTypes.float},
	'image-depth':{'comment': 'Use when camera below water, then it has positive values. Given in meters', 'dataType': dataTypes.float, 'alt-fields': ['image-altitude']},
	'image-altitude':{'comment': 'Optional: Use when camera above water, then it has positive values. Given in meters. You may also use image-depth with negative values instead.', 'dataType': dataTypes.float, 'alt-fields': ['image-depth']},
	'image-coordinate-reference-system':{'comment': 'The coordinate reference system, e.g. **EPSG:4326**'},
	'image-coordinate-uncertainty-meters':{'comment': 'Average/static uncertainty of coordinates in this dataset, given in meters', 'dataType': dataTypes.float},
...
ifdo_capture_fields = {
	...
	'image-camera-pose':{'comment': 'Information required to specify camera pose. For details on subfields see rows below.', 'dataType': dataTypes.dict,
			'subFields':{
					...
					'pose-utm-east-north-up-meters':{'comment': 'The resolution of the three coordinate axes in meters\nFormat: [float,float,float]'},
