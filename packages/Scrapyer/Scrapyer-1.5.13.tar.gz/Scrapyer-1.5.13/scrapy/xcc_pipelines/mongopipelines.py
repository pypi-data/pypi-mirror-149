...
class MongodbPipeline(object):
    def __init__(self,MONGO_HOST,MONGO_PORT,MONGO_PSW,MONGO_USER,MONGO_DB,AUTH_SOURCE):
        # 链接数据库
        if MONGO_PORT:
            # 兼容之前的settings配置
    ...
    def from_crawler(cls, crawler):
        ...
        return cls(MONGO_HOST=get_config().get(section=section_select,option='MONGO_HOST'),
                   MONGO_PORT=get_config().getint(section=section_select,option='MONGO_PORT'),
                   MONGO_PSW=get_config().get(section=section_select,option='MONGO_PSW'),
                   MONGO_USER=get_config().get(section=section_select,option='MONGO_USER'),
                   MONGO_DB=get_config().get(section=section_select,option='MONGO_DB'),
                   AUTH_SOURCE=get_config().get(section=section_select,option='AUTH_SOURCE'),
                   )

    def process_item(self, item, spider):
        date_time = datetime.now().strftime("_%Y_%m_%d")
        postItem = dict(item)
