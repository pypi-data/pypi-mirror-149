...
class MySQLPipeline:
    ...
    def from_crawler(cls, crawler):
        ...
        # 取 scrapy.cfg 参与配置
        return cls(host=get_config().get(section=section_select,option="MYSQL_HOST" ),
            database=get_config().get(section=section_select,option='MYSQL_DATABASE'),
            user=get_config().get(section=section_select,option='MYSQL_USER'),
            password=get_config().get(section=section_select,option='MYSQL_PASSWORD'),
            port=get_config().getint(section=section_select,option='MYSQL_PORT'),
        )
