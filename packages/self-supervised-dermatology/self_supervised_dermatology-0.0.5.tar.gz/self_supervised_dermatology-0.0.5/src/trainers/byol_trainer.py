...
class BYOLTrainer(Trainer):
    ...
    def fit(self):
        ...
        for epoch in range(self.start_epoch, self.config['epochs'] + 1):
            ...
            # evaluate on the downstream tasks if needed
            if epoch % self.config['downstream_every_n_epochs'] == 0:
                return_nodes = {
                    '4': 'feat1',
                    '5': 'feat3',
                    '6': 'feat4',
                ...
                if self.multi_gpu:
                    backbone = create_feature_extractor(
                        self.o_network.module.encoder,
                        return_nodes=return_nodes)
                else:
                    backbone = create_feature_extractor(
                        self.o_network.encoder, return_nodes=return_nodes)
                self.eval_downstream_tasks(self.o_network,
                                           backbone=backbone,
                                           n_iter=n_iter)
    ...
    def eval(self):
        ...
        self.config_eval = self.config
        # load the config from the checkpoint
        to_restore = {"epoch": 1, 'config': self.config}
        self.restart_from_checkpoint(
            self.config_eval['model_path'],
            run_variables=to_restore,
        )
        self.start_epoch = to_restore["epoch"]
        self.config = to_restore["config"]
        ...
        encoder = resnet_dict.get(self.config['model']['base_model'], np.nan)
        if encoder is np.nan:
            raise ValueError('Invalid base model name')
        self.o_network = BYOLModel(
            encoder, self.config['dataset']['augmentations']['target_size'],
            **self.config['model']['byol'])

        # load the trained model
        self.restart_from_checkpoint(
            self.config_eval['model_path'],
            state_dict=self.o_network,
        )
        self.o_network = self.o_network.to(self.device)
        self.o_network = self.distribute_model(self.o_network)
        self.o_network.eval()
        set_requires_grad(self.o_network, False)

        # copy the configs
        self.config['batch_size'] = self.config_eval['batch_size']
        self.config['downstream_train_epochs'] = self.config_eval[
            'downstream_train_epochs']
        self.config['downstream'] = self.config_eval['downstream']
        ...
        backbone = create_feature_extractor(self.o_network.online_encoder.net,
                                            return_nodes=return_nodes)

        # evaluate the model
        self.eval_downstream_tasks(self.o_network, backbone=backbone, n_iter=0)
