...
class TestEncryptedFolder(unittest.TestCase):
    def setUp(self):
        self.example_img_path = Path(
            'assets/illustrations/sweet-syndrome40.jpg')
    ...
    def test_decrypting_images(self):
        dataset = EncryptedImageFolder(self.tmp_dir,
    ...
    def test_decrypting_images_multiple_keys(self):
        dataset = EncryptedImageFolder(
            self.tmp_dir,
            enc_keys=self.rand_keys + [str(self.tmp_dir / 'key')])
        for img, _ in dataset:
            self.assertEqual(type(img), Image.Image)
            self.assertIsNotNone(img)

    def test_loading_not_encrypted_images(self):
        dataset = EncryptedImageFolder(self.tmp_dec_dir,
                                       enc_keys=None)
        for img, _ in dataset:
            self.assertEqual(type(img), Image.Image)
            self.assertIsNotNone(img)
