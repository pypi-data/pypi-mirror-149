...
class TestDINOAugmentation(unittest.TestCase):
    ...
    def test_augmentation(self):
        ...
        dino_aug = DINODataAugmentation(global_crops_scale=global_crops_scale,
                                        local_crops_scale=local_crops_scale,
                                        local_crops_number=local_crops_number)

        dino_crops = dino_aug(rand_img)

        global_crops = dino_crops[:2]
        local_crops = dino_crops[2:]

        self.assertEqual(len(global_crops), 2)
