...
def wrapNebula2Df(nebulaObj):
    ...
    nebulaObj=json.loads(nebulaObj.decode("latin1").encode("utf8").decode())
    
    dataList=nebulaObj["results"]["data"]
    
    dataList=[dict(list(zip(row["meta"],row["row"]))) for row in dataList]
    
    return pd.DataFrame(dataList).drop_duplicates()
...
def delEdge(gClient,sysIdList,edgeType="*"):
    ...
    if edgeType=="*":
        relDf=wrapNebula2Df(gClient.execute_json("SHOW EDGES"))["Name"]
    ...
    for relItem in tqdm.tqdm(relList,desc="del edges"):
        for srcSysIdItem in sysIdList:
            searchRelStr="GO FROM {srcSysId} OVER {edgeName} BIDIRECT YIELD {edgeName}._dst AS tgtSysId".format(
                                                                                                                ...
                                                                                                                edgeName=relItem)
            print(searchRelStr)
            relTailSysIdDf=wrapNebula2Df(gClient.execute_json(searchRelStr))
            if relTailSysIdDf.shape[0]>0:
                ...
                print(delStr)
                gClient.execute_json(delStr)

def delVertex(gClient,sysIdList,delRel=True):
    '''（关联）删除节点'''
    delReq={"data":[],"error_code":0}
    if delRel==True:
        relDf=wrapNebula2Df(gClient.execute_json("SHOW EDGES"))
        if relDf.shape[0]>0:
            ...
            for relItem in tqdm.tqdm(relList,desc="del edges with vertexes"):
                for srcSysIdItem in tqdm.tqdm(sysIdList,desc="del srcs from edges with vertexes"):
                    relTailStr="GO FROM {srcSysId} OVER {edgeName} BIDIRECT YIELD {edgeName}._dst AS tgtSysId".format(
                        ...
                        edgeName=relItem)
                    # print(relTailStr)
                    relTailSysIdDf=wrapNebula2Df(gClient.execute_json(relTailStr))
                    if relTailSysIdDf.shape[0]>0:
                        relTailSysIdList=relTailSysIdDf["tgtSysId"].values.flatten().tolist()
                        delOrderGroupStr=",".join(["{}->{}".format(srcSysIdItem,tailSysIdItem) for tailSysIdItem in relTailSysIdList])
                        delReverseGroupStr=",".join(["{}->{}".format(tailSysIdItem,srcSysIdItem) for tailSysIdItem in relTailSysIdList])
    ...
    for batchI in tqdm.tqdm(range(0,len(sysIdList),50),desc="del vertexes"): 
        ...
        # print(delStr)
        delReq=gClient.execute_json(delStr)
    return delReq
...
def existTag(nodeType,nodeIdAttr,nodeName,gClient):
    ...
    while passTag<5:
        try:
            searchTagDf=wrapNebula2Df(gClient.execute_json("LOOKUP ON {nodeType} WHERE {nodeType}.{nodeIdAttr}=='{nodeName}'|LIMIT 1".format(
                nodeType=nodeType,
...
def transferBetweenPlato(gHost,gPort,gUser,gPassword,
    ...
    srcClient.execute_json("use {}".format(srcGdbName))
    ...
    tgtClient.execute_json("use {}".format(tgtGdbName))

    # 1.构建data的项目

    # 获取schema
    srcVertexTypeDf=wrapNebula2Df(srcClient.execute_json("SHOW TAGS"))
    ...
    for srcVertexTypeItem in srcVertexTypeDf["Name"].values.tolist():
        ...
        srcVertexInfoDf=wrapNebula2Df(srcClient.execute_json(tagTypeListStr))
        srcVertexTypeAttrSetDict[srcVertexTypeItem]=dict(srcVertexInfoDf.loc[:,["Field","Type"]].values.tolist())
    
    srcEdgeTypeDf=wrapNebula2Df(srcClient.execute_json("SHOW EDGES"))
    ...
    for srcEdgeTypeItem in srcEdgeTypeDf["Name"].values.tolist():
        if len(edgeTypeList)==0 or srcEdgeTypeItem in edgeTypeList:
            ...
            srcEdgeInfoDf=wrapNebula2Df(srcClient.execute_json(edgeTypeListStr))
    ...
    for srcVertexTypeAttrSetItem in srcVertexTypeAttrSetDict:
        ...
        tgtClient.execute_json(buildTagSchemaStr)
    ...
    for srcEdgeTypeAttrSetItem in srcEdgeTypeAttrSetDict:
        ...
        tgtClient.execute_json(buildEdgeSchemaStr)
    ...
    for vertexTypeItem in srcVertexKeynameDict:
        ...
        tgtClient.execute_json("CREATE TAG INDEX IF NOT EXISTS {} ON {}({})".format(tagIndexName,vertexTypeItem,srcVertexKeynameDict[vertexTypeItem]))
        tgtClient.execute_json("REBUILD TAG INDEX {} OFFLINE".format(tagIndexName))
    ...
    for srcVertexTypeItem in tqdm.tqdm(srcVertexKeynameDict):
        batchI=0
        while True:
            vertexSysIdDf=wrapNebula2Df(srcClient.execute_json("LOOKUP ON {vertexType} WHERE {vertexType}.{attrKeyname}!='不可能的名字'|LIMIT {batchI},{batchSize}".format(
            ...
            vertexInfoDf=wrapNebula2Df(srcClient.execute_json("FETCH PROP ON {} {}".format(srcVertexTypeItem,",".join(vertexSysIdList))))
            ...
            for srcEdgeTypeItem in srcEdgeTypeAttrSetDict:
                for tgtVertexTypeItem in srcVertexKeynameDict:
                    ...
                    goDf=wrapNebula2Df(srcClient.execute_json("GO FROM {headSysId} OVER {edge} YIELD $^.{headType}.{headKeyname} AS headId,$$.{tailType}.{tailKeyname} AS tailId{attrList}".format(
...
def existEdge(headType,headIdAttr,headIdVal,tailType,tailIdAttr,tailIdVal,edgeType,edgeDir,gClient):
    ...
    searchEdgeDf=wrapNebula2Df(gClient.execute_json("LOOKUP ON {headType} WHERE {headType}.{headIdAttr}=='{headIdVal}'|\
...
def nlpPreprocessAttr(gClient,nodeType,nodeIdAttrName,nodeAttrName,tabuStr=""):
    ...
    nodeSysIdDf=wrapNebula2Df(gClient.execute_json("LOOKUP ON {nodeType} WHERE {nodeType}.{nodeIdAttr}!='不可能的名字'\
    ...
    if nodeSysIdDf.shape[0]>0:
        ...
        for nodeSysId,nodeAttrVal in nodeSysIdAttrList:
            gClient.execute_json("UPDATE VERTEX {nodeSysId} SET {nodeType}.{nodeAttr}='{nodeVal}'".format(
                                                                                                            nodeSysId=nodeSysId,
                                                                                                            nodeType=nodeType,
...
def addTokenForEntities(gClient,gDBName,nodeIdAttrList,tabuList=[],projectName="",email=""):
    ...
    for nodeItdPairItem in nodeIdAttrList:
        ...
        tgtNameDf=wrapNebula2Df(gClient.execute_json("LOOKUP ON {nodeType} WHERE {nodeType}.{nodeIdAttr}!='不可能的名字' \
...
def ucreateGDB(gClient,gdbname):
    '''创建图数据库'''
    gClient.execute_json("CREATE SPACE {}".format(gdbname))
...
def ucreateTag(gClient,tagAttrTypeDefaultDict,defaultTagAttrTypeDict={"Alias":{"Name":{"type":"string","default":"null"},
    ...
    for tagTypeItem in tagAttrTypeDefaultDict:
        ...
        gClient.execute_json("CREATE TAG IF NOT EXISTS {} ({})".format(tagTypeItem,attrListStr))
...
def ucreateEdge(gClient,edgeAttrTypeDefaultDict,defaultEdgeAttrTypeDict={"alias":{"checked":{"type":"string","default":"false"},
    ...
    for tagTypeItem in edgeAttrTypeDefaultDict:
        ...
        attrListStr=",".join(attrList)
        gClient.execute_json("CREATE EDGE IF NOT EXISTS {} ({})".format(tagTypeItem,attrListStr))
...
def uploadGraph(gClient,folderName,cookie,
    ...
    gClient.execute_json(useSpaceStr)
    ...
    gClient.execute_json("USE {}".format(gspace))
...
def downloadGraph(gClient,
    ...
    gClient.execute_json("USE {}".format(srcGdbName))
    ...
    for srcVertexTypeItem in srcVertexKeynameDict:
        ...
        vertexDfItem=wrapNebula2Df(gClient.execute_json(lookupVertexGroupStr))
        ...
        vertexAttrDTypeDf=wrapNebula2Df(gClient.execute_json("DESCRIBE TAG {}".format(srcVertexTypeItem)))
    ...
    for edgeTypeItem in edgeTypeList:
        for srcVertexTypeItem_head in srcVertexKeynameDict:
            for tgtVertexTypeItem_tail in srcVertexKeynameDict:
                edgeLookupReq=gClient.execute_json("LOOKUP ON {srcNodeType} WHERE {srcNodeType}.{srcNodeIdAttr}!='不可能的名字'|\
                ...
                edgeFetchReq=gClient.execute_json("FETCH PROP ON {edgeType} {edgeList}".format(
                                                                                                    edgeType=edgeTypeItem,
                ...
                edgeAttrDTypeDf=wrapNebula2Df(gClient.execute_json("DESCRIBE EDGE {}".format(edgeTypeItem)))
...
if __name__=="__main__":
    ...
    # start=time.time()
    adf=wrapNebula2Df(gClient.execute_json("SHOW SPACES"))
    # end=time.time()
    # print(end-start)
