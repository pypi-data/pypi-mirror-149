...
class ScienceCalibration(
    ...
    def run(self):
        ...
        with self.apm_task_step(
            ...
            f"{self.constants.num_dsps_repeats} dsps repeats and "
        ...
        for exp_time in self.constants.observe_exposure_times:
            for dsps_repeat in range(1, self.constants.num_dsps_repeats + 1):
                for raster_step in range(0, self.constants.num_raster_steps):
                    ...
                    header_storage = dict()
                    for beam in range(1, self.constants.num_beams + 1):
                        apm_str = f"{dsps_repeat = }, {raster_step = }, and {beam = }"
    ...
    def write_calibrated_array(self, calibrated_object) -> None:
        """
        ...
        if self.constants.correct_for_polarization:  # Write all 4 stokes params
            for i, stokes_param in enumerate(self.constants.stokes_params):
                ...
                self.write_cal_array(
                    data=final_data,
                    header=calibrated_object.header,
                    stokes=stokes_param,
                    raster_step=calibrated_object.raster_scan_step,
                    dsps_repeat=calibrated_object.current_dsps_repeat,
                )
    ...
    def correct_single_frame(
        ...
        if len(observe_object_list) > 1:
            raise ValueError(
                f"Found more than one observe frame for {dsps_repeat = }, {raster_step = }, {modstate = }, "
    ...
    def telescope_polarization_correction(
        ...
        """
        wavelength = inst_demod_obj.wavelength
        obstime = Time(inst_demod_obj.time_obs)
        tm = TelescopeModel(
            inst_demod_obj.azimuth, inst_demod_obj.elevation, inst_demod_obj.table_angle
        )
