...
class FeatureCreatorManager:
    ...
    def create_features_and_targets(self, instances: pd.DataFrame):
        data = f_cleaner.apply_basic_cleaning(instances=instances, resample_freq=self.resample_freq)



        targets = _create_targets(clean_data=data.copy(),
                                  target_settings=self.target_settings)

        features = _create_features_list(data=data.copy(),
                                         feature_settings_list=self.feature_settings_list)

        features_lags = _create_lags_list(data=data.copy(),
                                          lag_settings_list=self.lag_settings_list)

        # todo: needs to be configurable otherwise close will be gone
