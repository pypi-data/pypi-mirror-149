...
def _get_target_settings(setting_key: str):
    ind = IndicatorTargetSettings(**{
        'win_len': 80,
        'lookahead_len': 3,
        'feature_name': HistoricalReturnOperator.__name__,
        'target_col_feature_type': 'Return',
        'target_col_feature_subtype': HistoricalReturnOperator.get_column_names()[0],
    })
    settings = {"ind": ind}
...
def _get_feature_settings(setting_key: str):
    ind = IndicatorFeatureSettings(**{
        'win_lens': [70],
        'feature_names': [HistoricalReturnOperator.__name__]
    })
