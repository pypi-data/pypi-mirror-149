...
class AKIDASHAREDLIB_EXPORT Tensor {
 public:
  ...
  /**
   * @brief Downcast a Tensor to a Sparse or create a Sparse copy
   * @return : a pointer to the underlying Sparse
   */
  static std::shared_ptr<const Sparse> ensure_sparse(TensorConstPtr tensor);

  /**
   * @brief Splits a Tensor along the specified dimension
   ...
   * @param : the dimension index
   * @return : a vector of sub-tensors
   */
  static std::vector<TensorConstPtr> split(TensorConstPtr t, Index n = 0);
};
