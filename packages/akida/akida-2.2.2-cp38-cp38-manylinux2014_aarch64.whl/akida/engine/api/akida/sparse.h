...
class AKIDASHAREDLIB_EXPORT Sparse : public Tensor {
 public:
  ...
  /**
   * @brief Create a Sparse from a C/C++ coordinates array and a C/C++ byte data
   * array
   * @param dims    : the Sparse dimensions
   * @param coords  : a pointer to the source coordinates array
   * @param data    : a pointer to the source data byte array
   * @param n_items : the number of sparse items
   * @param type    : the Sparse data type
   */
  static SparsePtr from_buffers(const Shape& dims, const Index* coords,
  ...
  /**
   * @brief Create a Sparse from a Dense
   * @param dense : the Dense object to clone
   */
  static SparsePtr from_dense(const Dense& sparse);
};
