...
struct RegDetail {
  ...
  explicit constexpr RegDetail(uint32_t offset, uint32_t last)
      : offset(offset), nb_bits(last - offset + 1) {}

  explicit constexpr RegDetail(uint32_t offset) : offset(offset), nb_bits(1) {}
};
