...
def visualize(image, boxes, classes, scores, category_index, font, max_boxes_to_draw=101, min_score_thresh=.5,
    ...
    max_boxes_to_draw = len(boxes) if not max_boxes_to_draw else min(max_boxes_to_draw, len(boxes))
    for i in range(max_boxes_to_draw):
        if scores is None or scores[i] > min_score_thresh:
            ...
            if scores is None:
                ...
                if classes[i] < len(category_index):
