...
general = {
    ...
    "nthRegression()": ["num r=0", "Computes an nth-degree regression by the least squares method and returns the best fit polynomial. Printing output restricted if r==1."]
}
...
def modulehelp(rep=0):
    if(rep == 0):
        print("This is the Matrix Module V0.0.10, a package created by Elliott Walker. Type 'funcs' for a list of available functions, 'details' for a more detailed list including parameters, or 'exit' to exit this help function.")
    else:
...
def nthRegression(restrict=0):
    # Performs least-squares regression in order to find the best fitting nth-degree polynomial.
    ...
    for i in range(ndeg+1):
        outpoly += ("%sx^%d" % (Fraction(coeffs[i][0]).limit_denominator(), i))
        if(i < ndeg):
            outpoly += " + "
    if(restrict != 1):
        print("outpoly")
