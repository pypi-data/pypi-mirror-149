...
def nthRegression(restrict=0,inMatrix=["empty"],ndeg=0):
    ...
    if(restrict != 1):
        print("outpoly")
    return [coeffs,outpoly]
    
def pwrRegression(restrict=0):
    ...
    # Need to take user input, like for nthRegression.
    lnMt_reg = nthRegression(1,lnMt,1)[0] # Performs linear regression (n=1) on logarithmic dataset
    ...
    outeq = "y = %sx^(%s)" % (lnMt_reg[0][0],lnMt_reg[1][0]) # Readable format y = ax^(b)
    if(restrict != 1):
        print(outeq)
    return [lnMt_reg,outeq]