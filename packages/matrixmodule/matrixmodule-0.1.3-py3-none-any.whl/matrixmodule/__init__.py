...
def modulehelp(rep=0):
    if(rep == 0):
        print("This is the Matrix Module V0.1.3, a package created by Elliott Walker. Type 'funcs' for a list of available functions, 'details' for a more detailed list including parameters, or 'exit' to exit this help function.")
    else:
...
def regression(inmat,model="lin",degree=2,type="ls",rcomp=1, restrict=1):
    ...
    if (model == "lin" or model == "linear"):
        ...
        a = 0
        p = 0
        if(type=="leastsquares" or type=="ls"): #also not working completely right. This time the Jupyter fit ends up with a=0.
            for i in range(m):
