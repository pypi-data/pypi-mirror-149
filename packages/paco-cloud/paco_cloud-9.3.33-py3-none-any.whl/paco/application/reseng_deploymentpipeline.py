...
"""
    ...
    def init_stage_action_codedeploy_deploy(self, action_config):
        ...
        if action_config.paco_service_hook != None and registry.CODEPIPELINE_CODEDEPLOY_INIT_STAGE_HOOK != None:
            asg_roles_list = registry.CODEPIPELINE_CODEDEPLOY_INIT_STAGE_HOOK(self.paco_ctx, action_config)
            self.artifacts_bucket_policy_resource_arns.extend(asg_roles_list)
        else:
            if action_config.auto_scaling_group == None:
                breakpoint()
        ...
        stack_hooks = None
        if action_config.paco_service_hook != None and registry.CODEPIPELINE_CODEDEPLOY_UPDATE_DEPLOYMENT_GROUP_HOOK != None:
            hook_config = {
                'action_config': action_config,
                'paco_ctx': self.paco_ctx
            }
            stack_hooks = registry.CODEPIPELINE_CODEDEPLOY_UPDATE_DEPLOYMENT_GROUP_HOOK(hook_config)
