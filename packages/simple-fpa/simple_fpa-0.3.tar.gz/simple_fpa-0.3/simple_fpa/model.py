...
class Model:
    ...
    def fit(self, smoothing_rate = 0.2, trim_percent = 5, reflect = True):
        ...
        self.hat_f = f_smooth(self.observations, self.kernel, *self.band_options, paste_ends = False, reflect = reflect)/self.scale # might be unnecessary
        self.hat_q = self.scale*q_smooth(self.observations, self.kernel, *self.band_options, is_sorted = True, reflect = reflect)
