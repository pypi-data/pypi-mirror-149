from ..solutions import assignment1
import torch
...
def grade_11_helper(testcase_id, error_gradient, weights_true, weights_current):
    ...
    student_output = error_gradient(weights_current, inputs, targets)
    teacher_output = assignment1.error_gradient(weights_current, inputs, targets)
    is_correct = torch.allclose(student_output, teacher_output)
...
def grade_12_helper(testcase_id, gradient_descent, weights_true, weights_initial):
    ...
    learning_rate = 0.001
    num_iterations = 20
    student_output = gradient_descent(weights_initial, inputs, targets, learning_rate, num_iterations)
    teacher_output = assignment1.gradient_descent(weights_initial, inputs, targets, learning_rate, num_iterations)
    is_correct = torch.allclose(student_output, teacher_output)
    print(to_message(testcase_id, is_correct))
...
def grade_14_helper(testcase_id, gradient_descent_with_logger, weights_true, weights_initial):
    ...
    learning_rate = 0.001
    num_iterations = 20
    _, student_output = gradient_descent_with_logger(weights_initial, inputs, targets, learning_rate, num_iterations)
    _, teacher_output = assignment1.gradient_descent_with_logger(weights_initial, inputs, targets, learning_rate, num_iterations)
    is_correct = torch.allclose(student_output, teacher_output)
    print(to_message(testcase_id, is_correct))
