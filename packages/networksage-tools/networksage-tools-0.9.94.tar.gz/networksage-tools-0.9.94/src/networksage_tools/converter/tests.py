...
def run_tests():
    ...
    for tc in test_cases:
        ...
        if len(ordered_expected) != ordered_output:
            print("Different number of secflows: Expected (" + str(len(ordered_expected)) + ") output vs. Output (" + str(len(ordered_output)) + ")")
        for i in range(0, len(ordered_expected)):
            try:
                if len(ordered_expected[i] ^ ordered_output[i]) != 0:
                    print("Mismatch for secflow", str(i)+": Expected", ordered_expected[i], "vs. Output", ordered_output[i])
                    diffs += 1
            except:
