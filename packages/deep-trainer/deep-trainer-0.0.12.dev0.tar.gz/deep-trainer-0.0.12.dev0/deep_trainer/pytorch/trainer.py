...
class PytorchTrainer:  # pylint: disable=too-many-instance-attributes
    ...
    def _handle_validation_metrics(self, metrics: Dict[str, float]) -> None:
        ...
        if val_metric.minimize and metrics[val_metric.display_name] >= self.best_val:
            return

        if not val_metric.minimize and metrics[val_metric.display_name] <= self.best_val:
            return
    ...
    def train(
        ...
        while self.epoch < n_epochs:
            ...
            if val_loader is not None:
                ...
                self._handle_validation_metrics(metrics)
