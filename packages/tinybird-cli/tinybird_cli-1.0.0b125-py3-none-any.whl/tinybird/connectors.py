...
class GCS():
    ...
    def compose(self, source_prefix: str, destination_blob_name: str, mode: str) -> List[str]:
        ...
        for i, blob in enumerate(sources):
            ...
            if chunk_size > self.MAX_FILE_SIZE or i == len(sources) - 1:
                destination_name: str = f'{destination_blob_name}_part{count}.csv'
                destination: Blob = bucket.blob(destination_name)
                destination.content_type = "text/csv"
                destination.compose(chunk)

                urls.append(destination_name)
        ...
        if mode == 'replace':
            chunk = [bucket.get_blob(url) for url in urls]

            destination_name = f'{destination_blob_name}.csv'
            destination = bucket.blob(destination_name)
            destination.content_type = "text/csv"
            destination.compose(chunk)

            urls = [destination_name]
            blobs_to_delete += chunk
