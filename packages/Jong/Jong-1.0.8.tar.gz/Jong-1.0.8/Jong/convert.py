...
class PinholeLens(Projection):
    def __init__(self, focal_length: typing.Union[float, list]):
        self.focal_length = focal_length if isinstance(focal_length, (float, int)) else focal_length[0]
        self.K = np.array([self.focal_length*0.7, self.focal_length*0.7, 1], dtype=float)
...
def make_perspective_cam(cam: Camera):
...
def preprocess(img, json, dst):
    ...
    perspective_cam = make_perspective_cam(fisheye_cam)

    # load example image and re-project it to a central cylindrical projection
    fisheye_image = cv2.imread(img)
    map1, map2 = create_img_projection_maps(fisheye_cam, perspective_cam)
...
def To_prespective():
    ...
    parser.add_argument('input_folder_path', help="input fisheye image folder path")
    parser.add_argument('json_folder_path', help = "input json file folder path which correspond to fisheye image")
    parser.add_argument('output_folder_path', help = "output file path")

    args = parser.parse_args()

    imgs = sorted(os.listdir(args.input_folder_path))
    jsons = sorted(os.listdir(args.json_folder_path))

    if not os.path.exists(args.output_folder_path):
        os.mkdir(args.output_folder_path)

    for i in range(1, len(imgs)):
        print(imgs[i] + " complete.")
        preprocess(args.input_folder_path+imgs[i], args.json_folder_path+jsons[i], args.output_folder_path+imgs[i])