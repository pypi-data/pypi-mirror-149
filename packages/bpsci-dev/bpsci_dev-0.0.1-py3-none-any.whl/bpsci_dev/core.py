...
class dyn_obj:
    ...
    def apply_streamline(self, staticity, int_x, int_y, int_z, thickness):
        ...
        if staticity == "static":
            ...
            self.streamline = bpy.data.objects[self.name+'_streamline']

        if staticity == 'dynamic':
            int_size = len(int_x)
            int_coords = np.zeros((int_size, 3))
            int_coords[:, 0] = np.flip(int_x*self.scale)
            int_coords[:, 1] = np.flip(int_y*self.scale)
            int_coords[:, 2] = np.flip(int_z*self.scale)

            self.streamline.shape_key_add(from_mix=False)

            for i in range(len(self.frames)):
                cur_frame = self.frames[i]
                cur_zeros = np.arange(0, len(self.frames)-i)

                tmp_coords = np.zeros([int_size, 3])
                tmp_coords[cur_zeros, :] = 0

                tmp_curve = bpy.data.curves.new(name, type='CURVE')
                tmp_curve.dimensions = '3D'
                tmp_curve_curve.resolution_u = 2

                # map coords to spline
                tmp_curve_line = tmp_curve_curve.splines.new('NURBS')
                tmp_curve_line.points.add(len(tmp_coords))
                for i, coord in enumerate(tmp_coords):
                    tmp_,tmp_y,tmp_z = coord
                    tmp_line.points[i].co = (tmp_x, tmp_y, tmp_z, 1)

                # create Object
                tmp_curveOB = bpy.data.objects.new(name, tmp_curve)

                # attach to scene and validate context
                bpy.context.collection.objects.link(tmp_curveOB)
                
                tmp_curveOB.data.bevel_depth = thickness
                tmp_curveOB.data.use_fill_caps = True

                tmp_curveOB.data.bevel_factor_start = 0

                curve_to_mesh(tmp_curveOB)

                tmp_curve_mesh = bpy.data.objects[self.name+'_streamline']
                
                bpy.context.scene.objects.active = tmp_curve_mesh
                self.streamline.select_set(state=True)

                bpy.ops.object.join_shapes()

                bpy.data.objects.remove(tmp_curve_mesh)
