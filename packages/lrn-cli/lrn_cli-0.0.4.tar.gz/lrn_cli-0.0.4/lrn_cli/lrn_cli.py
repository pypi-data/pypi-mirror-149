...
def data(ctx, endpoint_url, references=None, limit=None,
    ...
    try:
        r = _send_json_request(endpoint_url, consumer_key, consumer_secret, data_request, action,
                               logger, do_recurse, page_size, data_attribute)
...
def _send_www_encoded_request(api, endpoint_url, consumer_key, consumer_secret, logger,
    ...
    logger.debug(f'Sending request: {form}')
...
def _send_json_request(endpoint_url, consumer_key, consumer_secret,
                       ...
                       logger,
                       do_recurse=False,
                       page_size=None, data_attribute='data'):
    data_api = DataApi()
...
def _validate_response(r, ctx=None):
    logger = logging
    dump_meta = False
    if ctx and ctx.obj:
        logger = ctx.obj['logger']
