...
def get_16s_copy_num(args):
    ...
    if mag_depth_gc_txt is None:
        ...
        if reads_subsample_pct_for_mag_cov_gc == 100:
            #mapping_cmd_mag = 'bowtie2 -x %s/%s -1 %s -2 %s -S %s -p %s -f --xeq --no-mixed --no-discordant --no-1mm-upfront --no-unal' % (index_folder, combined_prefixed_mags_no_ext, reads_file_r1, reads_file_r2, sam_file_reads_to_mag, num_threads)
            ...
            #mapping_cmd_mag = 'bowtie2 -x %s/%s -1 %s -2 %s -S %s -p %s -f --xeq --no-mixed --no-discordant --no-1mm-upfront --no-unal' % (index_folder, combined_prefixed_mags_no_ext, reads_file_r1_subset, reads_file_r2_subset, sam_file_reads_to_mag, num_threads)
            mapping_cmd_mag = 'bowtie2 -x %s/%s -1 %s -2 %s -S %s -p %s -f --xeq --no-unal' % (index_folder, combined_prefixed_mags_no_ext, reads_file_r1_subset, reads_file_r2_subset, sam_file_reads_to_mag, num_threads)
    ...
    if sorted_read_to_16s_sam_file is None:
        ...
        if matam_16s_reads is None:
            map_16s_reads_to_linked_16s_cmd = 'bowtie2 -x %s -U %s,%s -S %s -p %s -f --local --xeq --all --no-unal -N 1 -L 30' % (marker_seq_index, reads_file_r1, reads_file_r2, all_16s_sam_file, num_threads)
        else:
            map_16s_reads_to_linked_16s_cmd = 'bowtie2 -x %s -U %s -S %s -p %s -f --local --xeq --all --no-unal -N 1 -L 30' % (marker_seq_index, matam_16s_reads, all_16s_sam_file, num_threads)
