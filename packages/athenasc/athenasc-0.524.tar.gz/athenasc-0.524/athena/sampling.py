...
class Sampling:
    ...
    def sample(self, ncells=10000, pop_fp=None, cache=True, sim_fp=None):
        ...
        if cache:
            ...
            cells_meta.to_csv(os.path.join(self.metadata_dir, 'cells_metadata.csv'), compression='bz2', index=False)
            gene_expr.to_csv(os.path.join(self.metadata_dir, 'gene_expression.csv.gz'), compression='bz2', index=False)
    ...
    def sampling_molecules(self, gene_expr, pop_fp=None):
        ...
        realcounts = pd.DataFrame(pop.X.toarray(), columns=pop.var.index.values, dtype=np.int16)
    ...
    def calc_cpm(self, realcount, sim_counts):
        realcount = self.rank_order_gene_expr(realcount, sim_counts.shape[1])
        realcount = realcount.to_numpy(dtype=np.int16)
        ...
        if self.map_reference_cpm:
            ...
            realcount_cpm = realcount_cpm.flatten().astype(np.float32)
            sim_counts_cpm = sim_counts_cpm.flatten().astype(np.float32)
    ...
    def downsampling(self, sim_counts_cpm, lib_sizes):
        nlib_sizes = len(lib_sizes)
        
        for index in tqdm(range(nlib_sizes)):
    ...
    def rank_order_gene_expr(self, expr_mat, nsim_genes):
        expr_mat = expr_mat.T
        for cell in tqdm(expr_mat.columns):
            expr_mat[cell] = expr_mat[cell].sort_values(ascending=False).values
        
        expr_mat = expr_mat.iloc[:nsim_genes]
        return expr_mat.T