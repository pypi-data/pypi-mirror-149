...
def getTargetFileParamsJson(targetFileName, mapName):
...
def correctJsonData(content, mapName):
    return str(content.replace("Map<String, String> {0} =".format(mapName), "")).replace("\n", "").strip(",};").__add__("}")
...
def mapListCreater(emptyParams):
    if emptyParams:
        return ""
    with open(tdfIntlDir + '/i18n.json', 'r', encoding='utf-8') as jsonF:
    ...
    if (os.path.exists('i18n.json') == False):
        ...
        with open(tdfIntlDir + '/i18n.json', 'r', encoding='utf-8') as originF:
            ...
            for targetFileNameSuffix in i18nList:
                ...
                paramsJson = getTargetFileParamsJson(
                ...
                for item in json_data:
                    ...
                    if (targetFileNameSuffix.find('zh-ch') != -1):
                        paramsJson[r'{0}'.format(
                            item)] = r'{0}'.format(json_data[item])
                    elif (targetFileNameSuffix.find('zh-tw') != -1):
                        paramsJson[r'{0}'.format(
                            item)] = r'{0}'.format(tdf_translate(
                ...
                with open(targetFileName, 'a', encoding='utf-8') as targetFile:
                    ...
                    targetFile.write(
                        r"Map<String, String> {0}Map = {1};".format(targetFileNameSuffix.replace("-", ""), json.dumps(paramsJson, ensure_ascii=False)))
                ...
                #         targetFileName, 'w+', encoding='utf-8')
