...
def getTargetFileParamsList(targetFileName, mapName):
    ...
    if (os.path.exists('i18n.json') == False):
        ...
        with open(tdfIntlDir + '/i18n.json', 'r', encoding='utf8') as originF:
            ...
            for targetFileNameSuffix in i18nList:
                ...
                paramsResultList = getTargetFileParamsList(
                ...
                for item in json_data:
                    if len(paramsResultList) > 0 and item in paramsResultList:
                        continue
                    paramsResultList[r'{0}'.format(
                        item)] = r'{0}'.format(json_data[item])
                with open(targetFileName, 'a', encoding='utf8') as targetFile:
                    targetFile.seek(0)  # 定位
                    targetFile.truncate()  # 清空文件
                    targetFile.write(
                        r"Map<String, String> {0}Map = {1};".format(targetFileNameSuffix.replace("-", ""), json.dumps(paramsResultList)))
