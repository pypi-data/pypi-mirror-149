...
class Runpipe:
    ...
    def runpipe(self):
        ...
        count_cmd = 'DNBC4tools count --name %s --raw_matrix %s/%s/01.data/raw_matrix --bam %s/%s/01.data/final.bam --calling_method %s --expectcells %s --forcecells %s --minumi %s --cDNAbarcodeCount %s/%s/01.data/cDNA_barcode_counts_raw.txt --Indexreads %s/%s/01.data/Index_reads.fq.gz --oligobarcodeCount %s/%s/01.data/Index_barcode_counts_raw.txt --thread %s --oligotype %s --outdir %s'\
        %(self.name,self.outdir,self.name,self.outdir,self.name,self.calling_method,self.expectcells,self.forcecells,self.minumi,self.outdir,self.name,self.outdir,self.name,self.outdir,self.name,self.thread,self.oligotype,self.outdir)

        analysis_cmd = 'DNBC4tools analysis --name %s --matrix %s/%s/02.count/filter_matrix --species %s --outdir %s'\
...
def parse_run(parser):
    ...
    parser.add_argument('--cDNAconfig', metavar='JASON',help='whitelist of cell barcode and structure file in JSON format for cDNA fastq, [defalut: %s/config/DNBelabC4_scRNA_beads_readStructure.json].'%_root_dir,default='%s/config/DNBelabC4_scRNA_beads_readStructure.json'%_root_dir)
    parser.add_argument('--oligoconfig', metavar='JASON',help='whitelist of cell barcode and structure file in JSON format for oligo fastq, [defalut: %s/config/DNBelabC4_scRNA_oligo_readStructure.json].'%_root_dir,default='%s/config/DNBelabC4_scRNA_oligo_readStructure.json'%_root_dir)
    parser.add_argument('--oligotype', metavar='FILE',help='Whitelist of oligo index, [default: %s/config/oligo_type8.txt].'%_root_dir,default='%s/config/oligo_type8.txt'%_root_dir)
    ...
    parser.add_argument('--process', metavar='TEXT',help='Custom your analysis steps, steps are separated by comma, [default: data,count,analysis,report].',type=str,default='data,count,analysis,report')
