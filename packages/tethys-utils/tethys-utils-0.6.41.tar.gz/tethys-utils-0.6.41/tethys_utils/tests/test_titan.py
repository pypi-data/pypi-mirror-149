...
def main(stns, period, version_date, add_old):
    ...
    titan = tu.titan.Titan(temp_path=temp_path, add_old=add_old)
    ...
    for meas, ds in datasets.items():
        ...
        for stn in stns1.to_dict(orient="records"):
            ...
            data2 = titan.combine_obs_stn_data(data1, stn, mod_date=False)

            ###########################################
            ## Package up into the data_dict
            if not data1.empty:
                for ds in datasets[meas]:
                    results_paths1 = titan.save_preprocessed_results(data2, ds['dataset_id'])
    ...
    updated_results = titan.update_conflicting_results(results_paths3, threads=60, max_workers=1)

    titan.update_auxiliary_objects(updated_results, max_workers=1)

    print('-- Uploading final objects')
    resp = titan.upload_final_objects(threads=60)
