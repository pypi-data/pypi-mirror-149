...
class Titan(object):
    ...
    def status_checks(self, connection_config: tdm.base.ConnectionConfig, bucket: str, public_url: str = None, system_version: int = 4, load_diagnostics: bool = False):
        ...
        try:
            _ = s3.put_object_s3(s3, bucket, test_key, test1, {}, '', retries=1)
        ...
        # Version 3+ check
        objs1 = s3.list_objects_s3(s3, bucket, 'tethys/', delimiter='/')
        if not objs1.empty:
            exist_version = int(objs1.iloc[0].Key.split('/')[1].split('.')[0][1:])
        else:
        # Version 2 check
            objs2 = s3.list_objects_s3(s3, bucket, 'tethys/v2/', delimiter='/')
            if objs2.empty:
                exist_version = None
            else:
                exist_version = 2
    ...
    def save_diagnostics(self):
        ...
        obj_resp = s3.put_object_s3(s3, self.bucket, dgn_key, dgn_obj, {'run_date': run_date1}, 'application/json')
    ...
    def save_new_results(self, source_paths, correct_times=False, max_workers=4):
        ...
        if max_workers <= 1:
            ...
            for source_path in source_paths1:
                ...
                f = processing.save_new_results(source_path, metadata, self.max_version_date, correct_times, self.system_version)
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers, mp_context=mp.get_context("spawn")) as executor:
                ...
                for source_path in source_paths1:
                    ds_id = source_path.split('/')[-1].split('_')[0]
                    metadata = self.datasets[ds_id]
                    f = executor.submit(processing.save_new_results, source_path, metadata, self.max_version_date, correct_times, self.system_version)
                    futures.append(f)
    ...
    def _determine_results_chunks_diffs(self, source_paths, remote, add_old=False):
        ...
        """
        r_chunks = s3.determine_results_chunks_diffs(source_paths, remote, add_old)
    ...
    def _update_results_chunks(self, results_paths, max_workers=4):
        ...
        for ds_id, stn_files in paths_dict.items():
            ...
            if self.max_version_date in old_versions:
                old_rc_data = s3.get_remote_results_chunks(self.bucket, s3=None, connection_config=self.connection_config, public_url=self.public_url, dataset_id=ds_id, version_date=self.max_version_date, system_version=self.system_version)
            elif add_old and (len(old_versions) > 0):
                previous_version_date = old_versions[-1]
                old_rc_data = s3.get_remote_results_chunks(self.bucket, s3=None, connection_config=self.connection_config, public_url=self.public_url, dataset_id=ds_id, version_date=previous_version_date, system_version=self.system_version)
            else:
                old_rc_data = None

            with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers, mp_context=mp.get_context("spawn")) as executor:
                f = executor.submit(processing.update_results_chunks, stn_files, self.preprocessed_path, old_rc_data)
                futures.append(f)
    ...
    def _update_stations(self, results_paths, max_workers=4):
        ...
        for ds_id, data_paths in paths_dict.items():
            ...
            ## Get the old results chunks file
            if self.max_version_date in old_versions:
                old_stns_data = s3.get_remote_stations(self.bucket, s3=None, connection_config=self.connection_config, public_url=self.public_url, dataset_id=ds_id, version_date=self.max_version_date, system_version=self.system_version)
            elif add_old and (len(old_versions) > 0):
                previous_version_date = old_versions[-1]
                old_stns_data = s3.get_remote_stations(self.bucket, s3=None, connection_config=self.connection_config, public_url=self.public_url, dataset_id=ds_id, version_date=previous_version_date, system_version=self.system_version)
            else:
                old_stns_data = None

            with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers, mp_context=mp.get_context("spawn")) as executor:
                f = executor.submit(processing.update_stations, data_paths, rc_path, self.preprocessed_path, old_stns_data)
                futures.append(f)
    ...
    def upload_final_objects(self, threads=60):
        ...
        if len(rc_paths) > 0:
            ...
            for path in all_paths:
                dict1 = processing.prepare_file_for_s3(path, s3, self.bucket, self.system_version)
                append(dict1)
