...
def get_obj_list(glob_path, date_format=None, freq=None, from_date=None, to_date=None, connection_config=None, bucket=None):
    ...
    if isinstance(connection_config, dict) and isinstance(bucket, str):
        ...
        obj_list = s3.list_objects_s3(s3, bucket, glob_path3, delimiter='/', date_format=date_format)
    ...
    if isinstance(connection_config, (dict, str)) and isinstance(bucket, str):
        ...
        if isinstance(max_workers, int):
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
                futures = []
                for obj in objs:
                    f = executor.submit(data_io.copy_s3_file, source_path=obj, dest_path=dest_path, s3=s3, connection_config=connection_config, bucket=bucket, public_url=public_url, compression=compression)
                    futures.append(f)
                runs = concurrent.futures.wait(futures)
...
def copy_interim_objs(source_paths, dest_path, bucket, connection_config=None, public_url=None, threads=20):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        ...
        for source_path in source_paths:
            ...
            f = executor.submit(data_io.copy_s3_file, source_path, dest_path1, bucket, s3=s3, public_url=public_url, compression='zstd')
            futures.append(f)
        runs = concurrent.futures.wait(futures)
...
def multi_calc_new_variables(nc_paths, datasets, version_date, func_dict):
    ...
    for i, g in dates1.groupby('date'):
        # print(g)
        p = processing.calc_new_variables(g['path'].tolist(), datasets, version_date, func_dict)
...
class Grid(titan.Titan):
    ...
    def upload_interim_results(self, source_paths, threads=10):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
            ...
            for path in source_paths:
                f = executor.submit(s3.put_interim_results_s3, s3, self.bucket, path, self.run_id, system_version=self.system_version)
                futures.append(f)
            runs = concurrent.futures.wait(futures)
    ...
    def get_interim_results_list(self, max_size_gb=0.1):
        ...
        obj_list1 = s3.list_objects_s3(s3, self.bucket, key)
    ...
    def clear_interim_results(self):
        ...
        obj_list = s3.list_object_versions_s3(s3, self.bucket, prefix)
