        ...
        try:
            ...
            for guest in record.get('guestInfo').get('guest_list'):
                ...
                if guest.get('primary'):
                    docs = self.firestore_db.get_collection(f'Organizations/{record.get("orgId")}/users')\
                        .where("pmsGuestProfile", "==", guest.get('guest')).stream()
                    logging.info(f"Retrieve {len(docs)} profiles from firestore")
                    for doc in docs:
                        self.firestore_db.get_collection(f'Organizations/{record.get("orgId")}/users/{doc.id}/reservations')\
                            .document(str(record.get('_id'))).set(record, merge=True)
                        logging.info(f"{record.get('orgId')} : {record.get('_id')} added to {doc.id}")
        except Exception as e:
