import re
import dunlin.standardfile.dun.dun_string_reader as dsr

def parse_element(element, interpolators=None):
    interpolated = interpolate(element, interpolators)
    substituted  = substitute_dun(interpolated)
    result       = {} 
...
def interpolate(element, interpolators):
    def helper(match):
        i = match[0]
        if i in interpolators:
            return interpolators[i]
        else:
            raise SyntaxError(f'Encountered an unexpected interpolation: {i}') 
    
    interpolated = re.sub('#\w*(\W|$)', helper, element)
