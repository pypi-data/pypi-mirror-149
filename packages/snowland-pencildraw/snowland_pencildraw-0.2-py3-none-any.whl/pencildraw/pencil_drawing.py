...
def natural_histogram_matching(img, ub=225, ua=105):
    img_map = np.arange(257) / 256.0
    img_map[-1] += 1e-12  # 加个小数防止溢出
    po = np.zeros(256)
    img_int = (img // (1/256)).astype(int)
    counter = Counter(img_int.flatten())
    for k, v in counter.items():
        po[k] = v
    po = po / np.sum(po)
    ho = np.cumsum(po)
    p1 = lambda x: (1 / 9.0) * np.exp(-(ub - x) / 9.0)
    p2 = lambda x: (1.0 / (ub - ua)) * (ua <= x <= ub)
    ...
    Iadjusted = np.zeros_like(img)
    for x in range(img.shape[0]):
        for y in range(img.shape[1]):
            Iadjusted[x, y] = np.argmin(np.absolute(histo - ho[img_int[x, y]]))
...
def genToneMap(im, ub=225, ua=105):
    # Parameters
    p = np.zeros(256)
    p[ua: ub + 1] = 1.0 / (ub - ua)
