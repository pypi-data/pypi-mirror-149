...
class SwitchBeeAPI:
    ...
    async def _send_request(self, command, params={}):
        ...
        async def do_send_request(payload):
            try:
                resp = await self._session.post(url=f"https://{self._cunit_ip}/commands", json=payload)
...
async def main():
    ...
    api = SwitchBeeAPI("192.168.50.2", "atili.jafar@gmail.com", "Gafr6616", session)
    await api.login()
    json_resp = await api.get_configuration()
    if json_resp[ATTR_STATUS] == STATUS_OK:
        print(json_resp)
    else:
        print("Failed")
    await session.close()
