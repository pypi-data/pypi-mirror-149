...
class Stellar:
    ...
    def __init__(self, group, treatment, con, discharge_statuses = None, months_in_eval_period = 3, stars = False):
        '''
        ...
        months_in_eval_period : integer, optional
            Number of months the outcome period should span. The default is 3.
        ...
        self.months_in_eval_period = months_in_eval_period
        self.stars = stars
    ...
    def generate_queries(self):
        ...
        if self.treatment:
            ...
            if self.stars:
                ER_tbl_nm = f'{self.group}_er_eligible_from_IP'
            
            else:
                ER_tbl_nm = f'{self.group}_er_eligible'
                
            steps.append(self.split_utils(cohort_tbl_nm = f'{self.group}_cohort', ER_tbl_nm = ER_tbl_nm, IP_tbl_nm = f'{self.group}_ip_all'))

            #----------------These steps are all to get follow-up rates---------------------------------
            steps.append(self.unique_er_discharges(input_tbl_nm = ER_tbl_nm, # get follow-ups for ALL ED visits so can include gap closed flag on final table
            ...
            # If doing a stars analysis, return all the discharges
            # if self.stars:
            #     step12 = self.feature_tbl(cohort_tbl_nm = f'{self.group}_cohort', filter_to_obs_period=False)
                
            # # If doing the utilization analysis, filter discharges down to the correct observation period
            # else:
            #     step12 = self.feature_tbl(cohort_tbl_nm = f'{self.group}_cohort', filter_to_obs_period=True)

            # steps = [step1, step2a, step2b, step3, step4a, step4b, step4c, step4d, step5, 
        ...
        # Control pipeline  
            ...
            if self.stars:
                # This table will get created in using the remove_ineligible_due_to_ED() method
                ER_tbl_nm = f'{self.group}_er_eligible_from_IP'
            
            else:
                # This is just the volatile table created in the previous step
                ER_tbl_nm = f'{self.group}_er_eligible'
                
            steps.append(self.narrow_to_observation_period(input_tbl_nm = ER_tbl_nm, output_tbl_nm = f'{self.group}_cohort_mbi'))
            steps.append(self.mbi_to_pty_lookup(input_tbl_nm = f'{self.group}_cohort_mbi', output_tbl_nm = f'{self.group}_cohort'))
            steps.append(self.split_utils(cohort_tbl_nm = f'{self.group}_cohort', ER_tbl_nm = ER_tbl_nm, IP_tbl_nm = f'{self.group}_ip_all'))

            #----------------These steps are all to get follow-up rates---------------------------------
            steps.append(self.unique_er_discharges(input_tbl_nm = ER_tbl_nm, output_tbl_nm = f'{self.group}_er_unique')) # get follow-ups for ALL ED visits so can include gap closed flag on final table
            ...
            # steps += self.feature_tbl(cohort_tbl_nm = f'{self.group}_cohort', filter_to_obs_period = False) # Want to get back all discharges from 11-04-2021 through 02-28-2022

            
        return steps
    
    def build_cohort(self, steps = None, pre_ER_steps = None, post_ER_steps = None):
        '''
        ...
        if self.stars:
            ...
            # Tell the for loop when to break to run the STARS-eligible algo
            if pre_ER_steps is None and self.treatment:
                pre_ER_steps = 4
            
            elif pre_ER_steps is None and self.control:
                pre_ER_steps = 5
                
            
            for i, ele in enumerate(steps[:pre_ER_steps]):
                print(f'Running step {i}...')
                self.cur.execute(ele)
                
            # remove ineligible ER here
            self.remove_ineligible_due_to_ed(input_tbl_nm = f'{self.group}_er_eligible', output_tbl_nm = f'{self.group}_er_eligible_from_IP')
            
            for i, ele in enumerate(steps[pre_ER_steps:]):
                print(f'Running step {i+pre_ER_steps-1}...')
                self.cur.execute(ele)
            
        
        else:
        
            # Can run all the queries straight through for the affordability analysis
    ...
    def build_feature_tbl(self, tbls = None):
        ...
        if self.stars:        
            sql = self.feature_tbl2(cohort_tbl_nm = f'{self.group}_cohort', filter_to_obs_period = False, tbls = tbls)
            
        else:            
            sql = self.feature_tbl2(cohort_tbl_nm = f'{self.group}_cohort', filter_to_obs_period = True, tbls = tbls)
    ...
    def get_feature_tbl(self):
        ...
        tbl = pd.read_sql(f'SELECT a.*, {trt} as treatment FROM {self.group}_feature_tbl a', self.con)
        ...
        (SELECT DISTINCT b.alt_id_val, a.mbr_pty_id, b.orig_src_sys_cd, a.er_discharge
         FROM {input_tbl_nm} a
            ...
            LEFT JOIN udwbasesecureview1.mbr_f c
             ...
             AND a.er_discharge >= c.row_eff_dt
             AND a.er_discharge <= c.row_expir_dt
        ...
        if indexed_on_discharge:
            ...
            (SELECT DISTINCT a.alt_id_val, b.mbr_pty_id
             FROM {input_tbl_nm} a
                ...
                WHERE b.alt_id_typ_cd = 'HIC'
                AND EXTRACT(YEAR from b.row_eff_dt) <= 2021
                AND EXTRACT(YEAR from b.row_expir_dt) >= 2021)
    ...
    def create_starting_cohort(self, output_tbl_nm):
        ...
        if self.treatment and self.stars:
            ...
            (SELECT DISTINCT b.mbr_pty_id, b.alt_id_val, a.discharge_dt AS er_discharge
             ...
             AND a.discharge_dt >= b.row_eff_dt
             AND a.discharge_dt <= b.row_expir_dt
            ...
            sql = f'''CREATE VOLATILE TABLE {output_tbl_nm} AS (                 
                    ...
                    SELECT DISTINCT b.mbr_pty_id, b.proc_dt AS er_discharge
                    ...
                    WHERE b.proc_dt BETWEEN '2021-11-04' AND ADD_MONTHS(CURRENT_DATE(), -1) 
                    ...
                    WHERE c.clm_srvc_strt_dt BETWEEN '2021-11-04' AND ADD_MONTHS(CURRENT_DATE(), -1) 
    ...
    def remove_ineligible_due_to_ed_helper(self, input_tbl_nm):
        
        # Pull discharges into memory
        df = pd.read_sql(f'SELECT * FROM {input_tbl_nm};', self.con)
        df['er_discharge'] = pd.to_datetime(df['er_discharge'])
        ...
        for i in member_ids:
            ...
            # Put dates in order
            mbr_discharges.sort_values(by = 'er_discharge')
            
            # Remove ineligible discharges
            mbr_elig_discharges = self.get_mbrs_eligible_discharges(mbr_discharges['er_discharge'])
        ...
        eligible_discharges_df = pd.DataFrame([(k, x) for k, v in eligible_dict.items() for x in v], columns = ['alt_id_val', 'er_discharge'])
        ...
        with open(f'{self.group}_ed_eligible.csv', newline='') as f:
            self.cur.execute(f'''CREATE VOLATILE TABLE {output_tbl_nm}
                                ...
                                (alt_id_val VARCHAR(100),
                                 er_discharge DATE FORMAT 'YYYY-MM-DD'
                                 )
    ...
    def split_utils(self, cohort_tbl_nm, ER_tbl_nm, IP_tbl_nm):
        ...
        ip_outcome : string
            Query to pull member IP admissions in the outcome period (day after index date through +months in eval period of the instance).
        ...
        LEFT JOIN {ER_tbl_nm}  b
        ON a.alt_id_val = b.alt_id_val
        AND b.er BETWEEN (a.er_discharge + INTERVAL '1' DAY) AND ADD_MONTHS(a.er_discharge, {self.months_in_eval_period}))
        ...
        LEFT JOIN {IP_tbl_nm} b
        ON a.alt_id_val = b.alt_id_val
        AND b.ip_admit BETWEEN (a.er_discharge + INTERVAL '1' DAY) AND ADD_MONTHS(a.er_discharge, {self.months_in_eval_period}))
...
GROUP BY a.alt_id_val, a.er_discharge
    ...
    def feature_tbl2(self, cohort_tbl_nm, filter_to_obs_period, tbls = None):
        ...
        sql = f'''CREATE VOLATILE TABLE {self.group}_feature_tbl AS 
            ...
            (SELECT a.alt_id_val, a.er_discharge, ADD_MONTHS(CURRENT_DATE(), -{self.months_in_eval_period}-1) as obs_period_end_date  {select_statement}
             FROM {cohort_tbl_nm} a
