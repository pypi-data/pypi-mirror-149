...
def create(cluster_id, name):
    ...
    service = DatabaseService()
    if not click.confirm(f"Are you sure, that you want to create {name} database in {cluster_id} cluster?"):
...
def dump(cluster_id, db_name, db_user):
    """command for create db snapshot and save it into yours /tmp, db_user is optional (default is db_name)"""
    service = DatabaseService()
    ...
    if cluster.engine == "pg":
        dump_command = f"ssh swarm1 'PGPASSWORD={user.password} pg_dump -U {user.name} -h {cluster.privateConnection.host} -p {cluster.privateConnection.port} -d {db_name} > /tmp/{server_dumpfile_name}'"
    elif cluster.engine == "mysql":
        dump_command = f"ssh swarm1 'mysqldump -u {user.name} -p {user.password} -h {cluster.privateConnection.host} -P {cluster.privateConnection.port} {db_name} > /tmp/{server_dumpfile_name}'"
    else:
    ...
    click.echo(f"Database was successfully dumped. You can find it in your /tmp/{local_filename}")
...
#     """command for recreate database (when it needs to be dropped and created again)"""
#     service = DatabaseService()
#     if not confirm_action(f"Are you sure, that you want to recreate {name} database in {cluster_id} cluster?"):
