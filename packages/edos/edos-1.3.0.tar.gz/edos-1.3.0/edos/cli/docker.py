...
def ps(secret_name: str):
    ...
    try:
        ...
        command = f"docker --host ssh://swarm1 exec -t {container_name} cat /run/secrets/{d_secret.name}"
...
def edit(secret_name: str):
    ...
    try:
        ...
        while True:
            name = click.prompt("New secret name: (keep empty if you want to use default)", default=new_name)
            try:
