...
class DatabaseService:
    ...
    def create_database(self, cluster_id: str, project_name: str) -> str:
        ...
        so this command will be doing this:
            ...
            - get cluster
            - create db_url from private connection from cluster and database user/password
        ...
        return (
            f"{prefix}{user.name}:{user.password}@{private_connection.host}:{private_connection.port}/{database.name}"
        )
    ...
    #     """
    #     this command just deletes database and create it again (when it needs to be deleted like uploading a dump)
    #     :param cluster_id: cluster where is database stored
