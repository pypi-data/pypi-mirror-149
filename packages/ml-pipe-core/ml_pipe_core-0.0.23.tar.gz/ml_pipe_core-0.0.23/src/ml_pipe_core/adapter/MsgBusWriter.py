import time
import requests
from ml_pipe_core.config import OBSERVER_URL
...
class MessageBusWriter:
    ...
    def commit(self):
        ...
        self.producer.sync_produce(MACHINE_INPUT_TOPIC, data.serialize(), Headers(package_id=package_id, source=self.service_type, msg_type=self.service_type))
        self.cmd_queue = []
