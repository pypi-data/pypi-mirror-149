...
from src.ml_pipe_core.config import PIPE_CONFIG_FILEPATH

PROFILE = "pipeline"

PIPE_CONFIG_FILEPATH_HOST = os.environ.get("PIPE_CONFIG_FILEPATH_HOST")
...
class DockerComposeClient():
    ...
    def run_service(self, name: str, service_type: str, mode: str):
        ...
        command = f'docker-compose -p {PROFILE} run --rm --no-deps -e SERVICE_NAME={name} -e SERVICE_MODE={mode} -e PIPE_CONFIG_FILEPATH={PIPE_CONFIG_FILEPATH} -v {PIPE_CONFIG_FILEPATH_HOST}:{PIPE_CONFIG_FILEPATH} {service_type}'
        print(command)
