...
from exa.daemon_pb import daemon_pb2
...
class ModuleContextSpec:
    ...
    def _to_proto(self) -> daemon_pb2.ModuleContextSpec:
        return daemon_pb2.ModuleContextSpec(
...
class PlacementGroupSpec:
    ...
    def _to_proto(self, name) -> daemon_pb2.PlacementGroupSpec:
        return daemon_pb2.PlacementGroupSpec(
            name=name,
...
class Session:
    ...
    def __init__(
        ...
        session_config = daemon_pb2.SessionConfig(
        ...
        self._c = _C.Session(session_config.SerializeToString(), existing_session_id)
    ...
    def new_module(
        self, module_tag: str, config: Optional[Dict[str, bytes]] = None
    ) -> Module:
        """
    ...
    def new_module_from_hash(
        self, module_hash: str, config: Optional[Dict[str, bytes]] = None
    ) -> Module:
        """
    ...
    def new_module_from_cls(
        ...
        if "_py_module_cls" in config:
            raise ValueError(
                "Config for python interpreter module may not contain key _py_module_cls"
            )
        if "_py_module_name" in config:
            raise ValueError(
                "Config for python interpreter module may not contain key _py_module_name"
            )
