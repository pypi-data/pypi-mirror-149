...
void ForestSurvival::loadForest(size_t num_trees, std::vector<std::vector<std::vector<size_t>> >& forest_child_nodeIDs,
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    trees.push_back(
        ranger::make_unique<TreeSurvival>(forest_child_nodeIDs[i], forest_split_varIDs[i], forest_split_values[i],
...
void ForestSurvival::growInternal() {
  trees.reserve(num_trees);
  for (size_t i = 0; i < num_trees; ++i) {
    trees.push_back(ranger::make_unique<TreeSurvival>(&unique_timepoints, &response_timepointIDs));
  }
}
...
//    }
    ...
    // Create tree
    trees.push_back(
        ranger::make_unique<TreeSurvival>(child_nodeIDs, split_varIDs, split_values, chf, &unique_timepoints,
