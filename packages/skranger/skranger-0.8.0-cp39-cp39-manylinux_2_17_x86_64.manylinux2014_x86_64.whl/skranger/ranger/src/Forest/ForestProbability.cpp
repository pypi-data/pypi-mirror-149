...
void ForestProbability::loadForest(size_t num_trees,
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    trees.push_back(
        ranger::make_unique<TreeProbability>(forest_child_nodeIDs[i], forest_split_varIDs[i], forest_split_values[i],
...
void ForestProbability::growInternal() {
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    trees.push_back(
        ranger::make_unique<TreeProbability>(&class_values, &response_classIDs, &sampleIDs_per_class, &class_weights));
  }
}
...
// Read class_values
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    ...
    // Create tree
    trees.push_back(
        ranger::make_unique<TreeProbability>(child_nodeIDs, split_varIDs, split_values, &class_values, &response_classIDs,
