...
void ForestRegression::loadForest(size_t num_trees,
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    trees.push_back(
        ranger::make_unique<TreeRegression>(forest_child_nodeIDs[i], forest_split_varIDs[i], forest_split_values[i]));
  }
...
void ForestRegression::growInternal() {
  trees.reserve(num_trees);
  for (size_t i = 0; i < num_trees; ++i) {
    trees.push_back(ranger::make_unique<TreeRegression>());
  }
}
...
// Read treetype
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    ...
    // Create tree
    trees.push_back(ranger::make_unique<TreeRegression>(child_nodeIDs, split_varIDs, split_values));
  }
}
