...
void run_ranger(const ArgumentHandler& arg_handler, std::ostream& verbose_out) {
  ...
  case TREE_CLASSIFICATION:
    if (arg_handler.probability) {
      forest = ranger::make_unique<ForestProbability>();
    } else {
      forest = ranger::make_unique<ForestClassification>();
    }
    break;
  case TREE_REGRESSION:
    forest = ranger::make_unique<ForestRegression>();
    break;
  case TREE_SURVIVAL:
    forest = ranger::make_unique<ForestSurvival>();
    break;
