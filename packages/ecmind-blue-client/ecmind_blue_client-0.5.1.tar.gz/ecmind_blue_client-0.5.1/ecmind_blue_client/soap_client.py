    ...
    def execute(self, job: Job) -> Result:
        ...
        for param in job.params:
            if param.type == ParamTypes.INTEGER:
                params.append(
                    self.enaioWsTypesFactory.IntegerParameter(
                        name=param.name, value=param.value
                    )
                )
            elif param.type == ParamTypes.BOOLEAN:
                params.append(
                    self.enaioWsTypesFactory.BooleanParameter(
                        name=param.name, value=param.value
                    )
                )
            elif param.type == ParamTypes.DATE_TIME:
                params.append(
                    self.enaioWsTypesFactory.DateTimeParameter(
                        name=param.name, value=param.value
                    )
                )
            elif param.type == ParamTypes.DOUBLE:
                params.append(
                    self.enaioWsTypesFactory.DoubleParameter(
                        name=param.name, value=param.value
                    )
                )
            elif param.type == ParamTypes.STRING:
                params.append(
                    self.enaioWsTypesFactory.StringParameter(
                        name=param.name, value=param.value
                    )
                )
                ...
                if isinstance(param.value, (bytearray, bytes)):
                    params.append(
                        self.enaioWsTypesFactory.Base64Parameter(
                            name=param.name, value=param.value
                        )
                    )
                else:
                    params.append(
                        self.enaioWsTypesFactory.Base64Parameter(
                            name=param.name, value=bytearray(param.value, "UTF-8")
                        )
                    )
        ...
        try:
            native_result = self.client.service.execute(
                ...
                job=self.enaioWsTypesFactory.Job(
                    name=job.name, parameter=params, fileParameter=file_params
                ),
                property=[],
            )
            result_return_code = native_result.job.returnCode
            result_values = {
                e["name"]: e["value"].decode("UTF-8")
            ...
            if ft.message in [
                ...
                return Result(
                    return_code=result_return_code, error_message=error_message
                )
