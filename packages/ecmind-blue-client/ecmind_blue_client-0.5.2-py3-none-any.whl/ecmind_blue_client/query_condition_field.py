...
from .const import QueryOperators, SpecialValues, SystemFields
...
class QueryConditionField:
    def __init__(
        ...
        if not table and isinstance(field, SystemFields):
            ...
            self.system = True
        elif not table and field in [
            system_field.name for system_field in SystemFields
        ]:
    ...
    def __repr__(self) -> str:
        return (
            f"{self.table_name}:" if self.table_name else ""
        ) + f"{self.internal_name} {self.query_operator.value} {self.values}"
    ...
    def to_xml_element(self) -> XmlElement:
        ...
        def values_to_xml_elements(self) -> List[XmlElement]:
            ...
            result = []
            for value in (
                self.values if isinstance(self.values, list) else [self.values]
            ):
                if value is None:
                    ...
                    result.append(
                        XmlElement(
                            "Value", t=datetime.strftime(value, "%d.%m.%Y %H:%M:%S")
                        )
                    )
                elif isinstance(value, date):
                    v_datetime = datetime.combine(value, datetime.min.time())
                    result.append(
                        XmlElement("Value", t=datetime.strftime(v_datetime, "%d.%m.%Y"))
                    )
                else:
