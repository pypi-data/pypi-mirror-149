...
class TcpClient(Client):
    ...
    def __attach__(self, username: str, password: str):
        ...
        if (
            session_login_result.values["Description"] is not None
            and session_login_result.values["Description"] != ""
        ):
            raise RuntimeError(
                f'Login error: {session_login_result.values["Description"]}'
            )
    ...
    def _connect(self):
        ...
        self.job_caller = JobCaller(
            self.hostname, self.port, self.use_ssl, self.file_cache_byte_limit
        )
