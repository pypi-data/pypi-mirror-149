...
class Client(ABC):
    ...
    def lol_query(
        ...
        while (not max_hits) or (combined_count <= max_hits):
            ...
            if not columns and not tables:
                ...
                tables = {
                    table.attributes["internal_name"]: {
                        column.attributes["internal_name"]: column.attributes[
                            "datatype"
                        ]
                ...
                if columns:
                    for name, value in zip(columns, row["Value"]):
                        ...
                        if datatype == "TEXT":
                            ...
                            yield_result[name] = (
                                float(value.text) if value.text else None
                            )
                        elif datatype == "DATE":
                            yield_result[name] = (
            ...
            logging.debug(
                "Paging to next result frame with offset %s after %s combined hits.",
                current_offset,
                combined_count,
            )
