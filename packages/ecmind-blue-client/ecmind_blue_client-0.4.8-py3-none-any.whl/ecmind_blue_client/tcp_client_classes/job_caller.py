...
class JobCaller:
    ...
    def build_parameters(self, params: List[Param]):
        ...
        for param in params:
            ...
            if param.type == ParamTypes.BOOLEAN:
                parameter_data = JobParameterData(
                    name=param.name, value="1" if param.value else "0"
                )
            ...
            if param.type == ParamTypes.BASE64:
                if isinstance(param.value, (bytearray, bytes)):
                    parameter_data = JobParameterData(
                        name=param.name, value=base64.b64encode(param.value)
                    )
                else:
                    parameter_data = JobParameterData(
                        name=param.name,
    ...
    def parse_response_body(self, body_raw):
        ...
        internal_length = unpack(">i", body_raw[0:4])[
            0
        ]  # read byte length of internal parameters
    ...
    def parse_parameters(self, params_raw):
        ...
        for i in range(0, params_count):
            ...
            name_offset = unpack(
                ">i", params_raw[descriptionOffset : descriptionOffset + 4]
            )[0]
            param_type = unpack(
                ">i", params_raw[descriptionOffset + 4 : descriptionOffset + 8]
            )[0]
            value_offset = unpack(
                ">i", params_raw[descriptionOffset + 8 : descriptionOffset + 12]
            )[0]
            ...
            if i == params_count - 1:
                ...
                value_offset_end = (
                    unpack(
                        ">i",
                        params_raw[descriptionOffset + 12 : descriptionOffset + 16],
                    )[0]
                    - 1
                )
