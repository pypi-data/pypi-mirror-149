            ...
            if transform_img_width >= slot_width and transform_img_height >= slot_height:  # 槽的上下左右都能被满足
                ...
                if retain_x_or_y == "x":
                    ...
                    if left == 0.5 and right == 0.5:  # 图片能放在slot位内 human 会被裁掉
                        min_img_left_ratio = self.__divide(0.5 * img.width, interest_left + interest_width / 2)
                        min_img_right_ratio = self.__divide(0.5 * img.width, slot_width-interest_left-interest_width/2)
                        ...
                        min_human_left_ratio = self.__divide(interest_left, left * img.width)

                        min_human_right_ratio = self.__divide(slot_width-interest_left-interest_width, (1 - right)*img.width)
