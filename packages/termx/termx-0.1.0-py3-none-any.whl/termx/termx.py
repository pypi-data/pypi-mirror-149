...
def log(msg, style=None, header=None):
    ...
    if style == 'success':
        print(header[0] * header[1] + Fore.GREEN + str(msg) + Fore.RESET)
    elif style == 'error':
        print(header[0] * header[1] + Fore.RED + str(msg) + Fore.RESET)
    else:
        print(header[0] * header[1] + str(msg))
...
def add_command(command, arguments=0, usage=None, cmdinfo=None, action=None):
    if usage is None:
        usage = ""
    if cmdinfo is None:
        cmdinfo = ""
...
def cmd_result(command, input_):
    ...
    if not input_:
        return False, "[-] No input was given."
    if arg_amnt > 0:
        input_ = input_.split()
        cmd = input_[0]
        del input_[0]
