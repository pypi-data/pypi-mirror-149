...
class Regressor(LearnersPool):
    ...
    def score(self, metric: str, y_true: np.ndarray, y_pred: np.ndarray):
        dist_value = self._call_function(module=metrics, name=metric, y_true=y_true, y_pred=y_pred)
        return dist_value, np.exp(- dist_value / np.std(dist_value))
