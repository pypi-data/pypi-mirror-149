...
def bake(host=host_, port=port_, user=user_, pasw=pasw_, db=db_, cursor=curs_, mode=mode_, file=None):
    global host_, port_, user_, pasw_, db_, curs_, mode_, connection_
    if file is None:
        if host != "": host_=host
        if port != "": port_=port
        if user != "": user_=user
    ...
    connection_ = conn.cursor(pymysql.cursors.DictCursor) if curs_=="DICTIONARY" else conn.cursor()

# A query.
def query(query):
    conn_object = connection_
