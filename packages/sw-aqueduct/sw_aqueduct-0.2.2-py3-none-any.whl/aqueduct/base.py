...
class Base(metaclass=LoggingBase):
    ...
    def add_to_diff_log(self, name, type, subcomponent=None, value=''):
        if type in self.__diff_types:
            ...
            if subcomponent:
                getattr(parent, f'_{component}__logger').info(f"Dry Run: Component '{component}' named '{name}' with subcomponent '{subcomponent}' with value of '{value}' would have been {type} on destination.")
            else:
