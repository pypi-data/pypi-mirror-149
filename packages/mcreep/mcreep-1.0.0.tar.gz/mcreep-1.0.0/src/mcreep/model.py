...
class Model:
    ...
    Model for fitting creep data, containing namely:
        * fitting function (s function selected from mcreep.func)
        * methods to run the fitting (employing mcreep.io, mcreep.fit)
    ...
    * table_of_results_evp : pandas.DataFrame
        ...
        if self.fname == 'evp_s_d_1kv':
            func_with_fixed_constants = \
                lambda t,B0,Cv,D1,tau1 : \
                    func(t,self.EPAR.const,B0,Cv,D1,tau1)
        elif self.fname == 'evp_s_d_2kv':
            func_with_fixed_constants = \
    ...
    def describe(self, fh=sys.stdout):
        ...
        if self.fname == 'power_law':
            print('Model function: Power Law => deformation(t) = C * t**k')
            print('Units are relative; k = creep constant ~ creep rate.')
    ...
    def run(self, datafile, t_start, t_hold, t_fstart=None, t_fend=None):
        ...
        * we note that the printing data and saving them to file
        ...
        output_graph = Path(self.output_dir, output_filename)
        # (b) Create the graph
        self.plot_fitting_result(data, par, output_graph)
    ...
    def plot_fitting_result(self, data, par, output_graph):
        '''
        Save the results of creep data fitting with given model.
        (just a wrapper for function mcreep.io.print_fitting_result).
        '''
        mcreep.io.plot_result(self, data, par, output_graph)
        ...
        * self.table_of_results_evp = recalculated results for EVP models
        ...
        # (B) EVP models
            ...
            if self.fname == 'evp_s_d_1kv':
                ...
                if rtimes: D1 = par[-1:]; tau1=rtimes
                else: D1,tau1 = par[-2:]
