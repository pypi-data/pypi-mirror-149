...
#from local_workflow_engine.metrics_summary import TimeWindowSummary as Summary
from local_workflow_engine.metrics_summary import BasicSummary as Summary

from local_workflow_engine.state_engine_paths import (
    apply_path,
    apply_jsonpath,
    get_full_jsonpath,
    apply_resultpath,
...
from local_workflow_engine.open_tracing_factory import span_context, inject_span
from local_workflow_engine.store import (
    create_ASL_store,
    create_executions_store,
    create_history_store,
)
from local_workflow_engine.task_dispatcher import TaskDispatcher

from local_workflow_engine.asl_exceptions import *
from local_workflow_engine.arn import *
...
class StateEngine(object):
    def __init__(self, config):
        """
        """
        self.logger = init_logging(log_name="local_workflow_engine")
    ...
    def broadcast_notification(self, execution_arn, execution_detail, context):
        ...
        local_workflow_engine/<state_machine_arn>.<status>

        That is to say the local_workflow_engine topic exchange with a subject
        comprising the state_machine_arn and the status separated by a dot. e.g. 
        local_workflow_engine/arn:aws:states:local:0123456789:stateMachine:simple_state_machine.SUCCEEDED
        ...
        local_workflow_engine/arn:aws:states:local:0123456789:stateMachine:simple_state_machine.*
    ...
    def end_execution(self, state_machine_type, state_type, event):
        ...
        with opentracing.tracer.start_active_span(
            ...
            if execution_failed:
                ...
                self.logger=init_logging(log_name="asl_execution_history")
                self.logger.info(self.execution_history)
                if self.execution_metrics:
