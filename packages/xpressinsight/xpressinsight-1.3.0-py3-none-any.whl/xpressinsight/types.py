...
import pandas as pd
import numpy as np
from typing import Type, Dict, Union, Tuple, List, Any, Optional, get_type_hints
import re
...
class Param(ScalarBase):
    """
    Represents a *parameter* entity. Parameters can be used to configure an Xpress Insight app. When parameters are
    declared, their name, data type, and default value must be specified. Parameters are typically read-only.
...
class Index(Entity):
    """
    Represents an *index* entity. To be used in conjunction with `xpressinsight.Series` or `xpressinsight.DataFrame`
    objects.
