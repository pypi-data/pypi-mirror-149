...
def mi_request_metadata(ip: str, port: int, usr: str, pwd: str, program: str) -> requests.models.Request:
    url = f'http://{ip}:{port}/m3api-rest/metadata/{program}'
    request = mi_request(url, usr, pwd)

    return request

def mi_request_programs(ip: str, port: int, usr: str, pwd: str) -> requests.models.Request:
    url = f'http://{ip}:{port}/m3api-rest/metadata'
    request = mi_request(url, usr, pwd)

    return request

def mi_request_execute(ip: str, port: int, usr: str, pwd:str, program: str, transaction: str, **kwargs) -> requests.models.Response:
    param_str = dict_to_param_str(kwargs)
    url = f'http://{ip}:{port}/m3api-rest/execute/{program}/{transaction}{param_str}'
    request = mi_request(url, usr, pwd)
