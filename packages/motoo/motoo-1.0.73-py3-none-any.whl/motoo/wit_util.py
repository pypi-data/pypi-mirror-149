...
class WitData:
    ...
    def base_calc(self, type_node, begin_days, end_days=0, nan_day_nan=True):
        ...
        def post_process(data):
            ...
            indexed_data = np.concatenate((np.arange(len(data[0])).reshape(1, -1), data))
            ...
            return np.apply_along_axis(insert_nan, axis=0, arr=indexed_data)

        # 몰려있는 상태로 rolling 및 계산
        if type_node.endswith('POINT'):
            data = pd.DataFrame(nan_shifted).shift(-begin_days).values
        elif type_node.endswith('MOMENTUM'):
            first = pd.DataFrame(nan_shifted).shift(-begin_days).values
            last = pd.DataFrame(nan_shifted).shift(-end_days).values
            data = last / first * 100.0 - 100.0
        elif type_node.endswith('SUM'):
            data = pd.DataFrame(nan_shifted).shift(end_days).rolling(window_size).sum().values
        elif type_node.endswith('AVG'):
            data = pd.DataFrame(nan_shifted).shift(end_days).rolling(window_size).mean().values
        elif type_node.endswith('EMA'):
            data = pd.DataFrame(nan_shifted).shift(end_days).ewm(window_size).mean().values
        elif type_node.endswith('CHANGE'):
            data = pd.DataFrame(nan_shifted).shift(end_days).rolling(window_size).std(ddof=0).values
        elif type_node.endswith('MAX'):
            data = pd.DataFrame(nan_shifted).shift(end_days).rolling(window_size).max().values
        elif type_node.endswith('MIN'):
            data = pd.DataFrame(nan_shifted).shift(end_days).rolling(window_size).min().values
        else:
            return None
