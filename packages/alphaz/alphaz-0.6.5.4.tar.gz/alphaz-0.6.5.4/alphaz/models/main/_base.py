...
class AlphaDataclass(AlphaClass):
    def compare(self, other, attrs_list: List[str], attrs_eq: Dict[str, str]) -> bool:
        if not isinstance(other, self.__class__):
            raise AlphaException(f"Cannot compare {self.__class__} object with {other.__class__} object")
        
        for attr in attrs_eq:
            if not attr in attrs_list:
        ...
        for attr in attrs_list:
            if hasattr(self, attr):                
                ...
                custom_eq = None if not attr in attrs_eq else attrs_eq[attr]
