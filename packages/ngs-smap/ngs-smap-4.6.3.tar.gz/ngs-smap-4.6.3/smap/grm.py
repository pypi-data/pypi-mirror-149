...
def create_information_matrix(haplotypes: FrequencyMatrix | CountMatrix,
    ...
    samples_with_unique_col = pd.Series('', index=pd.Index(loci, name='Locus'))
    ...
    for (sample1_name, sample2_name), result in \
        ...
        informative_loci, shared_loci, *per_sample = result
        ...
        number_of_assessed_loci_per_sample_combination.at[sample1_name, sample2_name] = \
            shared_loci.size
        if per_sample and sample1_name != sample2_name:
            for sample_name, unique_per_sample in zip((sample1_name, sample2_name), per_sample):
                if not unique_per_sample.empty:
                    sample_to_add = [sample_name] * len(unique_per_sample)
                    new_col = samples_with_unique_col.loc[unique_per_sample]\
                                                     .str.cat(others=sample_to_add,
                                                              sep=',')
                    samples_with_unique_col.loc[unique_per_sample] = new_col
    ...
    if similarity_coefficient.locus_information_criterion == "Unique":
        samples_with_unique_col = samples_with_unique_col.str.removeprefix(',')

        def non_unique(s):
            non_na = s.dropna()
            non_na = non_na[non_na != '']
            if not non_na.empty:
                return non_na[non_na.duplicated()].values
            return non_na.values
        counts_per_locus['samples_with_unique'] = \
            samples_with_unique_col.str.split(',', expand=True)\
                                   .T.agg(non_unique).str.join(', ')
...
def locus_information_worker(similarity_coefficient: LocusInformationCoefficient,
    ...
    # Capture where unique loci came from
    if similarity_coefficient._locus_information_criterion == "Unique":
        sample1_loci = sample1.smap.haplotypes_with_counts()\
                              .droplevel([REFERENCE_COLUMN_NAME,
                                          HAPLOTYPES_COLUMN_NAME]).index.unique()
        sample2_loci = sample2.smap.haplotypes_with_counts()\
                              .droplevel([REFERENCE_COLUMN_NAME,
                                          HAPLOTYPES_COLUMN_NAME]).index.unique()
        unique_from_sample1 = sample1_loci.intersection(informative_loci)
        unique_from_sample2 = sample2_loci.intersection(informative_loci)
        return informative_loci, shared_loci, unique_from_sample1, unique_from_sample2
