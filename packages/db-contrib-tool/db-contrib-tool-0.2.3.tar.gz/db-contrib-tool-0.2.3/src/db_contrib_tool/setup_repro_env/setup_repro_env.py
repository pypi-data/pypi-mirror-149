...
class EvgURLInfo(NamedTuple):
    ...
    urls: Dict[str, Any] = {}
    evg_version_id: Optional[str] = None
...
class SetupReproEnv(Subcommand):
    ...
    def __init__(
        ...
        self.setup_repo_config = config.SETUP_REPRO_ENV_CONFIG
    ...
    def get_latest_urls(
        ...
        if buildvariant_name is None:
            LOGGER.warning("Unable to find matching build variant", version=version)
            return EvgURLInfo()
    ...
    def get_urls(self, version: str, buildvariant_name: Optional[str] = None) -> EvgURLInfo:
        ...
        if not buildvariant_name:
            ...
            if buildvariant_name is None:
                raise ValueError(
                    f"Unable to find matching build variant for '{major_minor_version}'."
                )
            LOGGER.debug("Found buildvariant.", buildvariant_name=buildvariant_name)
    ...
    def get_buildvariant_name(self, major_minor_version: str) -> Optional[str]:
        """
        Find the build variant to use for collecting artifacts.

        :param major_minor_version: Version to target.
        :return: Build variant to target.
        """
        if self.variant:
            return self.variant

        target = config.DownloadTarget(
            edition=self.edition, platform=self.platform, architecture=self.architecture
        )

        return self.setup_repo_config.find_matching_build_variant(target, major_minor_version)
