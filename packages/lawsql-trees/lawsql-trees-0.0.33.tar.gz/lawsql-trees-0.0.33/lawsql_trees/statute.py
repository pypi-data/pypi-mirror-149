...
def extract_base(folder: Path) -> Iterator[StatuteItem] | NoReturn:
    for detail in folder.glob("**/details.yaml"):
        try:
            ...
            raise Exception(f"Could not extract {detail.parent=}")
...
def extract_content(items: Iterator[StatuteItem]) -> Iterator[dict] | NoReturn:
    for item in items:
        try:
            ...
            raise Exception(f"Could not extract content {item=}")


def extract_statutes_only(folder: Path) -> Iterator[dict]:
    ...
    for content in contents:
        try:
            ...
            raise Exception(f"Could not extract statute content {content=}")


def extract_titles_only(folder: Path) -> Iterator[dict]:
    items = extract_base(folder)
    contents = extract_content(items)
    for content in contents:
        try:
            ...
            raise Exception(f"Could not extract statute title {content=}")
