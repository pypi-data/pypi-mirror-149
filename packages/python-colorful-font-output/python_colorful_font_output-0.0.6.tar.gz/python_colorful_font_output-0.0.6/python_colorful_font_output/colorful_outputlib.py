"""这是python-colorful-font-output-0.0.6库的源码"""
import sys
...
class Help:
    ...
    def ask_the_module_questions(self):
        ...
        if answer == "Run the module's tips":
            ...
            But, you don't run about rgb output, if your computer monitor does not support it. It will print, too.
            ...
            |      |    |  1,   ->(1/0) 1:colorful print; 0: default color print(if it was 0, 3th number please write down, too.
            |      |    | [0.2], -> (all float)If there 1 float, each word's wait time same.
            |      |    | [36]) -> (all int)If there 1 int, each word's color same. And you can write tuples in it.
            ...
            python-colorful-font-output               0.0.6(This version)
            <python-colorful-font-output-0.0.4> add/update items:
            ...
            No.3 Update class 'RichRgbOutput' to output colorful words with R-G-B.(R-G-B is RED-GREEN-BLUE's concentration.
            ...
            No.5 You can in Windows own color output use 3 numbers to print words, color more precision.
            No.6 Add WindowsUpdateModule class to update the module.
            """)
            ...
            python-colorful-font-output   2022.05.05 13:10       0.0.6(This version)
    ...
    def colorful_output(self):
        ...
        if self.colorful_ft == 0:
            ...
            if len(self.list_m) == 1:
                ...
                x = 0
                for i in self.object:
                    if type(self.list_m[0]) == tuple:
                        if len(self.list_m[x]) == 3:
                            print("\033[{};{};{}m{}".format(str(self.list_m[x][0]), str(self.list_m[x][1]),
                                                            str(self.list_m[x][2]), i))
                        elif len(self.list_m[x]) == 2:
                            print("\033[{};{}m{}".format(str(self.list_m[x][0]), str(self.list_m[x][1]), i))
                    else:
