...
class ElementContextManager:
    ...
    def exit(self, elem):
        ...
        if len(self.element_stack):
            self._app = self.element_stack[-1].app

    def add_child(self, elem):
        if len(self.element_stack):
            if elem.app is None:
                elem.set_app(self.element_stack[-1].app)
            self.element_stack[-1].add_child(elem)

        if elem.app is None:
            elem.set_app(self._app)

        if elem.app is None:
            print(f"No application available for element {elem._elem_name}")
...
class AbstractElement:
    ...
    def __init__(self, _elem_name, children=None, **kwargs):
        ...
        self._app = kwargs.get("trame_app")
        ...
        for _name in names:
            ...
            if name in self._py_attr:
                ...
                if isinstance(value, (tuple, list)):
                    if len(value) > 1:
                        self._app.state.setdefault(value[0], value[1])
...
class HtmlElement(AbstractElement):
    ...
    def __init__(self, _elem_name, children=None, **kwargs):
        ...
        if HtmlElement.MODULE and self._app:
            self._app.enable_module(HtmlElement.MODULE)
