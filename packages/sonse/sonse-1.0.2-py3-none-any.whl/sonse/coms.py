...
from sonse.clui import group, Stem


@group.command(short_help="Copy a note.")
@click.argument("stem", metavar="NOTE", type=Stem())
@click.argument("dest", type=Stem())
...
def import_(conf: Conf, stem: str, file: Path, edit: bool):
    ...
    path = jobs.get(conf.dire, conf.ext, stem, create=True)
...
@click.argument("text", default="", type=Stem())
@click.help_option("-h", "--help")
@click.pass_obj
...
@group.command(short_help="Move a note.")
@click.argument("stem", metavar="NOTE", type=Stem())
@click.argument("dest", type=Stem())
@click.help_option("-h", "--help")
@click.pass_obj
