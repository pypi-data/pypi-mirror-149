...
class Conf(NamedTuple):
    """
    A configuration tuple with a notes directory and extension.
    """
...
def get(dire: Path, ext: str, pre: str, *, create: Optional[bool] = False) -> Path:
    """
    Return a path in a directory matching an extension and a stem prefix, or
    raise errors for disambiguation. If 'create' is True and no matches are
    found, create and return a new file with the prefix as its entire name.
    """

    paths = list(fsys.match(dire, ext, pre))
    exact = [path for path in paths if path.stem == pre]
    ...
    if len(exact) == 1:
        ...
        if create:
            return fsys.create(dire, ext, pre)
        else:
            raise ClickException(f"no notes matching {pre}")

    else:
        opts = ", ".join(sorted(f"{path.stem!r}" for path in paths))
