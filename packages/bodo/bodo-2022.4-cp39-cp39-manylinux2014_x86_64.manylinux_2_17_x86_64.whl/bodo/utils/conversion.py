...
def overload_coerce_to_ndarray(data, error_on_nonarray=True,
    ...
    if isinstance(data, (types.List, types.UniTuple)):
        uvp__zjlza = data.dtype
        if isinstance(uvp__zjlza, types.Optional):
            uvp__zjlza = uvp__zjlza.type
            if bodo.utils.typing.is_scalar_type(uvp__zjlza):
                use_nullable_array = True
        if isinstance(uvp__zjlza, (types.Boolean, types.Integer,
            Decimal128Type)) or uvp__zjlza in [bodo.hiframes.
            ...
            elieg__ngss = dtype_to_array_type(uvp__zjlza)
            if not is_overload_none(use_nullable_array):
                elieg__ngss = to_nullable_type(elieg__ngss)

            def impl(data, error_on_nonarray=True, use_nullable_array=None,
                scalar_to_arr_len=None):
                cumk__thdbb = len(data)
                A = bodo.utils.utils.alloc_type(cumk__thdbb, elieg__ngss, (-1,)
                    )
                bodo.utils.utils.tuple_list_to_array(A, data, uvp__zjlza)
                return A
            return impl
        return (lambda data, error_on_nonarray=True, use_nullable_array=
    ...
    if not is_overload_none(scalar_to_arr_len):
        if isinstance(data, Decimal128Type):
            ppe__kcwp = data.precision
            erd__cfpjy = data.scale

            def impl_ts(data, error_on_nonarray=True, use_nullable_array=
                None, scalar_to_arr_len=None):
                cumk__thdbb = scalar_to_arr_len
                A = bodo.libs.decimal_arr_ext.alloc_decimal_array(cumk__thdbb,
                    ppe__kcwp, erd__cfpjy)
                for cgfep__hkae in numba.parfors.parfor.internal_prange(
                    cumk__thdbb):
                    A[cgfep__hkae] = data
                return A
            return impl_ts
        if data == bodo.hiframes.datetime_datetime_ext.datetime_datetime_type:
            etvx__tnil = np.dtype('datetime64[ns]')

            def impl_ts(data, error_on_nonarray=True, use_nullable_array=
                None, scalar_to_arr_len=None):
                cumk__thdbb = scalar_to_arr_len
                A = np.empty(cumk__thdbb, etvx__tnil)
                fpr__bpfb = (bodo.hiframes.pd_timestamp_ext.
                    datetime_datetime_to_dt64(data))
                zfdbz__awj = bodo.hiframes.pd_timestamp_ext.integer_to_dt64(
                    fpr__bpfb)
                for cgfep__hkae in numba.parfors.parfor.internal_prange(
                    cumk__thdbb):
                    A[cgfep__hkae] = zfdbz__awj
                return A
            return impl_ts
        if (data == bodo.hiframes.datetime_timedelta_ext.
            datetime_timedelta_type):
            bbcci__ncqs = np.dtype('timedelta64[ns]')

            def impl_ts(data, error_on_nonarray=True, use_nullable_array=
                None, scalar_to_arr_len=None):
                cumk__thdbb = scalar_to_arr_len
                A = np.empty(cumk__thdbb, bbcci__ncqs)
                wpqd__hwuch = (bodo.hiframes.pd_timestamp_ext.
                    datetime_timedelta_to_timedelta64(data))
                for cgfep__hkae in numba.parfors.parfor.internal_prange(
                    cumk__thdbb):
                    A[cgfep__hkae] = wpqd__hwuch
                return A
            return impl_ts
        ...
        if data == bodo.hiframes.pd_timestamp_ext.pd_timestamp_type:
            ...
            def impl_ts(data, error_on_nonarray=True, use_nullable_array=
                None, scalar_to_arr_len=None):
                cumk__thdbb = scalar_to_arr_len
                A = np.empty(scalar_to_arr_len, etvx__tnil)
                fpr__bpfb = bodo.hiframes.pd_timestamp_ext.integer_to_dt64(data
                    .value)
                for cgfep__hkae in numba.parfors.parfor.internal_prange(
                    cumk__thdbb):
                    A[cgfep__hkae] = fpr__bpfb
                return A
            return impl_ts
        ...
        if not is_overload_none(use_nullable_array) and isinstance(dtype,
            ...
            def impl_null_integer(data, error_on_nonarray=True,
                ...
                cumk__thdbb = scalar_to_arr_len
                ecyc__tvbuo = bodo.libs.int_arr_ext.alloc_int_array(cumk__thdbb
                    , dtype)
                for cgfep__hkae in numba.parfors.parfor.internal_prange(
                    cumk__thdbb):
                    ecyc__tvbuo[cgfep__hkae] = data
                return ecyc__tvbuo
        ...
        if not is_overload_none(use_nullable_array) and dtype == types.bool_:
            ...
            def impl_null_bool(data, error_on_nonarray=True,
                ...
                numba.parfors.parfor.init_prange()
                cumk__thdbb = scalar_to_arr_len
                ecyc__tvbuo = bodo.libs.bool_arr_ext.alloc_bool_array(
                    cumk__thdbb)
                for cgfep__hkae in numba.parfors.parfor.internal_prange(
                    cumk__thdbb):
                    ecyc__tvbuo[cgfep__hkae] = data
        ...
        def impl_num(data, error_on_nonarray=True, use_nullable_array=None,
            ...
            numba.parfors.parfor.init_prange()
            cumk__thdbb = scalar_to_arr_len
            ecyc__tvbuo = np.empty(cumk__thdbb, dtype)
            for cgfep__hkae in numba.parfors.parfor.internal_prange(cumk__thdbb
                ):
                ecyc__tvbuo[cgfep__hkae] = data
            return ecyc__tvbuo
        return impl_num
    if isinstance(data, types.BaseTuple) and all(isinstance(frqwi__cfix, (
        types.Float, types.Integer)) for frqwi__cfix in data.types):
        return (lambda data, error_on_nonarray=True, use_nullable_array=
...
def overload_coerce_to_array(data, error_on_nonarray=True,
    ...
    if isinstance(data, types.UniTuple) and isinstance(data.dtype, (types.
        ...
        ) and all(isinstance(frqwi__cfix, types.StringLiteral) for
        frqwi__cfix in data.types):
        return (lambda data, error_on_nonarray=True, use_nullable_array=
            None, scalar_to_arr_len=None: bodo.libs.str_arr_ext.
            str_arr_from_sequence(data))
    ...
    if isinstance(data, (types.List, types.UniTuple)) and isinstance(data.
        ...
        rmxnw__gyl = tuple(dtype_to_array_type(frqwi__cfix) for frqwi__cfix in
            data.dtype.types)

        def impl_tuple_list(data, error_on_nonarray=True,
            use_nullable_array=None, scalar_to_arr_len=None):
            cumk__thdbb = len(data)
            arr = bodo.libs.tuple_arr_ext.pre_alloc_tuple_array(cumk__thdbb,
                (-1,), rmxnw__gyl)
            for cgfep__hkae in range(cumk__thdbb):
                arr[cgfep__hkae] = data[cgfep__hkae]
            return arr
    ...
    if isinstance(data, types.List) and (bodo.utils.utils.is_array_typ(data
        ...
        ahm__opyvj = dtype_to_array_type(data.dtype.dtype)

        def impl_array_item_arr(data, error_on_nonarray=True,
            use_nullable_array=None, scalar_to_arr_len=None):
            cumk__thdbb = len(data)
            nbtar__lqmw = init_nested_counts(ahm__opyvj)
            for cgfep__hkae in range(cumk__thdbb):
                gkvxs__gceyn = bodo.utils.conversion.coerce_to_array(data[
                    cgfep__hkae], use_nullable_array=True)
                nbtar__lqmw = add_nested_counts(nbtar__lqmw, gkvxs__gceyn)
            ecyc__tvbuo = (bodo.libs.array_item_arr_ext.
                pre_alloc_array_item_array(cumk__thdbb, nbtar__lqmw,
                ahm__opyvj))
            jfkp__zmcah = bodo.libs.array_item_arr_ext.get_null_bitmap(
                ecyc__tvbuo)
            for cej__xbroy in range(cumk__thdbb):
                gkvxs__gceyn = bodo.utils.conversion.coerce_to_array(data[
                    cej__xbroy], use_nullable_array=True)
                ecyc__tvbuo[cej__xbroy] = gkvxs__gceyn
                bodo.libs.int_arr_ext.set_bit_to_arr(jfkp__zmcah, cej__xbroy, 1
                    )
            return ecyc__tvbuo
    ...
    if not is_overload_none(scalar_to_arr_len) and isinstance(data, (types.
        ...
        def impl_str(data, error_on_nonarray=True, use_nullable_array=None,
            scalar_to_arr_len=None):
            cumk__thdbb = scalar_to_arr_len
            A = bodo.libs.str_arr_ext.pre_alloc_string_array(cumk__thdbb, -1)
            for cgfep__hkae in numba.parfors.parfor.internal_prange(cumk__thdbb
                ):
                A[cgfep__hkae] = data
            return A
    ...
    if isinstance(data, types.List) and isinstance(data.dtype, bodo.
        ...
        def impl_list_timestamp(data, error_on_nonarray=True,
            use_nullable_array=None, scalar_to_arr_len=None):
            cumk__thdbb = len(data)
            A = np.empty(cumk__thdbb, np.dtype('datetime64[ns]'))
            for cgfep__hkae in range(cumk__thdbb):
                A[cgfep__hkae
    ...
    if isinstance(data, types.List) and data.dtype == bodo.pd_timedelta_type:
        ...
        def impl_list_timedelta(data, error_on_nonarray=True,
            use_nullable_array=None, scalar_to_arr_len=None):
            cumk__thdbb = len(data)
            A = np.empty(cumk__thdbb, np.dtype('timedelta64[ns]'))
            for cgfep__hkae in range(cumk__thdbb):
                A[cgfep__hkae
                    ] = bodo.hiframes.pd_timestamp_ext.integer_to_timedelta64(
                    data[cgfep__hkae].value)
            return A
    ...
    if not is_overload_none(scalar_to_arr_len) and data in [bodo.
        ...
        ntcn__kyxf = ('datetime64[ns]' if data == bodo.pd_timestamp_type else
            'timedelta64[ns]')

        def impl_timestamp(data, error_on_nonarray=True, use_nullable_array
            =None, scalar_to_arr_len=None):
            cumk__thdbb = scalar_to_arr_len
            A = np.empty(cumk__thdbb, ntcn__kyxf)
...
def overload_fix_arr_dtype(data, new_dtype, copy=None, nan_to_str=True,
    ...
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(data,
        'fix_arr_dtype()')
    myqk__qrnoj = is_overload_true(copy)
    auklq__qijmh = is_overload_constant_str(new_dtype
        ) and get_overload_const_str(new_dtype) == 'object'
    if is_overload_none(new_dtype) or auklq__qijmh:
        if myqk__qrnoj:
    ...
    if isinstance(data, NullableTupleType):
        ...
        cwtyt__qre = {types.unicode_type: '', boolean_dtype: False, types.
        ...
        ers__jlcw = {types.unicode_type: str, types.bool_: bool,
            ...
            pd.to_timedelta}
        nqzw__nya = cwtyt__qre.keys()
        obmdr__udmwk = list(data._tuple_typ.types)
        if nb_dtype not in nqzw__nya:
            raise BodoError(f'type conversion to {nb_dtype} types unsupported.'
                )
        for kylf__utzjj in obmdr__udmwk:
            if kylf__utzjj == bodo.datetime64ns:
                if nb_dtype not in (types.unicode_type, types.int64, types.
                    uint64, bodo.datetime64ns):
                    raise BodoError(
                        f'invalid type conversion from {kylf__utzjj} to {nb_dtype}.'
                        )
            elif kylf__utzjj == bodo.timedelta64ns:
                if nb_dtype not in (types.unicode_type, types.int64, types.
        ...
        gnj__cwp += '  data_tup = data._data\n'
        gnj__cwp += '  null_tup = data._null_values\n'
        for cgfep__hkae in range(len(obmdr__udmwk)):
            gnj__cwp += f'  val_{cgfep__hkae} = convert_func(default_value)\n'
            gnj__cwp += f'  if not null_tup[{cgfep__hkae}]:\n'
            gnj__cwp += (
                f'    val_{cgfep__hkae} = convert_func(data_tup[{cgfep__hkae}])\n'
                )
        divfv__sdbd = ', '.join(f'val_{cgfep__hkae}' for cgfep__hkae in
            range(len(obmdr__udmwk)))
        gnj__cwp += f'  vals_tup = ({divfv__sdbd},)\n'
        gnj__cwp += """  res_tup = bodo.libs.nullable_tuple_ext.build_nullable_tuple(vals_tup, null_tup)
"""
        gnj__cwp += '  return res_tup\n'
        odni__knb = {}
        hyla__xrxm = ers__jlcw[nb_dtype]
        nrm__wkjwi = cwtyt__qre[nb_dtype]
        exec(gnj__cwp, {'bodo': bodo, 'np': np, 'pd': pd, 'default_value':
            nrm__wkjwi, 'convert_func': hyla__xrxm}, odni__knb)
        impl = odni__knb['impl']
        return impl
    if _is_str_dtype(new_dtype):
        if isinstance(data.dtype, types.Integer):
            ...
            def impl_int_str(data, new_dtype, copy=None, nan_to_str=True,
                ...
                numba.parfors.parfor.init_prange()
                cumk__thdbb = len(data)
                A = bodo.libs.str_arr_ext.pre_alloc_string_array(cumk__thdbb,
                    -1)
                for lfxxn__iaib in numba.parfors.parfor.internal_prange(
                    cumk__thdbb):
                    if bodo.libs.array_kernels.isna(data, lfxxn__iaib):
                        if nan_to_str:
                            bodo.libs.str_arr_ext.str_arr_setitem_NA_str(A,
                                lfxxn__iaib)
                        else:
                            bodo.libs.array_kernels.setna(A, lfxxn__iaib)
                    else:
                        bodo.libs.str_arr_ext.str_arr_setitem_int_to_str(A,
                            lfxxn__iaib, data[lfxxn__iaib])
                return A
            return impl_int_str
            ...
            def impl_binary(data, new_dtype, copy=None, nan_to_str=True,
                ...
                for lfxxn__iaib in numba.parfors.parfor.internal_prange(
                    ...
                    if bodo.libs.array_kernels.isna(data, lfxxn__iaib):
                        ...
                        A[lfxxn__iaib] = ''.join([chr(vuygk__smeg) for
                            vuygk__smeg in data[lfxxn__iaib]])
                return A
            return impl_binary
                ...
                for lfxxn__iaib in numba.parfors.parfor.internal_prange(
                    ...
                    if bodo.libs.array_kernels.isna(data, lfxxn__iaib):
                        if nan_to_str:
                            ...
                            bodo.libs.array_kernels.setna(A, lfxxn__iaib)
                        continue
                    A[lfxxn__iaib] = str(box_if_dt64(data[lfxxn__iaib]))
                return A
                ...
                for lfxxn__iaib in numba.parfors.parfor.internal_prange(
                    ...
                    if bodo.libs.array_kernels.isna(data, lfxxn__iaib):
                        if nan_to_str:
                            ...
                            bodo.libs.array_kernels.setna(A, lfxxn__iaib)
                        continue
                    A[lfxxn__iaib] = str(data[lfxxn__iaib])
                return A
    ...
    if isinstance(new_dtype, bodo.hiframes.pd_categorical_ext.
        ...
        def impl_cat_dtype(data, new_dtype, copy=None, nan_to_str=True,
            ...
            cumk__thdbb = len(data)
            numba.parfors.parfor.init_prange()
            lsdh__eakn = (bodo.hiframes.pd_categorical_ext.
                get_label_dict_from_categories(new_dtype.categories.values))
            A = bodo.hiframes.pd_categorical_ext.alloc_categorical_array(
                cumk__thdbb, new_dtype)
            prmvu__ttzs = (bodo.hiframes.pd_categorical_ext.
                get_categorical_arr_codes(A))
            for cgfep__hkae in numba.parfors.parfor.internal_prange(cumk__thdbb
                ):
                if bodo.libs.array_kernels.isna(data, cgfep__hkae):
                    bodo.libs.array_kernels.setna(A, cgfep__hkae)
                    continue
                val = data[cgfep__hkae]
                if val not in lsdh__eakn:
                    bodo.libs.array_kernels.setna(A, cgfep__hkae)
                    continue
                prmvu__ttzs[cgfep__hkae] = lsdh__eakn[val]
            return A
    ...
    if is_overload_constant_str(new_dtype) and get_overload_const_str(new_dtype
        ...
        def impl_category(data, new_dtype, copy=None, nan_to_str=True,
            from_series=False):
            vbj__mbt = bodo.libs.array_kernels.unique(data, dropna=True)
            vbj__mbt = pd.Series(vbj__mbt).sort_values().values
            vbj__mbt = bodo.allgatherv(vbj__mbt, False)
            xpp__yjwdd = bodo.hiframes.pd_categorical_ext.init_cat_dtype(bodo
                .utils.conversion.index_from_array(vbj__mbt, None), False,
                None, None)
            cumk__thdbb = len(data)
            numba.parfors.parfor.init_prange()
            lsdh__eakn = (bodo.hiframes.pd_categorical_ext.
                get_label_dict_from_categories_no_duplicates(vbj__mbt))
            A = bodo.hiframes.pd_categorical_ext.alloc_categorical_array(
                cumk__thdbb, xpp__yjwdd)
            prmvu__ttzs = (bodo.hiframes.pd_categorical_ext.
                get_categorical_arr_codes(A))
            for cgfep__hkae in numba.parfors.parfor.internal_prange(cumk__thdbb
                ):
    ...
    nb_dtype = bodo.utils.typing.parse_dtype(new_dtype)
    if isinstance(data, bodo.libs.int_arr_ext.IntegerArrayType):
        tntp__fbkq = isinstance(nb_dtype, bodo.libs.int_arr_ext.IntDtype
            ) and data.dtype == nb_dtype.dtype
    else:
        tntp__fbkq = data.dtype == nb_dtype
    if myqk__qrnoj and tntp__fbkq:
        return (lambda data, new_dtype, copy=None, nan_to_str=True,
            from_series=False: data.copy())
    if tntp__fbkq:
        return (lambda data, new_dtype, copy=None, nan_to_str=True,
    ...
    if isinstance(nb_dtype, bodo.libs.int_arr_ext.IntDtype):
        ...
        if isinstance(data.dtype, types.Float):
            ...
            def impl_float(data, new_dtype, copy=None, nan_to_str=True,
                ...
                cumk__thdbb = len(data)
                numba.parfors.parfor.init_prange()
                zsd__ynsu = bodo.libs.int_arr_ext.alloc_int_array(cumk__thdbb,
                    ntcn__kyxf)
                for cgfep__hkae in numba.parfors.parfor.internal_prange(
                    cumk__thdbb):
                    if bodo.libs.array_kernels.isna(data, cgfep__hkae):
                        bodo.libs.array_kernels.setna(zsd__ynsu, cgfep__hkae)
                    else:
                        zsd__ynsu[cgfep__hkae] = int(data[cgfep__hkae])
                return zsd__ynsu
            return impl_float
        else:
            ...
            def impl(data, new_dtype, copy=None, nan_to_str=True,
                ...
                for cgfep__hkae in numba.parfors.parfor.internal_prange(
                    ...
                    if bodo.libs.array_kernels.isna(data, cgfep__hkae):
                        ...
                        zsd__ynsu[cgfep__hkae] = np.int64(data[cgfep__hkae])
                return zsd__ynsu
            return impl
    ...
    if nb_dtype == bodo.libs.bool_arr_ext.boolean_dtype:
        ...
        def impl_bool(data, new_dtype, copy=None, nan_to_str=True,
            ...
            cumk__thdbb = len(data)
            numba.parfors.parfor.init_prange()
            zsd__ynsu = bodo.libs.bool_arr_ext.alloc_bool_array(cumk__thdbb)
            for cgfep__hkae in numba.parfors.parfor.internal_prange(cumk__thdbb
                ):
                if bodo.libs.array_kernels.isna(data, cgfep__hkae):
                    bodo.libs.array_kernels.setna(zsd__ynsu, cgfep__hkae)
                else:
                    zsd__ynsu[cgfep__hkae] = bool(data[cgfep__hkae])
            return zsd__ynsu
        return impl_bool
    if nb_dtype == bodo.datetime_date_type:
        if data.dtype == bodo.datetime64ns:
            ...
            def impl_date(data, new_dtype, copy=None, nan_to_str=True,
                from_series=False):
                cumk__thdbb = len(data)
                ecyc__tvbuo = (bodo.hiframes.datetime_date_ext.
                    alloc_datetime_date_array(cumk__thdbb))
                for cgfep__hkae in numba.parfors.parfor.internal_prange(
                    cumk__thdbb):
                    if bodo.libs.array_kernels.isna(data, cgfep__hkae):
                        bodo.libs.array_kernels.setna(ecyc__tvbuo, cgfep__hkae)
                    else:
                        ecyc__tvbuo[cgfep__hkae
    ...
    if nb_dtype == bodo.datetime64ns:
        ...
        if isinstance(data.dtype, types.Number) or data.dtype in [bodo.
            ...
            def impl_numeric(data, new_dtype, copy=None, nan_to_str=True,
                ...
                cumk__thdbb = len(data)
                numba.parfors.parfor.init_prange()
                ecyc__tvbuo = np.empty(cumk__thdbb, dtype=np.dtype(
                    'datetime64[ns]'))
                for cgfep__hkae in numba.parfors.parfor.internal_prange(
                    cumk__thdbb):
                    if bodo.libs.array_kernels.isna(data, cgfep__hkae):
            ...
            if myqk__qrnoj:
                ...
                def impl_numeric(data, new_dtype, copy=None, nan_to_str=
                    ...
                    for cgfep__hkae in numba.parfors.parfor.internal_prange(
                        cumk__thdbb):
                        if bodo.libs.array_kernels.isna(data, cgfep__hkae):
                            bodo.libs.array_kernels.setna(ecyc__tvbuo,
                                cgfep__hkae)
                        else:
                            ecyc__tvbuo[cgfep__hkae] = (bodo.hiframes.
                                pd_timestamp_ext.integer_to_timedelta64(np.
                                int64(data[cgfep__hkae])))
                    return ecyc__tvbuo
                return impl_numeric
            else:
                return (lambda data, new_dtype, copy=None, nan_to_str=True,
    ...
    if nb_dtype == types.int64 and data.dtype in [bodo.datetime64ns, bodo.
        ...
        def impl_datelike_to_integer(data, new_dtype, copy=None, nan_to_str
            ...
            cumk__thdbb = len(data)
            numba.parfors.parfor.init_prange()
            A = np.empty(cumk__thdbb, types.int64)
            for cgfep__hkae in numba.parfors.parfor.internal_prange(cumk__thdbb
                ):
                if bodo.libs.array_kernels.isna(data, cgfep__hkae):
                    bodo.libs.array_kernels.setna(A, cgfep__hkae)
                else:
                    A[cgfep__hkae] = np.int64(data[cgfep__hkae])
            return A
...
def flatten_array(A):
    ...
    cumk__thdbb = len(A)
    for cgfep__hkae in range(cumk__thdbb):
        hymc__eqlu = A[cgfep__hkae]
        for kjmwq__qbpz in hymc__eqlu:
            qqpj__gpy.append(kjmwq__qbpz)
    return bodo.utils.conversion.coerce_to_array(qqpj__gpy)
...
def overload_parse_datetimes_from_strings(data):
    ...
    def parse_impl(data):
        numba.parfors.parfor.init_prange()
        cumk__thdbb = len(data)
        pbuw__mhjb = np.empty(cumk__thdbb, bodo.utils.conversion.NS_DTYPE)
        for cgfep__hkae in numba.parfors.parfor.internal_prange(cumk__thdbb):
            pbuw__mhjb[cgfep__hkae
...
def overload_convert_to_index(data, name=None):
    ...
    def impl(data, name=None):
        mpobz__vqia = bodo.utils.conversion.coerce_to_array(data)
        return bodo.utils.conversion.index_from_array(mpobz__vqia, name)
    return impl
    ...
    if val == types.Optional(bodo.hiframes.pd_timestamp_ext.pd_timestamp_type):
        ...
        def impl_optional(val):
            if val is None:
                ...
                scuv__kgfsf = bodo.hiframes.pd_timestamp_ext.integer_to_dt64(
    ...
    if val == types.Optional(bodo.hiframes.datetime_timedelta_ext.
        ...
        def impl_optional_td(val):
            if val is None:
                ...
                scuv__kgfsf = (bodo.hiframes.pd_timestamp_ext.
...
def overload_to_tuple(val):
    if not isinstance(val, types.BaseTuple) and is_overload_constant_list(val):
        gqvr__sekh = len(val.types if isinstance(val, types.LiteralList) else
            get_overload_const_list(val))
        gnj__cwp = 'def f(val):\n'
        nmt__clgkw = ','.join(f'val[{cgfep__hkae}]' for cgfep__hkae in
            range(gqvr__sekh))
        gnj__cwp += f'  return ({nmt__clgkw},)\n'
        odni__knb = {}
        exec(gnj__cwp, {}, odni__knb)
        impl = odni__knb['f']
        return impl
...
def overload_get_array_if_series_or_index(data):
    ...
    if isinstance(data, bodo.hiframes.pd_index_ext.HeterogeneousIndexType):
        if not is_heterogeneous_tuple_type(data.data):
            ...
            def impl(data):
                etxj__qgj = bodo.hiframes.pd_index_ext.get_index_data(data)
                return bodo.utils.conversion.coerce_to_array(etxj__qgj)
            return impl
...
def overload_extract_index_array(A):
    ...
    if isinstance(A, SeriesType):
        ...
        def impl(A):
            ...
            ktcb__etx = bodo.utils.conversion.coerce_to_array(index)
            return ktcb__etx
        return impl
...
def struct_if_heter_dict(values, names):
    return {ixyi__znhls: fpr__bpfb for ixyi__znhls, fpr__bpfb in zip(names,
        values)}
...
def overload_struct_if_heter_dict(values, names):
    if not types.is_homogeneous(*values.types):
        return lambda values, names: bodo.libs.struct_arr_ext.init_struct(
            values, names)
    pozle__xemkk = len(values.types)
    gnj__cwp = 'def f(values, names):\n'
    nmt__clgkw = ','.join("'{}': values[{}]".format(get_overload_const_str(
        names.types[cgfep__hkae]), cgfep__hkae) for cgfep__hkae in range(
        pozle__xemkk))
    gnj__cwp += '  return {{{}}}\n'.format(nmt__clgkw)
    odni__knb = {}
    exec(gnj__cwp, {}, odni__knb)
