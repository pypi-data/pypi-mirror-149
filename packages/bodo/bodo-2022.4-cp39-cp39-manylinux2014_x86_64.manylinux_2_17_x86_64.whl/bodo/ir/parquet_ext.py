...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    aiv__xhere = pq_node.out_vars[0].name
    pnla__gtes = pq_node.out_vars[1].name
    if aiv__xhere not in lives and pnla__gtes not in lives:
        return None
    elif aiv__xhere not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.type_usecol_offset = []
    elif pnla__gtes not in lives:
