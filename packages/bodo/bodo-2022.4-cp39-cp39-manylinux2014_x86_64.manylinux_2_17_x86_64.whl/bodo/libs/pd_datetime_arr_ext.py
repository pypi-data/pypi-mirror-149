...
def get_pytz_type_info(pytz_type):
    if isinstance(pytz_type, pytz._FixedOffset):
        phhvt__vpipr = pd.Timedelta(pytz_type._offset).value
    else:
        phhvt__vpipr = pytz_type.zone
        if phhvt__vpipr not in pytz.all_timezones_set:
            raise BodoError(
...
def nanoseconds_to_offset(nanoseconds):
    kycn__efsk = nanoseconds // (60 * 1000 * 1000 * 1000)
    return pytz.FixedOffset(kycn__efsk)
...
class PandasDatetimeArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        jwhs__sgkfw = [('data', fe_type.data_array_type)]
        models.StructModel.__init__(self, dmm, fe_type, jwhs__sgkfw)
...
def unbox_pd_datetime_array(typ, val, c):
    ifsjd__tkr = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    pfg__jtlj = c.pyapi.string_from_constant_string('datetime64[ns]')
    rhfmk__kxlt = c.pyapi.call_method(val, 'to_numpy', (pfg__jtlj,))
    ifsjd__tkr.data = c.unbox(typ.data_array_type, rhfmk__kxlt).value
    c.pyapi.decref(rhfmk__kxlt)
    jyugr__hefbz = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(ifsjd__tkr._getvalue(), is_error=jyugr__hefbz)
...
def box_pd_datetime_array(typ, val, c):
    ifsjd__tkr = cgutils.create_struct_proxy(typ)(c.context, c.builder,
        value=val)
    c.context.nrt.incref(c.builder, typ.data_array_type, ifsjd__tkr.data)
    wuhgt__pshy = c.pyapi.from_native_value(typ.data_array_type, ifsjd__tkr
        .data, c.env_manager)
    baj__nvyl = c.context.get_constant_generic(c.builder, types.
        unicode_type, 'ns')
    jtso__edh = c.pyapi.from_native_value(types.unicode_type, baj__nvyl, c.
        env_manager)
    if isinstance(typ.tz, str):
        ffja__kfzip = c.context.get_constant_generic(c.builder, types.
            unicode_type, typ.tz)
        hjgzn__ilmb = c.pyapi.from_native_value(types.unicode_type,
            ffja__kfzip, c.env_manager)
    else:
        yhjv__yiqdy = nanoseconds_to_offset(typ.tz)
        hjgzn__ilmb = c.pyapi.unserialize(c.pyapi.serialize_object(yhjv__yiqdy)
            )
    qigac__usr = c.context.insert_const_string(c.builder.module, 'pandas')
    jbwhx__bxlkt = c.pyapi.import_module_noblock(qigac__usr)
    dcs__abut = c.pyapi.call_method(jbwhx__bxlkt, 'DatetimeTZDtype', (
        jtso__edh, hjgzn__ilmb))
    utvpk__fua = c.pyapi.object_getattr_string(jbwhx__bxlkt, 'arrays')
    cwwm__ufq = c.pyapi.call_method(utvpk__fua, 'DatetimeArray', (
        wuhgt__pshy, dcs__abut))
    c.pyapi.decref(wuhgt__pshy)
    c.pyapi.decref(jtso__edh)
    c.pyapi.decref(hjgzn__ilmb)
    c.pyapi.decref(jbwhx__bxlkt)
    c.pyapi.decref(dcs__abut)
    c.pyapi.decref(utvpk__fua)
...
def init_pandas_datetime_array(typingctx, data, tz):
    ...
    def codegen(context, builder, sig, args):
        data, tz = args
        awq__sugmn = cgutils.create_struct_proxy(sig.return_type)(context,
            builder)
        awq__sugmn.data = data
        context.nrt.incref(builder, sig.args[0], data)
        return awq__sugmn._getvalue()
    if is_overload_constant_str(tz) or is_overload_constant_int(tz):
        ffja__kfzip = get_literal_value(tz)
    else:
        raise BodoError('tz must be a constant string or Fixed Offset')
    pzcx__cexu = DatetimeArrayType(ffja__kfzip)
    sig = pzcx__cexu(pzcx__cexu.data_array_type, tz)
    return sig, codegen
...
def lower_constant_pd_datetime_arr(context, builder, typ, pyval):
    gntpw__jfiv = context.get_constant_generic(builder, typ.data_array_type,
        pyval.to_numpy('datetime64[ns]'))
    sowf__xdoy = lir.Constant.literal_struct([gntpw__jfiv])
    return sowf__xdoy
...
def overload_getitem(A, ind):
    ...
    if is_list_like_index_type(ind) and ind.dtype == types.bool_:
        ...
        def impl_bool(A, ind):
            ...
            ikonl__rbxll = ensure_contig_if_np(A._data[ind])
            return init_pandas_datetime_array(ikonl__rbxll, tz)
        return impl_bool
...
def unwrap_tz_array_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    ympi__sqw = args[0]
    if equiv_set.has_shape(ympi__sqw):
        return ArrayAnalysis.AnalyzeResult(shape=ympi__sqw, pre=[])
    return None
