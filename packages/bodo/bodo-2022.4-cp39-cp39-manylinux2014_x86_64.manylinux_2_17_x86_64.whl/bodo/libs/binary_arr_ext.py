...
def init_binary_arr(typingctx, data_typ=None):
    ...
    def codegen(context, builder, sig, args):
        rem__foxy, = args
        pmjo__jkxn = context.make_helper(builder, binary_array_type)
        pmjo__jkxn.data = rem__foxy
        context.nrt.incref(builder, data_typ, rem__foxy)
        return pmjo__jkxn._getvalue()
...
def init_bytes_type(typingctx, data_typ, length_type):
    ...
    def codegen(context, builder, sig, args):
        nbkd__eejdr = cgutils.create_struct_proxy(sig.args[0])(context,
            builder, value=args[0])
        inw__twej = args[1]
        kfdam__dmu = cgutils.create_struct_proxy(bytes_type)(context, builder)
        kfdam__dmu.meminfo = context.nrt.meminfo_alloc(builder, inw__twej)
        kfdam__dmu.nitems = inw__twej
        kfdam__dmu.itemsize = lir.Constant(kfdam__dmu.itemsize.type, 1)
        kfdam__dmu.data = context.nrt.meminfo_data(builder, kfdam__dmu.meminfo)
        kfdam__dmu.parent = cgutils.get_null_value(kfdam__dmu.parent.type)
        kfdam__dmu.shape = cgutils.pack_array(builder, [inw__twej], context
            .get_value_type(types.intp))
        kfdam__dmu.strides = nbkd__eejdr.strides
        cgutils.memcpy(builder, kfdam__dmu.data, nbkd__eejdr.data, inw__twej)
        return kfdam__dmu._getvalue()
...
def binary_arr_hex(arr):
    cob__bcdm = numba.cpython.unicode.PY_UNICODE_1BYTE_KIND

    def impl(arr):
        inw__twej = len(arr) * 2
        output = numba.cpython.unicode._empty_string(cob__bcdm, inw__twej, 1)
...
def bytes_to_hex(typingctx, output, arr):
    ...
    def codegen(context, builder, sig, args):
        rnhn__qzfk = cgutils.create_struct_proxy(sig.args[0])(context,
            builder, value=args[0])
        vwwba__ssbae = cgutils.create_struct_proxy(sig.args[1])(context,
            builder, value=args[1])
        yxi__cpie = lir.FunctionType(lir.IntType(8).as_pointer(), [lir.
            IntType(8).as_pointer(), lir.IntType(8).as_pointer(), lir.
            IntType(64)])
        wbuwq__jsvg = cgutils.get_or_insert_function(builder.module,
            yxi__cpie, name='bytes_to_hex')
        builder.call(wbuwq__jsvg, (rnhn__qzfk.data, vwwba__ssbae.data,
            vwwba__ssbae.nitems))
...
def binary_arr_getitem(arr, ind):
    ...
    if isinstance(ind, types.Integer):
        ...
        def impl(arr, ind):
            ctsb__vfyqu = arr._data[ind]
            return init_bytes_type(ctsb__vfyqu, len(ctsb__vfyqu))
        return impl
...
def overload_bytes_fromhex(hex_str):
    ...
    if hex_str == bodo.string_type:
        ...
        def impl(hex_str):
            if not hex_str._is_ascii or hex_str._kind != cob__bcdm:
                raise TypeError(
                    'bytes.fromhex is only supported on ascii strings')
            rem__foxy = np.empty(len(hex_str) // 2, np.uint8)
            inw__twej = _bytes_fromhex(rem__foxy.ctypes, hex_str._data, len
                (hex_str))
            cxvpl__wsmvn = init_bytes_type(rem__foxy, inw__twej)
            return cxvpl__wsmvn
        return impl
...
def create_binary_cmp_op_overload(op):
    ...
    def overload_binary_cmp(lhs, rhs):
        saq__dculw = lhs == binary_array_type
        itdf__dozd = rhs == binary_array_type
        cuvx__qyr = 'lhs' if saq__dculw else 'rhs'
        kbz__izkra = 'def impl(lhs, rhs):\n'
        kbz__izkra += '  numba.parfors.parfor.init_prange()\n'
        kbz__izkra += f'  n = len({cuvx__qyr})\n'
        kbz__izkra += (
            '  out_arr = bodo.libs.bool_arr_ext.alloc_bool_array(n)\n')
        kbz__izkra += '  for i in numba.parfors.parfor.internal_prange(n):\n'
        skhvo__hdfu = []
        if saq__dculw:
            skhvo__hdfu.append('bodo.libs.array_kernels.isna(lhs, i)')
        if itdf__dozd:
            skhvo__hdfu.append('bodo.libs.array_kernels.isna(rhs, i)')
        kbz__izkra += f"    if {' or '.join(skhvo__hdfu)}:\n"
        kbz__izkra += '      bodo.libs.array_kernels.setna(out_arr, i)\n'
        kbz__izkra += '      continue\n'
        sond__ghca = 'lhs[i]' if saq__dculw else 'lhs'
        tkyk__coii = 'rhs[i]' if itdf__dozd else 'rhs'
        kbz__izkra += f'    out_arr[i] = op({sond__ghca}, {tkyk__coii})\n'
        kbz__izkra += '  return out_arr\n'
        domj__rylvg = {}
        exec(kbz__izkra, {'bodo': bodo, 'numba': numba, 'op': op}, domj__rylvg)
        return domj__rylvg['impl']
