...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        igh__rtavm = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(igh__rtavm)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ztoc__qnz = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, ztoc__qnz)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        fpt__hemhx, = args
        mvqx__oogf = signature.return_type
        pku__wjghl = cgutils.create_struct_proxy(mvqx__oogf)(context, builder)
        pku__wjghl.obj = fpt__hemhx
        context.nrt.incref(builder, signature.args[0], fpt__hemhx)
        return pku__wjghl._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                ppu__yzgsu = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        igh__rtavm = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(igh__rtavm)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ztoc__qnz = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, ztoc__qnz)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        fpt__hemhx, = args
        mbk__tsf = signature.return_type
        lkhy__erfl = cgutils.create_struct_proxy(mbk__tsf)(context, builder)
        lkhy__erfl.obj = fpt__hemhx
        context.nrt.incref(builder, signature.args[0], fpt__hemhx)
        return lkhy__erfl._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                urcsy__hetjg = bodo.utils.conversion.coerce_to_ndarray(idx)
                dpu__tzmz = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    urcsy__hetjg]
                col__ecxmk = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    urcsy__hetjg]
                igh__rtavm = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(dpu__tzmz,
                    col__ecxmk, igh__rtavm)
            return impl
        if isinstance(idx, types.SliceType):

            def impl_slice(I, idx):
                S = I._obj
                dpu__tzmz = bodo.hiframes.pd_series_ext.get_series_data(S)[idx]
                col__ecxmk = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    idx]
                igh__rtavm = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(dpu__tzmz,
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            if is_scalar_type(val):
                if I.stype.dtype == types.NPDatetime('ns'
                    ...
                    def impl_dt(I, idx, val):
                        s = integer_to_dt64(val.value)
                        urcsy__hetjg = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        ppu__yzgsu = (bodo.hiframes.datetime_timedelta_ext.
                            _to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(ppu__yzgsu))
                        urcsy__hetjg = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            urcsy__hetjg] = s
            ...
            def impl_arr(I, idx, val):
                urcsy__hetjg = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                    urcsy__hetjg] = bodo.utils.conversion.coerce_to_array(val,
                    False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        igh__rtavm = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(igh__rtavm)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ztoc__qnz = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, ztoc__qnz)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        fpt__hemhx, = args
        quld__ipkse = signature.return_type
        covi__iqd = cgutils.create_struct_proxy(quld__ipkse)(context, builder)
        covi__iqd.obj = fpt__hemhx
        context.nrt.incref(builder, signature.args[0], fpt__hemhx)
        return covi__iqd._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            dpu__tzmz = bodo.hiframes.pd_series_ext.get_series_data(S)
            col__ecxmk = bodo.hiframes.pd_series_ext.get_series_index(S)
            kvbyb__hvrtk = (idx - col__ecxmk._start) // col__ecxmk._step
            return dpu__tzmz[kvbyb__hvrtk]
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    ...
                    urcsy__hetjg = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(dpu__tzmz[
                        urcsy__hetjg])
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    gjmgp__cfp = bodo.hiframes.pd_series_ext.get_series_index(S
                        )
                    cbgmg__vbpjo = gjmgp__cfp.get_loc(idx)
                    if cbgmg__vbpjo is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(cbgmg__vbpjo)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                urcsy__hetjg = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[urcsy__hetjg
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        ulnhc__uapw = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(jbtup__dfl,
            int) for jbtup__dfl in ulnhc__uapw):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            uto__sla = get_literal_value(idx)
            if uto__sla not in ulnhc__uapw:
                raise_bodo_error(
                    f"Series label-based getitem: '{uto__sla}' not in {ulnhc__uapw}"
                    )
            arr_ind = ulnhc__uapw.index(uto__sla)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    pku__wjghl = cgutils.create_struct_proxy(fromty)(context, builder, val)
    qdrxd__blpxm = context.cast(builder, pku__wjghl.obj, fromty.stype, toty
        .stype)
    hau__mrw = cgutils.create_struct_proxy(toty)(context, builder)
    hau__mrw.obj = qdrxd__blpxm
    return hau__mrw._getvalue()
