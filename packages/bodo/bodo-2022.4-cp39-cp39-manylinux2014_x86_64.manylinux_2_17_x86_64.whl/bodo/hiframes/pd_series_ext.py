...
def series_getiter(context, builder, sig, args):
    lfeo__mkbl = get_series_payload(context, builder, sig.args[0], args[0])
    impl = context.get_function('getiter', sig.return_type(sig.args[0].data))
    return impl(builder, (lfeo__mkbl.data,))
...
class HeterSeriesAttribute(OverloadedKeyAttributeTemplate):
    ...
    def generic_resolve(self, S, attr):
        ...
        if isinstance(S.index, HeterogeneousIndexType
            ...
            xnpn__rgiqf = get_overload_const_tuple(S.index.data)
            if attr in xnpn__rgiqf:
                ftx__dfu = xnpn__rgiqf.index(attr)
                return S.data[ftx__dfu]
...
class SeriesPayloadModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        flbs__zpf = [('data', fe_type.series_type.data), ('index', fe_type.
            series_type.index), ('name', fe_type.series_type.name_typ)]
        super(SeriesPayloadModel, self).__init__(dmm, fe_type, flbs__zpf)
...
class SeriesModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ...
        flbs__zpf = [('meminfo', types.MemInfoPointer(payload_type)), (
            'parent', types.pyobject)]
        super(SeriesModel, self).__init__(dmm, fe_type, flbs__zpf)


def define_series_dtor(context, builder, series_type, payload_type):
    xfq__gtiy = builder.module
    segvl__ojd = lir.FunctionType(lir.VoidType(), [cgutils.voidptr_t])
    uzyu__mbnrd = cgutils.get_or_insert_function(xfq__gtiy, segvl__ojd,
        name='.dtor.series.{}'.format(series_type))
    if not uzyu__mbnrd.is_declaration:
        return uzyu__mbnrd
    uzyu__mbnrd.linkage = 'linkonce_odr'
    builder = lir.IRBuilder(uzyu__mbnrd.append_basic_block())
    lhys__xnfwe = uzyu__mbnrd.args[0]
    har__uicm = context.get_value_type(payload_type).as_pointer()
    yfztg__iktz = builder.bitcast(lhys__xnfwe, har__uicm)
    rqu__nfgj = context.make_helper(builder, payload_type, ref=yfztg__iktz)
    context.nrt.decref(builder, series_type.data, rqu__nfgj.data)
    context.nrt.decref(builder, series_type.index, rqu__nfgj.index)
    context.nrt.decref(builder, series_type.name_typ, rqu__nfgj.name)
    builder.ret_void()
    return uzyu__mbnrd
...
def construct_series(context, builder, series_type, data_val, index_val,
    ...
    lfeo__mkbl = cgutils.create_struct_proxy(payload_type)(context, builder)
    lfeo__mkbl.data = data_val
    lfeo__mkbl.index = index_val
    lfeo__mkbl.name = name_val
    djrjd__tacd = context.get_value_type(payload_type)
    bgfr__atvba = context.get_abi_sizeof(djrjd__tacd)
    atbdt__quyz = define_series_dtor(context, builder, series_type,
        payload_type)
    twb__zwdi = context.nrt.meminfo_alloc_dtor(builder, context.
        get_constant(types.uintp, bgfr__atvba), atbdt__quyz)
    zxgv__ddd = context.nrt.meminfo_data(builder, twb__zwdi)
    bqe__rndo = builder.bitcast(zxgv__ddd, djrjd__tacd.as_pointer())
    builder.store(lfeo__mkbl._getvalue(), bqe__rndo)
    series = cgutils.create_struct_proxy(series_type)(context, builder)
    series.meminfo = twb__zwdi
...
def init_series(typingctx, data, index, name=None):
    ...
    def codegen(context, builder, signature, args):
        ...
        dgsm__xop = construct_series(context, builder, series_type,
    ...
    if is_heterogeneous_tuple_type(data):
        tuu__rerc = HeterogeneousSeriesType(data, index, name)
    else:
        dtype = data.dtype
        data = if_series_to_array_type(data)
        tuu__rerc = SeriesType(dtype, data, index, name)
    sig = signature(tuu__rerc, data, index, name)
    return sig, codegen
...
def init_series_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    data = args[0]
    index = args[1]
    ahny__sjn = self.typemap[data.name]
    if is_heterogeneous_tuple_type(ahny__sjn) or isinstance(ahny__sjn,
        types.BaseTuple):
        return None
    kjy__ifsb = self.typemap[index.name]
    if not isinstance(kjy__ifsb, HeterogeneousIndexType
...
def get_series_payload(context, builder, series_type, value):
    twb__zwdi = cgutils.create_struct_proxy(series_type)(context, builder,
        value).meminfo
    payload_type = SeriesPayloadType(series_type)
    rqu__nfgj = context.nrt.meminfo_data(builder, twb__zwdi)
    har__uicm = context.get_value_type(payload_type).as_pointer()
    rqu__nfgj = builder.bitcast(rqu__nfgj, har__uicm)
    return context.make_helper(builder, payload_type, ref=rqu__nfgj)
...
def get_series_data(typingctx, series_typ=None):
    ...
    def codegen(context, builder, signature, args):
        lfeo__mkbl = get_series_payload(context, builder, signature.args[0],
            args[0])
        return impl_ret_borrowed(context, builder, series_typ.data,
            lfeo__mkbl.data)
    tuu__rerc = series_typ.data
    sig = signature(tuu__rerc, series_typ)
    return sig, codegen
...
def get_series_data_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    nvktj__oajjs = args[0]
    ahny__sjn = self.typemap[nvktj__oajjs.name].data
    if is_heterogeneous_tuple_type(ahny__sjn) or isinstance(ahny__sjn,
        types.BaseTuple):
        return None
    if equiv_set.has_shape(nvktj__oajjs):
        return ArrayAnalysis.AnalyzeResult(shape=nvktj__oajjs, pre=[])
    return None
...
def get_series_index_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    assert len(args) == 1 and not kws
    nvktj__oajjs = args[0]
    kjy__ifsb = self.typemap[nvktj__oajjs.name].index
    if isinstance(kjy__ifsb, HeterogeneousIndexType):
        return None
    if equiv_set.has_shape(nvktj__oajjs):
        return ArrayAnalysis.AnalyzeResult(shape=nvktj__oajjs, pre=[])
    return None
...
def cast_series(context, builder, fromty, toty, val):
    if fromty.copy(index=toty.index) == toty and isinstance(fromty.index,
        ...
        lfeo__mkbl = get_series_payload(context, builder, fromty, val)
        ysc__ndjua = context.cast(builder, lfeo__mkbl.index, fromty.index,
            toty.index)
        context.nrt.incref(builder, fromty.data, lfeo__mkbl.data)
        context.nrt.incref(builder, fromty.name_typ, lfeo__mkbl.name)
        return construct_series(context, builder, toty, lfeo__mkbl.data,
            ysc__ndjua, lfeo__mkbl.name)
...
class SeriesAttribute(OverloadedKeyAttributeTemplate):
    ...
    def resolve_head(self, ary, args, kws):
        yyn__qvmfr = 'Series.head'
        bxme__jdyk = 'n',
        aexgq__yssew = {'n': 5}
        pysig, eyxu__nyr = bodo.utils.typing.fold_typing_args(yyn__qvmfr,
            args, kws, bxme__jdyk, aexgq__yssew)
        lhgz__rvwuq = eyxu__nyr[0]
        if not is_overload_int(lhgz__rvwuq):
            raise BodoError(f"{yyn__qvmfr}(): 'n' must be an Integer")
        xvw__nan = ary
        return xvw__nan(*eyxu__nyr).replace(pysig=pysig)
    ...
    def _resolve_map_func(self, ary, func, pysig, fname, f_args=None, kws=None
        ...
        if f_args is not None:
            fro__idrn += tuple(f_args.types)
        if kws is None:
            kws = {}
        fvcu__qqc = False
        zaxxm__mdug = True
        if fname == 'map' and isinstance(func, types.DictType):
            brf__jvov = func.value_type
            fvcu__qqc = True
        else:
            try:
                if types.unliteral(func) == types.unicode_type:
                    ...
                    brf__jvov = bodo.utils.transform.get_udf_str_return_type(
                        ary, get_overload_const_str(func), self.context,
                        'Series.apply')
                    zaxxm__mdug = False
                elif bodo.utils.typing.is_numpy_ufunc(func):
                    brf__jvov = func.get_call_type(self.context, (ary,), {}
                        ).return_type
                    zaxxm__mdug = False
                else:
                    brf__jvov = get_const_func_output_type(func, fro__idrn,
                        kws, self.context, numba.core.registry.cpu_target.
                        target_context)
            except Exception as zlg__nvcxz:
                raise BodoError(get_udf_error_msg(f'Series.{fname}()',
                    zlg__nvcxz))
        if zaxxm__mdug:
            if isinstance(brf__jvov, (SeriesType, HeterogeneousSeriesType)
                ) and brf__jvov.const_info is None:
                raise BodoError(
                    'Invalid Series output in UDF (Series with constant length and constant Index value expected)'
                    )
            if isinstance(brf__jvov, HeterogeneousSeriesType):
                jrbx__sma, rmvgx__ize = brf__jvov.const_info
                if isinstance(brf__jvov.data, bodo.libs.nullable_tuple_ext.
                    NullableTupleType):
                    aleh__gxki = brf__jvov.data.tuple_typ.types
                elif isinstance(brf__jvov.data, types.Tuple):
                    aleh__gxki = brf__jvov.data.types
                bfyw__gtu = tuple(to_nullable_type(dtype_to_array_type(t)) for
                    t in aleh__gxki)
                niq__wafsk = bodo.DataFrameType(bfyw__gtu, ary.index,
                    rmvgx__ize)
            elif isinstance(brf__jvov, SeriesType):
                zbnie__cxtpg, rmvgx__ize = brf__jvov.const_info
                bfyw__gtu = tuple(to_nullable_type(dtype_to_array_type(
                    brf__jvov.dtype)) for jrbx__sma in range(zbnie__cxtpg))
                niq__wafsk = bodo.DataFrameType(bfyw__gtu, ary.index,
                    rmvgx__ize)
            else:
                iuv__dgmo = get_udf_out_arr_type(brf__jvov, fvcu__qqc)
                niq__wafsk = SeriesType(iuv__dgmo.dtype, iuv__dgmo, ary.
                    index, ary.name_typ)
        else:
            niq__wafsk = brf__jvov
        return signature(niq__wafsk, (func,)).replace(pysig=pysig)
    ...
    def resolve_map(self, ary, args, kws):
        ...
        na_action = args[1] if len(args) > 1 else kws.pop('na_action',
            types.none)
        xvhuh__qjh = dict(na_action=na_action)
        mhx__pyqp = dict(na_action=None)
        check_unsupported_args('Series.map', xvhuh__qjh, mhx__pyqp,
    ...
    def resolve_apply(self, ary, args, kws):
        ...
        kws.pop('func', None)
        ctpg__qmtg = args[1] if len(args) > 1 else kws.pop('convert_dtype',
            types.literal(True))
        f_args = args[2] if len(args) > 2 else kws.pop('args', None)
        xvhuh__qjh = dict(convert_dtype=ctpg__qmtg)
        dmavn__ilc = dict(convert_dtype=True)
        check_unsupported_args('Series.apply', xvhuh__qjh, dmavn__ilc,
            package_name='pandas', module_name='Series')
        ksf__pxgb = ', '.join("{} = ''".format(jmov__lspk) for jmov__lspk in
            kws.keys())
        qges__onv = (
            f'def apply_stub(func, convert_dtype=True, args=(), {ksf__pxgb}):\n'
            )
        qges__onv += '    pass\n'
        uol__gpvxn = {}
        exec(qges__onv, {}, uol__gpvxn)
        peasj__fjhb = uol__gpvxn['apply_stub']
        pysig = numba.core.utils.pysignature(peasj__fjhb)
    ...
    def _resolve_combine_func(self, ary, args, kws):
        ...
        awi__bnssb = ary.dtype
        bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(ary,
            'Series.combine()')
        bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(other,
            'Series.combine()')
        if awi__bnssb == types.NPDatetime('ns'):
            awi__bnssb = pd_timestamp_type
        zleo__zpb = other.dtype
        if zleo__zpb == types.NPDatetime('ns'):
            zleo__zpb = pd_timestamp_type
        brf__jvov = get_const_func_output_type(func, (awi__bnssb, zleo__zpb
            ), {}, self.context, numba.core.registry.cpu_target.target_context)
        sig = signature(SeriesType(brf__jvov, index=ary.index, name_typ=
...
def pd_series_overload(data=None, index=None, dtype=None, name=None, copy=
    ...
    yftct__brnd = is_overload_none(data)
    sza__cht = is_overload_none(index)
    wuo__vqg = is_overload_none(dtype)
    if yftct__brnd and sza__cht and wuo__vqg:
        raise BodoError(
            'pd.Series() requires at least 1 of data, index, and dtype to not be none'
            )
    if is_series_type(data) and not sza__cht:
        raise BodoError(
    ...
    if is_heterogeneous_tuple_type(data) and is_overload_none(dtype):
        ehbvv__kglxz = tuple(len(data) * [False])

        def impl_heter(data=None, index=None, dtype=None, name=None, copy=
            False, fastpath=False):
            trv__wtx = bodo.utils.conversion.extract_index_if_none(data, index)
            sxg__qnvl = bodo.utils.conversion.to_tuple(data)
            data_val = bodo.libs.nullable_tuple_ext.build_nullable_tuple(
                sxg__qnvl, ehbvv__kglxz)
            return bodo.hiframes.pd_series_ext.init_series(data_val, bodo.
                utils.conversion.convert_to_index(trv__wtx), name)
        return impl_heter
    if yftct__brnd:
        if wuo__vqg:
            ...
            def impl(data=None, index=None, dtype=None, name=None, copy=
                False, fastpath=False):
                lodl__ehbqp = bodo.utils.conversion.extract_name_if_none(data,
                    name)
                trv__wtx = bodo.utils.conversion.extract_index_if_none(data,
                    index)
                numba.parfors.parfor.init_prange()
                raji__yndwa = len(trv__wtx)
                sxg__qnvl = np.empty(raji__yndwa, np.float64)
                for jtgqo__khse in numba.parfors.parfor.internal_prange(
                    raji__yndwa):
                    bodo.libs.array_kernels.setna(sxg__qnvl, jtgqo__khse)
                return bodo.hiframes.pd_series_ext.init_series(sxg__qnvl,
                    bodo.utils.conversion.convert_to_index(trv__wtx),
                    lodl__ehbqp)
            return impl
        if bodo.utils.conversion._is_str_dtype(dtype):
            dztl__lqi = bodo.string_array_type
        else:
            dts__xyp = bodo.utils.typing.parse_dtype(dtype, 'pandas.Series')
            if isinstance(dts__xyp, bodo.libs.int_arr_ext.IntDtype):
                dztl__lqi = bodo.IntegerArrayType(dts__xyp.dtype)
            elif dts__xyp == bodo.libs.bool_arr_ext.boolean_dtype:
                dztl__lqi = bodo.boolean_array
            elif isinstance(dts__xyp, types.Number) or dts__xyp in [bodo.
                datetime64ns, bodo.timedelta64ns]:
                dztl__lqi = types.Array(dts__xyp, 1, 'C')
            else:
                raise BodoError(
        ...
        if sza__cht:
            ...
            def impl(data=None, index=None, dtype=None, name=None, copy=
                ...
                lodl__ehbqp = bodo.utils.conversion.extract_name_if_none(data,
                    name)
                trv__wtx = bodo.hiframes.pd_index_ext.init_range_index(0, 0,
                    1, None)
                numba.parfors.parfor.init_prange()
                raji__yndwa = len(trv__wtx)
                sxg__qnvl = bodo.utils.utils.alloc_type(raji__yndwa,
                    dztl__lqi, (-1,))
                return bodo.hiframes.pd_series_ext.init_series(sxg__qnvl,
                    trv__wtx, lodl__ehbqp)
            return impl
        else:
    ...
    def impl(data=None, index=None, dtype=None, name=None, copy=False,
        fastpath=False):
        lodl__ehbqp = bodo.utils.conversion.extract_name_if_none(data, name)
        trv__wtx = bodo.utils.conversion.extract_index_if_none(data, index)
        qwh__glhxd = bodo.utils.conversion.coerce_to_array(data, True,
            scalar_to_arr_len=len(trv__wtx))
        jvazu__uygj = bodo.utils.conversion.fix_arr_dtype(qwh__glhxd, dtype,
            None, False)
        return bodo.hiframes.pd_series_ext.init_series(jvazu__uygj, bodo.
            utils.conversion.convert_to_index(trv__wtx), lodl__ehbqp)
    return impl
...
def lower_constant_series(context, builder, series_type, pyval):
    if isinstance(series_type.data, bodo.DatetimeArrayType):
        caa__hlm = pyval.array
    else:
        caa__hlm = pyval.values
    data_val = context.get_constant_generic(builder, series_type.data, caa__hlm
        )
    ...
    name_val = context.get_constant_generic(builder, series_type.name_typ,
        pyval.name)
    rqu__nfgj = lir.Constant.literal_struct([data_val, index_val, name_val])
    rqu__nfgj = cgutils.global_constant(builder, '.const.payload', rqu__nfgj
        ).bitcast(cgutils.voidptr_t)
    icf__nuspb = context.get_constant(types.int64, -1)
    twts__kcxh = context.get_constant_null(types.voidptr)
    twb__zwdi = lir.Constant.literal_struct([icf__nuspb, twts__kcxh,
        twts__kcxh, rqu__nfgj, icf__nuspb])
    twb__zwdi = cgutils.global_constant(builder, '.const.meminfo', twb__zwdi
        ).bitcast(cgutils.voidptr_t)
    dgsm__xop = lir.Constant.literal_struct([twb__zwdi, twts__kcxh])
    return dgsm__xop
...
def _install_series_unsupported():
    for xkcyd__yki in series_unsupported_attrs:
        fdz__zdqw = 'Series.' + xkcyd__yki
        overload_attribute(SeriesType, xkcyd__yki)(create_unsupported_overload
            (fdz__zdqw))
    for fname in series_unsupported_methods:
        fdz__zdqw = 'Series.' + fname
        overload_method(SeriesType, fname, no_unliteral=True)(
            create_unsupported_overload(fdz__zdqw))
...
def _install_heter_series_unsupported():
    for xkcyd__yki in heter_series_unsupported_attrs:
        fdz__zdqw = 'HeterogeneousSeries.' + xkcyd__yki
        overload_attribute(HeterogeneousSeriesType, xkcyd__yki)(
            create_unsupported_overload(fdz__zdqw))
    for fname in heter_series_unsupported_methods:
        fdz__zdqw = 'HeterogeneousSeries.' + fname
