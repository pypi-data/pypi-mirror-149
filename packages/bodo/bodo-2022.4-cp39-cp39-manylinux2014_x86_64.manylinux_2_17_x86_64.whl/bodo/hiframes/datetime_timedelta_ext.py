...
class PDTimeDeltaModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        znpqk__fnoug = [('value', types.int64)]
        super(PDTimeDeltaModel, self).__init__(dmm, fe_type, znpqk__fnoug)


@box(PDTimeDeltaType)
def box_pd_timedelta(typ, val, c):
    vdc__esgw = cgutils.create_struct_proxy(typ)(c.context, c.builder,
        value=val)
    duh__kie = c.pyapi.long_from_longlong(vdc__esgw.value)
    wga__pth = c.pyapi.unserialize(c.pyapi.serialize_object(pd.Timedelta))
    res = c.pyapi.call_function_objargs(wga__pth, (duh__kie,))
    c.pyapi.decref(duh__kie)
    c.pyapi.decref(wga__pth)
    return res


@unbox(PDTimeDeltaType)
def unbox_pd_timedelta(typ, val, c):
    duh__kie = c.pyapi.object_getattr_string(val, 'value')
    bky__tnlvl = c.pyapi.long_as_longlong(duh__kie)
    vdc__esgw = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    vdc__esgw.value = bky__tnlvl
    c.pyapi.decref(duh__kie)
    xsdld__iqwu = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(vdc__esgw._getvalue(), is_error=xsdld__iqwu)
...
def pd_timedelta(value=_no_input, unit='ns', days=0, seconds=0,
    ...
    if value == _no_input:
        ...
        def impl_timedelta_kw(value=_no_input, unit='ns', days=0, seconds=0,
            ...
            czxl__bttu = 1000 * microseconds
            return init_pd_timedelta(czxl__bttu)
    ...
    lqwr__mkwu, qdwj__nilm = pd._libs.tslibs.conversion.precision_from_unit(
        unit)

    def impl_timedelta(value=_no_input, unit='ns', days=0, seconds=0,
        microseconds=0, milliseconds=0, minutes=0, hours=0, weeks=0):
        return init_pd_timedelta(value * lqwr__mkwu)
...
def overload_add_operator_datetime_timedelta(lhs, rhs):
    ...
    if lhs == pd_timedelta_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            zngdk__axz = (rhs.microseconds + (rhs.seconds + rhs.days * 60 *
                60 * 24) * 1000 * 1000) * 1000
            val = lhs.value + zngdk__axz
            return pd.Timedelta(val)
        return impl
    if lhs == datetime_timedelta_type and rhs == pd_timedelta_type:

        def impl(lhs, rhs):
            rbng__qfumq = (lhs.microseconds + (lhs.seconds + lhs.days * 60 *
                60 * 24) * 1000 * 1000) * 1000
            val = rbng__qfumq + rhs.value
            return pd.Timedelta(val)
        return impl
    if lhs == pd_timedelta_type and rhs == datetime_datetime_type:
        ...
        def impl(lhs, rhs):
            bosy__inujr = rhs.toordinal()
            larqf__xtf = rhs.second + rhs.minute * 60 + rhs.hour * 3600
            ofv__zah = rhs.microsecond
            symmb__lsl = lhs.value // 1000
            yopk__vhsn = lhs.nanoseconds
            nzoq__aclg = ofv__zah + symmb__lsl
            xxnm__krrpm = 1000000 * (bosy__inujr * 86400 + larqf__xtf
                ) + nzoq__aclg
            epth__zxtkt = yopk__vhsn
            return compute_pd_timestamp(xxnm__krrpm, epth__zxtkt)
        return impl
    ...
    if lhs == datetime_timedelta_type and rhs == datetime_datetime_type:
        ...
        def impl(lhs, rhs):
            dmobm__kzf = datetime.timedelta(rhs.toordinal(), hours=rhs.hour,
                minutes=rhs.minute, seconds=rhs.second, microseconds=rhs.
                microsecond)
            dmobm__kzf = dmobm__kzf + lhs
            twco__sdze, xdf__xnzyf = divmod(dmobm__kzf.seconds, 3600)
            urd__iiol, byu__biz = divmod(xdf__xnzyf, 60)
            if 0 < dmobm__kzf.days <= _MAXORDINAL:
                d = bodo.hiframes.datetime_date_ext.fromordinal_impl(dmobm__kzf
                    .days)
                return datetime.datetime(d.year, d.month, d.day, twco__sdze,
                    urd__iiol, byu__biz, dmobm__kzf.microseconds)
            raise OverflowError('result out of range')
        return impl
    if lhs == datetime_datetime_type and rhs == datetime_timedelta_type:

        def impl(lhs, rhs):
            dmobm__kzf = datetime.timedelta(lhs.toordinal(), hours=lhs.hour,
                minutes=lhs.minute, seconds=lhs.second, microseconds=lhs.
                microsecond)
            dmobm__kzf = dmobm__kzf + rhs
            twco__sdze, xdf__xnzyf = divmod(dmobm__kzf.seconds, 3600)
            urd__iiol, byu__biz = divmod(xdf__xnzyf, 60)
            if 0 < dmobm__kzf.days <= _MAXORDINAL:
                d = bodo.hiframes.datetime_date_ext.fromordinal_impl(dmobm__kzf
...
def overload_sub_operator_datetime_timedelta(lhs, rhs):
    if lhs == pd_timedelta_type and rhs == pd_timedelta_type:
        ...
        def impl(lhs, rhs):
            oygx__uuq = lhs.value - rhs.value
            return pd.Timedelta(oygx__uuq)
        return impl
    if lhs == pd_timedelta_type and rhs == datetime_timedelta_type:
    ...
    if lhs == datetime_timedelta_array_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            ...
            for dqkv__uczmt in numba.parfors.parfor.internal_prange(n):
                A[dqkv__uczmt] = ippqv__lrzc[dqkv__uczmt] - rhs
            return A
        return impl
...
def overload_mod_operator_timedeltas(lhs, rhs):
    ...
    if lhs == datetime_timedelta_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            qye__avv = _to_microseconds(lhs) % _to_microseconds(rhs)
            return datetime.timedelta(0, 0, qye__avv)
        return impl
...
def pd_timedelta_divmod(lhs, rhs):
    if lhs == pd_timedelta_type and rhs == pd_timedelta_type:
        ...
        def impl(lhs, rhs):
            aty__aiif, qye__avv = divmod(lhs.value, rhs.value)
            return aty__aiif, pd.Timedelta(qye__avv)
        return impl
...
class DatetimeTimeDeltaModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        znpqk__fnoug = [('days', types.int64), ('seconds', types.int64), (
            'microseconds', types.int64)]
        super(DatetimeTimeDeltaModel, self).__init__(dmm, fe_type, znpqk__fnoug
            )
...
def box_datetime_timedelta(typ, val, c):
    vdc__esgw = cgutils.create_struct_proxy(typ)(c.context, c.builder,
        value=val)
    czl__hjzlv = c.pyapi.long_from_longlong(vdc__esgw.days)
    abcp__pqdn = c.pyapi.long_from_longlong(vdc__esgw.seconds)
    yqro__pnj = c.pyapi.long_from_longlong(vdc__esgw.microseconds)
    wga__pth = c.pyapi.unserialize(c.pyapi.serialize_object(datetime.timedelta)
        )
    res = c.pyapi.call_function_objargs(wga__pth, (czl__hjzlv, abcp__pqdn,
        yqro__pnj))
    c.pyapi.decref(czl__hjzlv)
    c.pyapi.decref(abcp__pqdn)
    c.pyapi.decref(yqro__pnj)
    c.pyapi.decref(wga__pth)
    return res
...
def unbox_datetime_timedelta(typ, val, c):
    czl__hjzlv = c.pyapi.object_getattr_string(val, 'days')
    abcp__pqdn = c.pyapi.object_getattr_string(val, 'seconds')
    yqro__pnj = c.pyapi.object_getattr_string(val, 'microseconds')
    pqqs__koq = c.pyapi.long_as_longlong(czl__hjzlv)
    swqgj__hrnw = c.pyapi.long_as_longlong(abcp__pqdn)
    onbeu__flmt = c.pyapi.long_as_longlong(yqro__pnj)
    vdc__esgw = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    vdc__esgw.days = pqqs__koq
    vdc__esgw.seconds = swqgj__hrnw
    vdc__esgw.microseconds = onbeu__flmt
    c.pyapi.decref(czl__hjzlv)
    c.pyapi.decref(abcp__pqdn)
    c.pyapi.decref(yqro__pnj)
    xsdld__iqwu = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
...
def _divide_and_round(a, b):
    aty__aiif, qye__avv = divmod(a, b)
    qye__avv *= 2
    qxnyu__qdoh = qye__avv > b if b > 0 else qye__avv < b
    if qxnyu__qdoh or qye__avv == b and aty__aiif % 2 == 1:
        aty__aiif += 1
    return aty__aiif
...
def create_cmp_op_overload(op):
    ...
    def overload_timedelta_cmp(lhs, rhs):
        if lhs == datetime_timedelta_type and rhs == datetime_timedelta_type:
            ...
            def impl(lhs, rhs):
                cuimy__crt = _cmp(_getstate(lhs), _getstate(rhs))
                return op(cuimy__crt, 0)
            return impl
...
def timedelta_divmod(lhs, rhs):
    if lhs == datetime_timedelta_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            aty__aiif, qye__avv = divmod(_to_microseconds(lhs),
                _to_microseconds(rhs))
            return aty__aiif, datetime.timedelta(0, 0, qye__avv)
        return impl
...
def timedelta_to_bool(timedelta):
    if timedelta != datetime_timedelta_type:
        return
    mfoq__dcn = datetime.timedelta(0)

    def impl(timedelta):
        return timedelta != mfoq__dcn
    return impl
...
class DatetimeTimeDeltaArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        znpqk__fnoug = [('days_data', days_data_type), ('seconds_data',
            seconds_data_type), ('microseconds_data',
            microseconds_data_type), ('null_bitmap', nulls_type)]
        models.StructModel.__init__(self, dmm, fe_type, znpqk__fnoug)
...
def unbox_datetime_timedelta_array(typ, val, c):
    ...
    kiav__fxm = types.Array(types.intp, 1, 'C')
    fqov__ogdk = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        kiav__fxm, [n])
    uig__zmqno = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        kiav__fxm, [n])
    qauv__wmbf = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        kiav__fxm, [n])
    vamo__zro = c.builder.udiv(c.builder.add(n, lir.Constant(lir.IntType(64
        ), 7)), lir.Constant(lir.IntType(64), 8))
    ofi__iaem = bodo.utils.utils._empty_nd_impl(c.context, c.builder, types
        .Array(types.uint8, 1, 'C'), [vamo__zro])
    tms__largo = lir.FunctionType(lir.VoidType(), [lir.IntType(8).
        as_pointer(), lir.IntType(64), lir.IntType(64).as_pointer(), lir.
        IntType(64).as_pointer(), lir.IntType(64).as_pointer(), lir.IntType
        (8).as_pointer()])
    edal__xew = cgutils.get_or_insert_function(c.builder.module, tms__largo,
        name='unbox_datetime_timedelta_array')
    c.builder.call(edal__xew, [val, n, fqov__ogdk.data, uig__zmqno.data,
        qauv__wmbf.data, ofi__iaem.data])
    qql__kqbkv = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    qql__kqbkv.days_data = fqov__ogdk._getvalue()
    qql__kqbkv.seconds_data = uig__zmqno._getvalue()
    qql__kqbkv.microseconds_data = qauv__wmbf._getvalue()
    qql__kqbkv.null_bitmap = ofi__iaem._getvalue()
    xsdld__iqwu = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(qql__kqbkv._getvalue(), is_error=xsdld__iqwu)


@box(DatetimeTimeDeltaArrayType)
def box_datetime_timedelta_array(typ, val, c):
    ippqv__lrzc = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    fqov__ogdk = c.context.make_array(types.Array(types.int64, 1, 'C'))(c.
        context, c.builder, ippqv__lrzc.days_data)
    uig__zmqno = c.context.make_array(types.Array(types.int64, 1, 'C'))(c.
        context, c.builder, ippqv__lrzc.seconds_data).data
    qauv__wmbf = c.context.make_array(types.Array(types.int64, 1, 'C'))(c.
        context, c.builder, ippqv__lrzc.microseconds_data).data
    rve__pjyr = c.context.make_array(types.Array(types.uint8, 1, 'C'))(c.
        context, c.builder, ippqv__lrzc.null_bitmap).data
    n = c.builder.extract_value(fqov__ogdk.shape, 0)
    tms__largo = lir.FunctionType(c.pyapi.pyobj, [lir.IntType(64), lir.
        IntType(64).as_pointer(), lir.IntType(64).as_pointer(), lir.IntType
        (64).as_pointer(), lir.IntType(8).as_pointer()])
    jtm__yhev = cgutils.get_or_insert_function(c.builder.module, tms__largo,
        name='box_datetime_timedelta_array')
    cbcc__jvea = c.builder.call(jtm__yhev, [n, fqov__ogdk.data, uig__zmqno,
        qauv__wmbf, rve__pjyr])
...
def init_datetime_timedelta_array(typingctx, days_data, seconds_data,
    ...
    def codegen(context, builder, signature, args):
        afak__udvn, jzmhm__oaimx, wheqv__pkze, vlptg__rmj = args
        zacu__yjduy = cgutils.create_struct_proxy(signature.return_type)(
            context, builder)
        zacu__yjduy.days_data = afak__udvn
        zacu__yjduy.seconds_data = jzmhm__oaimx
        zacu__yjduy.microseconds_data = wheqv__pkze
        zacu__yjduy.null_bitmap = vlptg__rmj
        context.nrt.incref(builder, signature.args[0], afak__udvn)
        context.nrt.incref(builder, signature.args[1], jzmhm__oaimx)
        context.nrt.incref(builder, signature.args[2], wheqv__pkze)
        context.nrt.incref(builder, signature.args[3], vlptg__rmj)
        return zacu__yjduy._getvalue()
    kfyb__elzq = datetime_timedelta_array_type(days_data, seconds_data,
        microseconds_data, nulls)
    return kfyb__elzq, codegen


@lower_constant(DatetimeTimeDeltaArrayType)
def lower_constant_datetime_timedelta_arr(context, builder, typ, pyval):
    n = len(pyval)
    fqov__ogdk = np.empty(n, np.int64)
    uig__zmqno = np.empty(n, np.int64)
    qauv__wmbf = np.empty(n, np.int64)
    ihxq__zvpx = np.empty(n + 7 >> 3, np.uint8)
    for dqkv__uczmt, s in enumerate(pyval):
        edlv__ikkl = pd.isna(s)
        bodo.libs.int_arr_ext.set_bit_to_arr(ihxq__zvpx, dqkv__uczmt, int(
            not edlv__ikkl))
        if not edlv__ikkl:
            fqov__ogdk[dqkv__uczmt] = s.days
            uig__zmqno[dqkv__uczmt] = s.seconds
            qauv__wmbf[dqkv__uczmt] = s.microseconds
    oljk__cxcao = context.get_constant_generic(builder, days_data_type,
        fqov__ogdk)
    iww__dns = context.get_constant_generic(builder, seconds_data_type,
        uig__zmqno)
    qai__opmi = context.get_constant_generic(builder,
        microseconds_data_type, qauv__wmbf)
    mots__rtwlc = context.get_constant_generic(builder, nulls_type, ihxq__zvpx)
    return lir.Constant.literal_struct([oljk__cxcao, iww__dns, qai__opmi,
        mots__rtwlc])


@numba.njit(no_cpython_wrapper=True)
def alloc_datetime_timedelta_array(n):
    fqov__ogdk = np.empty(n, dtype=np.int64)
    uig__zmqno = np.empty(n, dtype=np.int64)
    qauv__wmbf = np.empty(n, dtype=np.int64)
    nulls = np.full(n + 7 >> 3, 255, np.uint8)
    return init_datetime_timedelta_array(fqov__ogdk, uig__zmqno, qauv__wmbf,
        nulls)
...
def dt_timedelta_arr_getitem(A, ind):
    ...
    if is_list_like_index_type(ind) and ind.dtype == types.bool_:
        ...
        def impl_bool(A, ind):
            xkp__ehhkh = bodo.utils.conversion.coerce_to_ndarray(ind)
            pcx__pio = A._null_bitmap
            gfasj__xuqb = A._days_data[xkp__ehhkh]
            yponz__xma = A._seconds_data[xkp__ehhkh]
            tye__zbs = A._microseconds_data[xkp__ehhkh]
            n = len(gfasj__xuqb)
            yxhvp__gxh = get_new_null_mask_bool_index(pcx__pio, ind, n)
            return init_datetime_timedelta_array(gfasj__xuqb, yponz__xma,
                tye__zbs, yxhvp__gxh)
        return impl_bool
    if is_list_like_index_type(ind) and isinstance(ind.dtype, types.Integer):
        ...
        def impl(A, ind):
            ...
            tye__zbs = A._microseconds_data[xkp__ehhkh]
            n = len(gfasj__xuqb)
            yxhvp__gxh = get_new_null_mask_int_index(pcx__pio, xkp__ehhkh, n)
            return init_datetime_timedelta_array(gfasj__xuqb, yponz__xma,
                tye__zbs, yxhvp__gxh)
        return impl
    if isinstance(ind, types.SliceType):
        ...
        def impl_slice(A, ind):
            n = len(A._days_data)
            pcx__pio = A._null_bitmap
            gfasj__xuqb = np.ascontiguousarray(A._days_data[ind])
            yponz__xma = np.ascontiguousarray(A._seconds_data[ind])
            tye__zbs = np.ascontiguousarray(A._microseconds_data[ind])
            yxhvp__gxh = get_new_null_mask_slice_index(pcx__pio, ind, n)
            return init_datetime_timedelta_array(gfasj__xuqb, yponz__xma,
                tye__zbs, yxhvp__gxh)
        return impl_slice
    raise BodoError(
    ...
    if is_list_like_index_type(ind) and isinstance(ind.dtype, types.Integer):
        if types.unliteral(val) == datetime_timedelta_type:
            ...
            def impl_arr_ind_scalar(A, ind, val):
                n = len(A)
                for dqkv__uczmt in range(n):
                    A._days_data[ind[dqkv__uczmt]] = val._days
                    A._seconds_data[ind[dqkv__uczmt]] = val._seconds
                    A._microseconds_data[ind[dqkv__uczmt]] = val._microseconds
            ...
            def impl_arr_ind(A, ind, val):
                ...
                for dqkv__uczmt in range(n):
                    A._days_data[ind[dqkv__uczmt]] = val._days_data[dqkv__uczmt
                        ]
                    A._seconds_data[ind[dqkv__uczmt]] = val._seconds_data[
                        dqkv__uczmt]
                    A._microseconds_data[ind[dqkv__uczmt]
                        ] = val._microseconds_data[dqkv__uczmt]
                    lubmz__cipc = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val
                        ._null_bitmap, dqkv__uczmt)
                    bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                        ind[dqkv__uczmt], lubmz__cipc)
            return impl_arr_ind
    if is_list_like_index_type(ind) and ind.dtype == types.bool_:
        if types.unliteral(val) == datetime_timedelta_type:
            ...
            def impl_bool_ind_mask_scalar(A, ind, val):
                n = len(ind)
                for dqkv__uczmt in range(n):
                    if not bodo.libs.array_kernels.isna(ind, dqkv__uczmt
                        ) and ind[dqkv__uczmt]:
                        A._days_data[dqkv__uczmt] = val._days
                        A._seconds_data[dqkv__uczmt] = val._seconds
                        A._microseconds_data[dqkv__uczmt] = val._microseconds
                        bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                            dqkv__uczmt, 1)
            ...
            def impl_bool_ind_mask(A, ind, val):
                ...
                for dqkv__uczmt in range(n):
                    if not bodo.libs.array_kernels.isna(ind, dqkv__uczmt
                        ...
                        A._days_data[dqkv__uczmt] = val._days_data[yszv__nyu]
                        A._seconds_data[dqkv__uczmt] = val._seconds_data[
                            yszv__nyu]
                        A._microseconds_data[dqkv__uczmt
                            ] = val._microseconds_data[yszv__nyu]
                        lubmz__cipc = bodo.libs.int_arr_ext.get_bit_bitmap_arr(
                            val._null_bitmap, yszv__nyu)
                        bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                            dqkv__uczmt, lubmz__cipc)
                        yszv__nyu += 1
    ...
    if isinstance(ind, types.SliceType):
        if types.unliteral(val) == datetime_timedelta_type:
            ...
            def impl_slice_scalar(A, ind, val):
                rhy__uriai = numba.cpython.unicode._normalize_slice(ind, len(A)
                    )
                for dqkv__uczmt in range(rhy__uriai.start, rhy__uriai.stop,
                    rhy__uriai.step):
                    A._days_data[dqkv__uczmt] = val._days
                    A._seconds_data[dqkv__uczmt] = val._seconds
                    A._microseconds_data[dqkv__uczmt] = val._microseconds
            ...
            def impl_slice_mask(A, ind, val):
                ...
                zyvqk__ugken = val._null_bitmap.copy()
                setitem_slice_index_null_bits(A._null_bitmap, zyvqk__ugken,
                    ind, n)
...
def overload_datetime_timedelta_arr_sub(arg1, arg2):
    if (arg1 == datetime_timedelta_array_type and arg2 ==
        ...
        def impl(arg1, arg2):
            ...
            A = alloc_datetime_timedelta_array(n)
            for dqkv__uczmt in numba.parfors.parfor.internal_prange(n):
                A[dqkv__uczmt] = ippqv__lrzc[dqkv__uczmt] - arg2
            return A
        return impl
    ...
    def overload_date_arr_cmp(lhs, rhs):
        ...
        if (lhs == datetime_timedelta_array_type and rhs ==
            ...
            def impl(lhs, rhs):
                numba.parfors.parfor.init_prange()
                n = len(lhs)
                pqzw__pgzb = bodo.libs.bool_arr_ext.alloc_bool_array(n)
                for dqkv__uczmt in numba.parfors.parfor.internal_prange(n):
                    joaqd__caw = bodo.libs.array_kernels.isna(lhs, dqkv__uczmt)
                    dqy__hick = bodo.libs.array_kernels.isna(rhs, dqkv__uczmt)
                    if joaqd__caw or dqy__hick:
                        ofmwe__dnmey = xcc__tsdk
                    else:
                        ofmwe__dnmey = op(lhs[dqkv__uczmt], rhs[dqkv__uczmt])
                    pqzw__pgzb[dqkv__uczmt] = ofmwe__dnmey
                return pqzw__pgzb
            return impl
            ...
            def impl(lhs, rhs):
                ...
                pqzw__pgzb = bodo.libs.bool_arr_ext.alloc_bool_array(n)
                for dqkv__uczmt in numba.parfors.parfor.internal_prange(n):
                    lubmz__cipc = bodo.libs.array_kernels.isna(lhs, dqkv__uczmt
                        )
                    if lubmz__cipc:
                        ofmwe__dnmey = xcc__tsdk
                    else:
                        ofmwe__dnmey = op(lhs[dqkv__uczmt], rhs)
                    pqzw__pgzb[dqkv__uczmt] = ofmwe__dnmey
                return pqzw__pgzb
            return impl
            ...
            def impl(lhs, rhs):
                ...
                pqzw__pgzb = bodo.libs.bool_arr_ext.alloc_bool_array(n)
                for dqkv__uczmt in numba.parfors.parfor.internal_prange(n):
                    lubmz__cipc = bodo.libs.array_kernels.isna(rhs, dqkv__uczmt
                        )
                    if lubmz__cipc:
                        ofmwe__dnmey = xcc__tsdk
                    else:
                        ofmwe__dnmey = op(lhs, rhs[dqkv__uczmt])
                    pqzw__pgzb[dqkv__uczmt] = ofmwe__dnmey
                return pqzw__pgzb
            return impl
...
def _intstall_pd_timedelta_unsupported():
    ...
    for vgovl__klxp in timedelta_unsupported_attrs:
        zcd__dap = 'pandas.Timedelta.' + vgovl__klxp
        overload_attribute(PDTimeDeltaType, vgovl__klxp)(
            create_unsupported_overload(zcd__dap))
    for zaki__jvr in timedelta_unsupported_methods:
        zcd__dap = 'pandas.Timedelta.' + zaki__jvr
        overload_method(PDTimeDeltaType, zaki__jvr)(create_unsupported_overload
            (zcd__dap + '()'))
