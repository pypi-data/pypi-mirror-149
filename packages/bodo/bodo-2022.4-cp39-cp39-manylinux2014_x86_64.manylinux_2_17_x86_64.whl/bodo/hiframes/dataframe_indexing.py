...
class DataFrameGetItemTemplate(AbstractTemplate):
    ...
    def typecheck_loc_getitem(self, args):
        ...
        if is_list_like_index_type(idx) and idx.dtype == types.bool_:
            ...
            xzol__bmj = df.data
            iem__uqi = df.columns
            mom__gygik = self.replace_range_with_numeric_idx_if_needed(df,
                hvdo__oopd)
            jhxr__ijnvo = DataFrameType(xzol__bmj, mom__gygik, iem__uqi)
            return jhxr__ijnvo(*args)
        if isinstance(idx, types.BaseTuple) and len(idx) == 2:
            iwqqh__epx = idx.types[0]
            znnqv__fzk = idx.types[1]
            if isinstance(iwqqh__epx, types.Integer):
                ...
                if is_overload_constant_str(znnqv__fzk):
                    boys__bvxk = get_overload_const_str(znnqv__fzk)
                    if boys__bvxk not in df.columns:
                        raise_bodo_error(
                            'dataframe {} does not include column {}'.
                            format(df, boys__bvxk))
                    qsdn__rzv = df.columns.index(boys__bvxk)
                    return df.data[qsdn__rzv].dtype(*args)
                if isinstance(znnqv__fzk, types.UnicodeType):
                    raise_bodo_error(
            ...
            if is_list_like_index_type(iwqqh__epx
                ) and iwqqh__epx.dtype == types.bool_ or isinstance(iwqqh__epx,
                types.SliceType):
                mom__gygik = self.replace_range_with_numeric_idx_if_needed(df,
                    iwqqh__epx)
                if is_overload_constant_str(znnqv__fzk):
                    lmf__teie = get_overload_const_str(znnqv__fzk)
                    if lmf__teie not in df.columns:
                        raise_bodo_error(
                            f'dataframe {df} does not include column {lmf__teie}'
                            )
                    qsdn__rzv = df.columns.index(lmf__teie)
                    ddgzg__pnc = df.data[qsdn__rzv]
                    cxdk__qbg = ddgzg__pnc.dtype
                    nutrd__ronvw = types.literal(df.columns[qsdn__rzv])
                    jhxr__ijnvo = bodo.SeriesType(cxdk__qbg, ddgzg__pnc,
                        mom__gygik, nutrd__ronvw)
                    return jhxr__ijnvo(*args)
                if isinstance(znnqv__fzk, types.UnicodeType):
                    raise_bodo_error(
                        f'DataFrame.loc[] getitem (location-based indexing) requires constant column names. For more information, see https://docs.bodo.ai/latest/source/programming_with_bodo/require_constants.html.'
                        )
                elif is_overload_constant_list(znnqv__fzk):
                    xlt__vkctn = get_overload_const_list(znnqv__fzk)
                    qavac__deyp = types.unliteral(znnqv__fzk)
                    if qavac__deyp.dtype == types.bool_:
                        if len(df.columns) != len(xlt__vkctn):
                            raise_bodo_error(
                                f'dataframe {df} has {len(df.columns)} columns, but boolean array used with DataFrame.loc[] {xlt__vkctn} has {len(xlt__vkctn)} values'
                                )
                        lgwnw__jwkdp = []
                        gplqp__juouu = []
                        for mob__hvu in range(len(xlt__vkctn)):
                            if xlt__vkctn[mob__hvu]:
                                lgwnw__jwkdp.append(df.columns[mob__hvu])
                                gplqp__juouu.append(df.data[mob__hvu])
                        cect__tbxdu = tuple()
                        jhxr__ijnvo = DataFrameType(tuple(gplqp__juouu),
                            mom__gygik, tuple(lgwnw__jwkdp))
                        return jhxr__ijnvo(*args)
                    elif qavac__deyp.dtype == bodo.string_type:
                        cect__tbxdu, gplqp__juouu = (self.
                            get_kept_cols_and_data(df, xlt__vkctn))
                        jhxr__ijnvo = DataFrameType(gplqp__juouu,
                            mom__gygik, cect__tbxdu)
                        return jhxr__ijnvo(*args)
        raise_bodo_error(
    ...
    def typecheck_df_getitem(self, args):
        ...
        if is_overload_constant_str(ind) or is_overload_constant_int(ind):
            ...
            if isinstance(df.columns[0], tuple):
                lgwnw__jwkdp = []
                gplqp__juouu = []
                for mob__hvu, aurts__tluuh in enumerate(df.columns):
                    if aurts__tluuh[0] != ind_val:
                        continue
                    lgwnw__jwkdp.append(aurts__tluuh[1] if len(aurts__tluuh
                        ) == 2 else aurts__tluuh[1:])
                    gplqp__juouu.append(df.data[mob__hvu])
                ddgzg__pnc = tuple(gplqp__juouu)
                kuul__qrq = df.index
                sbpan__tgcv = tuple(lgwnw__jwkdp)
                jhxr__ijnvo = DataFrameType(ddgzg__pnc, kuul__qrq, sbpan__tgcv)
                return jhxr__ijnvo(*args)
            else:
                if ind_val not in df.columns:
                    raise_bodo_error('dataframe {} does not include column {}'
                        .format(df, ind_val))
                qsdn__rzv = df.columns.index(ind_val)
                ddgzg__pnc = df.data[qsdn__rzv]
                cxdk__qbg = ddgzg__pnc.dtype
                kuul__qrq = df.index
                nutrd__ronvw = types.literal(df.columns[qsdn__rzv])
                jhxr__ijnvo = bodo.SeriesType(cxdk__qbg, ddgzg__pnc,
                    kuul__qrq, nutrd__ronvw)
                return jhxr__ijnvo(*args)
        ...
        if is_list_like_index_type(ind
            ...
            ddgzg__pnc = df.data
            kuul__qrq = self.replace_range_with_numeric_idx_if_needed(df, ind)
            sbpan__tgcv = df.columns
            jhxr__ijnvo = DataFrameType(ddgzg__pnc, kuul__qrq, sbpan__tgcv,
                is_table_format=df.is_table_format)
            return jhxr__ijnvo(*args)
        elif is_overload_constant_list(ind):
            hctd__mhyr = get_overload_const_list(ind)
            sbpan__tgcv, ddgzg__pnc = self.get_kept_cols_and_data(df,
                hctd__mhyr)
            kuul__qrq = df.index
            jhxr__ijnvo = DataFrameType(ddgzg__pnc, kuul__qrq, sbpan__tgcv)
            return jhxr__ijnvo(*args)
    ...
    def get_kept_cols_and_data(self, df, cols_to_keep_list):
        for vialh__cqpac in cols_to_keep_list:
            if vialh__cqpac not in df.columns:
                raise_bodo_error('Column {} not found in dataframe columns {}'
                    .format(vialh__cqpac, df.columns))
        sbpan__tgcv = tuple(cols_to_keep_list)
        ddgzg__pnc = tuple(df.data[df.columns.index(zjr__wzigi)] for
            zjr__wzigi in sbpan__tgcv)
        return sbpan__tgcv, ddgzg__pnc

    def replace_range_with_numeric_idx_if_needed(self, df, ind):
        mom__gygik = bodo.hiframes.pd_index_ext.NumericIndexType(types.
...
def df_getitem_overload(df, ind):
        ...
        if isinstance(df.columns[0], tuple):
            ...
            for mob__hvu, aurts__tluuh in enumerate(df.columns):
                ...
                gplqp__juouu.append(
                    ...
                    .format(mob__hvu))
            nuxja__ghnih = 'def impl(df, ind):\n'
            ysfz__grm = (
                'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df)')
            return bodo.hiframes.dataframe_impl._gen_init_df(nuxja__ghnih,
                lgwnw__jwkdp, ', '.join(gplqp__juouu), ysfz__grm)
        if ind_val not in df.columns:
            raise_bodo_error('dataframe {} does not include column {}'.
                format(df, ind_val))
    ...
    if is_overload_constant_list(ind):
        hctd__mhyr = get_overload_const_list(ind)
        for vialh__cqpac in hctd__mhyr:
            if vialh__cqpac not in df.columns:
                raise_bodo_error('Column {} not found in dataframe columns {}'
                    .format(vialh__cqpac, df.columns))
        gplqp__juouu = ', '.join(
            'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {}).copy()'
            .format(df.columns.index(vialh__cqpac)) for vialh__cqpac in
            hctd__mhyr)
        nuxja__ghnih = 'def impl(df, ind):\n'
        ysfz__grm = 'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df)'
        return bodo.hiframes.dataframe_impl._gen_init_df(nuxja__ghnih,
            hctd__mhyr, gplqp__juouu, ysfz__grm)
    if is_list_like_index_type(ind) and ind.dtype == types.bool_ or isinstance(
        ...
        if df.is_table_format:
            ...
            gplqp__juouu = ', '.join(
                f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {df.columns.index(vialh__cqpac)})[ind]'
                 for vialh__cqpac in df.columns)
        return bodo.hiframes.dataframe_impl._gen_init_df(nuxja__ghnih, df.
            columns, gplqp__juouu, ysfz__grm, out_df_type=df)
...
class DataFrameILocType(types.Type):
    ...
    def __init__(self, df_type):
        ...
        zjr__wzigi = 'DataFrameILocType({})'.format(df_type)
        super(DataFrameILocType, self).__init__(zjr__wzigi)
...
class DataFrameILocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        tnaxo__vqpp = [('obj', fe_type.df_type)]
        super(DataFrameILocModel, self).__init__(dmm, fe_type, tnaxo__vqpp)
...
def init_dataframe_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        bub__egg, = args
        ljjjo__buwc = signature.return_type
        batoy__mpq = cgutils.create_struct_proxy(ljjjo__buwc)(context, builder)
        batoy__mpq.obj = bub__egg
        context.nrt.incref(builder, signature.args[0], bub__egg)
        return batoy__mpq._getvalue()
...
def overload_iloc_getitem(I, idx):
    ...
    if isinstance(idx, types.BaseTuple) and len(idx) == 2 and not isinstance(
        ...
        zkfq__maj = len(df.data)
        if is_overload_constant_int(idx.types[1]):
            is_out_series = True
            yld__bce = get_overload_const_int(idx.types[1])
            if yld__bce < 0 or yld__bce >= zkfq__maj:
                raise BodoError(
            ...
            is_out_series = False
            aqlop__orr = get_overload_const_list(idx.types[1])
            if any(not isinstance(ind, int) or ind < 0 or ind >= zkfq__maj for
                ind in aqlop__orr):
                raise BodoError(
                    'df.iloc: column list must be integers referring to a valid column number'
                    )
        col_names = tuple(pd.Series(df.columns, dtype=object)[aqlop__orr])
        if isinstance(idx.types[0], types.Integer):
            if isinstance(idx.types[1], types.Integer):
                yld__bce = aqlop__orr[0]

                def impl(I, idx):
                    df = I._obj
                    return bodo.utils.conversion.box_if_dt64(bodo.hiframes.
                        pd_dataframe_ext.get_dataframe_data(df, yld__bce)[
                        idx[0]])
                return impl
...
def _gen_iloc_getitem_bool_slice_impl(df, col_names, idx_typ, idx,
    is_out_series):
    nuxja__ghnih = 'def impl(I, idx):\n'
    nuxja__ghnih += '  df = I._obj\n'
    if isinstance(idx_typ, types.SliceType):
        nuxja__ghnih += f'  idx_t = {idx}\n'
    else:
        nuxja__ghnih += (
            f'  idx_t = bodo.utils.conversion.coerce_to_ndarray({idx})\n')
    ysfz__grm = 'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df)[idx_t]'
    gplqp__juouu = ', '.join(
        f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {df.columns.index(vialh__cqpac)})[idx_t]'
         for vialh__cqpac in col_names)
    if is_out_series:
        lbbb__fmmdw = f"'{col_names[0]}'" if isinstance(col_names[0], str
            ) else f'{col_names[0]}'
        nuxja__ghnih += f"""  return bodo.hiframes.pd_series_ext.init_series({gplqp__juouu}, {ysfz__grm}, {lbbb__fmmdw})
"""
        cbu__mbv = {}
        exec(nuxja__ghnih, {'bodo': bodo}, cbu__mbv)
        return cbu__mbv['impl']
    return bodo.hiframes.dataframe_impl._gen_init_df(nuxja__ghnih,
        col_names, gplqp__juouu, ysfz__grm)
...
def _gen_iloc_getitem_row_impl(df, col_names, idx):
    ...
    nuxja__ghnih += '  df = I._obj\n'
    xiom__eol = ', '.join(
        f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {df.columns.index(vialh__cqpac)})[{idx}]'
         for vialh__cqpac in col_names)
    nuxja__ghnih += f"""  row_idx = bodo.hiframes.pd_index_ext.init_heter_index({gen_const_tup(col_names)}, None)
"""
    nuxja__ghnih += f"""  return bodo.hiframes.pd_series_ext.init_series(({xiom__eol},), row_idx, None)
"""
    cbu__mbv = {}
    exec(nuxja__ghnih, {'bodo': bodo}, cbu__mbv)
    impl = cbu__mbv['impl']
...
class DataFrameLocType(types.Type):
    ...
    def __init__(self, df_type):
        self.df_type = df_type
        zjr__wzigi = 'DataFrameLocType({})'.format(df_type)
        super(DataFrameLocType, self).__init__(zjr__wzigi)
...
class DataFrameLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        tnaxo__vqpp = [('obj', fe_type.df_type)]
        super(DataFrameLocModel, self).__init__(dmm, fe_type, tnaxo__vqpp)
...
def init_dataframe_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        bub__egg, = args
        jgyon__bcskr = signature.return_type
        xvz__xzy = cgutils.create_struct_proxy(jgyon__bcskr)(context, builder)
        xvz__xzy.obj = bub__egg
        context.nrt.incref(builder, signature.args[0], bub__egg)
        return xvz__xzy._getvalue()
...
def overload_loc_getitem(I, idx):
    ...
    if is_list_like_index_type(idx) and idx.dtype == types.bool_:
        ...
        gplqp__juouu = ', '.join(
            ...
            .format(df.columns.index(vialh__cqpac)) for vialh__cqpac in df.
            columns)
        return bodo.hiframes.dataframe_impl._gen_init_df(nuxja__ghnih, df.
            columns, gplqp__juouu, ysfz__grm)
    if isinstance(idx, types.BaseTuple) and len(idx) == 2:
        tkdd__yaua = idx.types[1]
        if is_overload_constant_str(tkdd__yaua):
            bwe__aisf = get_overload_const_str(tkdd__yaua)
            yld__bce = df.columns.index(bwe__aisf)

            def impl_col_name(I, idx):
                df = I._obj
                ysfz__grm = bodo.hiframes.pd_dataframe_ext.get_dataframe_index(
                    df)
                xdp__lojyk = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(
                    df, yld__bce)
                return bodo.hiframes.pd_series_ext.init_series(xdp__lojyk,
                    ysfz__grm, bwe__aisf).loc[idx[0]]
            return impl_col_name
        if is_overload_constant_list(tkdd__yaua):
            col_idx_list = get_overload_const_list(tkdd__yaua)
            if len(col_idx_list) > 0 and not isinstance(col_idx_list[0], (
                bool, np.bool_)) and not all(vialh__cqpac in df.columns for
                vialh__cqpac in col_idx_list):
                raise_bodo_error(
...
def gen_df_loc_col_select_impl(df, col_idx_list):
    ...
    nuxja__ghnih += '  df = I._obj\n'
    return bodo.hiframes.dataframe_impl._gen_init_df(nuxja__ghnih,
        col_idx_list, gplqp__juouu, ysfz__grm)
...
class DataFrameIatType(types.Type):
    ...
    def __init__(self, df_type):
        self.df_type = df_type
        zjr__wzigi = 'DataFrameIatType({})'.format(df_type)
        super(DataFrameIatType, self).__init__(zjr__wzigi)
...
class DataFrameIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        tnaxo__vqpp = [('obj', fe_type.df_type)]
        super(DataFrameIatModel, self).__init__(dmm, fe_type, tnaxo__vqpp)
...
def init_dataframe_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        bub__egg, = args
        ehbc__icwm = signature.return_type
        dju__vci = cgutils.create_struct_proxy(ehbc__icwm)(context, builder)
        dju__vci.obj = bub__egg
        context.nrt.incref(builder, signature.args[0], bub__egg)
        return dju__vci._getvalue()
...
def overload_iat_getitem(I, idx):
    ...
    if isinstance(idx, types.BaseTuple) and len(idx) == 2:
        ...
        def impl_col_ind(I, idx):
            df = I._obj
            xdp__lojyk = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df,
                yld__bce)
            return xdp__lojyk[idx[0]]
        return impl_col_ind
...
def overload_iat_setitem(I, idx, val):
    ...
    if isinstance(idx, types.BaseTuple) and len(idx) == 2:
        ...
        yld__bce = get_overload_const_int(idx.types[1])
        if is_immutable_array(I.df_type.data[yld__bce]):
            raise BodoError(
        ...
        def impl_col_ind(I, idx, val):
            ...
            xdp__lojyk = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df,
                yld__bce)
            xdp__lojyk[idx[0]] = bodo.utils.conversion.unbox_if_timestamp(val)
        return impl_col_ind
...
def cast_series_iat(context, builder, fromty, toty, val):
    dju__vci = cgutils.create_struct_proxy(fromty)(context, builder, val)
    ubzt__acbm = context.cast(builder, dju__vci.obj, fromty.df_type, toty.
        df_type)
    rtjg__fza = cgutils.create_struct_proxy(toty)(context, builder)
    rtjg__fza.obj = ubzt__acbm
    return rtjg__fza._getvalue()
