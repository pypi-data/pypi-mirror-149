...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        yene__dfw = []
        tqwq__ztoax = h5g_get_num_objs(obj_id)
        for mraa__sgvke in range(tqwq__ztoax):
            rcjaj__rzl = h5g_get_objname_by_idx(obj_id, mraa__sgvke)
            yene__dfw.append(rcjaj__rzl)
        return yene__dfw
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    omhac__jyjfe = parse_dtype(dtype)
    hemjk__kpg = np.int32(numba_to_c_type(omhac__jyjfe))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            fecm__ylu = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    fyvbv__xtf = tuple_to_ptr(starts)
    cfqkr__ooxei = tuple_to_ptr(counts)
    ynwl__wdrtw = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, fyvbv__xtf,
        cfqkr__ooxei, is_parallel, out_arr.ctypes, ynwl__wdrtw)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    cfqkr__ooxei = tuple_to_ptr(counts)
    ynwl__wdrtw = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, fyvbv__xtf,
        cfqkr__ooxei, is_parallel, out_arr.ctypes, ynwl__wdrtw)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        ajxi__tye = types.Array(dtype, ndim, 'C')
        upd__tjfyt = args[3]
        ajxi__tye = self.context.resolve_function_type(operator.getitem, [
            ajxi__tye, upd__tjfyt], {}).return_type.copy(layout='C')
        return signature(ajxi__tye, *args)
...
def get_filter_read_indices(bool_arr):
    ewb__ompcl = bool_arr.nonzero()[0]
    oivx__yqn = bodo.libs.distributed_api.get_rank()
    iuud__esp = bodo.libs.distributed_api.get_size()
    objbk__ahma = np.empty(iuud__esp, np.int64)
    apg__rapj = len(bool_arr)
    bodo.libs.distributed_api.allgather(objbk__ahma, apg__rapj)
    crkp__hjjf = objbk__ahma.cumsum()[oivx__yqn] - apg__rapj
    ewb__ompcl += crkp__hjjf
    alz__uhii = bodo.libs.distributed_api.dist_reduce(len(ewb__ompcl), np.
        int32(sum_op))
    fqn__pgq = bodo.libs.distributed_api.gatherv(ewb__ompcl)
    if oivx__yqn == 0:
        lno__fwz = fqn__pgq
    else:
        lno__fwz = np.empty(alz__uhii, ewb__ompcl.dtype)
    bodo.libs.distributed_api.bcast(lno__fwz)
    pgn__usg = bodo.libs.distributed_api.get_start(alz__uhii, iuud__esp,
        oivx__yqn)
    bsb__bkoah = bodo.libs.distributed_api.get_end(alz__uhii, iuud__esp,
        oivx__yqn)
    return lno__fwz[pgn__usg:bsb__bkoah]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        poeid__spya = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], poeid__spya)
        return builder.bitcast(poeid__spya, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    cfqkr__ooxei = tuple_to_ptr(counts)
    ynwl__wdrtw = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, fyvbv__xtf,
        cfqkr__ooxei, is_parallel, out_arr.ctypes, ynwl__wdrtw,
