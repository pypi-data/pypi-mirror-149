...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        ged__hxzov = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, ged__hxzov)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            csk__usql = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(csk__usql.dtype)
            file_write(unicode_to_utf8(fname), csk__usql.ctypes, dtype_size *
                csk__usql.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            csk__usql = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(csk__usql.dtype)
            eeu__unri = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                csk__usql.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), csk__usql.ctypes,
                start, count, eeu__unri)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            ouzn__dlb = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                ouzn__dlb = min(ouzn__dlb, count * dtype_size)
            if ouzn__dlb < 0:
                return -1
            return ouzn__dlb
        return impl
