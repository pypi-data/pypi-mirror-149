...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        wbnf__eaduh = []
        cbzw__fjord = h5g_get_num_objs(obj_id)
        for vebv__oehr in range(cbzw__fjord):
            odmcu__qft = h5g_get_objname_by_idx(obj_id, vebv__oehr)
            wbnf__eaduh.append(odmcu__qft)
        return wbnf__eaduh
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    noep__ipya = parse_dtype(dtype)
    gzfi__jhbhg = np.int32(numba_to_c_type(noep__ipya))
    ...
    def impl(obj_id, name, shape=None, dtype=None, data=None):
        ...
        return h5_create_dset(unify_h5_id(obj_id), unicode_to_utf8(name),
            ndim, counts.ctypes, gzfi__jhbhg)
    return impl
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            eox__ovf = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ymdmw__hwdad = tuple_to_ptr(starts)
    sams__eqkn = tuple_to_ptr(counts)
    jhfl__bqe = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, ymdmw__hwdad,
        sams__eqkn, is_parallel, out_arr.ctypes, jhfl__bqe)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    sams__eqkn = tuple_to_ptr(counts)
    jhfl__bqe = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, ymdmw__hwdad,
        sams__eqkn, is_parallel, out_arr.ctypes, jhfl__bqe)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        toqt__nsy = types.Array(dtype, ndim, 'C')
        osh__xmlv = args[3]
        toqt__nsy = self.context.resolve_function_type(operator.getitem, [
            toqt__nsy, osh__xmlv], {}).return_type.copy(layout='C')
        return signature(toqt__nsy, *args)
...
def get_filter_read_indices(bool_arr):
    ibxxa__kbn = bool_arr.nonzero()[0]
    mhs__eopee = bodo.libs.distributed_api.get_rank()
    zwi__qzcfd = bodo.libs.distributed_api.get_size()
    vezw__offov = np.empty(zwi__qzcfd, np.int64)
    rur__lcv = len(bool_arr)
    bodo.libs.distributed_api.allgather(vezw__offov, rur__lcv)
    vrfl__tqjiv = vezw__offov.cumsum()[mhs__eopee] - rur__lcv
    ibxxa__kbn += vrfl__tqjiv
    ubef__pthjj = bodo.libs.distributed_api.dist_reduce(len(ibxxa__kbn), np
        .int32(sum_op))
    bnd__ptmgi = bodo.libs.distributed_api.gatherv(ibxxa__kbn)
    if mhs__eopee == 0:
        jewsa__txi = bnd__ptmgi
    else:
        jewsa__txi = np.empty(ubef__pthjj, ibxxa__kbn.dtype)
    bodo.libs.distributed_api.bcast(jewsa__txi)
    hiky__fhql = bodo.libs.distributed_api.get_start(ubef__pthjj,
        zwi__qzcfd, mhs__eopee)
    ycvow__das = bodo.libs.distributed_api.get_end(ubef__pthjj, zwi__qzcfd,
        mhs__eopee)
    return jewsa__txi[hiky__fhql:ycvow__das]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        yyk__kgu = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], yyk__kgu)
        return builder.bitcast(yyk__kgu, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    sams__eqkn = tuple_to_ptr(counts)
    jhfl__bqe = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, ymdmw__hwdad,
        sams__eqkn, is_parallel, out_arr.ctypes, jhfl__bqe, read_indices.
