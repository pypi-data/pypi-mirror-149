...
def _global_except_hook(exctype, value, traceback):
    ...
    jkdtv__ecf = MPI.COMM_WORLD
    pdz__ela = jkdtv__ecf.Get_rank()
    faw__esbm = 3.0
    dhdqx__seeox = True
    dplm__oukgd = jkdtv__ecf.Ibarrier()
    ilgjg__ipm = time.time()
    while time.time() - ilgjg__ipm < faw__esbm:
        time.sleep(0.1)
        if dplm__oukgd.Test():
            dhdqx__seeox = False
            break
    try:
        ...
        if dhdqx__seeox:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {pdz__ela}. \n')
