...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        ixmb__iwp = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(ixmb__iwp)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ajv__ifwg = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, ajv__ifwg)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        jpvpt__zibny, = args
        quh__kssjw = signature.return_type
        brkt__yywa = cgutils.create_struct_proxy(quh__kssjw)(context, builder)
        brkt__yywa.obj = jpvpt__zibny
        context.nrt.incref(builder, signature.args[0], jpvpt__zibny)
        return brkt__yywa._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                mgavu__zzgm = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        ixmb__iwp = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(ixmb__iwp)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ajv__ifwg = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, ajv__ifwg)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        jpvpt__zibny, = args
        xbufc__vbudn = signature.return_type
        bcnxo__exc = cgutils.create_struct_proxy(xbufc__vbudn)(context, builder
            )
        bcnxo__exc.obj = jpvpt__zibny
        context.nrt.incref(builder, signature.args[0], jpvpt__zibny)
        return bcnxo__exc._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                xoui__fjt = bodo.utils.conversion.coerce_to_ndarray(idx)
                nwn__fnmrr = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    xoui__fjt]
                zngjf__azbu = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    xoui__fjt]
                ixmb__iwp = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(nwn__fnmrr,
                    zngjf__azbu, ixmb__iwp)
            return impl
        if isinstance(idx, types.SliceType):

            def impl_slice(I, idx):
                S = I._obj
                nwn__fnmrr = bodo.hiframes.pd_series_ext.get_series_data(S)[idx
                    ]
                zngjf__azbu = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    idx]
                ixmb__iwp = bodo.hiframes.pd_series_ext.get_series_name(S)
    ...
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        mgavu__zzgm = (bodo.hiframes.datetime_timedelta_ext
                            ._to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(mgavu__zzgm))
                        xoui__fjt = bodo.utils.conversion.coerce_to_ndarray(idx
                            )
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            xoui__fjt] = s
            ...
            def impl_arr(I, idx, val):
                xoui__fjt = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[xoui__fjt
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        ixmb__iwp = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(ixmb__iwp)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ajv__ifwg = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, ajv__ifwg)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        jpvpt__zibny, = args
        jtnm__oapjo = signature.return_type
        sjp__mmis = cgutils.create_struct_proxy(jtnm__oapjo)(context, builder)
        sjp__mmis.obj = jpvpt__zibny
        context.nrt.incref(builder, signature.args[0], jpvpt__zibny)
        return sjp__mmis._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            nwn__fnmrr = bodo.hiframes.pd_series_ext.get_series_data(S)
            zngjf__azbu = bodo.hiframes.pd_series_ext.get_series_index(S)
            osq__yoihm = (idx - zngjf__azbu._start) // zngjf__azbu._step
            return nwn__fnmrr[osq__yoihm]
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    ...
                    xoui__fjt = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(nwn__fnmrr[
                        xoui__fjt])
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    znc__taj = bodo.hiframes.pd_series_ext.get_series_index(S)
                    nvjlb__bvb = znc__taj.get_loc(idx)
                    if nvjlb__bvb is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(nvjlb__bvb)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                xoui__fjt = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[xoui__fjt
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        xbhpm__sego = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(fkg__gxpj,
            int) for fkg__gxpj in xbhpm__sego):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            wfumj__xeshy = get_literal_value(idx)
            if wfumj__xeshy not in xbhpm__sego:
                raise_bodo_error(
                    f"Series label-based getitem: '{wfumj__xeshy}' not in {xbhpm__sego}"
                    )
            arr_ind = xbhpm__sego.index(wfumj__xeshy)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    brkt__yywa = cgutils.create_struct_proxy(fromty)(context, builder, val)
    ypm__btjkp = context.cast(builder, brkt__yywa.obj, fromty.stype, toty.stype
        )
    buj__run = cgutils.create_struct_proxy(toty)(context, builder)
    buj__run.obj = ypm__btjkp
    return buj__run._getvalue()
