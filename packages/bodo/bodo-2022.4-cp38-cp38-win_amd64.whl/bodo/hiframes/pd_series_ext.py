...
def series_getiter(context, builder, sig, args):
    asjo__lpil = get_series_payload(context, builder, sig.args[0], args[0])
    impl = context.get_function('getiter', sig.return_type(sig.args[0].data))
    return impl(builder, (asjo__lpil.data,))
...
class HeterSeriesAttribute(OverloadedKeyAttributeTemplate):
    ...
    def generic_resolve(self, S, attr):
        ...
        if isinstance(S.index, HeterogeneousIndexType
            ...
            gjww__vgyzv = get_overload_const_tuple(S.index.data)
            if attr in gjww__vgyzv:
                vfm__iyzyq = gjww__vgyzv.index(attr)
                return S.data[vfm__iyzyq]
...
class SeriesPayloadModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        wiir__zlce = [('data', fe_type.series_type.data), ('index', fe_type
            .series_type.index), ('name', fe_type.series_type.name_typ)]
        super(SeriesPayloadModel, self).__init__(dmm, fe_type, wiir__zlce)
...
class SeriesModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ...
        wiir__zlce = [('meminfo', types.MemInfoPointer(payload_type)), (
            'parent', types.pyobject)]
        super(SeriesModel, self).__init__(dmm, fe_type, wiir__zlce)


def define_series_dtor(context, builder, series_type, payload_type):
    dbjs__lpu = builder.module
    twbh__uxa = lir.FunctionType(lir.VoidType(), [cgutils.voidptr_t])
    glzm__nrdyz = cgutils.get_or_insert_function(dbjs__lpu, twbh__uxa, name
        ='.dtor.series.{}'.format(series_type))
    if not glzm__nrdyz.is_declaration:
        return glzm__nrdyz
    glzm__nrdyz.linkage = 'linkonce_odr'
    builder = lir.IRBuilder(glzm__nrdyz.append_basic_block())
    ulxyz__mszm = glzm__nrdyz.args[0]
    kroqe__nvzdq = context.get_value_type(payload_type).as_pointer()
    nayn__xkqy = builder.bitcast(ulxyz__mszm, kroqe__nvzdq)
    qdof__neov = context.make_helper(builder, payload_type, ref=nayn__xkqy)
    context.nrt.decref(builder, series_type.data, qdof__neov.data)
    context.nrt.decref(builder, series_type.index, qdof__neov.index)
    context.nrt.decref(builder, series_type.name_typ, qdof__neov.name)
    builder.ret_void()
    return glzm__nrdyz
...
def construct_series(context, builder, series_type, data_val, index_val,
    ...
    asjo__lpil = cgutils.create_struct_proxy(payload_type)(context, builder)
    asjo__lpil.data = data_val
    asjo__lpil.index = index_val
    asjo__lpil.name = name_val
    fax__ixmtn = context.get_value_type(payload_type)
    qwydl__sswn = context.get_abi_sizeof(fax__ixmtn)
    twkw__jdoq = define_series_dtor(context, builder, series_type, payload_type
        )
    zkycg__hdb = context.nrt.meminfo_alloc_dtor(builder, context.
        get_constant(types.uintp, qwydl__sswn), twkw__jdoq)
    xaac__nflq = context.nrt.meminfo_data(builder, zkycg__hdb)
    ydj__hnj = builder.bitcast(xaac__nflq, fax__ixmtn.as_pointer())
    builder.store(asjo__lpil._getvalue(), ydj__hnj)
    series = cgutils.create_struct_proxy(series_type)(context, builder)
    series.meminfo = zkycg__hdb
...
def init_series(typingctx, data, index, name=None):
    ...
    def codegen(context, builder, signature, args):
        ...
        vvem__rxqmi = construct_series(context, builder, series_type,
    ...
    if is_heterogeneous_tuple_type(data):
        kau__rubgi = HeterogeneousSeriesType(data, index, name)
    else:
        dtype = data.dtype
        data = if_series_to_array_type(data)
        kau__rubgi = SeriesType(dtype, data, index, name)
    sig = signature(kau__rubgi, data, index, name)
    return sig, codegen
...
def init_series_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    data = args[0]
    index = args[1]
    kkv__evi = self.typemap[data.name]
    if is_heterogeneous_tuple_type(kkv__evi) or isinstance(kkv__evi, types.
        BaseTuple):
        return None
    fzoae__eymy = self.typemap[index.name]
    if not isinstance(fzoae__eymy, HeterogeneousIndexType
...
def get_series_payload(context, builder, series_type, value):
    zkycg__hdb = cgutils.create_struct_proxy(series_type)(context, builder,
        value).meminfo
    payload_type = SeriesPayloadType(series_type)
    qdof__neov = context.nrt.meminfo_data(builder, zkycg__hdb)
    kroqe__nvzdq = context.get_value_type(payload_type).as_pointer()
    qdof__neov = builder.bitcast(qdof__neov, kroqe__nvzdq)
    return context.make_helper(builder, payload_type, ref=qdof__neov)
...
def get_series_data(typingctx, series_typ=None):
    ...
    def codegen(context, builder, signature, args):
        asjo__lpil = get_series_payload(context, builder, signature.args[0],
            args[0])
        return impl_ret_borrowed(context, builder, series_typ.data,
            asjo__lpil.data)
    kau__rubgi = series_typ.data
    sig = signature(kau__rubgi, series_typ)
    return sig, codegen
...
def get_series_data_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    xfb__yjc = args[0]
    kkv__evi = self.typemap[xfb__yjc.name].data
    if is_heterogeneous_tuple_type(kkv__evi) or isinstance(kkv__evi, types.
        BaseTuple):
        return None
    if equiv_set.has_shape(xfb__yjc):
        return ArrayAnalysis.AnalyzeResult(shape=xfb__yjc, pre=[])
    return None
...
def get_series_index_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    assert len(args) == 1 and not kws
    xfb__yjc = args[0]
    fzoae__eymy = self.typemap[xfb__yjc.name].index
    if isinstance(fzoae__eymy, HeterogeneousIndexType):
        return None
    if equiv_set.has_shape(xfb__yjc):
        return ArrayAnalysis.AnalyzeResult(shape=xfb__yjc, pre=[])
    return None
...
def cast_series(context, builder, fromty, toty, val):
    if fromty.copy(index=toty.index) == toty and isinstance(fromty.index,
        ...
        asjo__lpil = get_series_payload(context, builder, fromty, val)
        noyme__imbdm = context.cast(builder, asjo__lpil.index, fromty.index,
            toty.index)
        context.nrt.incref(builder, fromty.data, asjo__lpil.data)
        context.nrt.incref(builder, fromty.name_typ, asjo__lpil.name)
        return construct_series(context, builder, toty, asjo__lpil.data,
            noyme__imbdm, asjo__lpil.name)
...
class SeriesAttribute(OverloadedKeyAttributeTemplate):
    ...
    def resolve_head(self, ary, args, kws):
        tfwap__jcla = 'Series.head'
        yojy__fkn = 'n',
        mpeoa__bdfqq = {'n': 5}
        pysig, enj__xchfd = bodo.utils.typing.fold_typing_args(tfwap__jcla,
            args, kws, yojy__fkn, mpeoa__bdfqq)
        lgyt__xay = enj__xchfd[0]
        if not is_overload_int(lgyt__xay):
            raise BodoError(f"{tfwap__jcla}(): 'n' must be an Integer")
        muo__lkbge = ary
        return muo__lkbge(*enj__xchfd).replace(pysig=pysig)
    ...
    def _resolve_map_func(self, ary, func, pysig, fname, f_args=None, kws=None
        ...
        if f_args is not None:
            xbzxb__dpc += tuple(f_args.types)
        if kws is None:
            kws = {}
        vputv__phtk = False
        anems__fiznb = True
        if fname == 'map' and isinstance(func, types.DictType):
            acwxf__awgi = func.value_type
            vputv__phtk = True
        else:
            try:
                if types.unliteral(func) == types.unicode_type:
                    ...
                    acwxf__awgi = bodo.utils.transform.get_udf_str_return_type(
                        ary, get_overload_const_str(func), self.context,
                        'Series.apply')
                    anems__fiznb = False
                elif bodo.utils.typing.is_numpy_ufunc(func):
                    acwxf__awgi = func.get_call_type(self.context, (ary,), {}
                        ).return_type
                    anems__fiznb = False
                else:
                    acwxf__awgi = get_const_func_output_type(func,
                        xbzxb__dpc, kws, self.context, numba.core.registry.
                        cpu_target.target_context)
            except Exception as jor__dbich:
                raise BodoError(get_udf_error_msg(f'Series.{fname}()',
                    jor__dbich))
        if anems__fiznb:
            if isinstance(acwxf__awgi, (SeriesType, HeterogeneousSeriesType)
                ) and acwxf__awgi.const_info is None:
                raise BodoError(
                    'Invalid Series output in UDF (Series with constant length and constant Index value expected)'
                    )
            if isinstance(acwxf__awgi, HeterogeneousSeriesType):
                wjxy__lody, ixif__eipk = acwxf__awgi.const_info
                if isinstance(acwxf__awgi.data, bodo.libs.
                    nullable_tuple_ext.NullableTupleType):
                    agltj__hpvem = acwxf__awgi.data.tuple_typ.types
                elif isinstance(acwxf__awgi.data, types.Tuple):
                    agltj__hpvem = acwxf__awgi.data.types
                fntwy__dmjk = tuple(to_nullable_type(dtype_to_array_type(t)
                    ) for t in agltj__hpvem)
                mxyhu__rcce = bodo.DataFrameType(fntwy__dmjk, ary.index,
                    ixif__eipk)
            elif isinstance(acwxf__awgi, SeriesType):
                lrq__gsvln, ixif__eipk = acwxf__awgi.const_info
                fntwy__dmjk = tuple(to_nullable_type(dtype_to_array_type(
                    acwxf__awgi.dtype)) for wjxy__lody in range(lrq__gsvln))
                mxyhu__rcce = bodo.DataFrameType(fntwy__dmjk, ary.index,
                    ixif__eipk)
            else:
                ggp__uaz = get_udf_out_arr_type(acwxf__awgi, vputv__phtk)
                mxyhu__rcce = SeriesType(ggp__uaz.dtype, ggp__uaz, ary.
                    index, ary.name_typ)
        else:
            mxyhu__rcce = acwxf__awgi
        return signature(mxyhu__rcce, (func,)).replace(pysig=pysig)
    ...
    def resolve_map(self, ary, args, kws):
        ...
        na_action = args[1] if len(args) > 1 else kws.pop('na_action',
            types.none)
        tpyl__rvy = dict(na_action=na_action)
        izn__sqpyq = dict(na_action=None)
        check_unsupported_args('Series.map', tpyl__rvy, izn__sqpyq,
    ...
    def resolve_apply(self, ary, args, kws):
        ...
        kws.pop('func', None)
        mzya__cdc = args[1] if len(args) > 1 else kws.pop('convert_dtype',
            types.literal(True))
        f_args = args[2] if len(args) > 2 else kws.pop('args', None)
        tpyl__rvy = dict(convert_dtype=mzya__cdc)
        lkmod__qrnnc = dict(convert_dtype=True)
        check_unsupported_args('Series.apply', tpyl__rvy, lkmod__qrnnc,
            package_name='pandas', module_name='Series')
        xxwsi__wfi = ', '.join("{} = ''".format(jgcbz__niw) for jgcbz__niw in
            kws.keys())
        fsgze__voo = (
            f'def apply_stub(func, convert_dtype=True, args=(), {xxwsi__wfi}):\n'
            )
        fsgze__voo += '    pass\n'
        tqoc__wqo = {}
        exec(fsgze__voo, {}, tqoc__wqo)
        ehwbg__qztg = tqoc__wqo['apply_stub']
        pysig = numba.core.utils.pysignature(ehwbg__qztg)
    ...
    def _resolve_combine_func(self, ary, args, kws):
        ...
        zmpnz__qyf = ary.dtype
        bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(ary,
            'Series.combine()')
        bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(other,
            'Series.combine()')
        if zmpnz__qyf == types.NPDatetime('ns'):
            zmpnz__qyf = pd_timestamp_type
        tgv__ecd = other.dtype
        if tgv__ecd == types.NPDatetime('ns'):
            tgv__ecd = pd_timestamp_type
        acwxf__awgi = get_const_func_output_type(func, (zmpnz__qyf,
            tgv__ecd), {}, self.context, numba.core.registry.cpu_target.
            target_context)
        sig = signature(SeriesType(acwxf__awgi, index=ary.index, name_typ=
...
def pd_series_overload(data=None, index=None, dtype=None, name=None, copy=
    ...
    zjj__lsmlm = is_overload_none(data)
    tvwfn__optpo = is_overload_none(index)
    gqa__pnzm = is_overload_none(dtype)
    if zjj__lsmlm and tvwfn__optpo and gqa__pnzm:
        raise BodoError(
            'pd.Series() requires at least 1 of data, index, and dtype to not be none'
            )
    if is_series_type(data) and not tvwfn__optpo:
        raise BodoError(
    ...
    if is_heterogeneous_tuple_type(data) and is_overload_none(dtype):
        bor__ojoty = tuple(len(data) * [False])

        def impl_heter(data=None, index=None, dtype=None, name=None, copy=
            False, fastpath=False):
            xduz__sth = bodo.utils.conversion.extract_index_if_none(data, index
                )
            mvpst__aozpd = bodo.utils.conversion.to_tuple(data)
            data_val = bodo.libs.nullable_tuple_ext.build_nullable_tuple(
                mvpst__aozpd, bor__ojoty)
            return bodo.hiframes.pd_series_ext.init_series(data_val, bodo.
                utils.conversion.convert_to_index(xduz__sth), name)
        return impl_heter
    if zjj__lsmlm:
        if gqa__pnzm:
            ...
            def impl(data=None, index=None, dtype=None, name=None, copy=
                False, fastpath=False):
                zsdnw__ueex = bodo.utils.conversion.extract_name_if_none(data,
                    name)
                xduz__sth = bodo.utils.conversion.extract_index_if_none(data,
                    index)
                numba.parfors.parfor.init_prange()
                bfxt__vtiz = len(xduz__sth)
                mvpst__aozpd = np.empty(bfxt__vtiz, np.float64)
                for puwvl__lpbw in numba.parfors.parfor.internal_prange(
                    bfxt__vtiz):
                    bodo.libs.array_kernels.setna(mvpst__aozpd, puwvl__lpbw)
                return bodo.hiframes.pd_series_ext.init_series(mvpst__aozpd,
                    bodo.utils.conversion.convert_to_index(xduz__sth),
                    zsdnw__ueex)
            return impl
        if bodo.utils.conversion._is_str_dtype(dtype):
            dawc__kxy = bodo.string_array_type
        else:
            uqa__mkxkn = bodo.utils.typing.parse_dtype(dtype, 'pandas.Series')
            if isinstance(uqa__mkxkn, bodo.libs.int_arr_ext.IntDtype):
                dawc__kxy = bodo.IntegerArrayType(uqa__mkxkn.dtype)
            elif uqa__mkxkn == bodo.libs.bool_arr_ext.boolean_dtype:
                dawc__kxy = bodo.boolean_array
            elif isinstance(uqa__mkxkn, types.Number) or uqa__mkxkn in [bodo
                .datetime64ns, bodo.timedelta64ns]:
                dawc__kxy = types.Array(uqa__mkxkn, 1, 'C')
            else:
                raise BodoError(
        ...
        if tvwfn__optpo:
            ...
            def impl(data=None, index=None, dtype=None, name=None, copy=
                ...
                zsdnw__ueex = bodo.utils.conversion.extract_name_if_none(data,
                    name)
                xduz__sth = bodo.hiframes.pd_index_ext.init_range_index(0, 
                    0, 1, None)
                numba.parfors.parfor.init_prange()
                bfxt__vtiz = len(xduz__sth)
                mvpst__aozpd = bodo.utils.utils.alloc_type(bfxt__vtiz,
                    dawc__kxy, (-1,))
                return bodo.hiframes.pd_series_ext.init_series(mvpst__aozpd,
                    xduz__sth, zsdnw__ueex)
            return impl
        else:
    ...
    def impl(data=None, index=None, dtype=None, name=None, copy=False,
        fastpath=False):
        zsdnw__ueex = bodo.utils.conversion.extract_name_if_none(data, name)
        xduz__sth = bodo.utils.conversion.extract_index_if_none(data, index)
        mzz__xigm = bodo.utils.conversion.coerce_to_array(data, True,
            scalar_to_arr_len=len(xduz__sth))
        doj__ons = bodo.utils.conversion.fix_arr_dtype(mzz__xigm, dtype,
            None, False)
        return bodo.hiframes.pd_series_ext.init_series(doj__ons, bodo.utils
            .conversion.convert_to_index(xduz__sth), zsdnw__ueex)
    return impl
...
def lower_constant_series(context, builder, series_type, pyval):
    if isinstance(series_type.data, bodo.DatetimeArrayType):
        zbini__vcaap = pyval.array
    else:
        zbini__vcaap = pyval.values
    ...
    name_val = context.get_constant_generic(builder, series_type.name_typ,
        pyval.name)
    qdof__neov = lir.Constant.literal_struct([data_val, index_val, name_val])
    qdof__neov = cgutils.global_constant(builder, '.const.payload', qdof__neov
        ).bitcast(cgutils.voidptr_t)
    fbh__wctb = context.get_constant(types.int64, -1)
    duc__oozyf = context.get_constant_null(types.voidptr)
    zkycg__hdb = lir.Constant.literal_struct([fbh__wctb, duc__oozyf,
        duc__oozyf, qdof__neov, fbh__wctb])
    zkycg__hdb = cgutils.global_constant(builder, '.const.meminfo', zkycg__hdb
        ).bitcast(cgutils.voidptr_t)
    vvem__rxqmi = lir.Constant.literal_struct([zkycg__hdb, duc__oozyf])
    return vvem__rxqmi
...
def _install_series_unsupported():
    for adiy__jlqh in series_unsupported_attrs:
        buiq__bava = 'Series.' + adiy__jlqh
        overload_attribute(SeriesType, adiy__jlqh)(create_unsupported_overload
            (buiq__bava))
    for fname in series_unsupported_methods:
        buiq__bava = 'Series.' + fname
        overload_method(SeriesType, fname, no_unliteral=True)(
            create_unsupported_overload(buiq__bava))
...
def _install_heter_series_unsupported():
    for adiy__jlqh in heter_series_unsupported_attrs:
        buiq__bava = 'HeterogeneousSeries.' + adiy__jlqh
        overload_attribute(HeterogeneousSeriesType, adiy__jlqh)(
            create_unsupported_overload(buiq__bava))
    for fname in heter_series_unsupported_methods:
        buiq__bava = 'HeterogeneousSeries.' + fname
