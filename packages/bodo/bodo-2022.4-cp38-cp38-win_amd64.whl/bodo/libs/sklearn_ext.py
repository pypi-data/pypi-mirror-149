...
except ImportError as zhgzi__fit:
    pass


def check_sklearn_version():
    if not _is_sklearn_supported_version:
        ukcs__aymq = f""" Bodo supports scikit-learn version >= {_min_sklearn_ver_str} and < {_max_sklearn_ver_str}.
             Installed version is {sklearn.__version__}.
"""
        raise BodoError(ukcs__aymq)


def random_forest_model_fit(m, X, y):
    lgs__ryl = m.n_estimators
    ljci__dro = MPI.Get_processor_name()
    lqgwq__vhyne = get_host_ranks()
    csi__nebfc = len(lqgwq__vhyne)
    fwp__fbp = bodo.get_rank()
    m.n_estimators = bodo.libs.distributed_api.get_node_portion(lgs__ryl,
        csi__nebfc, fwp__fbp)
    if fwp__fbp == lqgwq__vhyne[ljci__dro][0]:
        m.n_jobs = len(lqgwq__vhyne[ljci__dro])
    ...
    obmdl__bfx = create_subcomm_mpi4py(first_rank_node)
    if obmdl__bfx != MPI.COMM_NULL:
        enwsi__kxis = 10
        cta__oti = bodo.libs.distributed_api.get_node_portion(lgs__ryl,
            csi__nebfc, 0)
        yes__bata = cta__oti // enwsi__kxis
        if cta__oti % enwsi__kxis != 0:
            yes__bata += 1
        rgyhs__besxn = []
        for gfxy__npp in range(yes__bata):
            xzq__gkaz = obmdl__bfx.gather(m.estimators_[gfxy__npp *
                enwsi__kxis:gfxy__npp * enwsi__kxis + enwsi__kxis])
            if fwp__fbp == 0:
                rgyhs__besxn += list(itertools.chain.from_iterable(xzq__gkaz))
        if fwp__fbp == 0:
            m.estimators_ = rgyhs__besxn
    fuhpz__aezxg = MPI.COMM_WORLD
    if fwp__fbp == 0:
        for gfxy__npp in range(0, lgs__ryl, 10):
            fuhpz__aezxg.bcast(m.estimators_[gfxy__npp:gfxy__npp + 10])
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            fuhpz__aezxg.bcast(m.n_classes_)
            fuhpz__aezxg.bcast(m.classes_)
        fuhpz__aezxg.bcast(m.n_outputs_)
    else:
        ijr__kbexz = []
        for gfxy__npp in range(0, lgs__ryl, 10):
            ijr__kbexz += fuhpz__aezxg.bcast(None)
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            m.n_classes_ = fuhpz__aezxg.bcast(None)
            m.classes_ = fuhpz__aezxg.bcast(None)
        m.n_outputs_ = fuhpz__aezxg.bcast(None)
        m.estimators_ = ijr__kbexz
    assert len(m.estimators_) == lgs__ryl
    m.n_estimators = lgs__ryl
...
def precision_recall_fscore_support_helper(MCM, average):
    ...
    def multilabel_confusion_matrix(y_true, y_pred, *, sample_weight=None,
        ...
        return MCM
    qjd__yob = sklearn.metrics._classification.multilabel_confusion_matrix
    result = -1.0
    try:
        ...
        sklearn.metrics._classification.multilabel_confusion_matrix = qjd__yob
    return result
...
def precision_recall_fscore_parallel(y_true, y_pred, operation, average=
    ...
    nlmmh__udet = len(labels)
    wyc__taf = np.zeros(nlmmh__udet, np.int64)
    kdq__qggb = np.zeros(nlmmh__udet, np.int64)
    mllt__asya = np.zeros(nlmmh__udet, np.int64)
    mxnch__fcjaa = (bodo.hiframes.pd_categorical_ext.
        get_label_dict_from_categories(labels))
    for gfxy__npp in range(len(y_true)):
        kdq__qggb[mxnch__fcjaa[y_true[gfxy__npp]]] += 1
        if y_pred[gfxy__npp] not in mxnch__fcjaa:
            continue
        mls__bman = mxnch__fcjaa[y_pred[gfxy__npp]]
        mllt__asya[mls__bman] += 1
        if y_true[gfxy__npp] == y_pred[gfxy__npp]:
            wyc__taf[mls__bman] += 1
    wyc__taf = bodo.libs.distributed_api.dist_reduce(wyc__taf, np.int32(
        Reduce_Type.Sum.value))
    kdq__qggb = bodo.libs.distributed_api.dist_reduce(kdq__qggb, np.int32(
        Reduce_Type.Sum.value))
    mllt__asya = bodo.libs.distributed_api.dist_reduce(mllt__asya, np.int32
        (Reduce_Type.Sum.value))
    zan__pnwag = mllt__asya - wyc__taf
    fbij__nxdaj = kdq__qggb - wyc__taf
    bcm__tzxw = wyc__taf
    pro__nesv = y_true.shape[0] - bcm__tzxw - zan__pnwag - fbij__nxdaj
    with numba.objmode(result='float64[:]'):
        MCM = np.array([pro__nesv, zan__pnwag, fbij__nxdaj, bcm__tzxw]
            ).T.reshape(-1, 2, 2)
...
def mse_mae_dist_helper(y_true, y_pred, sample_weight, multioutput, squared,
    metric):
    if metric == 'mse':
        gfw__tmjs = sklearn.metrics.mean_squared_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values', squared=True
            )
    elif metric == 'mae':
        gfw__tmjs = sklearn.metrics.mean_absolute_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values')
    else:
        raise RuntimeError(
            f"Unrecognized metric {metric}. Must be one of 'mae' and 'mse'")
    fuhpz__aezxg = MPI.COMM_WORLD
    tpt__lhj = fuhpz__aezxg.Get_size()
    if sample_weight is not None:
        ksahz__ujl = np.sum(sample_weight)
    else:
        ksahz__ujl = np.float64(y_true.shape[0])
    wfhan__mmp = np.zeros(tpt__lhj, dtype=type(ksahz__ujl))
    fuhpz__aezxg.Allgather(ksahz__ujl, wfhan__mmp)
    bdhw__ykmgr = np.zeros((tpt__lhj, *gfw__tmjs.shape), dtype=gfw__tmjs.dtype)
    fuhpz__aezxg.Allgather(gfw__tmjs, bdhw__ykmgr)
    lpbx__ojfpq = np.average(bdhw__ykmgr, weights=wfhan__mmp, axis=0)
    if metric == 'mse' and not squared:
        lpbx__ojfpq = np.sqrt(lpbx__ojfpq)
    if isinstance(multioutput, str) and multioutput == 'raw_values':
        return lpbx__ojfpq
    elif isinstance(multioutput, str) and multioutput == 'uniform_average':
        return np.average(lpbx__ojfpq)
    else:
        return np.average(lpbx__ojfpq, weights=multioutput)
...
def accuracy_score_dist_helper(y_true, y_pred, normalize, sample_weight):
    ...
    fuhpz__aezxg = MPI.COMM_WORLD
    score = fuhpz__aezxg.allreduce(score, op=MPI.SUM)
    if normalize:
        effu__vlq = np.sum(sample_weight
            ) if sample_weight is not None else len(y_true)
        effu__vlq = fuhpz__aezxg.allreduce(effu__vlq, op=MPI.SUM)
        score = score / effu__vlq
    return score
...
def check_consistent_length_parallel(*arrays):
    fuhpz__aezxg = MPI.COMM_WORLD
    ivx__sqsm = True
    arhmp__fqd = [len(nll__nkzed) for nll__nkzed in arrays if nll__nkzed is not
        None]
    if len(np.unique(arhmp__fqd)) > 1:
        ivx__sqsm = False
    ivx__sqsm = fuhpz__aezxg.allreduce(ivx__sqsm, op=MPI.LAND)
    return ivx__sqsm
...
def r2_score_dist_helper(y_true, y_pred, sample_weight, multioutput):
    ...
    acdky__jqst = y_true.shape[0]
    hag__agtf = fuhpz__aezxg.allreduce(acdky__jqst, op=MPI.SUM)
    if hag__agtf < 2:
        warnings.warn(
    ...
    if sample_weight is not None:
        ...
        mqyu__vxg = sample_weight[:, np.newaxis]
    else:
        sample_weight = np.float64(y_true.shape[0])
        mqyu__vxg = 1.0
    yze__sqkcd = (mqyu__vxg * (y_true - y_pred) ** 2).sum(axis=0, dtype=np.
        float64)
    sdu__vlp = np.zeros(yze__sqkcd.shape, dtype=yze__sqkcd.dtype)
    fuhpz__aezxg.Allreduce(yze__sqkcd, sdu__vlp, op=MPI.SUM)
    eid__fmu = np.nansum(y_true * mqyu__vxg, axis=0, dtype=np.float64)
    prla__thffr = np.zeros_like(eid__fmu)
    fuhpz__aezxg.Allreduce(eid__fmu, prla__thffr, op=MPI.SUM)
    usmij__yrw = np.nansum(sample_weight, dtype=np.float64)
    uvup__rhz = fuhpz__aezxg.allreduce(usmij__yrw, op=MPI.SUM)
    crv__ofwbt = prla__thffr / uvup__rhz
    wkjs__crwnj = (mqyu__vxg * (y_true - crv__ofwbt) ** 2).sum(axis=0,
        dtype=np.float64)
    mfdf__hnssv = np.zeros(wkjs__crwnj.shape, dtype=wkjs__crwnj.dtype)
    fuhpz__aezxg.Allreduce(wkjs__crwnj, mfdf__hnssv, op=MPI.SUM)
    dlmn__pilhv = mfdf__hnssv != 0
    btqyo__wnugq = sdu__vlp != 0
    bvbu__cfw = dlmn__pilhv & btqyo__wnugq
    uflp__tbry = np.ones([y_true.shape[1] if len(y_true.shape) > 1 else 1])
    uflp__tbry[bvbu__cfw] = 1 - sdu__vlp[bvbu__cfw] / mfdf__hnssv[bvbu__cfw]
    uflp__tbry[btqyo__wnugq & ~dlmn__pilhv] = 0.0
    if isinstance(multioutput, str):
        if multioutput == 'raw_values':
            ...
            wzb__tix = mfdf__hnssv
            if not np.any(dlmn__pilhv):
                if not np.any(btqyo__wnugq):
                    return np.array([1.0])
                else:
                    return np.array([0.0])
    else:
        wzb__tix = multioutput
    return np.array([np.average(uflp__tbry, weights=wzb__tix)])
...
def confusion_matrix_dist_helper(y_true, y_pred, labels=None, sample_weight
    ...
    fuhpz__aezxg = MPI.COMM_WORLD
    try:
        lxzs__xrkm = sklearn.metrics.confusion_matrix(y_true, y_pred,
            labels=labels, sample_weight=sample_weight, normalize=None)
    except ValueError as ljl__eizx:
        lxzs__xrkm = ljl__eizx
    kpa__ydym = isinstance(lxzs__xrkm, ValueError
        ) and 'At least one label specified must be in y_true' in lxzs__xrkm.args[
        0]
    ifd__wvi = fuhpz__aezxg.allreduce(kpa__ydym, op=MPI.LAND)
    if ifd__wvi:
        raise lxzs__xrkm
    elif kpa__ydym:
        dtype = np.int64
        if sample_weight is not None and sample_weight.dtype.kind not in {'i',
            'u', 'b'}:
            dtype = np.float64
        tjz__fjtbf = np.zeros((labels.size, labels.size), dtype=dtype)
    else:
        tjz__fjtbf = lxzs__xrkm
    tjy__nqsup = np.zeros_like(tjz__fjtbf)
    fuhpz__aezxg.Allreduce(tjz__fjtbf, tjy__nqsup)
    with np.errstate(all='ignore'):
        if normalize == 'true':
            tjy__nqsup = tjy__nqsup / tjy__nqsup.sum(axis=1, keepdims=True)
        elif normalize == 'pred':
            tjy__nqsup = tjy__nqsup / tjy__nqsup.sum(axis=0, keepdims=True)
        elif normalize == 'all':
            tjy__nqsup = tjy__nqsup / tjy__nqsup.sum()
        tjy__nqsup = np.nan_to_num(tjy__nqsup)
    return tjy__nqsup
...
def overload_confusion_matrix(y_true, y_pred, labels=None, sample_weight=
    ...
    check_sklearn_version()
    axfne__yyf = 'def _confusion_matrix_impl(\n'
    axfne__yyf += '    y_true, y_pred, labels=None,\n'
    axfne__yyf += '    sample_weight=None, normalize=None,\n'
    axfne__yyf += '    _is_data_distributed=False,\n'
    axfne__yyf += '):\n'
    axfne__yyf += (
        '    y_true = bodo.utils.conversion.coerce_to_array(y_true)\n')
    axfne__yyf += (
    ...
    oohhz__omxjw = 'int64[:,:]', 'np.int64'
    if not is_overload_none(normalize):
        oohhz__omxjw = 'float64[:,:]', 'np.float64'
    ...
    if not is_overload_none(labels):
        ...
        axfne__yyf += '    labels = bodo.allgatherv(labels, False)\n'
        axfne__yyf += """    labels = bodo.libs.array_kernels.sort(labels, ascending=True, inplace=False)
"""
    axfne__yyf += f"    with numba.objmode(cm='{oohhz__omxjw[0]}'):\n"
    if is_overload_false(_is_data_distributed):
        axfne__yyf += '      cm = sklearn.metrics.confusion_matrix(\n'
    else:
        axfne__yyf += '      cm = confusion_matrix_dist_helper(\n'
    axfne__yyf += '        y_true, y_pred, labels=labels,\n'
    axfne__yyf += '        sample_weight=sample_weight, normalize=normalize,\n'
    axfne__yyf += f'      ).astype({oohhz__omxjw[1]})\n'
    axfne__yyf += '    return cm\n'
    wag__tqe = {}
    exec(axfne__yyf, globals(), wag__tqe)
    gciq__lrtd = wag__tqe['_confusion_matrix_impl']
    return gciq__lrtd
...
def fit_sgd(m, X, y, y_classes=None, _is_data_distributed=False):
    fuhpz__aezxg = MPI.COMM_WORLD
    olws__dyk = fuhpz__aezxg.allreduce(len(X), op=MPI.SUM)
    tzc__ntas = len(X) / olws__dyk
    ejkp__zwm = fuhpz__aezxg.Get_size()
    m.n_jobs = 1
    m.early_stopping = False
    czl__chjhs = np.inf
    qywcx__fpuq = 0
    if m.loss == 'hinge':
        lcpmm__fbdwa = hinge_loss
    elif m.loss == 'log':
        lcpmm__fbdwa = log_loss
    elif m.loss == 'squared_error':
        lcpmm__fbdwa = mean_squared_error
    else:
        raise ValueError('loss {} not supported'.format(m.loss))
    fnt__yty = False
    if isinstance(m, sklearn.linear_model.SGDRegressor):
        fnt__yty = True
    for kwfn__ztlym in range(m.max_iter):
        if fnt__yty:
            m.partial_fit(X, y)
        else:
            m.partial_fit(X, y, classes=y_classes)
        m.coef_ = m.coef_ * tzc__ntas
        m.coef_ = fuhpz__aezxg.allreduce(m.coef_, op=MPI.SUM)
        m.intercept_ = m.intercept_ * tzc__ntas
        m.intercept_ = fuhpz__aezxg.allreduce(m.intercept_, op=MPI.SUM)
        if fnt__yty:
            y_pred = m.predict(X)
            bqet__nhqy = lcpmm__fbdwa(y, y_pred)
        else:
            y_pred = m.decision_function(X)
            bqet__nhqy = lcpmm__fbdwa(y, y_pred, labels=y_classes)
        kfgm__hfj = fuhpz__aezxg.allreduce(bqet__nhqy, op=MPI.SUM)
        bqet__nhqy = kfgm__hfj / ejkp__zwm
        if m.tol > np.NINF and bqet__nhqy > czl__chjhs - m.tol * olws__dyk:
            qywcx__fpuq += 1
        else:
            qywcx__fpuq = 0
        if bqet__nhqy < czl__chjhs:
            czl__chjhs = bqet__nhqy
        if qywcx__fpuq >= m.n_iter_no_change:
            break
    return m
...
def kmeans_fit_helper(m, len_X, all_X, all_sample_weight, _is_data_distributed
    ...
    fuhpz__aezxg = MPI.COMM_WORLD
    fwp__fbp = fuhpz__aezxg.Get_rank()
    ljci__dro = MPI.Get_processor_name()
    lqgwq__vhyne = get_host_ranks()
    ypqcy__wbb = m.n_jobs if hasattr(m, 'n_jobs') else None
    vfh__jujhm = m._n_threads if hasattr(m, '_n_threads') else None
    m._n_threads = len(lqgwq__vhyne[ljci__dro])
    if fwp__fbp == 0:
        m.fit(X=all_X, y=None, sample_weight=all_sample_weight)
    if fwp__fbp == 0:
        fuhpz__aezxg.bcast(m.cluster_centers_)
        fuhpz__aezxg.bcast(m.inertia_)
        fuhpz__aezxg.bcast(m.n_iter_)
    else:
        m.cluster_centers_ = fuhpz__aezxg.bcast(None)
        m.inertia_ = fuhpz__aezxg.bcast(None)
        m.n_iter_ = fuhpz__aezxg.bcast(None)
    if _is_data_distributed:
        csd__cwyek = fuhpz__aezxg.allgather(len_X)
        if fwp__fbp == 0:
            rowtf__fjd = np.empty(len(csd__cwyek) + 1, dtype=int)
            np.cumsum(csd__cwyek, out=rowtf__fjd[1:])
            rowtf__fjd[0] = 0
            zew__irdj = [m.labels_[rowtf__fjd[lxvt__enxyn]:rowtf__fjd[
                lxvt__enxyn + 1]] for lxvt__enxyn in range(len(csd__cwyek))]
            ioqel__vlz = fuhpz__aezxg.scatter(zew__irdj)
        else:
            ioqel__vlz = fuhpz__aezxg.scatter(None)
        m.labels_ = ioqel__vlz
    elif fwp__fbp == 0:
        fuhpz__aezxg.bcast(m.labels_)
    else:
        m.labels_ = fuhpz__aezxg.bcast(None)
    m._n_threads = vfh__jujhm
    return m
...
def overload_kmeans_clustering_score(m, X, y=None, sample_weight=None,
    ...
    def _cluster_kmeans_score(m, X, y=None, sample_weight=None,
        ...
        with numba.objmode(result='float64'):
            ...
            if _is_data_distributed:
                fuhpz__aezxg = MPI.COMM_WORLD
                result = fuhpz__aezxg.allreduce(result, op=MPI.SUM)
            m._n_threads = vfh__jujhm
        return result
...
def overload_multinomial_nb_model_fit(m, X, y, sample_weight=None,
    ...
    if is_overload_false(_is_data_distributed):
        ...
        axfne__yyf = 'def _model_multinomial_nb_fit_impl(\n'
        axfne__yyf += (
            '    m, X, y, sample_weight=None, _is_data_distributed=False\n')
        axfne__yyf += '):  # pragma: no cover\n'
        axfne__yyf += '    y = bodo.utils.conversion.coerce_to_ndarray(y)\n'
        if isinstance(X, DataFrameType):
            axfne__yyf += '    X = X.to_numpy()\n'
        else:
            axfne__yyf += (
                '    X = bodo.utils.conversion.coerce_to_ndarray(X)\n')
        axfne__yyf += '    my_rank = bodo.get_rank()\n'
        axfne__yyf += '    nranks = bodo.get_size()\n'
        axfne__yyf += '    total_cols = X.shape[1]\n'
        axfne__yyf += '    for i in range(nranks):\n'
        axfne__yyf += """        start = bodo.libs.distributed_api.get_start(total_cols, nranks, i)
"""
        axfne__yyf += (
            '        end = bodo.libs.distributed_api.get_end(total_cols, nranks, i)\n'
            )
        axfne__yyf += '        if i == my_rank:\n'
        axfne__yyf += (
            '            X_train = bodo.gatherv(X[:, start:end:1], root=i)\n')
        axfne__yyf += '        else:\n'
        axfne__yyf += '            bodo.gatherv(X[:, start:end:1], root=i)\n'
        axfne__yyf += '    y_train = bodo.allgatherv(y, False)\n'
        axfne__yyf += '    with numba.objmode(m="multinomial_nb_type"):\n'
        axfne__yyf += '        m = fit_multinomial_nb(\n'
        axfne__yyf += """            m, X_train, y_train, sample_weight, total_cols, _is_data_distributed
"""
        axfne__yyf += '        )\n'
        axfne__yyf += '    bodo.barrier()\n'
        axfne__yyf += '    return m\n'
        wag__tqe = {}
        exec(axfne__yyf, globals(), wag__tqe)
        zni__cgqy = wag__tqe['_model_multinomial_nb_fit_impl']
        return zni__cgqy
...
def fit_multinomial_nb(m, X_train, y_train, sample_weight=None, total_cols=
    ...
    kwfn__ztlym, n_features = X_train.shape
    m.n_features_in_ = n_features
    hdroi__eheif = LabelBinarizer()
    occjz__trw = hdroi__eheif.fit_transform(y_train)
    m.classes_ = hdroi__eheif.classes_
    if occjz__trw.shape[1] == 1:
        occjz__trw = np.concatenate((1 - occjz__trw, occjz__trw), axis=1)
    if sample_weight is not None:
        occjz__trw = occjz__trw.astype(np.float64, copy=False)
        sample_weight = _check_sample_weight(sample_weight, X_train)
        sample_weight = np.atleast_2d(sample_weight)
        occjz__trw *= sample_weight.T
    class_prior = m.class_prior
    aws__djo = occjz__trw.shape[1]
    m._init_counters(aws__djo, n_features)
    m._count(X_train.astype('float64'), occjz__trw)
    alpha = m._check_alpha()
    m._update_class_log_prior(class_prior=class_prior)
    rkwy__etduc = m.feature_count_ + alpha
    mxgv__swpi = rkwy__etduc.sum(axis=1)
    fuhpz__aezxg = MPI.COMM_WORLD
    ejkp__zwm = fuhpz__aezxg.Get_size()
    baa__rmlnh = np.zeros(aws__djo)
    fuhpz__aezxg.Allreduce(mxgv__swpi, baa__rmlnh, op=MPI.SUM)
    pctlz__xgv = np.log(rkwy__etduc) - np.log(baa__rmlnh.reshape(-1, 1))
    yawsm__cxhnm = pctlz__xgv.T.reshape(n_features * aws__djo)
    qtouk__htowi = np.ones(ejkp__zwm) * (total_cols // ejkp__zwm)
    iwr__vre = total_cols % ejkp__zwm
    for nhdj__bpg in range(iwr__vre):
        qtouk__htowi[nhdj__bpg] += 1
    qtouk__htowi *= aws__djo
    vzxua__avupq = np.zeros(ejkp__zwm, dtype=np.int32)
    vzxua__avupq[1:] = np.cumsum(qtouk__htowi)[:-1]
    wwuvo__sfoj = np.zeros((total_cols, aws__djo), dtype=np.float64)
    fuhpz__aezxg.Allgatherv(yawsm__cxhnm, [wwuvo__sfoj, qtouk__htowi,
        vzxua__avupq, MPI.DOUBLE_PRECISION])
    m.feature_log_prob_ = wwuvo__sfoj.T
...
def sklearn_preprocessing_standard_scaler_fit_dist_helper(m, X):
    ...
    tpt__lhj = fuhpz__aezxg.Get_size()
    ixa__cot = m.with_std
    ykq__mtclb = m.with_mean
    m.with_std = False
    if ixa__cot:
        m.with_mean = True
    m = m.fit(X)
    m.with_std = ixa__cot
    m.with_mean = ykq__mtclb
    if not isinstance(m.n_samples_seen_, numbers.Integral):
        ...
        m.n_samples_seen_ = np.repeat(m.n_samples_seen_, X.shape[1]).astype(np
            .int64, copy=False)
    tgxq__loxq = np.zeros((tpt__lhj, *m.n_samples_seen_.shape), dtype=m.
        n_samples_seen_.dtype)
    fuhpz__aezxg.Allgather(m.n_samples_seen_, tgxq__loxq)
    mumy__hbfa = np.sum(tgxq__loxq, axis=0)
    m.n_samples_seen_ = mumy__hbfa
    if m.with_mean or m.with_std:
        bal__hbe = np.zeros((tpt__lhj, *m.mean_.shape), dtype=m.mean_.dtype)
        fuhpz__aezxg.Allgather(m.mean_, bal__hbe)
        bal__hbe[np.isnan(bal__hbe)] = 0
        auge__fqxe = np.average(bal__hbe, axis=0, weights=tgxq__loxq)
        m.mean_ = auge__fqxe
    if m.with_std:
        vkmcj__gwol = sklearn_safe_accumulator_op(np.nansum, (X -
            auge__fqxe) ** 2, axis=0) / mumy__hbfa
        ava__mvdvf = np.zeros_like(vkmcj__gwol)
        fuhpz__aezxg.Allreduce(vkmcj__gwol, ava__mvdvf, op=MPI.SUM)
        m.var_ = ava__mvdvf
        m.scale_ = sklearn_handle_zeros_in_scale(np.sqrt(m.var_))
    yhejw__lljzq = fuhpz__aezxg.allreduce(yhejw__lljzq, op=MPI.LAND)
    if yhejw__lljzq:
...
def get_data_slice_parallel(data, labels, len_train):
    khy__iufo = data[:len_train]
    crpfg__zuvt = data[len_train:]
    khy__iufo = bodo.rebalance(khy__iufo)
    crpfg__zuvt = bodo.rebalance(crpfg__zuvt)
    fqup__yns = labels[:len_train]
    gbo__dibx = labels[len_train:]
    fqup__yns = bodo.rebalance(fqup__yns)
    gbo__dibx = bodo.rebalance(gbo__dibx)
    return khy__iufo, crpfg__zuvt, fqup__yns, gbo__dibx
...
def overload_train_test_split(data, labels=None, train_size=None, test_size
    ...
    _is_data_distributed=False):
    check_sklearn_version()
    xfkt__ztf = {'stratify': stratify}
    vjgsq__sofz = {'stratify': None}
    check_unsupported_args('train_test_split', xfkt__ztf, vjgsq__sofz, 'ml')
    if is_overload_false(_is_data_distributed):
        gezdw__zqkp = f'data_split_type_{numba.core.ir_utils.next_label()}'
        jvbrr__hju = f'labels_split_type_{numba.core.ir_utils.next_label()}'
        for pkl__lcpfw, jtsl__ttv in ((data, gezdw__zqkp), (labels, jvbrr__hju)
            ):
            if isinstance(pkl__lcpfw, (DataFrameType, SeriesType)):
                yaks__mvyqa = pkl__lcpfw.copy(index=NumericIndexType(types.
                    int64))
                setattr(types, jtsl__ttv, yaks__mvyqa)
            else:
                setattr(types, jtsl__ttv, pkl__lcpfw)
        axfne__yyf = 'def _train_test_split_impl(\n'
        axfne__yyf += '    data,\n'
        axfne__yyf += '    labels=None,\n'
        axfne__yyf += '    train_size=None,\n'
        axfne__yyf += '    test_size=None,\n'
        axfne__yyf += '    random_state=None,\n'
        axfne__yyf += '    shuffle=True,\n'
        axfne__yyf += '    stratify=None,\n'
        axfne__yyf += '    _is_data_distributed=False,\n'
        axfne__yyf += '):  # pragma: no cover\n'
        axfne__yyf += (
            """    with numba.objmode(data_train='{}', data_test='{}', labels_train='{}', labels_test='{}'):
"""
            .format(gezdw__zqkp, gezdw__zqkp, jvbrr__hju, jvbrr__hju))
        axfne__yyf += """        data_train, data_test, labels_train, labels_test = sklearn.model_selection.train_test_split(
"""
        axfne__yyf += '            data,\n'
        axfne__yyf += '            labels,\n'
        axfne__yyf += '            train_size=train_size,\n'
        axfne__yyf += '            test_size=test_size,\n'
        axfne__yyf += '            random_state=random_state,\n'
        axfne__yyf += '            shuffle=shuffle,\n'
        axfne__yyf += '            stratify=stratify,\n'
        axfne__yyf += '        )\n'
        axfne__yyf += (
            '    return data_train, data_test, labels_train, labels_test\n')
        wag__tqe = {}
        exec(axfne__yyf, globals(), wag__tqe)
        _train_test_split_impl = wag__tqe['_train_test_split_impl']
        ...
        def _train_test_split_impl(data, labels=None, train_size=None,
            ...
            elu__ktwne = bodo.libs.distributed_api.dist_reduce(len(data),
                np.int32(Reduce_Type.Sum.value))
            len_train = int(elu__ktwne * train_size)
            cdza__qqu = elu__ktwne - len_train
            if shuffle:
                labels = set_labels_type(labels, label_type)
                fwp__fbp = bodo.get_rank()
                ejkp__zwm = bodo.get_size()
                qmj__ubad = np.empty(ejkp__zwm, np.int64)
                bodo.libs.distributed_api.allgather(qmj__ubad, len(data))
                ugjj__ztek = np.cumsum(qmj__ubad[0:fwp__fbp + 1])
                sojxi__bfw = np.full(elu__ktwne, True)
                sojxi__bfw[:cdza__qqu] = False
                np.random.seed(42)
                np.random.permutation(sojxi__bfw)
                if fwp__fbp:
                    agvfy__gxmef = ugjj__ztek[fwp__fbp - 1]
                else:
                    agvfy__gxmef = 0
                oft__edhby = ugjj__ztek[fwp__fbp]
                ylj__ikm = sojxi__bfw[agvfy__gxmef:oft__edhby]
                khy__iufo = data[ylj__ikm]
                crpfg__zuvt = data[~ylj__ikm]
                fqup__yns = labels[ylj__ikm]
                gbo__dibx = labels[~ylj__ikm]
                khy__iufo = bodo.random_shuffle(khy__iufo, seed=
                    random_state, parallel=True)
                crpfg__zuvt = bodo.random_shuffle(crpfg__zuvt, seed=
                    random_state, parallel=True)
                fqup__yns = bodo.random_shuffle(fqup__yns, seed=
                    random_state, parallel=True)
                gbo__dibx = bodo.random_shuffle(gbo__dibx, seed=
                    random_state, parallel=True)
                fqup__yns = reset_labels_type(fqup__yns, label_type)
                gbo__dibx = reset_labels_type(gbo__dibx, label_type)
            else:
                khy__iufo, crpfg__zuvt, fqup__yns, gbo__dibx = (
...
def sklearn_preprocessing_minmax_scaler_fit_dist_helper(m, X):
    ...
    tpt__lhj = fuhpz__aezxg.Get_size()
    m = m.fit(X)
    mumy__hbfa = fuhpz__aezxg.allreduce(m.n_samples_seen_, op=MPI.SUM)
    m.n_samples_seen_ = mumy__hbfa
    bku__yko = np.zeros((tpt__lhj, *m.data_min_.shape), dtype=m.data_min_.dtype
        )
    fuhpz__aezxg.Allgather(m.data_min_, bku__yko)
    gnd__puv = np.nanmin(bku__yko, axis=0)
    spe__ndq = np.zeros((tpt__lhj, *m.data_max_.shape), dtype=m.data_max_.dtype
        )
    fuhpz__aezxg.Allgather(m.data_max_, spe__ndq)
    mmsh__pljf = np.nanmax(spe__ndq, axis=0)
    tsl__bftal = mmsh__pljf - gnd__puv
    m.scale_ = (m.feature_range[1] - m.feature_range[0]
        ) / sklearn_handle_zeros_in_scale(tsl__bftal)
    m.min_ = m.feature_range[0] - gnd__puv * m.scale_
    m.data_min_ = gnd__puv
    m.data_max_ = mmsh__pljf
    m.data_range_ = tsl__bftal
    return m
...
def overload_rf_classifier_model_fit(m, X, y, sample_weight=None,
    _is_data_distributed=False):
    swli__nlrwl = 'RandomForestClassifier'
    if isinstance(m, BodoRandomForestRegressorType):
        swli__nlrwl = 'RandomForestRegressor'
    if not is_overload_none(sample_weight):
        raise BodoError(
            f"sklearn.ensemble.{swli__nlrwl}.fit() : 'sample_weight' is not supported for distributed data."
            )
    ...
    def _model_fit_impl(m, X, y, sample_weight=None, _is_data_distributed=False
        ...
        with numba.objmode(first_rank_node='int32[:]'):
            first_rank_node = get_nodes_first_ranks()
        if _is_data_distributed:
            csi__nebfc = len(first_rank_node)
            X = bodo.gatherv(X)
            y = bodo.gatherv(y)
            if csi__nebfc > 1:
                X = bodo.libs.distributed_api.bcast_comm(X, comm_ranks=
                    first_rank_node, nranks=csi__nebfc)
...
def _cv_fit_transform_helper(m, X):
    ...
    if m.vocabulary is None:
        ...
        bqqco__bnya = True
    return bqqco__bnya, local_vocabulary
    ...
    if is_overload_true(_is_data_distributed):
        ...
        def _count_vectorizer_fit_transform_impl(m, X, y=None,
            ...
            if changeVoc:
                ...
                oxd__aojx = bodo.libs.array_kernels.unique(local_vocabulary,
                    parallel=True)
                oxd__aojx = bodo.allgatherv(oxd__aojx, False)
                oxd__aojx = bodo.libs.array_kernels.sort(oxd__aojx,
                    ascending=True, inplace=True)
                mrvlw__qze = {}
                for gfxy__npp in range(len(oxd__aojx)):
                    mrvlw__qze[oxd__aojx[gfxy__npp]] = gfxy__npp
            else:
                mrvlw__qze = local_vocabulary
            with numba.objmode(transformed_X='csr_matrix_int64_int64'):
                if changeVoc:
                    m.vocabulary = mrvlw__qze
                transformed_X = m.fit_transform(X, y)
                transformed_X.indices = transformed_X.indices.astype(np.int64)
                transformed_X.indptr = transformed_X.indptr.astype(np.int64)
            return transformed_X
