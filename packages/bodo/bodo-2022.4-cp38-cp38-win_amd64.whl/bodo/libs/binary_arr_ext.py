...
def init_binary_arr(typingctx, data_typ=None):
    ...
    def codegen(context, builder, sig, args):
        gjk__glofg, = args
        inxd__zyx = context.make_helper(builder, binary_array_type)
        inxd__zyx.data = gjk__glofg
        context.nrt.incref(builder, data_typ, gjk__glofg)
        return inxd__zyx._getvalue()
...
def init_bytes_type(typingctx, data_typ, length_type):
    ...
    def codegen(context, builder, sig, args):
        atq__lwr = cgutils.create_struct_proxy(sig.args[0])(context,
            builder, value=args[0])
        bdaon__zfe = args[1]
        efr__hhcr = cgutils.create_struct_proxy(bytes_type)(context, builder)
        efr__hhcr.meminfo = context.nrt.meminfo_alloc(builder, bdaon__zfe)
        efr__hhcr.nitems = bdaon__zfe
        efr__hhcr.itemsize = lir.Constant(efr__hhcr.itemsize.type, 1)
        efr__hhcr.data = context.nrt.meminfo_data(builder, efr__hhcr.meminfo)
        efr__hhcr.parent = cgutils.get_null_value(efr__hhcr.parent.type)
        efr__hhcr.shape = cgutils.pack_array(builder, [bdaon__zfe], context
            .get_value_type(types.intp))
        efr__hhcr.strides = atq__lwr.strides
        cgutils.memcpy(builder, efr__hhcr.data, atq__lwr.data, bdaon__zfe)
        return efr__hhcr._getvalue()
...
def binary_arr_hex(arr):
    qld__mgpds = numba.cpython.unicode.PY_UNICODE_1BYTE_KIND

    def impl(arr):
        bdaon__zfe = len(arr) * 2
        output = numba.cpython.unicode._empty_string(qld__mgpds, bdaon__zfe, 1)
...
def bytes_to_hex(typingctx, output, arr):
    ...
    def codegen(context, builder, sig, args):
        zvobi__mol = cgutils.create_struct_proxy(sig.args[0])(context,
            builder, value=args[0])
        lveb__vsf = cgutils.create_struct_proxy(sig.args[1])(context,
            builder, value=args[1])
        ily__dpu = lir.FunctionType(lir.IntType(8).as_pointer(), [lir.
            IntType(8).as_pointer(), lir.IntType(8).as_pointer(), lir.
            IntType(64)])
        uwz__zlf = cgutils.get_or_insert_function(builder.module, ily__dpu,
            name='bytes_to_hex')
        builder.call(uwz__zlf, (zvobi__mol.data, lveb__vsf.data, lveb__vsf.
            nitems))
...
def binary_arr_getitem(arr, ind):
    ...
    if isinstance(ind, types.Integer):
        ...
        def impl(arr, ind):
            otqrl__hgody = arr._data[ind]
            return init_bytes_type(otqrl__hgody, len(otqrl__hgody))
        return impl
...
def overload_bytes_fromhex(hex_str):
    ...
    if hex_str == bodo.string_type:
        ...
        def impl(hex_str):
            if not hex_str._is_ascii or hex_str._kind != qld__mgpds:
                raise TypeError(
                    'bytes.fromhex is only supported on ascii strings')
            gjk__glofg = np.empty(len(hex_str) // 2, np.uint8)
            bdaon__zfe = _bytes_fromhex(gjk__glofg.ctypes, hex_str._data,
                len(hex_str))
            hoka__mtk = init_bytes_type(gjk__glofg, bdaon__zfe)
            return hoka__mtk
        return impl
...
def create_binary_cmp_op_overload(op):
    ...
    def overload_binary_cmp(lhs, rhs):
        tmobo__qeoq = lhs == binary_array_type
        xre__rog = rhs == binary_array_type
        nzgf__ifkcf = 'lhs' if tmobo__qeoq else 'rhs'
        jtdi__rlpgp = 'def impl(lhs, rhs):\n'
        jtdi__rlpgp += '  numba.parfors.parfor.init_prange()\n'
        jtdi__rlpgp += f'  n = len({nzgf__ifkcf})\n'
        jtdi__rlpgp += (
            '  out_arr = bodo.libs.bool_arr_ext.alloc_bool_array(n)\n')
        jtdi__rlpgp += '  for i in numba.parfors.parfor.internal_prange(n):\n'
        asmu__fhrwk = []
        if tmobo__qeoq:
            asmu__fhrwk.append('bodo.libs.array_kernels.isna(lhs, i)')
        if xre__rog:
            asmu__fhrwk.append('bodo.libs.array_kernels.isna(rhs, i)')
        jtdi__rlpgp += f"    if {' or '.join(asmu__fhrwk)}:\n"
        jtdi__rlpgp += '      bodo.libs.array_kernels.setna(out_arr, i)\n'
        jtdi__rlpgp += '      continue\n'
        wca__iewk = 'lhs[i]' if tmobo__qeoq else 'lhs'
        grgi__nkey = 'rhs[i]' if xre__rog else 'rhs'
        jtdi__rlpgp += f'    out_arr[i] = op({wca__iewk}, {grgi__nkey})\n'
        jtdi__rlpgp += '  return out_arr\n'
        lgwvl__zioa = {}
        exec(jtdi__rlpgp, {'bodo': bodo, 'numba': numba, 'op': op}, lgwvl__zioa
            )
        return lgwvl__zioa['impl']
