...
def box_regex_flag(typ, val, c):
    dbb__oiwk = c.box(types.int64, val)
    nivk__dsh = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(nivk__dsh, (dbb__oiwk,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    dbb__oiwk = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, dbb__oiwk)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    rkj__coqgh = context.get_python_api(builder)
    zlgj__xpxfu = builder.icmp_signed('==', val, rkj__coqgh.borrow_none())
    with builder.if_then(zlgj__xpxfu):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    mqt__rgdub = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(mqt__rgdub)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        xhe__tskr = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if xhe__tskr > 1:
            typ = types.List(types.Tuple([string_type] * xhe__tskr))
...
def overload_match_group(m, *args):
    ...
    sopho__xevu = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            mrajp__cbui = args[0]
            with numba.objmode(out=sopho__xevu):
                out = m.group(mrajp__cbui)
            return out
        return _match_group_impl_one
    uihz__wdz = 'tuple_str_{}'.format(len(args))
    setattr(types, uihz__wdz, types.Tuple([sopho__xevu] * len(args)))
    fsaw__djpnp = ', '.join('group{}'.format(lnz__iiv + 1) for lnz__iiv in
        range(len(args)))
    pim__foc = 'def _match_group_impl(m, *args):\n'
    pim__foc += '  ({}) = args\n'.format(fsaw__djpnp)
    pim__foc += "  with numba.objmode(out='{}'):\n".format(uihz__wdz)
    pim__foc += '    out = m.group({})\n'.format(fsaw__djpnp)
    pim__foc += '  return out\n'
    levf__pfoyo = {}
    exec(pim__foc, globals(), levf__pfoyo)
    msvjt__eoou = levf__pfoyo['_match_group_impl']
    return msvjt__eoou
...
def overload_match_groups(m, default=None):
    ehpgr__wsytm = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=ehpgr__wsytm):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(sbkps__luqj is None for sbkps__luqj in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=sopho__xevu):
