...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    edz__vvpt = pq_node.out_vars[0].name
    zyw__rffrr = pq_node.out_vars[1].name
    if edz__vvpt not in lives and zyw__rffrr not in lives:
        return None
    elif edz__vvpt not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.type_usecol_offset = []
    elif zyw__rffrr not in lives:
