...
def decode_if_dict_array_overload(A):
    if isinstance(A, types.BaseTuple):
        wlvd__seofm = len(A.types)
        mez__qhmqf = 'def f(A):\n'
        cjo__jcnat = ','.join(f'decode_if_dict_array(A[{i}])' for i in
            range(wlvd__seofm))
        mez__qhmqf += '  return ({}{})\n'.format(cjo__jcnat, ',' if 
            wlvd__seofm == 1 else '')
        bet__hjye = {}
        exec(mez__qhmqf, {'decode_if_dict_array': decode_if_dict_array},
            bet__hjye)
        impl = bet__hjye['f']
        return impl
    ...
    if isinstance(A, bodo.SeriesType):
        ...
        def impl(A):
            sie__fdx = bodo.hiframes.pd_series_ext.get_series_data(A)
            ywst__khtd = bodo.hiframes.pd_series_ext.get_series_index(A)
            name = bodo.hiframes.pd_series_ext.get_series_name(A)
            rer__vrux = decode_if_dict_array(sie__fdx)
            return bodo.hiframes.pd_series_ext.init_series(rer__vrux,
                ywst__khtd, name)
        return impl
...
def to_str_arr_if_dict_array(t):
    ...
    if isinstance(t, bodo.TableType):
        esrd__mnt = tuple(to_str_arr_if_dict_array(t) for t in t.arr_types)
        return bodo.TableType(esrd__mnt, t.has_runtime_cols)
...
class BodoError(NumbaError):
    ...
    def __init__(self, msg, loc=None, locs_in_msg=None):
        ...
        fpm__qgn = numba.core.errors.termcolor().errmsg
        super(BodoError, self).__init__(fpm__qgn(msg), loc)
...
def raise_bodo_error(msg, loc=None):
    if bodo.transforms.typing_pass.in_partial_typing:
        ...
        bxko__jqq = [] if loc is None else [loc]
        raise BodoError(msg, locs_in_msg=bxko__jqq)
...
class FileInfo:
    ...
    def get_schema(self, fname):
        qgc__wwi = self.get_full_filename(fname)
        return self._get_schema(qgc__wwi)
...
class FilenameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        tal__lltji = dmm.lookup(bodo.typeof(fe_type.fname))
        xjsgt__ksy = [(a, mqzj__klco) for a, mqzj__klco in zip(tal__lltji.
            _fields, tal__lltji._members)]
        super().__init__(dmm, fe_type, xjsgt__ksy)
...
def is_initial_value_type(t):
    if not isinstance(t, types.InitialValue) or t.initial_value is None:
        return False
    okdm__jptu = t.initial_value
    if isinstance(okdm__jptu, dict):
        okdm__jptu = okdm__jptu.values()
    return not any(isinstance(ypnq__aqs, (types.Poison, numba.core.
        interpreter._UNKNOWN_VALUE)) for ypnq__aqs in okdm__jptu)
...
def is_overload_constant_dict(val):
    return isinstance(val, types.LiteralStrKeyDict) and all(is_literal_type
        (ypnq__aqs) for ypnq__aqs in val.types) or is_initial_value_dict_type(
...
def is_overload_bool_list(val):
    return is_overload_constant_list(val) and all(is_overload_constant_bool
        (ypnq__aqs) for ypnq__aqs in get_overload_const_list(val))
...
def get_overload_const(val):
    ...
    if isinstance(val, types.LiteralList):
        pwcw__mmqr = []
        for ypnq__aqs in val.literal_value:
            iffru__zkdj = get_overload_const(ypnq__aqs)
            if iffru__zkdj == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                pwcw__mmqr.append(iffru__zkdj)
        return pwcw__mmqr
    ...
    if isinstance(val, types.BaseTuple):
        pwcw__mmqr = []
        for ypnq__aqs in val.types:
            iffru__zkdj = get_overload_const(ypnq__aqs)
            if iffru__zkdj == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                pwcw__mmqr.append(iffru__zkdj)
        return tuple(pwcw__mmqr)
...
def ensure_constant_arg(fname, arg_name, val, const_type):
    iffru__zkdj = get_overload_const(val)
    vlri__ncvig = _const_type_repr.get(const_type, str(const_type))
    if not isinstance(iffru__zkdj, const_type):
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant {vlri__ncvig} not {val}"
            )


def ensure_constant_values(fname, arg_name, val, const_values):
    iffru__zkdj = get_overload_const(val)
    if iffru__zkdj not in const_values:
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant value in {const_values} not '{iffru__zkdj}'"
            )
...
def check_unsupported_args(fname, args_dict, arg_defaults_dict,
    ...
    iqvml__lbki = False
    for a in args_dict:
        uhmeo__hlexa = get_overload_const(args_dict[a])
        lsbnd__upyq = arg_defaults_dict[a]
        if (uhmeo__hlexa is NOT_CONSTANT or uhmeo__hlexa is not None and 
            lsbnd__upyq is None or uhmeo__hlexa is None and lsbnd__upyq is not
            None or uhmeo__hlexa != lsbnd__upyq or uhmeo__hlexa is not
            _no_input and lsbnd__upyq is _no_input or uhmeo__hlexa is
            _no_input and lsbnd__upyq is not _no_input):
            gqd__wsbi = (
                f'{fn_str}: {a} parameter only supports default value {lsbnd__upyq}'
                )
            iqvml__lbki = True
            break
    if iqvml__lbki and package_name == 'pandas':
        if module_name == 'IO':
            gqd__wsbi += """
...
"""
    elif iqvml__lbki and package_name == 'ml':
        gqd__wsbi += """
...
"""
    elif iqvml__lbki and package_name == 'numpy':
        gqd__wsbi += """
...
def get_overload_constant_dict(val):
    if isinstance(val, types.LiteralStrKeyDict):
        return {get_literal_value(eqcws__ylvoa): get_literal_value(
            ypnq__aqs) for eqcws__ylvoa, ypnq__aqs in val.literal_value.items()
            }
    ...
    if isinstance(val, types.DictType):
        ...
        return val.initial_value
    giamf__nwp = [get_overload_const(ypnq__aqs) for ypnq__aqs in val.types[1:]]
    return {giamf__nwp[2 * i]: giamf__nwp[2 * i + 1] for i in range(len(
        giamf__nwp) // 2)}
...
def get_overload_const_list(val):
    ...
    if is_initial_value_list_type(val):
        return val.initial_value
    if isinstance(val, types.LiteralList):
        return [get_literal_value(ypnq__aqs) for ypnq__aqs in val.literal_value
            ]
...
def parse_dtype(dtype, func_name=None):
    ...
    if isinstance(dtype, types.Number) or dtype == bodo.string_type:
        return dtype
    try:
        rtfc__posx = get_overload_const_str(dtype)
        if rtfc__posx.startswith('Int') or rtfc__posx.startswith('UInt'):
            return bodo.libs.int_arr_ext.typeof_pd_int_dtype(pd.api.types.
                pandas_dtype(rtfc__posx), None)
        if rtfc__posx == 'boolean':
            return bodo.libs.bool_arr_ext.boolean_dtype
        if rtfc__posx == 'str':
            return bodo.string_type
        return numba.np.numpy_support.from_dtype(np.dtype(rtfc__posx))
    except:
        pass
...
def get_index_names(t, func_name, default_name):
    ...
    hcz__cgmhc = '{}: index name should be a constant string'.format(func_name)
    if isinstance(t, MultiIndexType):
        ciqz__fbh = []
        for i, znfpf__emt in enumerate(t.names_typ):
            if znfpf__emt == types.none:
                ciqz__fbh.append('level_{}'.format(i))
                continue
            if not is_overload_constant_str(znfpf__emt):
                raise BodoError(hcz__cgmhc)
            ciqz__fbh.append(get_overload_const_str(znfpf__emt))
        return tuple(ciqz__fbh)
...
if types.SliceLiteral in numba.core.pythonapi._boxers.functions:
    ...
    def box_slice_literal(typ, val, c):
        awit__fnbmj = typ.literal_value
        wlgt__xgtwc = []
        for wdww__frhqm in ('start', 'stop', 'step'):
            owo__seeo = getattr(typ.literal_value, wdww__frhqm)
            ejv__ctpux = c.pyapi.make_none(
                ) if owo__seeo is None else c.pyapi.from_native_value(types
                .literal(owo__seeo), owo__seeo, c.env_manager)
            wlgt__xgtwc.append(ejv__ctpux)
        nrfwq__gqkkm = c.pyapi.unserialize(c.pyapi.serialize_object(slice))
        yhfgo__rbwnf = c.pyapi.call_function_objargs(nrfwq__gqkkm, wlgt__xgtwc)
        for a in wlgt__xgtwc:
            c.pyapi.decref(a)
        c.pyapi.decref(nrfwq__gqkkm)
        return yhfgo__rbwnf
...
def box_list_literal(typ, val, c):
    qdd__tluby = typ.literal_value
    fpej__bckb = [c.pyapi.from_native_value(types.literal(ypnq__aqs),
        ypnq__aqs, c.env_manager) for ypnq__aqs in qdd__tluby]
    btd__vxplc = c.pyapi.list_pack(fpej__bckb)
    for a in fpej__bckb:
        c.pyapi.decref(a)
    return btd__vxplc
...
def list_literal_to_list(context, builder, fromty, toty, val):
    ...
    nifge__ulzp = types.List(toty.dtype)
...
def is_literal_type(t):
    ...
    return isinstance(t, (types.Literal, types.Omitted)) and not isinstance(t,
        ...
        is_literal_type(ypnq__aqs) for ypnq__aqs in t.types
...
def get_literal_value(t):
    ...
    if isinstance(t, types.Literal):
        if isinstance(t, types.LiteralStrKeyDict):
            return {get_literal_value(eqcws__ylvoa): get_literal_value(
                ypnq__aqs) for eqcws__ylvoa, ypnq__aqs in t.literal_value.
                items()}
        if isinstance(t, types.LiteralList):
            return [get_literal_value(ypnq__aqs) for ypnq__aqs in t.
                literal_value]
        return t.literal_value
    if isinstance(t, types.Omitted):
        return t.value
    if isinstance(t, types.BaseTuple):
        return tuple(get_literal_value(ypnq__aqs) for ypnq__aqs in t.types)
...
def dtype_to_array_type(dtype):
    ...
    if isinstance(dtype, (types.Number, types.Boolean, types.NPDatetime,
        types.NPTimedelta)):
        sie__fdx = types.Array(dtype, 1, 'C')
        if pzwo__vsp:
            return to_nullable_type(sie__fdx)
        return sie__fdx
...
def get_udf_out_arr_type(f_return_type, return_nullable=False):
    ...
    ktne__tcuf = dtype_to_array_type(f_return_type)
    ktne__tcuf = to_nullable_type(ktne__tcuf
        ) if return_nullable else ktne__tcuf
    return ktne__tcuf
...
def types_equality_exists(t1, t2):
    rzzko__hrz = numba.core.registry.cpu_target.typing_context
    try:
        rzzko__hrz.resolve_function_type(operator.eq, (t1, t2), {})
        return True
    except:
        return False
...
def is_hashable_type(t):
    whitelist_types = (types.UnicodeType, types.StringLiteral, types.
        ...
        PandasTimestampType)
    gfvb__hrmc = (types.bool_, bodo.datetime64ns, bodo.timedelta64ns, bodo.
        pd_timedelta_type)
    if isinstance(t, whitelist_types) or t in gfvb__hrmc:
        return True
    rzzko__hrz = numba.core.registry.cpu_target.typing_context
    try:
        rzzko__hrz.resolve_function_type(hash, (t,), {})
        return True
    except:
        return False
...
def to_nullable_type(t):
    ...
    if isinstance(t, DataFrameType):
        dnfnp__scnya = tuple(to_nullable_type(t) for t in t.data)
        return DataFrameType(dnfnp__scnya, t.index, t.columns, t.dist, t.
            is_table_format)
...
def is_common_scalar_dtype(scalar_types):
    kyv__ewqc, xjfqz__ufp = get_common_scalar_dtype(scalar_types)
    return xjfqz__ufp
...
def get_common_scalar_dtype(scalar_types):
    ...
    try:
        jffxk__kihag = np.find_common_type([numba.np.numpy_support.as_dtype
            (t) for t in scalar_types], [])
        if jffxk__kihag != object:
            return numba.np.numpy_support.from_dtype(jffxk__kihag), True
    except numba.core.errors.NumbaNotImplementedError as hpabz__agnt:
        pass
    ...
    igz__mecp = itertools.groupby(scalar_types)
    if next(igz__mecp, True) and not next(igz__mecp, False):
...
def get_nullable_and_non_nullable_types(array_of_types):
    ...
    for typ in array_of_types:
        if typ == bodo.libs.bool_arr_ext.boolean_array:
            isafy__xcrmz.append(types.Array(types.bool_, 1, 'C'))
        elif isinstance(typ, bodo.libs.int_arr_ext.IntegerArrayType):
            isafy__xcrmz.append(types.Array(typ.dtype, 1, 'C'))
        elif isinstance(typ, types.Array):
            if typ.dtype == types.bool_:
                isafy__xcrmz.append(bodo.libs.bool_arr_ext.boolean_array)
            if isinstance(typ.dtype, types.Integer):
                isafy__xcrmz.append(bodo.libs.int_arr_ext.IntegerArrayType(
                    typ.dtype))
        isafy__xcrmz.append(typ)
    return isafy__xcrmz
...
def _gen_objmode_overload(func, output_type, attr_name=None, is_function=
    ...
    if is_function:
        rdn__yeq = getfullargspec(func)
        assert rdn__yeq.varargs is None, 'varargs not supported'
        assert rdn__yeq.varkw is None, 'varkw not supported'
        defaults = [] if rdn__yeq.defaults is None else rdn__yeq.defaults
        rwd__ornd = len(rdn__yeq.args) - len(defaults)
        args = rdn__yeq.args[1:] if attr_name else rdn__yeq.args[:]
        lrx__egozt = []
        for i, onpdt__frqr in enumerate(rdn__yeq.args):
            if i < rwd__ornd:
                lrx__egozt.append(onpdt__frqr)
            elif str(defaults[i - rwd__ornd]) != '<deprecated parameter>':
                lrx__egozt.append(onpdt__frqr + '=' + str(defaults[i -
                    rwd__ornd]))
            else:
                args.remove(onpdt__frqr)
        if rdn__yeq.kwonlyargs is not None:
            for onpdt__frqr in rdn__yeq.kwonlyargs:
                args.append(f'{onpdt__frqr}={onpdt__frqr}')
                lrx__egozt.append(
                    f'{onpdt__frqr}={str(rdn__yeq.kwonlydefaults[onpdt__frqr])}'
                    )
        sig = ', '.join(lrx__egozt)
        args = ', '.join(args)
    else:
    ...
    qsq__tfn = f'self.{attr_name}' if attr_name else f'{func_name}'
    mez__qhmqf = f'def overload_impl({sig}):\n'
    mez__qhmqf += f'    def impl({sig}):\n'
    if single_rank:
        mez__qhmqf += f'        if bodo.get_rank() == 0:\n'
        esl__elng = '    '
    else:
        esl__elng = ''
    mez__qhmqf += (
        f"        {esl__elng}with numba.objmode(res='{type_name}'):\n")
    if is_function:
        mez__qhmqf += f'            {esl__elng}res = {qsq__tfn}({args})\n'
    else:
        mez__qhmqf += f'            {esl__elng}res = {qsq__tfn}\n'
    mez__qhmqf += f'        return res\n'
    mez__qhmqf += f'    return impl\n'
    bet__hjye = {}
    rguv__bhnst = globals()
    if not attr_name:
        rguv__bhnst[func_name] = func
    exec(mez__qhmqf, rguv__bhnst, bet__hjye)
    whvsk__qhcx = bet__hjye['overload_impl']
    return whvsk__qhcx


def gen_objmode_func_overload(func, output_type=None, single_rank=False):
    try:
        whvsk__qhcx = _gen_objmode_overload(func, output_type, is_function=
            True, single_rank=single_rank)
        overload(func, no_unliteral=True)(whvsk__qhcx)
    except Exception as hpabz__agnt:
        pass
...
def gen_objmode_method_overload(obj_type, method_name, method, output_type=
    ...
    try:
        whvsk__qhcx = _gen_objmode_overload(method, output_type,
            method_name, True, single_rank)
        overload_method(obj_type, method_name, no_unliteral=True)(whvsk__qhcx)
    except Exception as hpabz__agnt:
        pass
...
def gen_objmode_attr_overload(obj_type, attr_name, attr, output_type=None,
    single_rank=False):
    try:
        whvsk__qhcx = _gen_objmode_overload(attr, output_type, attr_name, 
            False, single_rank)
        overload_attribute(obj_type, attr_name, no_unliteral=True)(whvsk__qhcx)
    except Exception as hpabz__agnt:
        pass
...
class NumTypeStaticGetItem(AbstractTemplate):
    ...
    def generic(self, args, kws):
        val, jgpa__ipyub = args
        if isinstance(jgpa__ipyub, slice) and (isinstance(val, types.
            NumberClass) or isinstance(val, types.TypeRef) and isinstance(
            val.instance_type, (types.NPDatetime, types.NPTimedelta))):
            return signature(types.TypeRef(val.instance_type[jgpa__ipyub]),
                *args)
...
def construct_pysig(arg_names, defaults):
    mez__qhmqf = f'def stub('
    for onpdt__frqr in arg_names:
        mez__qhmqf += onpdt__frqr
        if onpdt__frqr in defaults:
            if isinstance(defaults[onpdt__frqr], str):
                mez__qhmqf += f"='{defaults[onpdt__frqr]}'"
            else:
                mez__qhmqf += f'={defaults[onpdt__frqr]}'
        mez__qhmqf += ', '
    mez__qhmqf += '):\n'
    mez__qhmqf += '    pass\n'
    bet__hjye = {}
    exec(mez__qhmqf, {}, bet__hjye)
    zkn__liif = bet__hjye['stub']
    return numba.core.utils.pysignature(zkn__liif)


def fold_typing_args(func_name, args, kws, arg_names, defaults,
    unsupported_arg_names=()):
    kws = dict(kws)
    mqvka__wsdic = len(arg_names)
    nqb__ntq = len(args) + len(kws)
    if nqb__ntq > mqvka__wsdic:
        cdc__flgul = 'argument' if mqvka__wsdic == 1 else 'arguments'
        dpz__hnn = 'was' if nqb__ntq == 1 else 'were'
        raise BodoError(
            f'{func_name}(): Too many arguments specified. Function takes {mqvka__wsdic} {cdc__flgul}, but {nqb__ntq} {dpz__hnn} provided.'
            )
    zoc__xzd = bodo.utils.typing.construct_pysig(arg_names, defaults)
    try:
        yua__uon = bodo.utils.transform.fold_argument_types(zoc__xzd, args, kws
            )
    except Exception as yncz__chaxc:
        raise_bodo_error(f'{func_name}(): {yncz__chaxc}')
    if unsupported_arg_names:
        ...
        for i, arg_name in enumerate(arg_names):
            if arg_name in unsupported_arg_names:
                ...
                qlew__iwx[arg_name] = yua__uon[i]
                xxvd__qoej[arg_name] = defaults[arg_name]
        check_unsupported_args(func_name, qlew__iwx, xxvd__qoej)
    return zoc__xzd, yua__uon
...
class BodoArrayIterator(types.SimpleIteratorType):
    ...
    def __init__(self, arr_type):
        ...
        iwc__yluic = arr_type.dtype
        super(BodoArrayIterator, self).__init__(name, iwc__yluic)
...
class BodoArrayIteratorModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        xjsgt__ksy = [('index', types.EphemeralPointer(types.uintp)), (
            'array', fe_type.arr_type)]
        super(BodoArrayIteratorModel, self).__init__(dmm, fe_type, xjsgt__ksy)
...
def iternext_bodo_array(context, builder, sig, args, result):
    [nai__zsjo] = sig.args
    [fkflu__kaz] = args
    ssjdf__tag = context.make_helper(builder, nai__zsjo, value=fkflu__kaz)
    lbx__siq = signature(types.intp, nai__zsjo.arr_type)
    hwmy__vfzdr = context.compile_internal(builder, lambda a: len(a),
        lbx__siq, [ssjdf__tag.array])
    ywst__khtd = builder.load(ssjdf__tag.index)
    emtk__cwjl = builder.icmp_signed('<', ywst__khtd, hwmy__vfzdr)
    result.set_valid(emtk__cwjl)
    with builder.if_then(emtk__cwjl):
        pjcx__ttf = signature(nai__zsjo.arr_type.dtype, nai__zsjo.arr_type,
            types.intp)
        value = context.compile_internal(builder, lambda a, i: a[i],
            pjcx__ttf, [ssjdf__tag.array, ywst__khtd])
        result.yield_(value)
        kvrdm__hhdxh = cgutils.increment_index(builder, ywst__khtd)
        builder.store(kvrdm__hhdxh, ssjdf__tag.index)


def index_typ_from_dtype_name(elem_dtype, name):
    byxi__diys = type(get_index_type_from_dtype(elem_dtype))
    if name is None:
        nmn__agu = None
    else:
        nmn__agu = types.StringLiteral(name)
    if byxi__diys == bodo.hiframes.pd_index_ext.NumericIndexType:
        ucovq__cozz = byxi__diys(elem_dtype, nmn__agu)
    elif byxi__diys == bodo.hiframes.pd_index_ext.CategoricalIndexType:
        ucovq__cozz = byxi__diys(bodo.CategoricalArrayType(elem_dtype),
            nmn__agu)
    else:
        ucovq__cozz = byxi__diys(nmn__agu)
    return ucovq__cozz
...
def check_objmode_output_type(ret_tup, ret_type):
    return tuple(_check_objmode_type(ypnq__aqs, t) for ypnq__aqs, t in zip(
...
def _fix_objmode_df_type(val, val_typ, typ):
    ...
    if val_typ != typ:
        lsp__grek = pd.Index(val_typ.columns)
        tpsl__lrmwq = pd.Index(typ.columns)
        dbkn__fsujr = lsp__grek.argsort()
        xoe__xle = tpsl__lrmwq.argsort()
        wqym__arx = val_typ.copy(data=tuple(np.array(val_typ.data)[
            dbkn__fsujr]), columns=tuple(lsp__grek[dbkn__fsujr]))
        bozvc__zchb = typ.copy(data=tuple(np.array(typ.data)[xoe__xle]),
            columns=tuple(tpsl__lrmwq[xoe__xle]))
        if wqym__arx == bozvc__zchb:
            val_typ = typ
