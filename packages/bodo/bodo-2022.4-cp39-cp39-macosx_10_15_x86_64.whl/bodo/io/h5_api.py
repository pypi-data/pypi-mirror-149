...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        mcvr__dsudn = []
        ulfgi__hqgfp = h5g_get_num_objs(obj_id)
        for kzteh__jafx in range(ulfgi__hqgfp):
            nila__fusy = h5g_get_objname_by_idx(obj_id, kzteh__jafx)
            mcvr__dsudn.append(nila__fusy)
        return mcvr__dsudn
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    ptik__lwr = parse_dtype(dtype)
    orpy__rueq = np.int32(numba_to_c_type(ptik__lwr))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            lcrb__tdhm = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    hzll__wehyg = tuple_to_ptr(starts)
    omoho__rgk = tuple_to_ptr(counts)
    vrvew__xat = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, hzll__wehyg, omoho__rgk,
        is_parallel, out_arr.ctypes, vrvew__xat)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    omoho__rgk = tuple_to_ptr(counts)
    vrvew__xat = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, hzll__wehyg,
        omoho__rgk, is_parallel, out_arr.ctypes, vrvew__xat)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        khivw__dpn = types.Array(dtype, ndim, 'C')
        mwjf__hcd = args[3]
        khivw__dpn = self.context.resolve_function_type(operator.getitem, [
            khivw__dpn, mwjf__hcd], {}).return_type.copy(layout='C')
        return signature(khivw__dpn, *args)
...
def get_filter_read_indices(bool_arr):
    ehoc__xuqrv = bool_arr.nonzero()[0]
    amelc__kdq = bodo.libs.distributed_api.get_rank()
    sded__kxn = bodo.libs.distributed_api.get_size()
    xrr__vayf = np.empty(sded__kxn, np.int64)
    ebh__devuv = len(bool_arr)
    bodo.libs.distributed_api.allgather(xrr__vayf, ebh__devuv)
    oba__qsg = xrr__vayf.cumsum()[amelc__kdq] - ebh__devuv
    ehoc__xuqrv += oba__qsg
    abzlp__jqu = bodo.libs.distributed_api.dist_reduce(len(ehoc__xuqrv), np
        .int32(sum_op))
    ohlrm__zpr = bodo.libs.distributed_api.gatherv(ehoc__xuqrv)
    if amelc__kdq == 0:
        rxcug__mlih = ohlrm__zpr
    else:
        rxcug__mlih = np.empty(abzlp__jqu, ehoc__xuqrv.dtype)
    bodo.libs.distributed_api.bcast(rxcug__mlih)
    ldom__xat = bodo.libs.distributed_api.get_start(abzlp__jqu, sded__kxn,
        amelc__kdq)
    kfcr__gqji = bodo.libs.distributed_api.get_end(abzlp__jqu, sded__kxn,
        amelc__kdq)
    return rxcug__mlih[ldom__xat:kfcr__gqji]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        ctoj__vrhgr = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], ctoj__vrhgr)
        return builder.bitcast(ctoj__vrhgr, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    omoho__rgk = tuple_to_ptr(counts)
    vrvew__xat = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, hzll__wehyg,
        omoho__rgk, is_parallel, out_arr.ctypes, vrvew__xat, read_indices.
