...
def _global_except_hook(exctype, value, traceback):
    ...
    bbzy__rln = MPI.COMM_WORLD
    coghq__mcozf = bbzy__rln.Get_rank()
    hiz__cfiu = 3.0
    cmph__qdqg = True
    fsti__izy = bbzy__rln.Ibarrier()
    gsgz__svlp = time.time()
    while time.time() - gsgz__svlp < hiz__cfiu:
        time.sleep(0.1)
        if fsti__izy.Test():
            cmph__qdqg = False
            break
    try:
        ...
        if cmph__qdqg:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {coghq__mcozf}. \n')
