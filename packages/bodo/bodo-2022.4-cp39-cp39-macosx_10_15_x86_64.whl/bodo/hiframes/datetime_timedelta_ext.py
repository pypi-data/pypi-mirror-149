...
class PDTimeDeltaModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        kiozk__pmtl = [('value', types.int64)]
        super(PDTimeDeltaModel, self).__init__(dmm, fe_type, kiozk__pmtl)


@box(PDTimeDeltaType)
def box_pd_timedelta(typ, val, c):
    jdfz__vztqn = cgutils.create_struct_proxy(typ)(c.context, c.builder,
        value=val)
    fonk__mejv = c.pyapi.long_from_longlong(jdfz__vztqn.value)
    loa__egmq = c.pyapi.unserialize(c.pyapi.serialize_object(pd.Timedelta))
    res = c.pyapi.call_function_objargs(loa__egmq, (fonk__mejv,))
    c.pyapi.decref(fonk__mejv)
    c.pyapi.decref(loa__egmq)
    return res


@unbox(PDTimeDeltaType)
def unbox_pd_timedelta(typ, val, c):
    fonk__mejv = c.pyapi.object_getattr_string(val, 'value')
    xlt__nku = c.pyapi.long_as_longlong(fonk__mejv)
    jdfz__vztqn = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    jdfz__vztqn.value = xlt__nku
    c.pyapi.decref(fonk__mejv)
    hzvu__jzxrq = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(jdfz__vztqn._getvalue(), is_error=hzvu__jzxrq)
...
def pd_timedelta(value=_no_input, unit='ns', days=0, seconds=0,
    ...
    if value == _no_input:
        ...
        def impl_timedelta_kw(value=_no_input, unit='ns', days=0, seconds=0,
            ...
            scjg__wmsbb = 1000 * microseconds
            return init_pd_timedelta(scjg__wmsbb)
    ...
    fcs__qtk, vabe__liy = pd._libs.tslibs.conversion.precision_from_unit(unit)

    def impl_timedelta(value=_no_input, unit='ns', days=0, seconds=0,
        microseconds=0, milliseconds=0, minutes=0, hours=0, weeks=0):
        return init_pd_timedelta(value * fcs__qtk)
...
def overload_add_operator_datetime_timedelta(lhs, rhs):
    ...
    if lhs == pd_timedelta_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            ipew__trjlo = (rhs.microseconds + (rhs.seconds + rhs.days * 60 *
                60 * 24) * 1000 * 1000) * 1000
            val = lhs.value + ipew__trjlo
            return pd.Timedelta(val)
        return impl
    if lhs == datetime_timedelta_type and rhs == pd_timedelta_type:

        def impl(lhs, rhs):
            lvw__zqhar = (lhs.microseconds + (lhs.seconds + lhs.days * 60 *
                60 * 24) * 1000 * 1000) * 1000
            val = lvw__zqhar + rhs.value
            return pd.Timedelta(val)
        return impl
    if lhs == pd_timedelta_type and rhs == datetime_datetime_type:
        ...
        def impl(lhs, rhs):
            bfduo__mog = rhs.toordinal()
            ual__jrxxi = rhs.second + rhs.minute * 60 + rhs.hour * 3600
            qkl__nhfbk = rhs.microsecond
            hvmaf__bircc = lhs.value // 1000
            ciut__cmfb = lhs.nanoseconds
            pvdg__phw = qkl__nhfbk + hvmaf__bircc
            qbnl__tibm = 1000000 * (bfduo__mog * 86400 + ual__jrxxi
                ) + pvdg__phw
            abazz__riq = ciut__cmfb
            return compute_pd_timestamp(qbnl__tibm, abazz__riq)
        return impl
    ...
    if lhs == datetime_timedelta_type and rhs == datetime_datetime_type:
        ...
        def impl(lhs, rhs):
            qhnx__rvwlu = datetime.timedelta(rhs.toordinal(), hours=rhs.
                hour, minutes=rhs.minute, seconds=rhs.second, microseconds=
                rhs.microsecond)
            qhnx__rvwlu = qhnx__rvwlu + lhs
            jepyw__utaq, plg__upih = divmod(qhnx__rvwlu.seconds, 3600)
            vsdm__iron, gbpj__zex = divmod(plg__upih, 60)
            if 0 < qhnx__rvwlu.days <= _MAXORDINAL:
                d = bodo.hiframes.datetime_date_ext.fromordinal_impl(
                    qhnx__rvwlu.days)
                return datetime.datetime(d.year, d.month, d.day,
                    jepyw__utaq, vsdm__iron, gbpj__zex, qhnx__rvwlu.
                    microseconds)
    ...
    if lhs == datetime_datetime_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            qhnx__rvwlu = datetime.timedelta(lhs.toordinal(), hours=lhs.
                hour, minutes=lhs.minute, seconds=lhs.second, microseconds=
                lhs.microsecond)
            qhnx__rvwlu = qhnx__rvwlu + rhs
            jepyw__utaq, plg__upih = divmod(qhnx__rvwlu.seconds, 3600)
            vsdm__iron, gbpj__zex = divmod(plg__upih, 60)
            if 0 < qhnx__rvwlu.days <= _MAXORDINAL:
                d = bodo.hiframes.datetime_date_ext.fromordinal_impl(
...
def overload_sub_operator_datetime_timedelta(lhs, rhs):
    if lhs == pd_timedelta_type and rhs == pd_timedelta_type:
        ...
        def impl(lhs, rhs):
            scxms__tdg = lhs.value - rhs.value
            return pd.Timedelta(scxms__tdg)
        return impl
    if lhs == pd_timedelta_type and rhs == datetime_timedelta_type:
    ...
    if lhs == datetime_timedelta_array_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            ...
            for qtpo__xryk in numba.parfors.parfor.internal_prange(n):
                A[qtpo__xryk] = ork__rersx[qtpo__xryk] - rhs
            return A
        return impl
...
def overload_mod_operator_timedeltas(lhs, rhs):
    ...
    if lhs == datetime_timedelta_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            yjh__nsm = _to_microseconds(lhs) % _to_microseconds(rhs)
            return datetime.timedelta(0, 0, yjh__nsm)
        return impl
...
def pd_timedelta_divmod(lhs, rhs):
    if lhs == pd_timedelta_type and rhs == pd_timedelta_type:
        ...
        def impl(lhs, rhs):
            ysk__xima, yjh__nsm = divmod(lhs.value, rhs.value)
            return ysk__xima, pd.Timedelta(yjh__nsm)
        return impl
...
class DatetimeTimeDeltaModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        kiozk__pmtl = [('days', types.int64), ('seconds', types.int64), (
            'microseconds', types.int64)]
        super(DatetimeTimeDeltaModel, self).__init__(dmm, fe_type, kiozk__pmtl)
...
def box_datetime_timedelta(typ, val, c):
    jdfz__vztqn = cgutils.create_struct_proxy(typ)(c.context, c.builder,
        value=val)
    mazvx__ejpxz = c.pyapi.long_from_longlong(jdfz__vztqn.days)
    tkolf__pynd = c.pyapi.long_from_longlong(jdfz__vztqn.seconds)
    gbw__dglx = c.pyapi.long_from_longlong(jdfz__vztqn.microseconds)
    loa__egmq = c.pyapi.unserialize(c.pyapi.serialize_object(datetime.
        timedelta))
    res = c.pyapi.call_function_objargs(loa__egmq, (mazvx__ejpxz,
        tkolf__pynd, gbw__dglx))
    c.pyapi.decref(mazvx__ejpxz)
    c.pyapi.decref(tkolf__pynd)
    c.pyapi.decref(gbw__dglx)
    c.pyapi.decref(loa__egmq)
    return res
...
def unbox_datetime_timedelta(typ, val, c):
    mazvx__ejpxz = c.pyapi.object_getattr_string(val, 'days')
    tkolf__pynd = c.pyapi.object_getattr_string(val, 'seconds')
    gbw__dglx = c.pyapi.object_getattr_string(val, 'microseconds')
    iuuuc__fjgm = c.pyapi.long_as_longlong(mazvx__ejpxz)
    baq__sdr = c.pyapi.long_as_longlong(tkolf__pynd)
    tfwbj__gcb = c.pyapi.long_as_longlong(gbw__dglx)
    jdfz__vztqn = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    jdfz__vztqn.days = iuuuc__fjgm
    jdfz__vztqn.seconds = baq__sdr
    jdfz__vztqn.microseconds = tfwbj__gcb
    c.pyapi.decref(mazvx__ejpxz)
    c.pyapi.decref(tkolf__pynd)
    c.pyapi.decref(gbw__dglx)
    hzvu__jzxrq = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
...
def _divide_and_round(a, b):
    ysk__xima, yjh__nsm = divmod(a, b)
    yjh__nsm *= 2
    hkbij__hnrsy = yjh__nsm > b if b > 0 else yjh__nsm < b
    if hkbij__hnrsy or yjh__nsm == b and ysk__xima % 2 == 1:
        ysk__xima += 1
    return ysk__xima
...
def create_cmp_op_overload(op):
    ...
    def overload_timedelta_cmp(lhs, rhs):
        if lhs == datetime_timedelta_type and rhs == datetime_timedelta_type:
            ...
            def impl(lhs, rhs):
                apic__kqkzy = _cmp(_getstate(lhs), _getstate(rhs))
                return op(apic__kqkzy, 0)
            return impl
...
def timedelta_divmod(lhs, rhs):
    if lhs == datetime_timedelta_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            ysk__xima, yjh__nsm = divmod(_to_microseconds(lhs),
                _to_microseconds(rhs))
            return ysk__xima, datetime.timedelta(0, 0, yjh__nsm)
        return impl
...
def timedelta_to_bool(timedelta):
    if timedelta != datetime_timedelta_type:
        return
    rhhkj__lko = datetime.timedelta(0)

    def impl(timedelta):
        return timedelta != rhhkj__lko
    return impl
...
class DatetimeTimeDeltaArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        kiozk__pmtl = [('days_data', days_data_type), ('seconds_data',
            seconds_data_type), ('microseconds_data',
            microseconds_data_type), ('null_bitmap', nulls_type)]
        models.StructModel.__init__(self, dmm, fe_type, kiozk__pmtl)
...
def unbox_datetime_timedelta_array(typ, val, c):
    ...
    grilq__alawp = types.Array(types.intp, 1, 'C')
    yltpy__xar = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        grilq__alawp, [n])
    nox__hwcw = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        grilq__alawp, [n])
    kimn__ipw = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        grilq__alawp, [n])
    ywkm__eykwm = c.builder.udiv(c.builder.add(n, lir.Constant(lir.IntType(
        64), 7)), lir.Constant(lir.IntType(64), 8))
    rqn__fejq = bodo.utils.utils._empty_nd_impl(c.context, c.builder, types
        .Array(types.uint8, 1, 'C'), [ywkm__eykwm])
    djgfn__gcx = lir.FunctionType(lir.VoidType(), [lir.IntType(8).
        as_pointer(), lir.IntType(64), lir.IntType(64).as_pointer(), lir.
        IntType(64).as_pointer(), lir.IntType(64).as_pointer(), lir.IntType
        (8).as_pointer()])
    ikkm__wtlny = cgutils.get_or_insert_function(c.builder.module,
        djgfn__gcx, name='unbox_datetime_timedelta_array')
    c.builder.call(ikkm__wtlny, [val, n, yltpy__xar.data, nox__hwcw.data,
        kimn__ipw.data, rqn__fejq.data])
    sxdg__wugg = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    sxdg__wugg.days_data = yltpy__xar._getvalue()
    sxdg__wugg.seconds_data = nox__hwcw._getvalue()
    sxdg__wugg.microseconds_data = kimn__ipw._getvalue()
    sxdg__wugg.null_bitmap = rqn__fejq._getvalue()
    hzvu__jzxrq = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(sxdg__wugg._getvalue(), is_error=hzvu__jzxrq)


@box(DatetimeTimeDeltaArrayType)
def box_datetime_timedelta_array(typ, val, c):
    ork__rersx = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    yltpy__xar = c.context.make_array(types.Array(types.int64, 1, 'C'))(c.
        context, c.builder, ork__rersx.days_data)
    nox__hwcw = c.context.make_array(types.Array(types.int64, 1, 'C'))(c.
        context, c.builder, ork__rersx.seconds_data).data
    kimn__ipw = c.context.make_array(types.Array(types.int64, 1, 'C'))(c.
        context, c.builder, ork__rersx.microseconds_data).data
    tkn__bfw = c.context.make_array(types.Array(types.uint8, 1, 'C'))(c.
        context, c.builder, ork__rersx.null_bitmap).data
    n = c.builder.extract_value(yltpy__xar.shape, 0)
    djgfn__gcx = lir.FunctionType(c.pyapi.pyobj, [lir.IntType(64), lir.
        IntType(64).as_pointer(), lir.IntType(64).as_pointer(), lir.IntType
        (64).as_pointer(), lir.IntType(8).as_pointer()])
    sjuen__safg = cgutils.get_or_insert_function(c.builder.module,
        djgfn__gcx, name='box_datetime_timedelta_array')
    idv__lngsi = c.builder.call(sjuen__safg, [n, yltpy__xar.data, nox__hwcw,
        kimn__ipw, tkn__bfw])
...
def init_datetime_timedelta_array(typingctx, days_data, seconds_data,
    ...
    def codegen(context, builder, signature, args):
        tle__xhtq, pcz__ovj, krot__buns, hti__ppvr = args
        nngd__qaw = cgutils.create_struct_proxy(signature.return_type)(context,
            builder)
        nngd__qaw.days_data = tle__xhtq
        nngd__qaw.seconds_data = pcz__ovj
        nngd__qaw.microseconds_data = krot__buns
        nngd__qaw.null_bitmap = hti__ppvr
        context.nrt.incref(builder, signature.args[0], tle__xhtq)
        context.nrt.incref(builder, signature.args[1], pcz__ovj)
        context.nrt.incref(builder, signature.args[2], krot__buns)
        context.nrt.incref(builder, signature.args[3], hti__ppvr)
        return nngd__qaw._getvalue()
    swoyx__dsz = datetime_timedelta_array_type(days_data, seconds_data,
        microseconds_data, nulls)
    return swoyx__dsz, codegen


@lower_constant(DatetimeTimeDeltaArrayType)
def lower_constant_datetime_timedelta_arr(context, builder, typ, pyval):
    n = len(pyval)
    yltpy__xar = np.empty(n, np.int64)
    nox__hwcw = np.empty(n, np.int64)
    kimn__ipw = np.empty(n, np.int64)
    gwtl__rfkx = np.empty(n + 7 >> 3, np.uint8)
    for qtpo__xryk, s in enumerate(pyval):
        hrvfx__jmrva = pd.isna(s)
        bodo.libs.int_arr_ext.set_bit_to_arr(gwtl__rfkx, qtpo__xryk, int(
            not hrvfx__jmrva))
        if not hrvfx__jmrva:
            yltpy__xar[qtpo__xryk] = s.days
            nox__hwcw[qtpo__xryk] = s.seconds
            kimn__ipw[qtpo__xryk] = s.microseconds
    kyi__durz = context.get_constant_generic(builder, days_data_type,
        yltpy__xar)
    uhrl__fndg = context.get_constant_generic(builder, seconds_data_type,
        nox__hwcw)
    wbsi__cdgw = context.get_constant_generic(builder,
        microseconds_data_type, kimn__ipw)
    dby__qkfi = context.get_constant_generic(builder, nulls_type, gwtl__rfkx)
    return lir.Constant.literal_struct([kyi__durz, uhrl__fndg, wbsi__cdgw,
        dby__qkfi])


@numba.njit(no_cpython_wrapper=True)
def alloc_datetime_timedelta_array(n):
    yltpy__xar = np.empty(n, dtype=np.int64)
    nox__hwcw = np.empty(n, dtype=np.int64)
    kimn__ipw = np.empty(n, dtype=np.int64)
    nulls = np.full(n + 7 >> 3, 255, np.uint8)
    return init_datetime_timedelta_array(yltpy__xar, nox__hwcw, kimn__ipw,
        nulls)
...
def dt_timedelta_arr_getitem(A, ind):
    ...
    if is_list_like_index_type(ind) and ind.dtype == types.bool_:
        ...
        def impl_bool(A, ind):
            jntyp__otgg = bodo.utils.conversion.coerce_to_ndarray(ind)
            amlp__kvp = A._null_bitmap
            ujlzs__pjvg = A._days_data[jntyp__otgg]
            qpjz__uifg = A._seconds_data[jntyp__otgg]
            pwa__who = A._microseconds_data[jntyp__otgg]
            n = len(ujlzs__pjvg)
            mya__jjl = get_new_null_mask_bool_index(amlp__kvp, ind, n)
            return init_datetime_timedelta_array(ujlzs__pjvg, qpjz__uifg,
                pwa__who, mya__jjl)
        return impl_bool
    if is_list_like_index_type(ind) and isinstance(ind.dtype, types.Integer):
        ...
        def impl(A, ind):
            ...
            pwa__who = A._microseconds_data[jntyp__otgg]
            n = len(ujlzs__pjvg)
            mya__jjl = get_new_null_mask_int_index(amlp__kvp, jntyp__otgg, n)
            return init_datetime_timedelta_array(ujlzs__pjvg, qpjz__uifg,
                pwa__who, mya__jjl)
        return impl
    if isinstance(ind, types.SliceType):
        ...
        def impl_slice(A, ind):
            n = len(A._days_data)
            amlp__kvp = A._null_bitmap
            ujlzs__pjvg = np.ascontiguousarray(A._days_data[ind])
            qpjz__uifg = np.ascontiguousarray(A._seconds_data[ind])
            pwa__who = np.ascontiguousarray(A._microseconds_data[ind])
            mya__jjl = get_new_null_mask_slice_index(amlp__kvp, ind, n)
            return init_datetime_timedelta_array(ujlzs__pjvg, qpjz__uifg,
                pwa__who, mya__jjl)
        return impl_slice
    raise BodoError(
    ...
    if is_list_like_index_type(ind) and isinstance(ind.dtype, types.Integer):
        if types.unliteral(val) == datetime_timedelta_type:
            ...
            def impl_arr_ind_scalar(A, ind, val):
                n = len(A)
                for qtpo__xryk in range(n):
                    A._days_data[ind[qtpo__xryk]] = val._days
                    A._seconds_data[ind[qtpo__xryk]] = val._seconds
                    A._microseconds_data[ind[qtpo__xryk]] = val._microseconds
            ...
            def impl_arr_ind(A, ind, val):
                ...
                for qtpo__xryk in range(n):
                    A._days_data[ind[qtpo__xryk]] = val._days_data[qtpo__xryk]
                    A._seconds_data[ind[qtpo__xryk]] = val._seconds_data[
                        qtpo__xryk]
                    A._microseconds_data[ind[qtpo__xryk]
                        ] = val._microseconds_data[qtpo__xryk]
                    oiti__tfbcd = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val
                        ._null_bitmap, qtpo__xryk)
                    bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                        ind[qtpo__xryk], oiti__tfbcd)
            return impl_arr_ind
    if is_list_like_index_type(ind) and ind.dtype == types.bool_:
        if types.unliteral(val) == datetime_timedelta_type:
            ...
            def impl_bool_ind_mask_scalar(A, ind, val):
                n = len(ind)
                for qtpo__xryk in range(n):
                    if not bodo.libs.array_kernels.isna(ind, qtpo__xryk
                        ) and ind[qtpo__xryk]:
                        A._days_data[qtpo__xryk] = val._days
                        A._seconds_data[qtpo__xryk] = val._seconds
                        A._microseconds_data[qtpo__xryk] = val._microseconds
                        bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                            qtpo__xryk, 1)
            ...
            def impl_bool_ind_mask(A, ind, val):
                ...
                for qtpo__xryk in range(n):
                    if not bodo.libs.array_kernels.isna(ind, qtpo__xryk
                        ...
                        A._days_data[qtpo__xryk] = val._days_data[pvwnu__amieb]
                        A._seconds_data[qtpo__xryk] = val._seconds_data[
                            pvwnu__amieb]
                        A._microseconds_data[qtpo__xryk
                            ] = val._microseconds_data[pvwnu__amieb]
                        oiti__tfbcd = bodo.libs.int_arr_ext.get_bit_bitmap_arr(
                            val._null_bitmap, pvwnu__amieb)
                        bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                            qtpo__xryk, oiti__tfbcd)
                        pvwnu__amieb += 1
    ...
    if isinstance(ind, types.SliceType):
        if types.unliteral(val) == datetime_timedelta_type:
            ...
            def impl_slice_scalar(A, ind, val):
                ctspm__evro = numba.cpython.unicode._normalize_slice(ind,
                    len(A))
                for qtpo__xryk in range(ctspm__evro.start, ctspm__evro.stop,
                    ctspm__evro.step):
                    A._days_data[qtpo__xryk] = val._days
                    A._seconds_data[qtpo__xryk] = val._seconds
                    A._microseconds_data[qtpo__xryk] = val._microseconds
            ...
            def impl_slice_mask(A, ind, val):
                ...
                wpj__olexc = val._null_bitmap.copy()
                setitem_slice_index_null_bits(A._null_bitmap, wpj__olexc,
                    ind, n)
...
def overload_datetime_timedelta_arr_sub(arg1, arg2):
    if (arg1 == datetime_timedelta_array_type and arg2 ==
        ...
        def impl(arg1, arg2):
            ...
            A = alloc_datetime_timedelta_array(n)
            for qtpo__xryk in numba.parfors.parfor.internal_prange(n):
                A[qtpo__xryk] = ork__rersx[qtpo__xryk] - arg2
            return A
        return impl
    ...
    def overload_date_arr_cmp(lhs, rhs):
        ...
        if (lhs == datetime_timedelta_array_type and rhs ==
            ...
            def impl(lhs, rhs):
                numba.parfors.parfor.init_prange()
                n = len(lhs)
                aoopo__siy = bodo.libs.bool_arr_ext.alloc_bool_array(n)
                for qtpo__xryk in numba.parfors.parfor.internal_prange(n):
                    dxr__yaaxj = bodo.libs.array_kernels.isna(lhs, qtpo__xryk)
                    atfy__eupz = bodo.libs.array_kernels.isna(rhs, qtpo__xryk)
                    if dxr__yaaxj or atfy__eupz:
                        kdqr__sfla = mxvwl__hmzu
                    else:
                        kdqr__sfla = op(lhs[qtpo__xryk], rhs[qtpo__xryk])
                    aoopo__siy[qtpo__xryk] = kdqr__sfla
                return aoopo__siy
            return impl
            ...
            def impl(lhs, rhs):
                ...
                aoopo__siy = bodo.libs.bool_arr_ext.alloc_bool_array(n)
                for qtpo__xryk in numba.parfors.parfor.internal_prange(n):
                    oiti__tfbcd = bodo.libs.array_kernels.isna(lhs, qtpo__xryk)
                    if oiti__tfbcd:
                        kdqr__sfla = mxvwl__hmzu
                    else:
                        kdqr__sfla = op(lhs[qtpo__xryk], rhs)
                    aoopo__siy[qtpo__xryk] = kdqr__sfla
                return aoopo__siy
            return impl
            ...
            def impl(lhs, rhs):
                ...
                aoopo__siy = bodo.libs.bool_arr_ext.alloc_bool_array(n)
                for qtpo__xryk in numba.parfors.parfor.internal_prange(n):
                    oiti__tfbcd = bodo.libs.array_kernels.isna(rhs, qtpo__xryk)
                    if oiti__tfbcd:
                        kdqr__sfla = mxvwl__hmzu
                    else:
                        kdqr__sfla = op(lhs, rhs[qtpo__xryk])
                    aoopo__siy[qtpo__xryk] = kdqr__sfla
                return aoopo__siy
            return impl
...
def _intstall_pd_timedelta_unsupported():
    ...
    for ofu__incu in timedelta_unsupported_attrs:
        xwgew__eei = 'pandas.Timedelta.' + ofu__incu
        overload_attribute(PDTimeDeltaType, ofu__incu)(
            create_unsupported_overload(xwgew__eei))
    for btng__gcelz in timedelta_unsupported_methods:
        xwgew__eei = 'pandas.Timedelta.' + btng__gcelz
        overload_method(PDTimeDeltaType, btng__gcelz)(
            create_unsupported_overload(xwgew__eei + '()'))
