...
def box_regex_flag(typ, val, c):
    opgfb__jefxr = c.box(types.int64, val)
    urk__gcg = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(urk__gcg, (opgfb__jefxr,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    opgfb__jefxr = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, opgfb__jefxr)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    hod__groi = context.get_python_api(builder)
    uydg__qnb = builder.icmp_signed('==', val, hod__groi.borrow_none())
    with builder.if_then(uydg__qnb):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    rato__fxxk = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(rato__fxxk)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        cmk__ivnl = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if cmk__ivnl > 1:
            typ = types.List(types.Tuple([string_type] * cmk__ivnl))
...
def overload_match_group(m, *args):
    ...
    ske__vhf = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            erlr__ylit = args[0]
            with numba.objmode(out=ske__vhf):
                out = m.group(erlr__ylit)
            return out
        return _match_group_impl_one
    kyysr__obpld = 'tuple_str_{}'.format(len(args))
    setattr(types, kyysr__obpld, types.Tuple([ske__vhf] * len(args)))
    rzx__ltdwd = ', '.join('group{}'.format(ixesb__loanv + 1) for
        ixesb__loanv in range(len(args)))
    ynsi__pdln = 'def _match_group_impl(m, *args):\n'
    ynsi__pdln += '  ({}) = args\n'.format(rzx__ltdwd)
    ynsi__pdln += "  with numba.objmode(out='{}'):\n".format(kyysr__obpld)
    ynsi__pdln += '    out = m.group({})\n'.format(rzx__ltdwd)
    ynsi__pdln += '  return out\n'
    qoctu__zces = {}
    exec(ynsi__pdln, globals(), qoctu__zces)
    zcaee__jlpn = qoctu__zces['_match_group_impl']
    return zcaee__jlpn
...
def overload_match_groups(m, default=None):
    axz__okqdh = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=axz__okqdh):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(lrsfb__oopan is None for lrsfb__oopan in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=ske__vhf):
