...
def decode_if_dict_array_overload(A):
    if isinstance(A, types.BaseTuple):
        hith__ggq = len(A.types)
        lxby__ldde = 'def f(A):\n'
        bpvj__ppla = ','.join(f'decode_if_dict_array(A[{i}])' for i in
            range(hith__ggq))
        lxby__ldde += '  return ({}{})\n'.format(bpvj__ppla, ',' if 
            hith__ggq == 1 else '')
        ahr__jdngn = {}
        exec(lxby__ldde, {'decode_if_dict_array': decode_if_dict_array},
            ahr__jdngn)
        impl = ahr__jdngn['f']
        return impl
    ...
    if isinstance(A, bodo.SeriesType):
        ...
        def impl(A):
            mjgc__rur = bodo.hiframes.pd_series_ext.get_series_data(A)
            idoq__qrec = bodo.hiframes.pd_series_ext.get_series_index(A)
            name = bodo.hiframes.pd_series_ext.get_series_name(A)
            iexk__qcyoj = decode_if_dict_array(mjgc__rur)
            return bodo.hiframes.pd_series_ext.init_series(iexk__qcyoj,
                idoq__qrec, name)
        return impl
...
def to_str_arr_if_dict_array(t):
    ...
    if isinstance(t, bodo.TableType):
        qcncw__xaeoy = tuple(to_str_arr_if_dict_array(t) for t in t.arr_types)
        return bodo.TableType(qcncw__xaeoy, t.has_runtime_cols)
...
class BodoError(NumbaError):
    ...
    def __init__(self, msg, loc=None, locs_in_msg=None):
        ...
        oyd__szgka = numba.core.errors.termcolor().errmsg
        super(BodoError, self).__init__(oyd__szgka(msg), loc)
...
def raise_bodo_error(msg, loc=None):
    if bodo.transforms.typing_pass.in_partial_typing:
        ...
        wtmnu__odin = [] if loc is None else [loc]
        raise BodoError(msg, locs_in_msg=wtmnu__odin)
...
class FileInfo:
    ...
    def get_schema(self, fname):
        gqnr__csgar = self.get_full_filename(fname)
        return self._get_schema(gqnr__csgar)
...
class FilenameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        iota__itbrj = dmm.lookup(bodo.typeof(fe_type.fname))
        xwl__qzx = [(a, axao__xztgc) for a, axao__xztgc in zip(iota__itbrj.
            _fields, iota__itbrj._members)]
        super().__init__(dmm, fe_type, xwl__qzx)
...
def is_initial_value_type(t):
    if not isinstance(t, types.InitialValue) or t.initial_value is None:
        return False
    nlopt__gfqy = t.initial_value
    if isinstance(nlopt__gfqy, dict):
        nlopt__gfqy = nlopt__gfqy.values()
    return not any(isinstance(utr__jrmy, (types.Poison, numba.core.
        interpreter._UNKNOWN_VALUE)) for utr__jrmy in nlopt__gfqy)
...
def is_overload_constant_dict(val):
    return isinstance(val, types.LiteralStrKeyDict) and all(is_literal_type
        (utr__jrmy) for utr__jrmy in val.types) or is_initial_value_dict_type(
...
def is_overload_bool_list(val):
    return is_overload_constant_list(val) and all(is_overload_constant_bool
        (utr__jrmy) for utr__jrmy in get_overload_const_list(val))
...
def get_overload_const(val):
    ...
    if isinstance(val, types.LiteralList):
        rhwc__tuf = []
        for utr__jrmy in val.literal_value:
            vah__rqhhb = get_overload_const(utr__jrmy)
            if vah__rqhhb == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                rhwc__tuf.append(vah__rqhhb)
        return rhwc__tuf
    ...
    if isinstance(val, types.BaseTuple):
        rhwc__tuf = []
        for utr__jrmy in val.types:
            vah__rqhhb = get_overload_const(utr__jrmy)
            if vah__rqhhb == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                rhwc__tuf.append(vah__rqhhb)
        return tuple(rhwc__tuf)
...
def ensure_constant_arg(fname, arg_name, val, const_type):
    vah__rqhhb = get_overload_const(val)
    womc__knwiu = _const_type_repr.get(const_type, str(const_type))
    if not isinstance(vah__rqhhb, const_type):
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant {womc__knwiu} not {val}"
            )


def ensure_constant_values(fname, arg_name, val, const_values):
    vah__rqhhb = get_overload_const(val)
    if vah__rqhhb not in const_values:
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant value in {const_values} not '{vah__rqhhb}'"
            )
...
def check_unsupported_args(fname, args_dict, arg_defaults_dict,
    ...
    dgnh__wdt = False
    for a in args_dict:
        eza__dfkjq = get_overload_const(args_dict[a])
        nxw__xxwp = arg_defaults_dict[a]
        if (eza__dfkjq is NOT_CONSTANT or eza__dfkjq is not None and 
            nxw__xxwp is None or eza__dfkjq is None and nxw__xxwp is not
            None or eza__dfkjq != nxw__xxwp or eza__dfkjq is not _no_input and
            nxw__xxwp is _no_input or eza__dfkjq is _no_input and nxw__xxwp
             is not _no_input):
            kzi__fjter = (
                f'{fn_str}: {a} parameter only supports default value {nxw__xxwp}'
                )
            dgnh__wdt = True
            break
    if dgnh__wdt and package_name == 'pandas':
        if module_name == 'IO':
            kzi__fjter += """
...
"""
    elif dgnh__wdt and package_name == 'ml':
        kzi__fjter += """
...
"""
    elif dgnh__wdt and package_name == 'numpy':
        kzi__fjter += """
...
def get_overload_constant_dict(val):
    if isinstance(val, types.LiteralStrKeyDict):
        return {get_literal_value(xqvo__fzvgj): get_literal_value(utr__jrmy
            ) for xqvo__fzvgj, utr__jrmy in val.literal_value.items()}
    ...
    if isinstance(val, types.DictType):
        ...
        return val.initial_value
    hlwd__gwn = [get_overload_const(utr__jrmy) for utr__jrmy in val.types[1:]]
    return {hlwd__gwn[2 * i]: hlwd__gwn[2 * i + 1] for i in range(len(
        hlwd__gwn) // 2)}
...
def get_overload_const_list(val):
    ...
    if is_initial_value_list_type(val):
        return val.initial_value
    if isinstance(val, types.LiteralList):
        return [get_literal_value(utr__jrmy) for utr__jrmy in val.literal_value
            ]
...
def parse_dtype(dtype, func_name=None):
    ...
    if isinstance(dtype, types.Number) or dtype == bodo.string_type:
        return dtype
    try:
        eqvs__kew = get_overload_const_str(dtype)
        if eqvs__kew.startswith('Int') or eqvs__kew.startswith('UInt'):
            return bodo.libs.int_arr_ext.typeof_pd_int_dtype(pd.api.types.
                pandas_dtype(eqvs__kew), None)
        if eqvs__kew == 'boolean':
            return bodo.libs.bool_arr_ext.boolean_dtype
        if eqvs__kew == 'str':
            return bodo.string_type
        return numba.np.numpy_support.from_dtype(np.dtype(eqvs__kew))
    except:
        pass
...
def get_index_names(t, func_name, default_name):
    ...
    cpt__lcvbs = '{}: index name should be a constant string'.format(func_name)
    if isinstance(t, MultiIndexType):
        ysdj__udaq = []
        for i, plmis__xhokh in enumerate(t.names_typ):
            if plmis__xhokh == types.none:
                ysdj__udaq.append('level_{}'.format(i))
                continue
            if not is_overload_constant_str(plmis__xhokh):
                raise BodoError(cpt__lcvbs)
            ysdj__udaq.append(get_overload_const_str(plmis__xhokh))
        return tuple(ysdj__udaq)
...
if types.SliceLiteral in numba.core.pythonapi._boxers.functions:
    ...
    def box_slice_literal(typ, val, c):
        fnx__wyds = typ.literal_value
        kdid__qogd = []
        for dre__jwa in ('start', 'stop', 'step'):
            pltpg__smcg = getattr(typ.literal_value, dre__jwa)
            rpzqp__btza = c.pyapi.make_none(
                ) if pltpg__smcg is None else c.pyapi.from_native_value(types
                .literal(pltpg__smcg), pltpg__smcg, c.env_manager)
            kdid__qogd.append(rpzqp__btza)
        mwtk__dxvz = c.pyapi.unserialize(c.pyapi.serialize_object(slice))
        xcgxi__kott = c.pyapi.call_function_objargs(mwtk__dxvz, kdid__qogd)
        for a in kdid__qogd:
            c.pyapi.decref(a)
        c.pyapi.decref(mwtk__dxvz)
        return xcgxi__kott
...
def box_list_literal(typ, val, c):
    vxxg__rzqv = typ.literal_value
    faiw__vrofo = [c.pyapi.from_native_value(types.literal(utr__jrmy),
        utr__jrmy, c.env_manager) for utr__jrmy in vxxg__rzqv]
    fgpx__axg = c.pyapi.list_pack(faiw__vrofo)
    for a in faiw__vrofo:
        c.pyapi.decref(a)
    return fgpx__axg
...
def list_literal_to_list(context, builder, fromty, toty, val):
    ...
    zereg__mscik = types.List(toty.dtype)
...
def is_literal_type(t):
    ...
    return isinstance(t, (types.Literal, types.Omitted)) and not isinstance(t,
        ...
        is_literal_type(utr__jrmy) for utr__jrmy in t.types
...
def get_literal_value(t):
    ...
    if isinstance(t, types.Literal):
        if isinstance(t, types.LiteralStrKeyDict):
            return {get_literal_value(xqvo__fzvgj): get_literal_value(
                utr__jrmy) for xqvo__fzvgj, utr__jrmy in t.literal_value.
                items()}
        if isinstance(t, types.LiteralList):
            return [get_literal_value(utr__jrmy) for utr__jrmy in t.
                literal_value]
        return t.literal_value
    if isinstance(t, types.Omitted):
        return t.value
    if isinstance(t, types.BaseTuple):
        return tuple(get_literal_value(utr__jrmy) for utr__jrmy in t.types)
...
def dtype_to_array_type(dtype):
    ...
    if isinstance(dtype, (types.Number, types.Boolean, types.NPDatetime,
        types.NPTimedelta)):
        mjgc__rur = types.Array(dtype, 1, 'C')
        if zghd__uzm:
            return to_nullable_type(mjgc__rur)
        return mjgc__rur
...
def get_udf_out_arr_type(f_return_type, return_nullable=False):
    ...
    fmng__hnf = dtype_to_array_type(f_return_type)
    fmng__hnf = to_nullable_type(fmng__hnf) if return_nullable else fmng__hnf
    return fmng__hnf
...
def types_equality_exists(t1, t2):
    xbryu__jxie = numba.core.registry.cpu_target.typing_context
    try:
        xbryu__jxie.resolve_function_type(operator.eq, (t1, t2), {})
        return True
    except:
        return False
...
def is_hashable_type(t):
    whitelist_types = (types.UnicodeType, types.StringLiteral, types.
        ...
        PandasTimestampType)
    zxmlf__skxfn = (types.bool_, bodo.datetime64ns, bodo.timedelta64ns,
        bodo.pd_timedelta_type)
    if isinstance(t, whitelist_types) or t in zxmlf__skxfn:
        return True
    xbryu__jxie = numba.core.registry.cpu_target.typing_context
    try:
        xbryu__jxie.resolve_function_type(hash, (t,), {})
        return True
    except:
        return False
...
def to_nullable_type(t):
    ...
    if isinstance(t, DataFrameType):
        gpasj__dvugz = tuple(to_nullable_type(t) for t in t.data)
        return DataFrameType(gpasj__dvugz, t.index, t.columns, t.dist, t.
            is_table_format)
...
def is_common_scalar_dtype(scalar_types):
    crhx__imf, wqdjz__bver = get_common_scalar_dtype(scalar_types)
    return wqdjz__bver
...
def get_common_scalar_dtype(scalar_types):
    ...
    try:
        ydk__umzat = np.find_common_type([numba.np.numpy_support.as_dtype(t
            ) for t in scalar_types], [])
        if ydk__umzat != object:
            return numba.np.numpy_support.from_dtype(ydk__umzat), True
    except numba.core.errors.NumbaNotImplementedError as rju__qmk:
        pass
    ...
    fzxy__mjx = itertools.groupby(scalar_types)
    if next(fzxy__mjx, True) and not next(fzxy__mjx, False):
...
def get_nullable_and_non_nullable_types(array_of_types):
    ...
    for typ in array_of_types:
        if typ == bodo.libs.bool_arr_ext.boolean_array:
            mjgfv__pfw.append(types.Array(types.bool_, 1, 'C'))
        elif isinstance(typ, bodo.libs.int_arr_ext.IntegerArrayType):
            mjgfv__pfw.append(types.Array(typ.dtype, 1, 'C'))
        elif isinstance(typ, types.Array):
            if typ.dtype == types.bool_:
                mjgfv__pfw.append(bodo.libs.bool_arr_ext.boolean_array)
            if isinstance(typ.dtype, types.Integer):
                mjgfv__pfw.append(bodo.libs.int_arr_ext.IntegerArrayType(
                    typ.dtype))
        mjgfv__pfw.append(typ)
    return mjgfv__pfw
...
def _gen_objmode_overload(func, output_type, attr_name=None, is_function=
    ...
    if is_function:
        ixw__qlo = getfullargspec(func)
        assert ixw__qlo.varargs is None, 'varargs not supported'
        assert ixw__qlo.varkw is None, 'varkw not supported'
        defaults = [] if ixw__qlo.defaults is None else ixw__qlo.defaults
        srpiq__ctqd = len(ixw__qlo.args) - len(defaults)
        args = ixw__qlo.args[1:] if attr_name else ixw__qlo.args[:]
        mhdq__saasp = []
        for i, yxqzk__cpn in enumerate(ixw__qlo.args):
            if i < srpiq__ctqd:
                mhdq__saasp.append(yxqzk__cpn)
            elif str(defaults[i - srpiq__ctqd]) != '<deprecated parameter>':
                mhdq__saasp.append(yxqzk__cpn + '=' + str(defaults[i -
                    srpiq__ctqd]))
            else:
                args.remove(yxqzk__cpn)
        if ixw__qlo.kwonlyargs is not None:
            for yxqzk__cpn in ixw__qlo.kwonlyargs:
                args.append(f'{yxqzk__cpn}={yxqzk__cpn}')
                mhdq__saasp.append(
                    f'{yxqzk__cpn}={str(ixw__qlo.kwonlydefaults[yxqzk__cpn])}')
        sig = ', '.join(mhdq__saasp)
        args = ', '.join(args)
    else:
        sig = 'self'
    ...
    ikym__daheu = f'self.{attr_name}' if attr_name else f'{func_name}'
    lxby__ldde = f'def overload_impl({sig}):\n'
    lxby__ldde += f'    def impl({sig}):\n'
    if single_rank:
        lxby__ldde += f'        if bodo.get_rank() == 0:\n'
        koq__empx = '    '
    else:
        koq__empx = ''
    lxby__ldde += (
        f"        {koq__empx}with numba.objmode(res='{type_name}'):\n")
    if is_function:
        lxby__ldde += f'            {koq__empx}res = {ikym__daheu}({args})\n'
    else:
        lxby__ldde += f'            {koq__empx}res = {ikym__daheu}\n'
    lxby__ldde += f'        return res\n'
    lxby__ldde += f'    return impl\n'
    ahr__jdngn = {}
    wpoje__mfv = globals()
    if not attr_name:
        wpoje__mfv[func_name] = func
    exec(lxby__ldde, wpoje__mfv, ahr__jdngn)
    lpv__wpkmo = ahr__jdngn['overload_impl']
    return lpv__wpkmo


def gen_objmode_func_overload(func, output_type=None, single_rank=False):
    try:
        lpv__wpkmo = _gen_objmode_overload(func, output_type, is_function=
            True, single_rank=single_rank)
        overload(func, no_unliteral=True)(lpv__wpkmo)
    except Exception as rju__qmk:
        pass
...
def gen_objmode_method_overload(obj_type, method_name, method, output_type=
    ...
    try:
        lpv__wpkmo = _gen_objmode_overload(method, output_type, method_name,
            True, single_rank)
        overload_method(obj_type, method_name, no_unliteral=True)(lpv__wpkmo)
    except Exception as rju__qmk:
        pass
...
def gen_objmode_attr_overload(obj_type, attr_name, attr, output_type=None,
    single_rank=False):
    try:
        lpv__wpkmo = _gen_objmode_overload(attr, output_type, attr_name, 
            False, single_rank)
        overload_attribute(obj_type, attr_name, no_unliteral=True)(lpv__wpkmo)
    except Exception as rju__qmk:
        pass
...
class NumTypeStaticGetItem(AbstractTemplate):
    ...
    def generic(self, args, kws):
        val, kzeh__jesc = args
        if isinstance(kzeh__jesc, slice) and (isinstance(val, types.
            NumberClass) or isinstance(val, types.TypeRef) and isinstance(
            val.instance_type, (types.NPDatetime, types.NPTimedelta))):
            return signature(types.TypeRef(val.instance_type[kzeh__jesc]),
                *args)
...
def construct_pysig(arg_names, defaults):
    lxby__ldde = f'def stub('
    for yxqzk__cpn in arg_names:
        lxby__ldde += yxqzk__cpn
        if yxqzk__cpn in defaults:
            if isinstance(defaults[yxqzk__cpn], str):
                lxby__ldde += f"='{defaults[yxqzk__cpn]}'"
            else:
                lxby__ldde += f'={defaults[yxqzk__cpn]}'
        lxby__ldde += ', '
    lxby__ldde += '):\n'
    lxby__ldde += '    pass\n'
    ahr__jdngn = {}
    exec(lxby__ldde, {}, ahr__jdngn)
    fqi__fzb = ahr__jdngn['stub']
    return numba.core.utils.pysignature(fqi__fzb)


def fold_typing_args(func_name, args, kws, arg_names, defaults,
    unsupported_arg_names=()):
    kws = dict(kws)
    ckp__uxoj = len(arg_names)
    otges__gmpn = len(args) + len(kws)
    if otges__gmpn > ckp__uxoj:
        dwrri__apmi = 'argument' if ckp__uxoj == 1 else 'arguments'
        fpn__tjy = 'was' if otges__gmpn == 1 else 'were'
        raise BodoError(
            f'{func_name}(): Too many arguments specified. Function takes {ckp__uxoj} {dwrri__apmi}, but {otges__gmpn} {fpn__tjy} provided.'
            )
    cjuf__fkckg = bodo.utils.typing.construct_pysig(arg_names, defaults)
    try:
        ksfh__mfbp = bodo.utils.transform.fold_argument_types(cjuf__fkckg,
            args, kws)
    except Exception as jhrw__ywb:
        raise_bodo_error(f'{func_name}(): {jhrw__ywb}')
    if unsupported_arg_names:
        ...
        for i, arg_name in enumerate(arg_names):
            if arg_name in unsupported_arg_names:
                ...
                cjqhx__zpq[arg_name] = ksfh__mfbp[i]
                kakqw__zbc[arg_name] = defaults[arg_name]
        check_unsupported_args(func_name, cjqhx__zpq, kakqw__zbc)
    return cjuf__fkckg, ksfh__mfbp
...
class BodoArrayIterator(types.SimpleIteratorType):
    ...
    def __init__(self, arr_type):
        ...
        rmca__lrv = arr_type.dtype
        super(BodoArrayIterator, self).__init__(name, rmca__lrv)
...
class BodoArrayIteratorModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        xwl__qzx = [('index', types.EphemeralPointer(types.uintp)), (
            'array', fe_type.arr_type)]
        super(BodoArrayIteratorModel, self).__init__(dmm, fe_type, xwl__qzx)


@lower_builtin('iternext', BodoArrayIterator)
@iternext_impl(RefType.NEW)
def iternext_bodo_array(context, builder, sig, args, result):
    [jtkbx__ajkv] = sig.args
    [viq__tekq] = args
    ppxhx__qhi = context.make_helper(builder, jtkbx__ajkv, value=viq__tekq)
    tal__nugzy = signature(types.intp, jtkbx__ajkv.arr_type)
    kefc__ltq = context.compile_internal(builder, lambda a: len(a),
        tal__nugzy, [ppxhx__qhi.array])
    idoq__qrec = builder.load(ppxhx__qhi.index)
    kke__qphb = builder.icmp_signed('<', idoq__qrec, kefc__ltq)
    result.set_valid(kke__qphb)
    with builder.if_then(kke__qphb):
        njku__gyum = signature(jtkbx__ajkv.arr_type.dtype, jtkbx__ajkv.
            arr_type, types.intp)
        value = context.compile_internal(builder, lambda a, i: a[i],
            njku__gyum, [ppxhx__qhi.array, idoq__qrec])
        result.yield_(value)
        rtqv__pojg = cgutils.increment_index(builder, idoq__qrec)
        builder.store(rtqv__pojg, ppxhx__qhi.index)


def index_typ_from_dtype_name(elem_dtype, name):
    keilh__mja = type(get_index_type_from_dtype(elem_dtype))
    if name is None:
        ass__bijv = None
    else:
        ass__bijv = types.StringLiteral(name)
    if keilh__mja == bodo.hiframes.pd_index_ext.NumericIndexType:
        cmda__kor = keilh__mja(elem_dtype, ass__bijv)
    elif keilh__mja == bodo.hiframes.pd_index_ext.CategoricalIndexType:
        cmda__kor = keilh__mja(bodo.CategoricalArrayType(elem_dtype), ass__bijv
            )
    else:
        cmda__kor = keilh__mja(ass__bijv)
    return cmda__kor
...
def check_objmode_output_type(ret_tup, ret_type):
    return tuple(_check_objmode_type(utr__jrmy, t) for utr__jrmy, t in zip(
...
def _fix_objmode_df_type(val, val_typ, typ):
    ...
    if val_typ != typ:
        wefcy__iormr = pd.Index(val_typ.columns)
        awr__gfgpx = pd.Index(typ.columns)
        gmh__ekhj = wefcy__iormr.argsort()
        nkkp__qjwt = awr__gfgpx.argsort()
        naho__skp = val_typ.copy(data=tuple(np.array(val_typ.data)[
            gmh__ekhj]), columns=tuple(wefcy__iormr[gmh__ekhj]))
        mqp__flejm = typ.copy(data=tuple(np.array(typ.data)[nkkp__qjwt]),
            columns=tuple(awr__gfgpx[nkkp__qjwt]))
        if naho__skp == mqp__flejm:
            val_typ = typ
