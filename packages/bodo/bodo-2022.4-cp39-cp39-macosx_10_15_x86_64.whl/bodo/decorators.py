...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        oxytq__nwx = self.overloads[sig]
        dwat__pff = oxytq__nwx.metadata.get('distributed_diagnostics', None)
        if dwat__pff is None:
            usyb__smox = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(usyb__smox)
        dwat__pff.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        kbqnm__pscy = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(kbqnm__pscy)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        nrjb__nphq = options.pop('distributed')
        pipeline_class = (pipeline_class if nrjb__nphq else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        xbet__yvhrb = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if xbet__yvhrb else
            pipeline_class)
    rsddu__vpemx = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(rsddu__vpemx, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(rsddu__vpemx)
        else:
            return master_mode_wrapper(rsddu__vpemx)
    else:
        return rsddu__vpemx
