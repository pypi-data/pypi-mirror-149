...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        accw__ziltz = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(accw__ziltz)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        gpwo__reuy = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, gpwo__reuy)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        jtugh__asqa, = args
        ubk__snct = signature.return_type
        gajp__uhjst = cgutils.create_struct_proxy(ubk__snct)(context, builder)
        gajp__uhjst.obj = jtugh__asqa
        context.nrt.incref(builder, signature.args[0], jtugh__asqa)
        return gajp__uhjst._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                kdfi__ptrty = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        accw__ziltz = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(accw__ziltz)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        gpwo__reuy = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, gpwo__reuy)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        jtugh__asqa, = args
        xdkoy__fog = signature.return_type
        mtzb__bndn = cgutils.create_struct_proxy(xdkoy__fog)(context, builder)
        mtzb__bndn.obj = jtugh__asqa
        context.nrt.incref(builder, signature.args[0], jtugh__asqa)
        return mtzb__bndn._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                yrvv__sezmy = bodo.utils.conversion.coerce_to_ndarray(idx)
                xubv__btbf = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    yrvv__sezmy]
                ckg__jpdl = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    yrvv__sezmy]
                accw__ziltz = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(xubv__btbf,
                    ckg__jpdl, accw__ziltz)
            return impl
        if isinstance(idx, types.SliceType):

            def impl_slice(I, idx):
                S = I._obj
                xubv__btbf = bodo.hiframes.pd_series_ext.get_series_data(S)[idx
                    ]
                ckg__jpdl = bodo.hiframes.pd_series_ext.get_series_index(S)[idx
                    ]
                accw__ziltz = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(xubv__btbf,
                    ckg__jpdl, accw__ziltz)
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            if is_scalar_type(val):
                if I.stype.dtype == types.NPDatetime('ns'
                    ...
                    def impl_dt(I, idx, val):
                        s = integer_to_dt64(val.value)
                        yrvv__sezmy = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        kdfi__ptrty = (bodo.hiframes.datetime_timedelta_ext
                            ._to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(kdfi__ptrty))
                        yrvv__sezmy = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            yrvv__sezmy] = s
            ...
            def impl_arr(I, idx, val):
                yrvv__sezmy = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[yrvv__sezmy
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        accw__ziltz = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(accw__ziltz)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        gpwo__reuy = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, gpwo__reuy)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        jtugh__asqa, = args
        emjdd__eein = signature.return_type
        znpvb__iqmbd = cgutils.create_struct_proxy(emjdd__eein)(context,
            builder)
        znpvb__iqmbd.obj = jtugh__asqa
        context.nrt.incref(builder, signature.args[0], jtugh__asqa)
        return znpvb__iqmbd._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            xubv__btbf = bodo.hiframes.pd_series_ext.get_series_data(S)
            ckg__jpdl = bodo.hiframes.pd_series_ext.get_series_index(S)
            chrfb__uun = (idx - ckg__jpdl._start) // ckg__jpdl._step
            return xubv__btbf[chrfb__uun]
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    ...
                    yrvv__sezmy = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(xubv__btbf[
                        yrvv__sezmy])
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    ezsfg__jqczo = (bodo.hiframes.pd_series_ext.
                        get_series_index(S))
                    opmar__ynoei = ezsfg__jqczo.get_loc(idx)
                    if opmar__ynoei is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(opmar__ynoei)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                yrvv__sezmy = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[yrvv__sezmy
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        jruir__dgkf = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(goz__ies,
            int) for goz__ies in jruir__dgkf):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            nyie__wtfqo = get_literal_value(idx)
            if nyie__wtfqo not in jruir__dgkf:
                raise_bodo_error(
                    f"Series label-based getitem: '{nyie__wtfqo}' not in {jruir__dgkf}"
                    )
            arr_ind = jruir__dgkf.index(nyie__wtfqo)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    gajp__uhjst = cgutils.create_struct_proxy(fromty)(context, builder, val)
    iseut__wdwk = context.cast(builder, gajp__uhjst.obj, fromty.stype, toty
        .stype)
    zrxgd__puepf = cgutils.create_struct_proxy(toty)(context, builder)
    zrxgd__puepf.obj = iseut__wdwk
    return zrxgd__puepf._getvalue()
