...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    ahibu__yju = pq_node.out_vars[0].name
    xmqva__avvwl = pq_node.out_vars[1].name
    if ahibu__yju not in lives and xmqva__avvwl not in lives:
        return None
    elif ahibu__yju not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.type_usecol_offset = []
    elif xmqva__avvwl not in lives:
