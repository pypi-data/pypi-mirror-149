...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        lfg__nuz = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, lfg__nuz)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            dqaoh__alhxn = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(dqaoh__alhxn.dtype)
            file_write(unicode_to_utf8(fname), dqaoh__alhxn.ctypes, 
                dtype_size * dqaoh__alhxn.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            dqaoh__alhxn = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(dqaoh__alhxn.dtype)
            gqtd__wxtlb = (dtype_size * bodo.libs.distributed_api.
                get_tuple_prod(dqaoh__alhxn.shape[1:]))
            _file_write_parallel(unicode_to_utf8(fname), dqaoh__alhxn.
                ctypes, start, count, gqtd__wxtlb)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            gprit__vqorb = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                gprit__vqorb = min(gprit__vqorb, count * dtype_size)
            if gprit__vqorb < 0:
                return -1
            return gprit__vqorb
        return impl
