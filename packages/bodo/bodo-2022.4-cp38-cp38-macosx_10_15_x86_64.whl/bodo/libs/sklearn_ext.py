...
except ImportError as prw__qfxe:
    pass


def check_sklearn_version():
    if not _is_sklearn_supported_version:
        rfd__rkrsy = f""" Bodo supports scikit-learn version >= {_min_sklearn_ver_str} and < {_max_sklearn_ver_str}.
             Installed version is {sklearn.__version__}.
"""
        raise BodoError(rfd__rkrsy)


def random_forest_model_fit(m, X, y):
    hitt__kkp = m.n_estimators
    dtkd__bemd = MPI.Get_processor_name()
    vqaz__ujriv = get_host_ranks()
    rtx__dszx = len(vqaz__ujriv)
    gyhgk__zra = bodo.get_rank()
    m.n_estimators = bodo.libs.distributed_api.get_node_portion(hitt__kkp,
        rtx__dszx, gyhgk__zra)
    if gyhgk__zra == vqaz__ujriv[dtkd__bemd][0]:
        m.n_jobs = len(vqaz__ujriv[dtkd__bemd])
    ...
    xyhax__cwx = create_subcomm_mpi4py(first_rank_node)
    if xyhax__cwx != MPI.COMM_NULL:
        udqde__rlvf = 10
        vil__gwqse = bodo.libs.distributed_api.get_node_portion(hitt__kkp,
            rtx__dszx, 0)
        sthgi__fkl = vil__gwqse // udqde__rlvf
        if vil__gwqse % udqde__rlvf != 0:
            sthgi__fkl += 1
        xee__pvdgx = []
        for hhy__rrkio in range(sthgi__fkl):
            mgxe__mpx = xyhax__cwx.gather(m.estimators_[hhy__rrkio *
                udqde__rlvf:hhy__rrkio * udqde__rlvf + udqde__rlvf])
            if gyhgk__zra == 0:
                xee__pvdgx += list(itertools.chain.from_iterable(mgxe__mpx))
        if gyhgk__zra == 0:
            m.estimators_ = xee__pvdgx
    dce__dbr = MPI.COMM_WORLD
    if gyhgk__zra == 0:
        for hhy__rrkio in range(0, hitt__kkp, 10):
            dce__dbr.bcast(m.estimators_[hhy__rrkio:hhy__rrkio + 10])
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            dce__dbr.bcast(m.n_classes_)
            dce__dbr.bcast(m.classes_)
        dce__dbr.bcast(m.n_outputs_)
    else:
        okoj__lzan = []
        for hhy__rrkio in range(0, hitt__kkp, 10):
            okoj__lzan += dce__dbr.bcast(None)
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            m.n_classes_ = dce__dbr.bcast(None)
            m.classes_ = dce__dbr.bcast(None)
        m.n_outputs_ = dce__dbr.bcast(None)
        m.estimators_ = okoj__lzan
    assert len(m.estimators_) == hitt__kkp
    m.n_estimators = hitt__kkp
...
def precision_recall_fscore_support_helper(MCM, average):
    ...
    def multilabel_confusion_matrix(y_true, y_pred, *, sample_weight=None,
        ...
        return MCM
    les__gpp = sklearn.metrics._classification.multilabel_confusion_matrix
    result = -1.0
    try:
        ...
        sklearn.metrics._classification.multilabel_confusion_matrix = les__gpp
    return result
...
def precision_recall_fscore_parallel(y_true, y_pred, operation, average=
    ...
    tyt__uxf = len(labels)
    xomu__etj = np.zeros(tyt__uxf, np.int64)
    gfjks__syurb = np.zeros(tyt__uxf, np.int64)
    sxjz__foxq = np.zeros(tyt__uxf, np.int64)
    vorp__gkroa = (bodo.hiframes.pd_categorical_ext.
        get_label_dict_from_categories(labels))
    for hhy__rrkio in range(len(y_true)):
        gfjks__syurb[vorp__gkroa[y_true[hhy__rrkio]]] += 1
        if y_pred[hhy__rrkio] not in vorp__gkroa:
            continue
        evm__crch = vorp__gkroa[y_pred[hhy__rrkio]]
        sxjz__foxq[evm__crch] += 1
        if y_true[hhy__rrkio] == y_pred[hhy__rrkio]:
            xomu__etj[evm__crch] += 1
    xomu__etj = bodo.libs.distributed_api.dist_reduce(xomu__etj, np.int32(
        Reduce_Type.Sum.value))
    gfjks__syurb = bodo.libs.distributed_api.dist_reduce(gfjks__syurb, np.
        int32(Reduce_Type.Sum.value))
    sxjz__foxq = bodo.libs.distributed_api.dist_reduce(sxjz__foxq, np.int32
        (Reduce_Type.Sum.value))
    jip__qhrtx = sxjz__foxq - xomu__etj
    kpi__zywqi = gfjks__syurb - xomu__etj
    hus__xzcxm = xomu__etj
    fcu__cqrw = y_true.shape[0] - hus__xzcxm - jip__qhrtx - kpi__zywqi
    with numba.objmode(result='float64[:]'):
        MCM = np.array([fcu__cqrw, jip__qhrtx, kpi__zywqi, hus__xzcxm]
            ).T.reshape(-1, 2, 2)
...
def mse_mae_dist_helper(y_true, y_pred, sample_weight, multioutput, squared,
    metric):
    if metric == 'mse':
        eclrl__itaj = sklearn.metrics.mean_squared_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values', squared=True
            )
    elif metric == 'mae':
        eclrl__itaj = sklearn.metrics.mean_absolute_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values')
    else:
        raise RuntimeError(
            f"Unrecognized metric {metric}. Must be one of 'mae' and 'mse'")
    dce__dbr = MPI.COMM_WORLD
    vayx__dhfc = dce__dbr.Get_size()
    if sample_weight is not None:
        dhlqn__lav = np.sum(sample_weight)
    else:
        dhlqn__lav = np.float64(y_true.shape[0])
    lyims__airnb = np.zeros(vayx__dhfc, dtype=type(dhlqn__lav))
    dce__dbr.Allgather(dhlqn__lav, lyims__airnb)
    ygizx__crxnr = np.zeros((vayx__dhfc, *eclrl__itaj.shape), dtype=
        eclrl__itaj.dtype)
    dce__dbr.Allgather(eclrl__itaj, ygizx__crxnr)
    dbw__hfhrj = np.average(ygizx__crxnr, weights=lyims__airnb, axis=0)
    if metric == 'mse' and not squared:
        dbw__hfhrj = np.sqrt(dbw__hfhrj)
    if isinstance(multioutput, str) and multioutput == 'raw_values':
        return dbw__hfhrj
    elif isinstance(multioutput, str) and multioutput == 'uniform_average':
        return np.average(dbw__hfhrj)
    else:
        return np.average(dbw__hfhrj, weights=multioutput)
...
def accuracy_score_dist_helper(y_true, y_pred, normalize, sample_weight):
    ...
    dce__dbr = MPI.COMM_WORLD
    score = dce__dbr.allreduce(score, op=MPI.SUM)
    if normalize:
        hwd__cyh = np.sum(sample_weight) if sample_weight is not None else len(
            y_true)
        hwd__cyh = dce__dbr.allreduce(hwd__cyh, op=MPI.SUM)
        score = score / hwd__cyh
    return score
...
def check_consistent_length_parallel(*arrays):
    dce__dbr = MPI.COMM_WORLD
    axj__lfj = True
    nvntl__znrdw = [len(irt__rtes) for irt__rtes in arrays if irt__rtes is not
        None]
    if len(np.unique(nvntl__znrdw)) > 1:
        axj__lfj = False
    axj__lfj = dce__dbr.allreduce(axj__lfj, op=MPI.LAND)
    return axj__lfj
...
def r2_score_dist_helper(y_true, y_pred, sample_weight, multioutput):
    ...
    abn__ipvtc = y_true.shape[0]
    nek__ejzo = dce__dbr.allreduce(abn__ipvtc, op=MPI.SUM)
    if nek__ejzo < 2:
        warnings.warn(
    ...
    if sample_weight is not None:
        ...
        borv__mfq = sample_weight[:, np.newaxis]
    else:
        sample_weight = np.float64(y_true.shape[0])
        borv__mfq = 1.0
    dgru__orpf = (borv__mfq * (y_true - y_pred) ** 2).sum(axis=0, dtype=np.
        float64)
    rbos__mva = np.zeros(dgru__orpf.shape, dtype=dgru__orpf.dtype)
    dce__dbr.Allreduce(dgru__orpf, rbos__mva, op=MPI.SUM)
    fnc__ohmad = np.nansum(y_true * borv__mfq, axis=0, dtype=np.float64)
    nmcur__vybfa = np.zeros_like(fnc__ohmad)
    dce__dbr.Allreduce(fnc__ohmad, nmcur__vybfa, op=MPI.SUM)
    lerc__dejq = np.nansum(sample_weight, dtype=np.float64)
    xwhp__ihjlm = dce__dbr.allreduce(lerc__dejq, op=MPI.SUM)
    cynhq__jetg = nmcur__vybfa / xwhp__ihjlm
    xsai__kspxk = (borv__mfq * (y_true - cynhq__jetg) ** 2).sum(axis=0,
        dtype=np.float64)
    tkb__qim = np.zeros(xsai__kspxk.shape, dtype=xsai__kspxk.dtype)
    dce__dbr.Allreduce(xsai__kspxk, tkb__qim, op=MPI.SUM)
    tuini__aej = tkb__qim != 0
    zrvr__hvci = rbos__mva != 0
    npav__uutoa = tuini__aej & zrvr__hvci
    ywz__wnc = np.ones([y_true.shape[1] if len(y_true.shape) > 1 else 1])
    ywz__wnc[npav__uutoa] = 1 - rbos__mva[npav__uutoa] / tkb__qim[npav__uutoa]
    ywz__wnc[zrvr__hvci & ~tuini__aej] = 0.0
    if isinstance(multioutput, str):
        if multioutput == 'raw_values':
            ...
            moisw__fwa = tkb__qim
            if not np.any(tuini__aej):
                if not np.any(zrvr__hvci):
                    return np.array([1.0])
                else:
                    return np.array([0.0])
    else:
        moisw__fwa = multioutput
    return np.array([np.average(ywz__wnc, weights=moisw__fwa)])
...
def confusion_matrix_dist_helper(y_true, y_pred, labels=None, sample_weight
    ...
    dce__dbr = MPI.COMM_WORLD
    try:
        fvk__evu = sklearn.metrics.confusion_matrix(y_true, y_pred, labels=
            labels, sample_weight=sample_weight, normalize=None)
    except ValueError as tzbun__hihdd:
        fvk__evu = tzbun__hihdd
    yfoy__rnm = isinstance(fvk__evu, ValueError
        ) and 'At least one label specified must be in y_true' in fvk__evu.args[
        0]
    ysi__kqyn = dce__dbr.allreduce(yfoy__rnm, op=MPI.LAND)
    if ysi__kqyn:
        raise fvk__evu
    elif yfoy__rnm:
        dtype = np.int64
        if sample_weight is not None and sample_weight.dtype.kind not in {'i',
            'u', 'b'}:
            dtype = np.float64
        nwwzj__iev = np.zeros((labels.size, labels.size), dtype=dtype)
    else:
        nwwzj__iev = fvk__evu
    wgp__njh = np.zeros_like(nwwzj__iev)
    dce__dbr.Allreduce(nwwzj__iev, wgp__njh)
    with np.errstate(all='ignore'):
        if normalize == 'true':
            wgp__njh = wgp__njh / wgp__njh.sum(axis=1, keepdims=True)
        elif normalize == 'pred':
            wgp__njh = wgp__njh / wgp__njh.sum(axis=0, keepdims=True)
        elif normalize == 'all':
            wgp__njh = wgp__njh / wgp__njh.sum()
        wgp__njh = np.nan_to_num(wgp__njh)
    return wgp__njh
...
def overload_confusion_matrix(y_true, y_pred, labels=None, sample_weight=
    ...
    check_sklearn_version()
    itfi__wguwo = 'def _confusion_matrix_impl(\n'
    itfi__wguwo += '    y_true, y_pred, labels=None,\n'
    itfi__wguwo += '    sample_weight=None, normalize=None,\n'
    itfi__wguwo += '    _is_data_distributed=False,\n'
    itfi__wguwo += '):\n'
    itfi__wguwo += (
        '    y_true = bodo.utils.conversion.coerce_to_array(y_true)\n')
    itfi__wguwo += (
    ...
    cpzs__mvfd = 'int64[:,:]', 'np.int64'
    if not is_overload_none(normalize):
        cpzs__mvfd = 'float64[:,:]', 'np.float64'
    ...
    if not is_overload_none(labels):
        ...
        itfi__wguwo += '    labels = bodo.allgatherv(labels, False)\n'
        itfi__wguwo += """    labels = bodo.libs.array_kernels.sort(labels, ascending=True, inplace=False)
"""
    itfi__wguwo += f"    with numba.objmode(cm='{cpzs__mvfd[0]}'):\n"
    if is_overload_false(_is_data_distributed):
        itfi__wguwo += '      cm = sklearn.metrics.confusion_matrix(\n'
    else:
        itfi__wguwo += '      cm = confusion_matrix_dist_helper(\n'
    itfi__wguwo += '        y_true, y_pred, labels=labels,\n'
    itfi__wguwo += (
        '        sample_weight=sample_weight, normalize=normalize,\n')
    itfi__wguwo += f'      ).astype({cpzs__mvfd[1]})\n'
    itfi__wguwo += '    return cm\n'
    xayn__hvosb = {}
    exec(itfi__wguwo, globals(), xayn__hvosb)
    vhf__uyq = xayn__hvosb['_confusion_matrix_impl']
    return vhf__uyq
...
def fit_sgd(m, X, y, y_classes=None, _is_data_distributed=False):
    dce__dbr = MPI.COMM_WORLD
    slqq__laq = dce__dbr.allreduce(len(X), op=MPI.SUM)
    onfy__klur = len(X) / slqq__laq
    jvx__sem = dce__dbr.Get_size()
    m.n_jobs = 1
    m.early_stopping = False
    fldg__eauw = np.inf
    oce__obx = 0
    if m.loss == 'hinge':
        imucl__jlb = hinge_loss
    elif m.loss == 'log':
        imucl__jlb = log_loss
    elif m.loss == 'squared_error':
        imucl__jlb = mean_squared_error
    else:
        raise ValueError('loss {} not supported'.format(m.loss))
    she__tgqla = False
    if isinstance(m, sklearn.linear_model.SGDRegressor):
        she__tgqla = True
    for domva__sxnn in range(m.max_iter):
        if she__tgqla:
            m.partial_fit(X, y)
        else:
            m.partial_fit(X, y, classes=y_classes)
        m.coef_ = m.coef_ * onfy__klur
        m.coef_ = dce__dbr.allreduce(m.coef_, op=MPI.SUM)
        m.intercept_ = m.intercept_ * onfy__klur
        m.intercept_ = dce__dbr.allreduce(m.intercept_, op=MPI.SUM)
        if she__tgqla:
            y_pred = m.predict(X)
            mmou__hlo = imucl__jlb(y, y_pred)
        else:
            y_pred = m.decision_function(X)
            mmou__hlo = imucl__jlb(y, y_pred, labels=y_classes)
        khub__qqz = dce__dbr.allreduce(mmou__hlo, op=MPI.SUM)
        mmou__hlo = khub__qqz / jvx__sem
        if m.tol > np.NINF and mmou__hlo > fldg__eauw - m.tol * slqq__laq:
            oce__obx += 1
        else:
            oce__obx = 0
        if mmou__hlo < fldg__eauw:
            fldg__eauw = mmou__hlo
        if oce__obx >= m.n_iter_no_change:
            break
    return m
...
def kmeans_fit_helper(m, len_X, all_X, all_sample_weight, _is_data_distributed
    ...
    dce__dbr = MPI.COMM_WORLD
    gyhgk__zra = dce__dbr.Get_rank()
    dtkd__bemd = MPI.Get_processor_name()
    vqaz__ujriv = get_host_ranks()
    aavld__trzo = m.n_jobs if hasattr(m, 'n_jobs') else None
    mhiuq__dglr = m._n_threads if hasattr(m, '_n_threads') else None
    m._n_threads = len(vqaz__ujriv[dtkd__bemd])
    if gyhgk__zra == 0:
        m.fit(X=all_X, y=None, sample_weight=all_sample_weight)
    if gyhgk__zra == 0:
        dce__dbr.bcast(m.cluster_centers_)
        dce__dbr.bcast(m.inertia_)
        dce__dbr.bcast(m.n_iter_)
    else:
        m.cluster_centers_ = dce__dbr.bcast(None)
        m.inertia_ = dce__dbr.bcast(None)
        m.n_iter_ = dce__dbr.bcast(None)
    if _is_data_distributed:
        ctvfy__foref = dce__dbr.allgather(len_X)
        if gyhgk__zra == 0:
            wnrh__sou = np.empty(len(ctvfy__foref) + 1, dtype=int)
            np.cumsum(ctvfy__foref, out=wnrh__sou[1:])
            wnrh__sou[0] = 0
            waclh__mvq = [m.labels_[wnrh__sou[fctyr__mnuoc]:wnrh__sou[
                fctyr__mnuoc + 1]] for fctyr__mnuoc in range(len(ctvfy__foref))
                ]
            bzsub__nkkd = dce__dbr.scatter(waclh__mvq)
        else:
            bzsub__nkkd = dce__dbr.scatter(None)
        m.labels_ = bzsub__nkkd
    elif gyhgk__zra == 0:
        dce__dbr.bcast(m.labels_)
    else:
        m.labels_ = dce__dbr.bcast(None)
    m._n_threads = mhiuq__dglr
    return m
...
def overload_kmeans_clustering_score(m, X, y=None, sample_weight=None,
    ...
    def _cluster_kmeans_score(m, X, y=None, sample_weight=None,
        ...
        with numba.objmode(result='float64'):
            ...
            if _is_data_distributed:
                dce__dbr = MPI.COMM_WORLD
                result = dce__dbr.allreduce(result, op=MPI.SUM)
            m._n_threads = mhiuq__dglr
        return result
...
def overload_multinomial_nb_model_fit(m, X, y, sample_weight=None,
    ...
    if is_overload_false(_is_data_distributed):
        ...
        itfi__wguwo = 'def _model_multinomial_nb_fit_impl(\n'
        itfi__wguwo += (
            '    m, X, y, sample_weight=None, _is_data_distributed=False\n')
        itfi__wguwo += '):  # pragma: no cover\n'
        itfi__wguwo += '    y = bodo.utils.conversion.coerce_to_ndarray(y)\n'
        if isinstance(X, DataFrameType):
            itfi__wguwo += '    X = X.to_numpy()\n'
        else:
            itfi__wguwo += (
                '    X = bodo.utils.conversion.coerce_to_ndarray(X)\n')
        itfi__wguwo += '    my_rank = bodo.get_rank()\n'
        itfi__wguwo += '    nranks = bodo.get_size()\n'
        itfi__wguwo += '    total_cols = X.shape[1]\n'
        itfi__wguwo += '    for i in range(nranks):\n'
        itfi__wguwo += """        start = bodo.libs.distributed_api.get_start(total_cols, nranks, i)
"""
        itfi__wguwo += (
            '        end = bodo.libs.distributed_api.get_end(total_cols, nranks, i)\n'
            )
        itfi__wguwo += '        if i == my_rank:\n'
        itfi__wguwo += (
            '            X_train = bodo.gatherv(X[:, start:end:1], root=i)\n')
        itfi__wguwo += '        else:\n'
        itfi__wguwo += '            bodo.gatherv(X[:, start:end:1], root=i)\n'
        itfi__wguwo += '    y_train = bodo.allgatherv(y, False)\n'
        itfi__wguwo += '    with numba.objmode(m="multinomial_nb_type"):\n'
        itfi__wguwo += '        m = fit_multinomial_nb(\n'
        itfi__wguwo += """            m, X_train, y_train, sample_weight, total_cols, _is_data_distributed
"""
        itfi__wguwo += '        )\n'
        itfi__wguwo += '    bodo.barrier()\n'
        itfi__wguwo += '    return m\n'
        xayn__hvosb = {}
        exec(itfi__wguwo, globals(), xayn__hvosb)
        uqtv__dpjg = xayn__hvosb['_model_multinomial_nb_fit_impl']
        return uqtv__dpjg
...
def fit_multinomial_nb(m, X_train, y_train, sample_weight=None, total_cols=
    ...
    domva__sxnn, n_features = X_train.shape
    m.n_features_in_ = n_features
    yjpua__gbxf = LabelBinarizer()
    bofco__zvj = yjpua__gbxf.fit_transform(y_train)
    m.classes_ = yjpua__gbxf.classes_
    if bofco__zvj.shape[1] == 1:
        bofco__zvj = np.concatenate((1 - bofco__zvj, bofco__zvj), axis=1)
    if sample_weight is not None:
        bofco__zvj = bofco__zvj.astype(np.float64, copy=False)
        sample_weight = _check_sample_weight(sample_weight, X_train)
        sample_weight = np.atleast_2d(sample_weight)
        bofco__zvj *= sample_weight.T
    class_prior = m.class_prior
    ngxj__cfr = bofco__zvj.shape[1]
    m._init_counters(ngxj__cfr, n_features)
    m._count(X_train.astype('float64'), bofco__zvj)
    alpha = m._check_alpha()
    m._update_class_log_prior(class_prior=class_prior)
    pmseb__ygi = m.feature_count_ + alpha
    wmh__qgpbq = pmseb__ygi.sum(axis=1)
    dce__dbr = MPI.COMM_WORLD
    jvx__sem = dce__dbr.Get_size()
    yrffg__ndkc = np.zeros(ngxj__cfr)
    dce__dbr.Allreduce(wmh__qgpbq, yrffg__ndkc, op=MPI.SUM)
    gtqsk__eaoxw = np.log(pmseb__ygi) - np.log(yrffg__ndkc.reshape(-1, 1))
    fwwlz__xbjag = gtqsk__eaoxw.T.reshape(n_features * ngxj__cfr)
    ysqv__odhvz = np.ones(jvx__sem) * (total_cols // jvx__sem)
    elrve__rrv = total_cols % jvx__sem
    for zhf__awt in range(elrve__rrv):
        ysqv__odhvz[zhf__awt] += 1
    ysqv__odhvz *= ngxj__cfr
    djk__tub = np.zeros(jvx__sem, dtype=np.int32)
    djk__tub[1:] = np.cumsum(ysqv__odhvz)[:-1]
    rciy__ozgdw = np.zeros((total_cols, ngxj__cfr), dtype=np.float64)
    dce__dbr.Allgatherv(fwwlz__xbjag, [rciy__ozgdw, ysqv__odhvz, djk__tub,
        MPI.DOUBLE_PRECISION])
    m.feature_log_prob_ = rciy__ozgdw.T
...
def sklearn_preprocessing_standard_scaler_fit_dist_helper(m, X):
    ...
    vayx__dhfc = dce__dbr.Get_size()
    vgo__tkf = m.with_std
    rssly__jcjj = m.with_mean
    m.with_std = False
    if vgo__tkf:
        m.with_mean = True
    m = m.fit(X)
    m.with_std = vgo__tkf
    m.with_mean = rssly__jcjj
    if not isinstance(m.n_samples_seen_, numbers.Integral):
        ...
        m.n_samples_seen_ = np.repeat(m.n_samples_seen_, X.shape[1]).astype(np
            .int64, copy=False)
    xfe__lksae = np.zeros((vayx__dhfc, *m.n_samples_seen_.shape), dtype=m.
        n_samples_seen_.dtype)
    dce__dbr.Allgather(m.n_samples_seen_, xfe__lksae)
    papjr__klyer = np.sum(xfe__lksae, axis=0)
    m.n_samples_seen_ = papjr__klyer
    if m.with_mean or m.with_std:
        hwatf__eldrf = np.zeros((vayx__dhfc, *m.mean_.shape), dtype=m.mean_
            .dtype)
        dce__dbr.Allgather(m.mean_, hwatf__eldrf)
        hwatf__eldrf[np.isnan(hwatf__eldrf)] = 0
        dbuhp__ykybf = np.average(hwatf__eldrf, axis=0, weights=xfe__lksae)
        m.mean_ = dbuhp__ykybf
    if m.with_std:
        rvg__rrbsu = sklearn_safe_accumulator_op(np.nansum, (X -
            dbuhp__ykybf) ** 2, axis=0) / papjr__klyer
        qbz__xemr = np.zeros_like(rvg__rrbsu)
        dce__dbr.Allreduce(rvg__rrbsu, qbz__xemr, op=MPI.SUM)
        m.var_ = qbz__xemr
        m.scale_ = sklearn_handle_zeros_in_scale(np.sqrt(m.var_))
    hvlvt__nlq = dce__dbr.allreduce(hvlvt__nlq, op=MPI.LAND)
    if hvlvt__nlq:
...
def get_data_slice_parallel(data, labels, len_train):
    wslr__jnog = data[:len_train]
    pezt__bbsae = data[len_train:]
    wslr__jnog = bodo.rebalance(wslr__jnog)
    pezt__bbsae = bodo.rebalance(pezt__bbsae)
    jaspb__pciuj = labels[:len_train]
    klro__qigwh = labels[len_train:]
    jaspb__pciuj = bodo.rebalance(jaspb__pciuj)
    klro__qigwh = bodo.rebalance(klro__qigwh)
    return wslr__jnog, pezt__bbsae, jaspb__pciuj, klro__qigwh
...
def overload_train_test_split(data, labels=None, train_size=None, test_size
    ...
    _is_data_distributed=False):
    check_sklearn_version()
    dgumf__dfwk = {'stratify': stratify}
    sog__jqqm = {'stratify': None}
    check_unsupported_args('train_test_split', dgumf__dfwk, sog__jqqm, 'ml')
    if is_overload_false(_is_data_distributed):
        jdvw__bgmjf = f'data_split_type_{numba.core.ir_utils.next_label()}'
        hdj__rfor = f'labels_split_type_{numba.core.ir_utils.next_label()}'
        for ide__zlv, brsf__dejj in ((data, jdvw__bgmjf), (labels, hdj__rfor)):
            if isinstance(ide__zlv, (DataFrameType, SeriesType)):
                qqxz__yuwc = ide__zlv.copy(index=NumericIndexType(types.int64))
                setattr(types, brsf__dejj, qqxz__yuwc)
            else:
                setattr(types, brsf__dejj, ide__zlv)
        itfi__wguwo = 'def _train_test_split_impl(\n'
        itfi__wguwo += '    data,\n'
        itfi__wguwo += '    labels=None,\n'
        itfi__wguwo += '    train_size=None,\n'
        itfi__wguwo += '    test_size=None,\n'
        itfi__wguwo += '    random_state=None,\n'
        itfi__wguwo += '    shuffle=True,\n'
        itfi__wguwo += '    stratify=None,\n'
        itfi__wguwo += '    _is_data_distributed=False,\n'
        itfi__wguwo += '):  # pragma: no cover\n'
        itfi__wguwo += (
            """    with numba.objmode(data_train='{}', data_test='{}', labels_train='{}', labels_test='{}'):
"""
            .format(jdvw__bgmjf, jdvw__bgmjf, hdj__rfor, hdj__rfor))
        itfi__wguwo += """        data_train, data_test, labels_train, labels_test = sklearn.model_selection.train_test_split(
"""
        itfi__wguwo += '            data,\n'
        itfi__wguwo += '            labels,\n'
        itfi__wguwo += '            train_size=train_size,\n'
        itfi__wguwo += '            test_size=test_size,\n'
        itfi__wguwo += '            random_state=random_state,\n'
        itfi__wguwo += '            shuffle=shuffle,\n'
        itfi__wguwo += '            stratify=stratify,\n'
        itfi__wguwo += '        )\n'
        itfi__wguwo += (
            '    return data_train, data_test, labels_train, labels_test\n')
        xayn__hvosb = {}
        exec(itfi__wguwo, globals(), xayn__hvosb)
        _train_test_split_impl = xayn__hvosb['_train_test_split_impl']
        ...
        def _train_test_split_impl(data, labels=None, train_size=None,
            ...
            lvcx__mynt = bodo.libs.distributed_api.dist_reduce(len(data),
                np.int32(Reduce_Type.Sum.value))
            len_train = int(lvcx__mynt * train_size)
            cptuk__nqhy = lvcx__mynt - len_train
            if shuffle:
                labels = set_labels_type(labels, label_type)
                gyhgk__zra = bodo.get_rank()
                jvx__sem = bodo.get_size()
                pum__wgss = np.empty(jvx__sem, np.int64)
                bodo.libs.distributed_api.allgather(pum__wgss, len(data))
                gvf__qeiph = np.cumsum(pum__wgss[0:gyhgk__zra + 1])
                zsenm__sie = np.full(lvcx__mynt, True)
                zsenm__sie[:cptuk__nqhy] = False
                np.random.seed(42)
                np.random.permutation(zsenm__sie)
                if gyhgk__zra:
                    mfws__wkeyu = gvf__qeiph[gyhgk__zra - 1]
                else:
                    mfws__wkeyu = 0
                lmhoy__owifq = gvf__qeiph[gyhgk__zra]
                onu__qzwe = zsenm__sie[mfws__wkeyu:lmhoy__owifq]
                wslr__jnog = data[onu__qzwe]
                pezt__bbsae = data[~onu__qzwe]
                jaspb__pciuj = labels[onu__qzwe]
                klro__qigwh = labels[~onu__qzwe]
                wslr__jnog = bodo.random_shuffle(wslr__jnog, seed=
                    random_state, parallel=True)
                pezt__bbsae = bodo.random_shuffle(pezt__bbsae, seed=
                    random_state, parallel=True)
                jaspb__pciuj = bodo.random_shuffle(jaspb__pciuj, seed=
                    random_state, parallel=True)
                klro__qigwh = bodo.random_shuffle(klro__qigwh, seed=
                    random_state, parallel=True)
                jaspb__pciuj = reset_labels_type(jaspb__pciuj, label_type)
                klro__qigwh = reset_labels_type(klro__qigwh, label_type)
            else:
                wslr__jnog, pezt__bbsae, jaspb__pciuj, klro__qigwh = (
...
def sklearn_preprocessing_minmax_scaler_fit_dist_helper(m, X):
    ...
    vayx__dhfc = dce__dbr.Get_size()
    m = m.fit(X)
    papjr__klyer = dce__dbr.allreduce(m.n_samples_seen_, op=MPI.SUM)
    m.n_samples_seen_ = papjr__klyer
    inw__bgw = np.zeros((vayx__dhfc, *m.data_min_.shape), dtype=m.data_min_
        .dtype)
    dce__dbr.Allgather(m.data_min_, inw__bgw)
    zzqw__bcvt = np.nanmin(inw__bgw, axis=0)
    svc__eyuuq = np.zeros((vayx__dhfc, *m.data_max_.shape), dtype=m.
        data_max_.dtype)
    dce__dbr.Allgather(m.data_max_, svc__eyuuq)
    gdv__pam = np.nanmax(svc__eyuuq, axis=0)
    wdy__thdv = gdv__pam - zzqw__bcvt
    m.scale_ = (m.feature_range[1] - m.feature_range[0]
        ) / sklearn_handle_zeros_in_scale(wdy__thdv)
    m.min_ = m.feature_range[0] - zzqw__bcvt * m.scale_
    m.data_min_ = zzqw__bcvt
    m.data_max_ = gdv__pam
    m.data_range_ = wdy__thdv
    return m
...
def overload_rf_classifier_model_fit(m, X, y, sample_weight=None,
    _is_data_distributed=False):
    xcdrl__nxa = 'RandomForestClassifier'
    if isinstance(m, BodoRandomForestRegressorType):
        xcdrl__nxa = 'RandomForestRegressor'
    if not is_overload_none(sample_weight):
        raise BodoError(
            f"sklearn.ensemble.{xcdrl__nxa}.fit() : 'sample_weight' is not supported for distributed data."
            )
    ...
    def _model_fit_impl(m, X, y, sample_weight=None, _is_data_distributed=False
        ...
        with numba.objmode(first_rank_node='int32[:]'):
            first_rank_node = get_nodes_first_ranks()
        if _is_data_distributed:
            rtx__dszx = len(first_rank_node)
            X = bodo.gatherv(X)
            y = bodo.gatherv(y)
            if rtx__dszx > 1:
                X = bodo.libs.distributed_api.bcast_comm(X, comm_ranks=
                    first_rank_node, nranks=rtx__dszx)
...
def _cv_fit_transform_helper(m, X):
    ...
    if m.vocabulary is None:
        ...
        jwh__vjdz = True
    return jwh__vjdz, local_vocabulary
    ...
    if is_overload_true(_is_data_distributed):
        ...
        def _count_vectorizer_fit_transform_impl(m, X, y=None,
            ...
            if changeVoc:
                ...
                irew__uigy = bodo.libs.array_kernels.unique(local_vocabulary,
                    parallel=True)
                irew__uigy = bodo.allgatherv(irew__uigy, False)
                irew__uigy = bodo.libs.array_kernels.sort(irew__uigy,
                    ascending=True, inplace=True)
                qgej__opc = {}
                for hhy__rrkio in range(len(irew__uigy)):
                    qgej__opc[irew__uigy[hhy__rrkio]] = hhy__rrkio
            else:
                qgej__opc = local_vocabulary
            with numba.objmode(transformed_X='csr_matrix_int64_int64'):
                if changeVoc:
                    m.vocabulary = qgej__opc
                transformed_X = m.fit_transform(X, y)
                transformed_X.indices = transformed_X.indices.astype(np.int64)
                transformed_X.indptr = transformed_X.indptr.astype(np.int64)
            return transformed_X
