...
def init_binary_arr(typingctx, data_typ=None):
    ...
    def codegen(context, builder, sig, args):
        zzg__brlnc, = args
        dyi__eme = context.make_helper(builder, binary_array_type)
        dyi__eme.data = zzg__brlnc
        context.nrt.incref(builder, data_typ, zzg__brlnc)
        return dyi__eme._getvalue()
...
def init_bytes_type(typingctx, data_typ, length_type):
    ...
    def codegen(context, builder, sig, args):
        eyq__jovnr = cgutils.create_struct_proxy(sig.args[0])(context,
            builder, value=args[0])
        usj__xnggy = args[1]
        paw__lbjce = cgutils.create_struct_proxy(bytes_type)(context, builder)
        paw__lbjce.meminfo = context.nrt.meminfo_alloc(builder, usj__xnggy)
        paw__lbjce.nitems = usj__xnggy
        paw__lbjce.itemsize = lir.Constant(paw__lbjce.itemsize.type, 1)
        paw__lbjce.data = context.nrt.meminfo_data(builder, paw__lbjce.meminfo)
        paw__lbjce.parent = cgutils.get_null_value(paw__lbjce.parent.type)
        paw__lbjce.shape = cgutils.pack_array(builder, [usj__xnggy],
            context.get_value_type(types.intp))
        paw__lbjce.strides = eyq__jovnr.strides
        cgutils.memcpy(builder, paw__lbjce.data, eyq__jovnr.data, usj__xnggy)
        return paw__lbjce._getvalue()
...
def binary_arr_hex(arr):
    ieuxa__qkypc = numba.cpython.unicode.PY_UNICODE_1BYTE_KIND

    def impl(arr):
        usj__xnggy = len(arr) * 2
        output = numba.cpython.unicode._empty_string(ieuxa__qkypc,
            usj__xnggy, 1)
...
def bytes_to_hex(typingctx, output, arr):
    ...
    def codegen(context, builder, sig, args):
        ggut__ksftn = cgutils.create_struct_proxy(sig.args[0])(context,
            builder, value=args[0])
        mtf__laave = cgutils.create_struct_proxy(sig.args[1])(context,
            builder, value=args[1])
        qegik__ifurf = lir.FunctionType(lir.IntType(8).as_pointer(), [lir.
            IntType(8).as_pointer(), lir.IntType(8).as_pointer(), lir.
            IntType(64)])
        apz__jxt = cgutils.get_or_insert_function(builder.module,
            qegik__ifurf, name='bytes_to_hex')
        builder.call(apz__jxt, (ggut__ksftn.data, mtf__laave.data,
            mtf__laave.nitems))
...
def binary_arr_getitem(arr, ind):
    ...
    if isinstance(ind, types.Integer):
        ...
        def impl(arr, ind):
            ila__ogimr = arr._data[ind]
            return init_bytes_type(ila__ogimr, len(ila__ogimr))
        return impl
...
def overload_bytes_fromhex(hex_str):
    ...
    if hex_str == bodo.string_type:
        ...
        def impl(hex_str):
            if not hex_str._is_ascii or hex_str._kind != ieuxa__qkypc:
                raise TypeError(
                    'bytes.fromhex is only supported on ascii strings')
            zzg__brlnc = np.empty(len(hex_str) // 2, np.uint8)
            usj__xnggy = _bytes_fromhex(zzg__brlnc.ctypes, hex_str._data,
                len(hex_str))
            hdf__jfvv = init_bytes_type(zzg__brlnc, usj__xnggy)
            return hdf__jfvv
        return impl
...
def create_binary_cmp_op_overload(op):
    ...
    def overload_binary_cmp(lhs, rhs):
        htf__oxup = lhs == binary_array_type
        ueupg__bhph = rhs == binary_array_type
        rrup__tvhyf = 'lhs' if htf__oxup else 'rhs'
        cbprq__xki = 'def impl(lhs, rhs):\n'
        cbprq__xki += '  numba.parfors.parfor.init_prange()\n'
        cbprq__xki += f'  n = len({rrup__tvhyf})\n'
        cbprq__xki += (
            '  out_arr = bodo.libs.bool_arr_ext.alloc_bool_array(n)\n')
        cbprq__xki += '  for i in numba.parfors.parfor.internal_prange(n):\n'
        udgxp__nzb = []
        if htf__oxup:
            udgxp__nzb.append('bodo.libs.array_kernels.isna(lhs, i)')
        if ueupg__bhph:
            udgxp__nzb.append('bodo.libs.array_kernels.isna(rhs, i)')
        cbprq__xki += f"    if {' or '.join(udgxp__nzb)}:\n"
        cbprq__xki += '      bodo.libs.array_kernels.setna(out_arr, i)\n'
        cbprq__xki += '      continue\n'
        vzsem__krvby = 'lhs[i]' if htf__oxup else 'lhs'
        qfemy__abq = 'rhs[i]' if ueupg__bhph else 'rhs'
        cbprq__xki += f'    out_arr[i] = op({vzsem__krvby}, {qfemy__abq})\n'
        cbprq__xki += '  return out_arr\n'
        ivpzb__yvhd = {}
        exec(cbprq__xki, {'bodo': bodo, 'numba': numba, 'op': op}, ivpzb__yvhd)
        return ivpzb__yvhd['impl']
