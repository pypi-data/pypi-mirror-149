...
def install_mpl_class(types_name, python_type):
    ymnh__gwy = ''.join(map(str.title, types_name.split('_')))
    cel__gqh = f'class {ymnh__gwy}(types.Opaque):\n'
    cel__gqh += '    def __init__(self):\n'
    cel__gqh += f"       types.Opaque.__init__(self, name='{ymnh__gwy}')\n"
    cel__gqh += '    def __reduce__(self):\n'
    cel__gqh += (
        f"        return (types.Opaque, ('{ymnh__gwy}',), self.__dict__)\n")
    pnk__ukoz = {}
    exec(cel__gqh, {'types': types, 'bodo': bodo}, pnk__ukoz)
    jcz__lgsh = pnk__ukoz[ymnh__gwy]
    scijg__noa = sys.modules[__name__]
    setattr(scijg__noa, ymnh__gwy, jcz__lgsh)
    class_instance = jcz__lgsh()
    setattr(types, types_name, class_instance)
    register_model(jcz__lgsh)(models.OpaqueModel)
    typeof_impl.register(python_type)(lambda val, c: class_instance)
    unbox(jcz__lgsh)(unbox_mpl_obj)
    box(jcz__lgsh)(box_mpl_obj)
...
def _install_mpl_types():
    niek__qce = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for oznbt__vjb, wfibi__xoybn in niek__qce:
        install_mpl_class(oznbt__vjb, wfibi__xoybn)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    zcsem__xjfn = ', '.join(f'e{usapv__fcg}' for usapv__fcg in range(len(args))
        )
    if zcsem__xjfn:
        zcsem__xjfn += ', '
    nhv__hgice = ', '.join(f"{xaq__gpoh} = ''" for xaq__gpoh in kws.keys())
    wya__tdm = 'matplotlib_obj, ' if obj_typ is not None else ''
    lpyw__jtq = f'def mpl_stub({wya__tdm} {zcsem__xjfn} {nhv__hgice}):\n'
    lpyw__jtq += '    pass\n'
    zbo__icv = {}
    exec(lpyw__jtq, {}, zbo__icv)
    dtjgv__ory = zbo__icv['mpl_stub']
    eprz__nwa = numba.core.utils.pysignature(dtjgv__ory)
    yltjx__zsg = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(yltjx__zsg)).replace(pysig=
        eprz__nwa)


def generate_axes_typing(mod_name, nrows, ncols):
    ini__mah = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(ini__mah.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(ini__mah.format(mod_name, 'ncols'))
    lltc__gosuz = get_overload_const_int(nrows)
    rhs__jhz = get_overload_const_int(ncols)
    if lltc__gosuz < 1:
        raise BodoError(ini__mah.format(mod_name, 'nrows'))
    if rhs__jhz < 1:
        raise BodoError(ini__mah.format(mod_name, 'ncols'))
    if lltc__gosuz == 1 and rhs__jhz == 1:
        gkev__vrp = types.mpl_axes_type
    else:
        if rhs__jhz == 1:
            dzu__vdj = types.mpl_axes_type
        else:
            dzu__vdj = types.Tuple([types.mpl_axes_type] * rhs__jhz)
        gkev__vrp = types.Tuple([dzu__vdj] * lltc__gosuz)
    return gkev__vrp


def generate_pie_return_type(args, kws):
    fecp__yyu = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if fecp__yyu == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    wrl__gqn = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(wrl__gqn):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(wrl__gqn):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        esbko__kvb = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, esbko__kvb]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    esbko__kvb = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    oznbt__vjb = str(esbko__kvb)
    if not hasattr(types, oznbt__vjb):
        oznbt__vjb = f'objmode_type{ir_utils.next_label()}'
        setattr(types, oznbt__vjb, esbko__kvb)
    lpyw__jtq = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{oznbt__vjb}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    zbo__icv = {}
    exec(lpyw__jtq, {'numba': numba, 'np': np}, zbo__icv)
    impl = zbo__icv['impl']
    return impl
