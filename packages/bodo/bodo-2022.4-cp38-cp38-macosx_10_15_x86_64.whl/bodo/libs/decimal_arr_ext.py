...
def decimal_to_str_codegen(context, builder, signature, args, scale):
    ...
    wyqfq__maqtb = cgutils.create_struct_proxy(types.unicode_type)(context,
        builder)
    izuw__dayh = lir.FunctionType(lir.VoidType(), [lir.IntType(128), lir.
        IntType(8).as_pointer().as_pointer(), lir.IntType(64).as_pointer(),
        lir.IntType(32)])
    oyqn__rdve = cgutils.get_or_insert_function(builder.module, izuw__dayh,
        name='decimal_to_str')
    builder.call(oyqn__rdve, [val, wyqfq__maqtb._get_ptr_by_name('meminfo'),
        wyqfq__maqtb._get_ptr_by_name('length'), scale])
    wyqfq__maqtb.kind = context.get_constant(types.int32, numba.cpython.
        unicode.PY_UNICODE_1BYTE_KIND)
    wyqfq__maqtb.is_ascii = context.get_constant(types.int32, 1)
    wyqfq__maqtb.hash = context.get_constant(numba.cpython.unicode.
        _Py_hash_t, -1)
    wyqfq__maqtb.data = context.nrt.meminfo_data(builder, wyqfq__maqtb.meminfo)
    wyqfq__maqtb.parent = cgutils.get_null_value(wyqfq__maqtb.parent.type)
    return wyqfq__maqtb._getvalue()
...
def str_to_decimal_codegen(context, builder, signature, args):
    val, nfwa__fgqb, nfwa__fgqb = args
    val = bodo.libs.str_ext.gen_unicode_to_std_str(context, builder, val)
    izuw__dayh = lir.FunctionType(lir.IntType(128), [lir.IntType(8).
        as_pointer()])
    oyqn__rdve = cgutils.get_or_insert_function(builder.module, izuw__dayh,
        name='str_to_decimal')
    xesq__htz = builder.call(oyqn__rdve, [val])
    return xesq__htz
...
def decimal128type_to_int64_tuple(typingctx, val):
    ...
    def codegen(context, builder, signature, args):
        smb__xqtdp = cgutils.alloca_once(builder, lir.ArrayType(lir.IntType
            (64), 2))
        builder.store(args[0], builder.bitcast(smb__xqtdp, lir.IntType(128)
            .as_pointer()))
        return builder.load(smb__xqtdp)
...
def decimal128type_cmp(typingctx, val1, scale1, val2, scale2, func_name):
    ...
    vxdkn__etqs = get_overload_const_str(func_name)

    def codegen(context, builder, signature, args):
        val1, scale1, val2, scale2, nfwa__fgqb = args
        izuw__dayh = lir.FunctionType(lir.IntType(1), [lir.IntType(128),
            lir.IntType(64), lir.IntType(128), lir.IntType(64)])
        oyqn__rdve = cgutils.get_or_insert_function(builder.module,
            izuw__dayh, name=vxdkn__etqs)
        return builder.call(oyqn__rdve, (val1, scale1, val2, scale2))
...
def decimal_create_cmp_op_overload(op):
    ...
    def overload_cmp(lhs, rhs):
        if isinstance(lhs, Decimal128Type) and isinstance(rhs, Decimal128Type):
            vxdkn__etqs = 'decimal_cmp_' + op.__name__
            scale1 = lhs.scale
            scale2 = rhs.scale

            def impl(lhs, rhs):
                return decimal128type_cmp(lhs, scale1, rhs, scale2, vxdkn__etqs
                    )
            return impl
    return overload_cmp
...
def lower_constant_decimal(context, builder, ty, pyval):
    osf__ynjqf = numba.njit(lambda v: decimal128type_to_int64_tuple(v))(pyval)
    hoxef__jiim = [context.get_constant_generic(builder, types.int64, v) for
        v in osf__ynjqf]
    mxtbe__oae = cgutils.pack_array(builder, hoxef__jiim)
    smb__xqtdp = cgutils.alloca_once(builder, lir.IntType(128))
    builder.store(mxtbe__oae, builder.bitcast(smb__xqtdp, lir.ArrayType(lir
...
def unbox_decimal(typ, val, c):
    izuw__dayh = lir.FunctionType(lir.VoidType(), [lir.IntType(8).
        as_pointer(), lir.IntType(128).as_pointer()])
    oyqn__rdve = cgutils.get_or_insert_function(c.builder.module,
        izuw__dayh, name='unbox_decimal')
    smb__xqtdp = cgutils.alloca_once(c.builder, c.context.get_value_type(
        int128_type))
    c.builder.call(oyqn__rdve, [val, smb__xqtdp])
    mrvzs__rsli = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    ohorf__wtkz = c.builder.load(smb__xqtdp)
    return NativeValue(ohorf__wtkz, is_error=mrvzs__rsli)


@box(Decimal128Type)
def box_decimal(typ, val, c):
    hxi__cbtt = decimal_to_str_codegen(c.context, c.builder, bodo.
        string_type(typ), (val,), typ.scale)
    kuof__imjo = c.pyapi.from_native_value(bodo.string_type, hxi__cbtt, c.
        env_manager)
    ctwcr__jih = c.context.insert_const_string(c.builder.module, 'decimal')
    okst__meiu = c.pyapi.import_module_noblock(ctwcr__jih)
    smb__xqtdp = c.pyapi.call_method(okst__meiu, 'Decimal', (kuof__imjo,))
    c.pyapi.decref(kuof__imjo)
    c.pyapi.decref(okst__meiu)
    return smb__xqtdp
...
class DecimalArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        cia__idpn = [('data', data_type), ('null_bitmap', nulls_type)]
        models.StructModel.__init__(self, dmm, fe_type, cia__idpn)
...
def init_decimal_array(typingctx, data, null_bitmap, precision_tp, scale_tp
    ...
    def codegen(context, builder, signature, args):
        wixzr__weur, tplt__ftnul, nfwa__fgqb, nfwa__fgqb = args
        psb__wknwd = cgutils.create_struct_proxy(signature.return_type)(context
            , builder)
        psb__wknwd.data = wixzr__weur
        psb__wknwd.null_bitmap = tplt__ftnul
        context.nrt.incref(builder, signature.args[0], wixzr__weur)
        context.nrt.incref(builder, signature.args[1], tplt__ftnul)
        return psb__wknwd._getvalue()
    precision = get_overload_const_int(precision_tp)
    scale = get_overload_const_int(scale_tp)
    rbs__ulyd = DecimalArrayType(precision, scale)
    psx__ywnws = rbs__ulyd(data, null_bitmap, precision_tp, scale_tp)
    return psx__ywnws, codegen


@lower_constant(DecimalArrayType)
def lower_constant_decimal_arr(context, builder, typ, pyval):
    n = len(pyval)
    bal__mxf = context.get_constant(types.int64, n)
    rcv__vwsgw = bodo.utils.utils._empty_nd_impl(context, builder, types.
        Array(int128_type, 1, 'C'), [bal__mxf])
    czw__orrwy = np.empty(n + 7 >> 3, np.uint8)

    def f(arr, idx, val):
        arr[idx] = decimal128type_to_int128(val)
    for zaq__cex, zze__wbzx in enumerate(pyval):
        hqr__orh = pd.isna(zze__wbzx)
        bodo.libs.int_arr_ext.set_bit_to_arr(czw__orrwy, zaq__cex, int(not
            hqr__orh))
        if not hqr__orh:
            context.compile_internal(builder, f, types.void(types.Array(
                int128_type, 1, 'C'), types.int64, Decimal128Type(typ.
                precision, typ.scale)), [rcv__vwsgw._getvalue(), context.
                get_constant(types.int64, zaq__cex), context.
                get_constant_generic(builder, Decimal128Type(typ.precision,
                typ.scale), zze__wbzx)])
    wmp__cwaw = context.get_constant_generic(builder, nulls_type, czw__orrwy)
    psb__wknwd = context.make_helper(builder, typ)
    psb__wknwd.data = rcv__vwsgw._getvalue()
    psb__wknwd.null_bitmap = wmp__cwaw
    return psb__wknwd._getvalue()


@numba.njit(no_cpython_wrapper=True)
def alloc_decimal_array(n, precision, scale):
    jieeu__lpr = np.empty(n, dtype=int128_type)
    ujp__dezmj = np.empty(n + 7 >> 3, dtype=np.uint8)
    return init_decimal_array(jieeu__lpr, ujp__dezmj, precision, scale)
...
def box_decimal_arr(typ, val, c):
    ossyu__mbul = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    jieeu__lpr = c.context.make_array(types.Array(int128_type, 1, 'C'))(c.
        context, c.builder, ossyu__mbul.data)
    fwgd__gch = c.context.make_array(types.Array(types.uint8, 1, 'C'))(c.
        context, c.builder, ossyu__mbul.null_bitmap).data
    n = c.builder.extract_value(jieeu__lpr.shape, 0)
    scale = c.context.get_constant(types.int32, typ.scale)
    izuw__dayh = lir.FunctionType(c.pyapi.pyobj, [lir.IntType(64), lir.
        IntType(128).as_pointer(), lir.IntType(8).as_pointer(), lir.IntType
        (32)])
    llo__xoqjx = cgutils.get_or_insert_function(c.builder.module,
        izuw__dayh, name='box_decimal_array')
    eiz__egeo = c.builder.call(llo__xoqjx, [n, jieeu__lpr.data, fwgd__gch,
        scale])
...
def unbox_decimal_arr(typ, val, c):
    psb__wknwd = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    otkkt__nlcnt = c.pyapi.call_method(val, '__len__', ())
    n = c.pyapi.long_as_longlong(otkkt__nlcnt)
    c.pyapi.decref(otkkt__nlcnt)
    ovbin__tpgk = c.builder.udiv(c.builder.add(n, lir.Constant(lir.IntType(
        64), 7)), lir.Constant(lir.IntType(64), 8))
    rcv__vwsgw = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        types.Array(int128_type, 1, 'C'), [n])
    wgs__xfg = bodo.utils.utils._empty_nd_impl(c.context, c.builder, types.
        Array(types.uint8, 1, 'C'), [ovbin__tpgk])
    izuw__dayh = lir.FunctionType(lir.VoidType(), [lir.IntType(8).
        as_pointer(), lir.IntType(64), lir.IntType(128).as_pointer(), lir.
        IntType(8).as_pointer()])
    oyqn__rdve = cgutils.get_or_insert_function(c.builder.module,
        izuw__dayh, name='unbox_decimal_array')
    c.builder.call(oyqn__rdve, [val, n, rcv__vwsgw.data, wgs__xfg.data])
    psb__wknwd.null_bitmap = wgs__xfg._getvalue()
    psb__wknwd.data = rcv__vwsgw._getvalue()
    mrvzs__rsli = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(psb__wknwd._getvalue(), is_error=mrvzs__rsli)
...
def decimal_arr_getitem(A, ind):
    ...
    if is_list_like_index_type(ind) and ind.dtype == types.bool_:
        ...
        def impl(A, ind):
            rjrg__due, ucfj__jdlny = array_getitem_bool_index(A, ind)
            return init_decimal_array(rjrg__due, ucfj__jdlny, precision, scale)
        return impl
    if is_list_like_index_type(ind) and isinstance(ind.dtype, types.Integer):
        ...
        def impl(A, ind):
            rjrg__due, ucfj__jdlny = array_getitem_int_index(A, ind)
            return init_decimal_array(rjrg__due, ucfj__jdlny, precision, scale)
        return impl
    if isinstance(ind, types.SliceType):
        ...
        def impl_slice(A, ind):
            rjrg__due, ucfj__jdlny = array_getitem_slice_index(A, ind)
            return init_decimal_array(rjrg__due, ucfj__jdlny, precision, scale)
        return impl_slice
    raise BodoError(
