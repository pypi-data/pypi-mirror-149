...
def get_new_null_mask_bool_index(old_mask, ind, n):
    fmhae__wdmc = n + 7 >> 3
    imfib__rts = np.empty(fmhae__wdmc, np.uint8)
    kaf__prqzt = 0
    for mra__zntb in range(len(ind)):
        if ind[mra__zntb]:
            hjfw__peq = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
                mra__zntb)
            bodo.libs.int_arr_ext.set_bit_to_arr(imfib__rts, kaf__prqzt,
                hjfw__peq)
            kaf__prqzt += 1
    return imfib__rts
...
def array_getitem_bool_index(A, ind):
    ...
    yfvik__kipph = A._data[ind]
    n = len(yfvik__kipph)
    imfib__rts = get_new_null_mask_bool_index(old_mask, ind, n)
    return yfvik__kipph, imfib__rts
...
def get_new_null_mask_int_index(old_mask, ind, n):
    ...
    kaf__prqzt = 0
    for mra__zntb in range(len(ind)):
        hjfw__peq = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask, ind[
            mra__zntb])
        bodo.libs.int_arr_ext.set_bit_to_arr(imfib__rts, kaf__prqzt, hjfw__peq)
        kaf__prqzt += 1
    return imfib__rts
...
def array_getitem_int_index(A, ind):
    ddtiw__qvzk = bodo.utils.conversion.coerce_to_ndarray(ind)
    old_mask = A._null_bitmap
    yfvik__kipph = A._data[ddtiw__qvzk]
    n = len(yfvik__kipph)
    imfib__rts = get_new_null_mask_int_index(old_mask, ddtiw__qvzk, n)
    return yfvik__kipph, imfib__rts


@register_jitable
def get_new_null_mask_slice_index(old_mask, ind, n):
    onbf__mom = numba.cpython.unicode._normalize_slice(ind, n)
    dlrf__xswt = numba.cpython.unicode._slice_span(onbf__mom)
    fmhae__wdmc = dlrf__xswt + 7 >> 3
    imfib__rts = np.empty(fmhae__wdmc, np.uint8)
    kaf__prqzt = 0
    for mra__zntb in range(onbf__mom.start, onbf__mom.stop, onbf__mom.step):
        hjfw__peq = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
            mra__zntb)
        bodo.libs.int_arr_ext.set_bit_to_arr(imfib__rts, kaf__prqzt, hjfw__peq)
        kaf__prqzt += 1
...
def array_getitem_slice_index(A, ind):
    n = len(A._data)
    old_mask = A._null_bitmap
    yfvik__kipph = np.ascontiguousarray(A._data[ind])
    imfib__rts = get_new_null_mask_slice_index(old_mask, ind, n)
    return yfvik__kipph, imfib__rts
...
def array_setitem_int_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            n = len(val._data)
            for mra__zntb in range(n):
                A._data[idx[mra__zntb]] = val._data[mra__zntb]
                hjfw__peq = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val.
                    _null_bitmap, mra__zntb)
    ...
    if bodo.utils.typing.is_scalar_type(val):
        ...
        def impl_scalar(A, idx, val):
            for mra__zntb in idx:
                A._data[mra__zntb] = val
...
def array_setitem_bool_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            rmdl__mtt = 0
            for mra__zntb in range(n):
                if not bodo.libs.array_kernels.isna(idx, mra__zntb) and idx[
                    mra__zntb]:
                    A._data[mra__zntb] = val._data[rmdl__mtt]
                    hjfw__peq = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val
                        ._null_bitmap, rmdl__mtt)
                    bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                        mra__zntb, hjfw__peq)
                    rmdl__mtt += 1
        return impl_arr
...
def setitem_slice_index_null_bits(dst_bitmap, src_bitmap, idx, n):
    onbf__mom = numba.cpython.unicode._normalize_slice(idx, n)
    rmdl__mtt = 0
    for mra__zntb in range(onbf__mom.start, onbf__mom.stop, onbf__mom.step):
        hjfw__peq = bodo.libs.int_arr_ext.get_bit_bitmap_arr(src_bitmap,
            rmdl__mtt)
        bodo.libs.int_arr_ext.set_bit_to_arr(dst_bitmap, mra__zntb, hjfw__peq)
        rmdl__mtt += 1
...
def array_setitem_slice_index_overload(A, idx, val):
    ...
    if bodo.utils.typing.is_scalar_type(val):
        ...
        def impl_scalar(A, idx, val):
            onbf__mom = numba.cpython.unicode._normalize_slice(idx, len(A))
            for mra__zntb in range(onbf__mom.start, onbf__mom.stop,
                onbf__mom.step):
                A._data[mra__zntb] = val
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                    mra__zntb, 1)
    ...
    if not bodo.utils.utils.is_array_typ(A, False):
        ...
        if isinstance(idx, types.Integer):
            ...
            def setitem_none_int_arr(A, idx, val):
                ...
                for mra__zntb in idx:
                    bodo.libs.array_kernels.setna(A, mra__zntb)
            ...
            if A == bodo.string_array_type:
                ...
                def string_arr_impl(A, idx, val):
                    n = len(A)
                    idx = bodo.utils.conversion.coerce_to_ndarray(idx)
                    lzbro__iep = bodo.libs.str_arr_ext.pre_alloc_string_array(n
                        , -1)
                    for mra__zntb in numba.parfors.parfor.internal_prange(n):
                        if idx[mra__zntb] or bodo.libs.array_kernels.isna(A,
                            mra__zntb):
                            lzbro__iep[mra__zntb] = ''
                            bodo.libs.str_arr_ext.str_arr_set_na(lzbro__iep,
                                mra__zntb)
                        else:
                            lzbro__iep[mra__zntb] = A[mra__zntb]
            ...
            def setitem_none_bool_arr(A, idx, val):
                ...
                n = len(idx)
                for mra__zntb in range(n):
                    if not bodo.libs.array_kernels.isna(idx, mra__zntb
                        ) and idx[mra__zntb]:
                        bodo.libs.array_kernels.setna(A, mra__zntb)
            ...
            def setitem_optional_int_arr(A, idx, val):
                ...
                for mra__zntb in idx:
                    if val is None:
                        bodo.libs.array_kernels.setna(A, mra__zntb)
                        continue
                    A[mra__zntb] = bodo.utils.indexing.unoptional(val)
...
def unoptional(typingctx, val_t=None):
    ...
    def codegen(context, builder, signature, args):
        llewh__scb = context.make_helper(builder, val_t, args[0])
        onr__awk = llewh__scb.data
        context.nrt.incref(builder, val_t.type, onr__awk)
        return onr__awk
