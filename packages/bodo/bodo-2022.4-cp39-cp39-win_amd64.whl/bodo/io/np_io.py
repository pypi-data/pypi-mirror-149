...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        pqxxh__lpb = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, pqxxh__lpb)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            qauns__fzo = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(qauns__fzo.dtype)
            file_write(unicode_to_utf8(fname), qauns__fzo.ctypes, 
                dtype_size * qauns__fzo.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            qauns__fzo = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(qauns__fzo.dtype)
            zvlg__hcxyj = (dtype_size * bodo.libs.distributed_api.
                get_tuple_prod(qauns__fzo.shape[1:]))
            _file_write_parallel(unicode_to_utf8(fname), qauns__fzo.ctypes,
                start, count, zvlg__hcxyj)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            isuh__xero = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                isuh__xero = min(isuh__xero, count * dtype_size)
            if isuh__xero < 0:
                return -1
            return isuh__xero
        return impl
