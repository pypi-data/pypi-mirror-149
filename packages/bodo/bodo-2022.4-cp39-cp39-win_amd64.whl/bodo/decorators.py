...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        rioc__txn = self.overloads[sig]
        yhz__igiq = rioc__txn.metadata.get('distributed_diagnostics', None)
        if yhz__igiq is None:
            uxdkp__ewl = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(uxdkp__ewl)
        yhz__igiq.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        gewc__wdam = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(gewc__wdam)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        yorl__oziw = options.pop('distributed')
        pipeline_class = (pipeline_class if yorl__oziw else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        ctdt__nqe = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if ctdt__nqe else
            pipeline_class)
    rsyg__xnjb = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(rsyg__xnjb, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(rsyg__xnjb)
        else:
            return master_mode_wrapper(rsyg__xnjb)
    else:
        return rsyg__xnjb
