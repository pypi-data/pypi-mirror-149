...
def get_pytz_type_info(pytz_type):
    if isinstance(pytz_type, pytz._FixedOffset):
        yiw__bcrw = pd.Timedelta(pytz_type._offset).value
    else:
        yiw__bcrw = pytz_type.zone
        if yiw__bcrw not in pytz.all_timezones_set:
            raise BodoError(
...
def nanoseconds_to_offset(nanoseconds):
    hyz__ajoj = nanoseconds // (60 * 1000 * 1000 * 1000)
    return pytz.FixedOffset(hyz__ajoj)
...
class PandasDatetimeArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        wbssh__mrh = [('data', fe_type.data_array_type)]
        models.StructModel.__init__(self, dmm, fe_type, wbssh__mrh)
...
def unbox_pd_datetime_array(typ, val, c):
    tae__sirk = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    xhuz__lhch = c.pyapi.string_from_constant_string('datetime64[ns]')
    eyr__lsse = c.pyapi.call_method(val, 'to_numpy', (xhuz__lhch,))
    tae__sirk.data = c.unbox(typ.data_array_type, eyr__lsse).value
    c.pyapi.decref(eyr__lsse)
    efgjt__mwf = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(tae__sirk._getvalue(), is_error=efgjt__mwf)
...
def box_pd_datetime_array(typ, val, c):
    tae__sirk = cgutils.create_struct_proxy(typ)(c.context, c.builder,
        value=val)
    c.context.nrt.incref(c.builder, typ.data_array_type, tae__sirk.data)
    hgr__bqsji = c.pyapi.from_native_value(typ.data_array_type, tae__sirk.
        data, c.env_manager)
    afr__oofup = c.context.get_constant_generic(c.builder, types.
        unicode_type, 'ns')
    hhou__ffkq = c.pyapi.from_native_value(types.unicode_type, afr__oofup,
        c.env_manager)
    if isinstance(typ.tz, str):
        jkcv__lka = c.context.get_constant_generic(c.builder, types.
            unicode_type, typ.tz)
        pwzm__cclbc = c.pyapi.from_native_value(types.unicode_type,
            jkcv__lka, c.env_manager)
    else:
        lag__bhw = nanoseconds_to_offset(typ.tz)
        pwzm__cclbc = c.pyapi.unserialize(c.pyapi.serialize_object(lag__bhw))
    dokl__kmu = c.context.insert_const_string(c.builder.module, 'pandas')
    rxq__aseh = c.pyapi.import_module_noblock(dokl__kmu)
    ubroq__kya = c.pyapi.call_method(rxq__aseh, 'DatetimeTZDtype', (
        hhou__ffkq, pwzm__cclbc))
    loyv__pqwe = c.pyapi.object_getattr_string(rxq__aseh, 'arrays')
    yiv__xxr = c.pyapi.call_method(loyv__pqwe, 'DatetimeArray', (hgr__bqsji,
        ubroq__kya))
    c.pyapi.decref(hgr__bqsji)
    c.pyapi.decref(hhou__ffkq)
    c.pyapi.decref(pwzm__cclbc)
    c.pyapi.decref(rxq__aseh)
    c.pyapi.decref(ubroq__kya)
    c.pyapi.decref(loyv__pqwe)
...
def init_pandas_datetime_array(typingctx, data, tz):
    ...
    def codegen(context, builder, sig, args):
        data, tz = args
        mmu__zdf = cgutils.create_struct_proxy(sig.return_type)(context,
            builder)
        mmu__zdf.data = data
        context.nrt.incref(builder, sig.args[0], data)
        return mmu__zdf._getvalue()
    if is_overload_constant_str(tz) or is_overload_constant_int(tz):
        jkcv__lka = get_literal_value(tz)
    else:
        raise BodoError('tz must be a constant string or Fixed Offset')
    wvj__ffc = DatetimeArrayType(jkcv__lka)
    sig = wvj__ffc(wvj__ffc.data_array_type, tz)
    return sig, codegen
...
def lower_constant_pd_datetime_arr(context, builder, typ, pyval):
    njzpm__qqufp = context.get_constant_generic(builder, typ.
        data_array_type, pyval.to_numpy('datetime64[ns]'))
    zyjw__mbwq = lir.Constant.literal_struct([njzpm__qqufp])
    return zyjw__mbwq
...
def overload_getitem(A, ind):
    ...
    if is_list_like_index_type(ind) and ind.dtype == types.bool_:
        ...
        def impl_bool(A, ind):
            ...
            tyard__wksxv = ensure_contig_if_np(A._data[ind])
            return init_pandas_datetime_array(tyard__wksxv, tz)
        return impl_bool
...
def unwrap_tz_array_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    tybpx__xcy = args[0]
    if equiv_set.has_shape(tybpx__xcy):
        return ArrayAnalysis.AnalyzeResult(shape=tybpx__xcy, pre=[])
    return None
