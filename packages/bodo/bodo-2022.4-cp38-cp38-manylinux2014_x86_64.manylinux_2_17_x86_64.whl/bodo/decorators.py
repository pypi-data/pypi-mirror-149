...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        wonw__tfdk = self.overloads[sig]
        kyhf__xhdtk = wonw__tfdk.metadata.get('distributed_diagnostics', None)
        if kyhf__xhdtk is None:
            srqt__bfma = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(srqt__bfma)
        kyhf__xhdtk.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        anuel__hvua = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(anuel__hvua)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        hzjq__mpg = options.pop('distributed')
        pipeline_class = (pipeline_class if hzjq__mpg else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        hmlwe__hrjjk = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if hmlwe__hrjjk else
            pipeline_class)
    jrhnp__hmpuv = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(jrhnp__hmpuv, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(jrhnp__hmpuv)
        else:
            return master_mode_wrapper(jrhnp__hmpuv)
    else:
        return jrhnp__hmpuv
