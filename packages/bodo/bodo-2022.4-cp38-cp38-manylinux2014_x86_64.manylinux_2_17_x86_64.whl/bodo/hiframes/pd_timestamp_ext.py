...
class PandasTimestampType(types.Type):
    ...
    def __init__(self, tz_val=None):
        self.tz = tz_val
        if tz_val is None:
            fmyax__fmsrc = 'PandasTimestampType()'
        else:
            fmyax__fmsrc = f'PandasTimestampType({tz_val})'
        super(PandasTimestampType, self).__init__(name=fmyax__fmsrc)
...
def check_tz_aware_unsupported(val, func_name):
    ...
    if isinstance(val, PandasTimestampType) and val.tz is not None:
        ...
        for poh__kqh in val.data:
            if isinstance(poh__kqh, bodo.DatetimeArrayType):
                raise BodoError(
...
class PandasTimestampModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ptrv__lkg = [('year', ts_field_typ), ('month', ts_field_typ), (
            ...
            ts_field_typ)]
        models.StructModel.__init__(self, dmm, fe_type, ptrv__lkg)
...
def unbox_pandas_timestamp(typ, val, c):
    priw__ifi = c.pyapi.object_getattr_string(val, 'year')
    jkgew__ieq = c.pyapi.object_getattr_string(val, 'month')
    mjn__ynyqo = c.pyapi.object_getattr_string(val, 'day')
    kzhuh__tsro = c.pyapi.object_getattr_string(val, 'hour')
    kmqyp__coe = c.pyapi.object_getattr_string(val, 'minute')
    oavf__kgrn = c.pyapi.object_getattr_string(val, 'second')
    tuuf__dpk = c.pyapi.object_getattr_string(val, 'microsecond')
    pjru__aho = c.pyapi.object_getattr_string(val, 'nanosecond')
    cdnr__eaubp = c.pyapi.object_getattr_string(val, 'value')
    ofj__fkz = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    ofj__fkz.year = c.pyapi.long_as_longlong(priw__ifi)
    ofj__fkz.month = c.pyapi.long_as_longlong(jkgew__ieq)
    ofj__fkz.day = c.pyapi.long_as_longlong(mjn__ynyqo)
    ofj__fkz.hour = c.pyapi.long_as_longlong(kzhuh__tsro)
    ofj__fkz.minute = c.pyapi.long_as_longlong(kmqyp__coe)
    ofj__fkz.second = c.pyapi.long_as_longlong(oavf__kgrn)
    ofj__fkz.microsecond = c.pyapi.long_as_longlong(tuuf__dpk)
    ofj__fkz.nanosecond = c.pyapi.long_as_longlong(pjru__aho)
    ofj__fkz.value = c.pyapi.long_as_longlong(cdnr__eaubp)
    c.pyapi.decref(priw__ifi)
    c.pyapi.decref(jkgew__ieq)
    c.pyapi.decref(mjn__ynyqo)
    c.pyapi.decref(kzhuh__tsro)
    c.pyapi.decref(kmqyp__coe)
    c.pyapi.decref(oavf__kgrn)
    c.pyapi.decref(tuuf__dpk)
    c.pyapi.decref(pjru__aho)
    c.pyapi.decref(cdnr__eaubp)
    powi__qho = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(ofj__fkz._getvalue(), is_error=powi__qho)


@box(PandasTimestampType)
def box_pandas_timestamp(typ, val, c):
    xok__xhjya = cgutils.create_struct_proxy(typ)(c.context, c.builder,
        value=val)
    priw__ifi = c.pyapi.long_from_longlong(xok__xhjya.year)
    jkgew__ieq = c.pyapi.long_from_longlong(xok__xhjya.month)
    mjn__ynyqo = c.pyapi.long_from_longlong(xok__xhjya.day)
    kzhuh__tsro = c.pyapi.long_from_longlong(xok__xhjya.hour)
    kmqyp__coe = c.pyapi.long_from_longlong(xok__xhjya.minute)
    oavf__kgrn = c.pyapi.long_from_longlong(xok__xhjya.second)
    kqd__xvops = c.pyapi.long_from_longlong(xok__xhjya.microsecond)
    orkhi__dxjbx = c.pyapi.long_from_longlong(xok__xhjya.nanosecond)
    hgw__juxz = c.pyapi.unserialize(c.pyapi.serialize_object(pd.Timestamp))
    if typ.tz is None:
        res = c.pyapi.call_function_objargs(hgw__juxz, (priw__ifi,
            jkgew__ieq, mjn__ynyqo, kzhuh__tsro, kmqyp__coe, oavf__kgrn,
            kqd__xvops, orkhi__dxjbx))
    else:
        if isinstance(typ.tz, int):
            rxgq__hpc = c.pyapi.long_from_longlong(lir.Constant(lir.IntType
                (64), typ.tz))
        else:
            ogfo__xjbkw = c.context.insert_const_string(c.builder.module,
                str(typ.tz))
            rxgq__hpc = c.pyapi.string_from_string(ogfo__xjbkw)
        args = c.pyapi.tuple_pack(())
        kwargs = c.pyapi.dict_pack([('year', priw__ifi), ('month',
            jkgew__ieq), ('day', mjn__ynyqo), ('hour', kzhuh__tsro), (
            'minute', kmqyp__coe), ('second', oavf__kgrn), ('microsecond',
            kqd__xvops), ('nanosecond', orkhi__dxjbx), ('tz', rxgq__hpc)])
        res = c.pyapi.call(hgw__juxz, args, kwargs)
        c.pyapi.decref(args)
        c.pyapi.decref(kwargs)
        c.pyapi.decref(rxgq__hpc)
    c.pyapi.decref(priw__ifi)
    c.pyapi.decref(jkgew__ieq)
    c.pyapi.decref(mjn__ynyqo)
    c.pyapi.decref(kzhuh__tsro)
    c.pyapi.decref(kmqyp__coe)
    c.pyapi.decref(oavf__kgrn)
    c.pyapi.decref(kqd__xvops)
    c.pyapi.decref(orkhi__dxjbx)
    return res
...
def init_timestamp(typingctx, year, month, day, hour, minute, second,
    ...
    def codegen(context, builder, sig, args):
        (year, month, day, hour, minute, second, xwxnl__ciorn, elm__nzpj,
            value, kttg__zgb) = args
        ...
        ts.minute = minute
        ts.second = second
        ts.microsecond = xwxnl__ciorn
        ts.nanosecond = elm__nzpj
        ts.value = value
        return ts._getvalue()
...
def overload_pd_timestamp(ts_input=_no_input, freq=None, tz=None, unit=None,
    ...
    if isinstance(ts_input, types.Number):
        ...
        xqpnr__ksat, precision = (pd._libs.tslibs.conversion.
        ...
        def impl_float(ts_input=_no_input, freq=None, tz=None, unit=None,
            year=None, month=None, day=None, hour=None, minute=None, second
            =None, microsecond=None, nanosecond=None, tzinfo=None):
            dfbc__vwiq = np.int64(ts_input)
            opr__ogb = ts_input - dfbc__vwiq
            if precision:
                opr__ogb = np.round(opr__ogb, precision)
            value = dfbc__vwiq * xqpnr__ksat + np.int64(opr__ogb * xqpnr__ksat)
            return convert_val_to_timestamp(value, tz)
        return impl_float
    ...
    if isinstance(ts_input, numba.core.types.scalars.NPDatetime):
        ...
        def impl_date(ts_input=_no_input, freq=None, tz=None, unit=None,
            year=None, month=None, day=None, hour=None, minute=None, second
            =None, microsecond=None, nanosecond=None, tzinfo=None):
            value = np.int64(ts_input) * xqpnr__ksat
...
def overload_week_number(ptt):
    ...
    def pd_week_number(ptt):
        kttg__zgb, wirh__lex, kttg__zgb = get_isocalendar(ptt.year, ptt.
            month, ptt.day)
        return wirh__lex
...
def overload_pd_timestamp_isocalendar(ptt):
    ...
    def impl(ptt):
        year, wirh__lex, wcd__hsft = get_isocalendar(ptt.year, ptt.month,
            ptt.day)
        return year, wirh__lex, wcd__hsft
    return impl
...
def overload_pd_timestamp_isoformat(ts, sep=None):
    if is_overload_none(sep):
        ...
        def timestamp_isoformat_impl(ts, sep=None):
            ...
            lvocn__lpds = str_2d(ts.hour) + ':' + str_2d(ts.minute
                ) + ':' + str_2d(ts.second)
...
def overload_pd_timestamp_day_name(ptt, locale=None):
    hep__kaij = dict(locale=locale)
    vieu__xde = dict(locale=None)
    check_unsupported_args('Timestamp.day_name', hep__kaij, vieu__xde,
        package_name='pandas', module_name='Timestamp')

    def impl(ptt, locale=None):
        haurm__xjbmk = ('Monday', 'Tuesday', 'Wednesday', 'Thursday',
            'Friday', 'Saturday', 'Sunday')
        kttg__zgb, kttg__zgb, lzp__utfoe = ptt.isocalendar()
        return haurm__xjbmk[lzp__utfoe - 1]
    return impl
...
def overload_pd_timestamp_month_name(ptt, locale=None):
    hep__kaij = dict(locale=locale)
    vieu__xde = dict(locale=None)
    check_unsupported_args('Timestamp.month_name', hep__kaij, vieu__xde,
        package_name='pandas', module_name='Timestamp')

    def impl(ptt, locale=None):
        auo__pdxg = ('January', 'February', 'March', 'April', 'May', 'June',
            'July', 'August', 'September', 'October', 'November', 'December')
        return auo__pdxg[ptt.month - 1]
    return impl
...
def overload_pd_timestamp_tz_localize(ptt, tz, ambiguous='raise',
    ...
    hep__kaij = dict(ambiguous=ambiguous, nonexistent=nonexistent)
    pucd__grue = dict(ambiguous='raise', nonexistent='raise')
    check_unsupported_args('Timestamp.tz_localize', hep__kaij, pucd__grue,
        package_name='pandas', module_name='Timestamp')
    if is_overload_none(tz):
...
def extract_year_days(typingctx, dt64_t=None):
    ...
    def codegen(context, builder, sig, args):
        lnce__fgnfr = cgutils.alloca_once(builder, lir.IntType(64))
        builder.store(args[0], lnce__fgnfr)
        year = cgutils.alloca_once(builder, lir.IntType(64))
        fou__fixi = cgutils.alloca_once(builder, lir.IntType(64))
        wihp__arg = lir.FunctionType(lir.VoidType(), [lir.IntType(64).
            as_pointer(), lir.IntType(64).as_pointer(), lir.IntType(64).
            as_pointer()])
        vuqv__smh = cgutils.get_or_insert_function(builder.module,
            wihp__arg, name='extract_year_days')
        builder.call(vuqv__smh, [lnce__fgnfr, year, fou__fixi])
        return cgutils.pack_array(builder, [builder.load(lnce__fgnfr),
            builder.load(year), builder.load(fou__fixi)])
...
def get_month_day(typingctx, year_t, days_t=None):
    ...
    def codegen(context, builder, sig, args):
        ...
        wihp__arg = lir.FunctionType(lir.VoidType(), [lir.IntType(64), lir.
            IntType(64), lir.IntType(64).as_pointer(), lir.IntType(64).
            as_pointer()])
        vuqv__smh = cgutils.get_or_insert_function(builder.module,
            wihp__arg, name='get_month_day')
        builder.call(vuqv__smh, [args[0], args[1], month, day])
...
def get_day_of_year(year, month, day):
    liua__may = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 365,
        0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335, 366]
    znru__bcz = is_leap_year(year)
    hfymo__dtr = liua__may[znru__bcz * 13 + month - 1]
    hpw__owu = hfymo__dtr + day
    return hpw__owu


@register_jitable
def get_day_of_week(y, m, d):
    gji__mqsyo = [0, 3, 2, 5, 0, 3, 5, 1, 4, 6, 2, 4]
    y -= m < 3
    day = (y + y // 4 - y // 100 + y // 400 + gji__mqsyo[m - 1] + d) % 7
    return (day + 6) % 7
...
def get_days_in_month(year, month):
    ...
    mdtge__gkk = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31, 31, 29, 
        31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
    return mdtge__gkk[12 * is_leap_year + month - 1]
...
def convert_val_to_timestamp(ts_input, tz=None, is_convert=True):
    rani__jpp = iziu__nscug = np.array([])
    cbjs__nsvv = '0'
    if is_overload_constant_str(tz):
        ogfo__xjbkw = get_overload_const_str(tz)
        rxgq__hpc = pytz.timezone(ogfo__xjbkw)
        if isinstance(rxgq__hpc, pytz.tzinfo.DstTzInfo):
            rani__jpp = np.array(rxgq__hpc._utc_transition_times, dtype=
                'M8[ns]').view('i8')
            iziu__nscug = np.array(rxgq__hpc._transition_info)[:, 0]
            iziu__nscug = (pd.Series(iziu__nscug).dt.total_seconds() * 
                1000000000).astype(np.int64).values
            cbjs__nsvv = (
                "deltas[np.searchsorted(trans, ts_input, side='right') - 1]")
        else:
            iziu__nscug = np.int64(rxgq__hpc._utcoffset.total_seconds() * 
                1000000000)
            cbjs__nsvv = 'deltas'
    elif is_overload_constant_int(tz):
        gyis__tlgmt = get_overload_const_int(tz)
        cbjs__nsvv = str(gyis__tlgmt)
    ...
    if is_convert:
        sgv__otcq = 'tz_ts_input'
        bot__rczoz = 'ts_input'
    else:
        sgv__otcq = 'ts_input'
        bot__rczoz = 'tz_ts_input'
    jyb__ioz = 'def impl(ts_input, tz=None, is_convert=True):\n'
    jyb__ioz += f'  tz_ts_input = ts_input + {cbjs__nsvv}\n'
    jyb__ioz += (
        f'  dt, year, days = extract_year_days(integer_to_dt64({sgv__otcq}))\n'
        )
    jyb__ioz += '  month, day = get_month_day(year, days)\n'
    jyb__ioz += '  return init_timestamp(\n'
    jyb__ioz += '    year=year,\n'
    jyb__ioz += '    month=month,\n'
    jyb__ioz += '    day=day,\n'
    jyb__ioz += '    hour=dt // (60 * 60 * 1_000_000_000),\n'
    jyb__ioz += '    minute=(dt // (60 * 1_000_000_000)) % 60,\n'
    jyb__ioz += '    second=(dt // 1_000_000_000) % 60,\n'
    jyb__ioz += '    microsecond=(dt // 1000) % 1_000_000,\n'
    jyb__ioz += '    nanosecond=dt % 1000,\n'
    jyb__ioz += f'    value={bot__rczoz},\n'
    jyb__ioz += '    tz=tz,\n'
    jyb__ioz += '  )\n'
    druv__skd = {}
    exec(jyb__ioz, {'np': np, 'pd': pd, 'trans': rani__jpp, 'deltas':
        iziu__nscug, 'integer_to_dt64': integer_to_dt64,
        'extract_year_days': extract_year_days, 'get_month_day':
        get_month_day, 'init_timestamp': init_timestamp, 'zero_if_none':
        zero_if_none}, druv__skd)
    impl = druv__skd['impl']
    return impl
...
def convert_datetime64_to_timestamp(dt64):
    lnce__fgnfr, year, fou__fixi = extract_year_days(dt64)
    month, day = get_month_day(year, fou__fixi)
    return init_timestamp(year=year, month=month, day=day, hour=lnce__fgnfr //
        (60 * 60 * 1000000000), minute=lnce__fgnfr // (60 * 1000000000) % 
        60, second=lnce__fgnfr // 1000000000 % 60, microsecond=lnce__fgnfr //
        1000 % 1000000, nanosecond=lnce__fgnfr % 1000, value=dt64, tz=None)


@numba.njit(no_cpython_wrapper=True)
def convert_numpy_timedelta64_to_datetime_timedelta(dt64):
    heo__vmmk = (bodo.hiframes.datetime_timedelta_ext.
        cast_numpy_timedelta_to_int(dt64))
    jhe__tgx = heo__vmmk // (86400 * 1000000000)
    yrs__iwuw = heo__vmmk - jhe__tgx * 86400 * 1000000000
    qyyi__ija = yrs__iwuw // 1000000000
    fuayh__nkycu = yrs__iwuw - qyyi__ija * 1000000000
    tjwlr__uhu = fuayh__nkycu // 1000
    return datetime.timedelta(jhe__tgx, qyyi__ija, tjwlr__uhu)
...
def pandas_dict_string_array_to_datetime(arr, errors, dayfirst, yearfirst,
    ...
    bruy__hvva = len(arr)
    lyqtw__dhir = np.empty(bruy__hvva, 'datetime64[ns]')
    oxn__njcb = arr._indices
    hjz__niuv = pandas_string_array_to_datetime(arr._data, errors, dayfirst,
        yearfirst, utc, format, exact, unit, infer_datetime_format, origin,
        cache).values
    for jwsi__fphq in range(bruy__hvva):
        if bodo.libs.array_kernels.isna(oxn__njcb, jwsi__fphq):
            bodo.libs.array_kernels.setna(lyqtw__dhir, jwsi__fphq)
            continue
        lyqtw__dhir[jwsi__fphq] = hjz__niuv[oxn__njcb[jwsi__fphq]]
    return lyqtw__dhir
...
def overload_to_datetime(arg_a, errors='raise', dayfirst=False, yearfirst=
    ...
    if isinstance(arg_a, bodo.hiframes.pd_series_ext.SeriesType):
        ...
        def impl_series(arg_a, errors='raise', dayfirst=False, yearfirst=
            ...
            ius__pdoc = bodo.hiframes.pd_series_ext.get_series_index(arg_a)
            fmyax__fmsrc = bodo.hiframes.pd_series_ext.get_series_name(arg_a)
            qrwx__ovswi = bodo.utils.conversion.coerce_to_ndarray(pd.
            ...
            return bodo.hiframes.pd_series_ext.init_series(qrwx__ovswi,
                ius__pdoc, fmyax__fmsrc)
        return impl_series
    if arg_a == bodo.hiframes.datetime_date_ext.datetime_date_array_type:
        lib__ezygs = np.dtype('datetime64[ns]')
        iNaT = pd._libs.tslibs.iNaT

        def impl_date_arr(arg_a, errors='raise', dayfirst=False, yearfirst=
            False, utc=None, format=None, exact=True, unit=None,
            infer_datetime_format=False, origin='unix', cache=True):
            bruy__hvva = len(arg_a)
            lyqtw__dhir = np.empty(bruy__hvva, lib__ezygs)
            for jwsi__fphq in numba.parfors.parfor.internal_prange(bruy__hvva):
                val = iNaT
                if not bodo.libs.array_kernels.isna(arg_a, jwsi__fphq):
                    data = arg_a[jwsi__fphq]
                    val = (bodo.hiframes.pd_timestamp_ext.
                        npy_datetimestruct_to_datetime(data.year, data.
                        month, data.day, 0, 0, 0, 0))
                lyqtw__dhir[jwsi__fphq
                    ] = bodo.hiframes.pd_timestamp_ext.integer_to_dt64(val)
            return bodo.hiframes.pd_index_ext.init_datetime_index(lyqtw__dhir,
                None)
    ...
    if isinstance(arg_a, CategoricalArrayType
        ...
        def impl_cat_arr(arg_a, errors='raise', dayfirst=False, yearfirst=
            ...
            bruy__hvva = len(arg_a)
            lyqtw__dhir = np.empty(bruy__hvva, lib__ezygs)
            ivq__rgohc = (bodo.hiframes.pd_categorical_ext.
                get_categorical_arr_codes(arg_a))
            hjz__niuv = pandas_string_array_to_datetime(arg_a.dtype.
                categories.values, errors, dayfirst, yearfirst, utc, format,
                exact, unit, infer_datetime_format, origin, cache).values
            for jwsi__fphq in numba.parfors.parfor.internal_prange(bruy__hvva):
                c = ivq__rgohc[jwsi__fphq]
                if c == -1:
                    bodo.libs.array_kernels.setna(lyqtw__dhir, jwsi__fphq)
                    continue
                lyqtw__dhir[jwsi__fphq] = hjz__niuv[c]
            return bodo.hiframes.pd_index_ext.init_datetime_index(lyqtw__dhir,
                None)
        return impl_cat_arr
    if arg_a == bodo.dict_str_arr_type:
        ...
        def impl_dict_str_arr(arg_a, errors='raise', dayfirst=False,
            yearfirst=False, utc=None, format=None, exact=True, unit=None,
            infer_datetime_format=False, origin='unix', cache=True):
            lyqtw__dhir = pandas_dict_string_array_to_datetime(arg_a,
...
def overload_to_timedelta(arg_a, unit='ns', errors='raise'):
    ...
    if isinstance(arg_a, types.Float):
        m, rodt__gqeh = pd._libs.tslibs.conversion.precision_from_unit(unit)

        def impl_float_scalar(arg_a, unit='ns', errors='raise'):
            val = float_to_timedelta_val(arg_a, rodt__gqeh, m)
            return pd.Timedelta(val)
        return impl_float_scalar
    if isinstance(arg_a, types.Integer):
        m, kttg__zgb = pd._libs.tslibs.conversion.precision_from_unit(unit)
    ...
    if is_iterable_type(arg_a) and not isinstance(arg_a, types.BaseTuple):
        m, rodt__gqeh = pd._libs.tslibs.conversion.precision_from_unit(unit)
        mak__zyieu = np.dtype('timedelta64[ns]')
        if isinstance(arg_a.dtype, types.Float):

            def impl_float(arg_a, unit='ns', errors='raise'):
                bruy__hvva = len(arg_a)
                lyqtw__dhir = np.empty(bruy__hvva, mak__zyieu)
                for jwsi__fphq in numba.parfors.parfor.internal_prange(
                    bruy__hvva):
                    val = iNaT
                    if not bodo.libs.array_kernels.isna(arg_a, jwsi__fphq):
                        val = float_to_timedelta_val(arg_a[jwsi__fphq],
                            rodt__gqeh, m)
                    lyqtw__dhir[jwsi__fphq
        ...
        if arg_a.dtype == datetime_timedelta_type:
            ...
            def impl_datetime_timedelta(arg_a, unit='ns', errors='raise'):
                ...
                for jwsi__fphq in numba.parfors.parfor.internal_prange(
                    ...
                    val = iNaT
                    if not bodo.libs.array_kernels.isna(arg_a, jwsi__fphq):
                        djyic__qegv = arg_a[jwsi__fphq]
                        val = (djyic__qegv.microseconds + 1000 * 1000 * (
                            djyic__qegv.seconds + 24 * 60 * 60 *
                            djyic__qegv.days)) * 1000
                    lyqtw__dhir[jwsi__fphq
                        ] = bodo.hiframes.pd_timestamp_ext.integer_to_timedelta64(
                        val)
                return bodo.hiframes.pd_index_ext.init_timedelta_index(
...
def float_to_timedelta_val(data, precision, multiplier):
    dfbc__vwiq = np.int64(data)
    opr__ogb = data - dfbc__vwiq
    if precision:
        opr__ogb = np.round(opr__ogb, precision)
    return dfbc__vwiq * multiplier + np.int64(opr__ogb * multiplier)
...
def overload_freq_methods(method):
    ...
    def freq_overload(td, freq, ambiguous='raise', nonexistent='raise'):
        ...
        hep__kaij = dict(ambiguous=ambiguous, nonexistent=nonexistent)
        bhsw__mquby = dict(ambiguous='raise', nonexistent='raise')
        check_unsupported_args(f'Timestamp.{method}', hep__kaij,
            bhsw__mquby, package_name='pandas', module_name='Timestamp')
        vxt__kvtq = ["freq == 'D'", "freq == 'H'",
            "freq == 'min' or freq == 'T'", "freq == 'S'",
            "freq == 'ms' or freq == 'L'", "freq == 'U' or freq == 'us'",
            "freq == 'N'"]
        dcylo__fedle = [24 * 60 * 60 * 1000000 * 1000, 60 * 60 * 1000000 * 
            1000, 60 * 1000000 * 1000, 1000000 * 1000, 1000 * 1000, 1000, 1]
        jyb__ioz = (
            "def impl(td, freq, ambiguous='raise', nonexistent='raise'):\n")
        for jwsi__fphq, bzm__kkf in enumerate(vxt__kvtq):
            fmi__qnzrh = 'if' if jwsi__fphq == 0 else 'elif'
            jyb__ioz += '    {} {}:\n'.format(fmi__qnzrh, bzm__kkf)
            jyb__ioz += '        unit_value = {}\n'.format(dcylo__fedle[
                jwsi__fphq])
        jyb__ioz += '    else:\n'
        jyb__ioz += (
...
"""
            ...
            if method == 'round':
                jyb__ioz += '    if unit_value == 1:\n'
                jyb__ioz += '        value = td.value\n'
                jyb__ioz += '    else:\n'
                jyb__ioz += (
                    '        quotient, remainder = np.divmod(td.value, unit_value)\n'
                    )
                jyb__ioz += """        mask = np.logical_or(remainder > (unit_value // 2), np.logical_and(remainder == (unit_value // 2), quotient % 2))
"""
                jyb__ioz += '        if mask:\n'
                jyb__ioz += '            quotient = quotient + 1\n'
                jyb__ioz += '        value = quotient * unit_value\n'
            jyb__ioz += '    return pd.Timestamp(value)\n'
        druv__skd = {}
        exec(jyb__ioz, {'np': np, 'pd': pd}, druv__skd)
        impl = druv__skd['impl']
        return impl
...
def _install_freq_methods():
    kdozf__vdj = ['ceil', 'floor', 'round']
    for method in kdozf__vdj:
        umd__aqlr = overload_freq_methods(method)
        overload_method(PDTimeDeltaType, method, no_unliteral=True)(umd__aqlr)
...
def compute_pd_timestamp(totmicrosec, nanosecond):
    ...
    xrn__veel = totmicrosec // 1000000
    second = xrn__veel % 60
    hxkfm__dsjty = xrn__veel // 60
    minute = hxkfm__dsjty % 60
    rqv__hdijr = hxkfm__dsjty // 60
    hour = rqv__hdijr % 24
    eiuw__bumg = rqv__hdijr // 24
    year, month, day = _ord2ymd(eiuw__bumg)
...
def overload_sub_operator_timestamp(lhs, rhs):
    if lhs == pd_timestamp_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            bgt__gsqvi = lhs.toordinal()
            gkedi__jlc = lhs.second + lhs.minute * 60 + lhs.hour * 3600
            tokn__xyx = lhs.microsecond
            nanosecond = lhs.nanosecond
            yvlih__paj = rhs.days
            cxo__pryhe = rhs.seconds
            xsahu__dgtxs = rhs.microseconds
            zvb__xxjgf = bgt__gsqvi - yvlih__paj
            dru__ljv = gkedi__jlc - cxo__pryhe
            enj__rgvo = tokn__xyx - xsahu__dgtxs
            totmicrosec = 1000000 * (zvb__xxjgf * 86400 + dru__ljv) + enj__rgvo
...
def overload_add_operator_timestamp(lhs, rhs):
    ...
    if lhs == pd_timestamp_type and rhs == pd_timedelta_type:
        ...
        def impl(lhs, rhs):
            ...
            gkedi__jlc = lhs.second + lhs.minute * 60 + lhs.hour * 3600
            tokn__xyx = lhs.microsecond
            cekih__cluwn = lhs.nanosecond
            xsahu__dgtxs = rhs.value // 1000
            mnj__njk = rhs.nanoseconds
            enj__rgvo = tokn__xyx + xsahu__dgtxs
            totmicrosec = 1000000 * (bgt__gsqvi * 86400 + gkedi__jlc
                ) + enj__rgvo
            zebqw__cem = cekih__cluwn + mnj__njk
            return compute_pd_timestamp(totmicrosec, zebqw__cem)
        return impl
...
def strftime(ts, format):
    if isinstance(ts, DatetimeDateType):
        lmsk__adhw = 'datetime.date'
    else:
        lmsk__adhw = 'pandas.Timestamp'
    if types.unliteral(format) != types.unicode_type:
        raise BodoError(
            f"{lmsk__adhw}.strftime(): 'strftime' argument must be a string")
...
def typeof_python_calendar(val, c):
    return types.Tuple([types.StringLiteral(gumvm__ydbsb) for gumvm__ydbsb in
        val])
...
def _install_pd_timestamp_unsupported():
    ...
    for nlqtk__rzusi in timestamp_unsupported_attrs:
        ezvrj__thpf = 'pandas.Timestamp.' + nlqtk__rzusi
        overload_attribute(PandasTimestampType, nlqtk__rzusi)(
            create_unsupported_overload(ezvrj__thpf))
    for tbvdm__kmyl in timestamp_unsupported_methods:
        ezvrj__thpf = 'pandas.Timestamp.' + tbvdm__kmyl
        overload_method(PandasTimestampType, tbvdm__kmyl)(
            create_unsupported_overload(ezvrj__thpf + '()'))
