...
def _global_except_hook(exctype, value, traceback):
    ...
    owfj__euqy = MPI.COMM_WORLD
    xusm__yeklh = owfj__euqy.Get_rank()
    fige__ebp = 3.0
    kwhv__dnq = True
    mhors__xmg = owfj__euqy.Ibarrier()
    kuj__zby = time.time()
    while time.time() - kuj__zby < fige__ebp:
        time.sleep(0.1)
        if mhors__xmg.Test():
            kwhv__dnq = False
            break
    try:
        ...
        if kwhv__dnq:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {xusm__yeklh}. \n')
