...
def decode_if_dict_array_overload(A):
    if isinstance(A, types.BaseTuple):
        hwy__emovp = len(A.types)
        xjuy__mxtvn = 'def f(A):\n'
        zqfjq__vzj = ','.join(f'decode_if_dict_array(A[{i}])' for i in
            range(hwy__emovp))
        xjuy__mxtvn += '  return ({}{})\n'.format(zqfjq__vzj, ',' if 
            hwy__emovp == 1 else '')
        nmfh__qjb = {}
        exec(xjuy__mxtvn, {'decode_if_dict_array': decode_if_dict_array},
            nmfh__qjb)
        impl = nmfh__qjb['f']
        return impl
    ...
    if isinstance(A, bodo.SeriesType):
        ...
        def impl(A):
            svhbx__lpqjs = bodo.hiframes.pd_series_ext.get_series_data(A)
            gqqm__yuxcu = bodo.hiframes.pd_series_ext.get_series_index(A)
            name = bodo.hiframes.pd_series_ext.get_series_name(A)
            yju__ekr = decode_if_dict_array(svhbx__lpqjs)
            return bodo.hiframes.pd_series_ext.init_series(yju__ekr,
                gqqm__yuxcu, name)
        return impl
...
def to_str_arr_if_dict_array(t):
    ...
    if isinstance(t, bodo.TableType):
        ymyp__mhsdv = tuple(to_str_arr_if_dict_array(t) for t in t.arr_types)
        return bodo.TableType(ymyp__mhsdv, t.has_runtime_cols)
...
class BodoError(NumbaError):
    ...
    def __init__(self, msg, loc=None, locs_in_msg=None):
        ...
        oepz__xymvd = numba.core.errors.termcolor().errmsg
        super(BodoError, self).__init__(oepz__xymvd(msg), loc)
...
def raise_bodo_error(msg, loc=None):
    if bodo.transforms.typing_pass.in_partial_typing:
        ...
        sxs__lynf = [] if loc is None else [loc]
        raise BodoError(msg, locs_in_msg=sxs__lynf)
...
class FileInfo:
    ...
    def get_schema(self, fname):
        vgvu__qwjrw = self.get_full_filename(fname)
        return self._get_schema(vgvu__qwjrw)
...
class FilenameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        jrh__cxfk = dmm.lookup(bodo.typeof(fe_type.fname))
        qitb__dkhu = [(a, hxis__dzdxm) for a, hxis__dzdxm in zip(jrh__cxfk.
            _fields, jrh__cxfk._members)]
        super().__init__(dmm, fe_type, qitb__dkhu)
...
def is_initial_value_type(t):
    if not isinstance(t, types.InitialValue) or t.initial_value is None:
        return False
    fiksk__gde = t.initial_value
    if isinstance(fiksk__gde, dict):
        fiksk__gde = fiksk__gde.values()
    return not any(isinstance(wqjw__ypy, (types.Poison, numba.core.
        interpreter._UNKNOWN_VALUE)) for wqjw__ypy in fiksk__gde)
...
def is_overload_constant_dict(val):
    return isinstance(val, types.LiteralStrKeyDict) and all(is_literal_type
        (wqjw__ypy) for wqjw__ypy in val.types) or is_initial_value_dict_type(
...
def is_overload_bool_list(val):
    return is_overload_constant_list(val) and all(is_overload_constant_bool
        (wqjw__ypy) for wqjw__ypy in get_overload_const_list(val))
...
def get_overload_const(val):
    ...
    if isinstance(val, types.LiteralList):
        ssr__iltgl = []
        for wqjw__ypy in val.literal_value:
            zmduf__qvhpr = get_overload_const(wqjw__ypy)
            if zmduf__qvhpr == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                ssr__iltgl.append(zmduf__qvhpr)
        return ssr__iltgl
    ...
    if isinstance(val, types.BaseTuple):
        ssr__iltgl = []
        for wqjw__ypy in val.types:
            zmduf__qvhpr = get_overload_const(wqjw__ypy)
            if zmduf__qvhpr == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                ssr__iltgl.append(zmduf__qvhpr)
        return tuple(ssr__iltgl)
...
def ensure_constant_arg(fname, arg_name, val, const_type):
    zmduf__qvhpr = get_overload_const(val)
    fwsvi__jseug = _const_type_repr.get(const_type, str(const_type))
    if not isinstance(zmduf__qvhpr, const_type):
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant {fwsvi__jseug} not {val}"
            )


def ensure_constant_values(fname, arg_name, val, const_values):
    zmduf__qvhpr = get_overload_const(val)
    if zmduf__qvhpr not in const_values:
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant value in {const_values} not '{zmduf__qvhpr}'"
            )
...
def check_unsupported_args(fname, args_dict, arg_defaults_dict,
    ...
    xeat__ikpc = False
    for a in args_dict:
        urt__dajf = get_overload_const(args_dict[a])
        lipd__pbvlf = arg_defaults_dict[a]
        if (urt__dajf is NOT_CONSTANT or urt__dajf is not None and 
            lipd__pbvlf is None or urt__dajf is None and lipd__pbvlf is not
            None or urt__dajf != lipd__pbvlf or urt__dajf is not _no_input and
            lipd__pbvlf is _no_input or urt__dajf is _no_input and 
            lipd__pbvlf is not _no_input):
            kcuxa__drc = (
                f'{fn_str}: {a} parameter only supports default value {lipd__pbvlf}'
                )
            xeat__ikpc = True
            break
    if xeat__ikpc and package_name == 'pandas':
        if module_name == 'IO':
            kcuxa__drc += """
...
"""
    elif xeat__ikpc and package_name == 'ml':
        kcuxa__drc += """
...
"""
    elif xeat__ikpc and package_name == 'numpy':
        kcuxa__drc += """
...
def get_overload_constant_dict(val):
    if isinstance(val, types.LiteralStrKeyDict):
        return {get_literal_value(kwbd__fpe): get_literal_value(wqjw__ypy) for
            kwbd__fpe, wqjw__ypy in val.literal_value.items()}
    ...
    if isinstance(val, types.DictType):
        ...
        return val.initial_value
    mylz__gwn = [get_overload_const(wqjw__ypy) for wqjw__ypy in val.types[1:]]
    return {mylz__gwn[2 * i]: mylz__gwn[2 * i + 1] for i in range(len(
        mylz__gwn) // 2)}
...
def get_overload_const_list(val):
    ...
    if is_initial_value_list_type(val):
        return val.initial_value
    if isinstance(val, types.LiteralList):
        return [get_literal_value(wqjw__ypy) for wqjw__ypy in val.literal_value
            ]
...
def parse_dtype(dtype, func_name=None):
    ...
    if isinstance(dtype, types.Number) or dtype == bodo.string_type:
        return dtype
    try:
        qkgzt__bqi = get_overload_const_str(dtype)
        if qkgzt__bqi.startswith('Int') or qkgzt__bqi.startswith('UInt'):
            return bodo.libs.int_arr_ext.typeof_pd_int_dtype(pd.api.types.
                pandas_dtype(qkgzt__bqi), None)
        if qkgzt__bqi == 'boolean':
            return bodo.libs.bool_arr_ext.boolean_dtype
        if qkgzt__bqi == 'str':
            return bodo.string_type
        return numba.np.numpy_support.from_dtype(np.dtype(qkgzt__bqi))
    except:
        pass
...
def get_index_names(t, func_name, default_name):
    ...
    oaqoi__gkvks = '{}: index name should be a constant string'.format(
        func_name)
    if isinstance(t, MultiIndexType):
        dhg__zyij = []
        for i, opdby__wakiz in enumerate(t.names_typ):
            if opdby__wakiz == types.none:
                dhg__zyij.append('level_{}'.format(i))
                continue
            if not is_overload_constant_str(opdby__wakiz):
                raise BodoError(oaqoi__gkvks)
            dhg__zyij.append(get_overload_const_str(opdby__wakiz))
        return tuple(dhg__zyij)
...
if types.SliceLiteral in numba.core.pythonapi._boxers.functions:
    ...
    def box_slice_literal(typ, val, c):
        xcoqw__cutt = typ.literal_value
        gjbut__ntc = []
        for nnk__ardt in ('start', 'stop', 'step'):
            weyt__ugyn = getattr(typ.literal_value, nnk__ardt)
            eaexq__cjf = c.pyapi.make_none(
                ) if weyt__ugyn is None else c.pyapi.from_native_value(types
                .literal(weyt__ugyn), weyt__ugyn, c.env_manager)
            gjbut__ntc.append(eaexq__cjf)
        mox__thr = c.pyapi.unserialize(c.pyapi.serialize_object(slice))
        dplp__tqzy = c.pyapi.call_function_objargs(mox__thr, gjbut__ntc)
        for a in gjbut__ntc:
            c.pyapi.decref(a)
        c.pyapi.decref(mox__thr)
        return dplp__tqzy
...
def box_list_literal(typ, val, c):
    huyay__vccpj = typ.literal_value
    ptli__wiigi = [c.pyapi.from_native_value(types.literal(wqjw__ypy),
        wqjw__ypy, c.env_manager) for wqjw__ypy in huyay__vccpj]
    eja__icbh = c.pyapi.list_pack(ptli__wiigi)
    for a in ptli__wiigi:
        c.pyapi.decref(a)
    return eja__icbh
...
def list_literal_to_list(context, builder, fromty, toty, val):
    ...
    tgx__ajz = types.List(toty.dtype)
...
def is_literal_type(t):
    ...
    return isinstance(t, (types.Literal, types.Omitted)) and not isinstance(t,
        ...
        is_literal_type(wqjw__ypy) for wqjw__ypy in t.types
...
def get_literal_value(t):
    ...
    if isinstance(t, types.Literal):
        if isinstance(t, types.LiteralStrKeyDict):
            return {get_literal_value(kwbd__fpe): get_literal_value(
                wqjw__ypy) for kwbd__fpe, wqjw__ypy in t.literal_value.items()}
        if isinstance(t, types.LiteralList):
            return [get_literal_value(wqjw__ypy) for wqjw__ypy in t.
                literal_value]
        return t.literal_value
    if isinstance(t, types.Omitted):
        return t.value
    if isinstance(t, types.BaseTuple):
        return tuple(get_literal_value(wqjw__ypy) for wqjw__ypy in t.types)
...
def dtype_to_array_type(dtype):
    ...
    if isinstance(dtype, (types.Number, types.Boolean, types.NPDatetime,
        types.NPTimedelta)):
        svhbx__lpqjs = types.Array(dtype, 1, 'C')
        if lcto__oxeb:
            return to_nullable_type(svhbx__lpqjs)
        return svhbx__lpqjs
...
def get_udf_out_arr_type(f_return_type, return_nullable=False):
    ...
    zac__jbtze = dtype_to_array_type(f_return_type)
    zac__jbtze = to_nullable_type(zac__jbtze
        ) if return_nullable else zac__jbtze
    return zac__jbtze
...
def types_equality_exists(t1, t2):
    nbfm__eaup = numba.core.registry.cpu_target.typing_context
    try:
        nbfm__eaup.resolve_function_type(operator.eq, (t1, t2), {})
        return True
    except:
        return False
...
def is_hashable_type(t):
    whitelist_types = (types.UnicodeType, types.StringLiteral, types.
        ...
        PandasTimestampType)
    pnko__qui = (types.bool_, bodo.datetime64ns, bodo.timedelta64ns, bodo.
        pd_timedelta_type)
    if isinstance(t, whitelist_types) or t in pnko__qui:
        return True
    nbfm__eaup = numba.core.registry.cpu_target.typing_context
    try:
        nbfm__eaup.resolve_function_type(hash, (t,), {})
        return True
    except:
        return False
...
def to_nullable_type(t):
    ...
    if isinstance(t, DataFrameType):
        mowz__ileuw = tuple(to_nullable_type(t) for t in t.data)
        return DataFrameType(mowz__ileuw, t.index, t.columns, t.dist, t.
            is_table_format)
...
def is_common_scalar_dtype(scalar_types):
    xic__okyi, irqf__ytdm = get_common_scalar_dtype(scalar_types)
    return irqf__ytdm
...
def get_common_scalar_dtype(scalar_types):
    ...
    try:
        bxtz__hnfl = np.find_common_type([numba.np.numpy_support.as_dtype(t
            ) for t in scalar_types], [])
        if bxtz__hnfl != object:
            return numba.np.numpy_support.from_dtype(bxtz__hnfl), True
    except numba.core.errors.NumbaNotImplementedError as gaih__pvaep:
        pass
    ...
    ijdyd__jcztn = itertools.groupby(scalar_types)
    if next(ijdyd__jcztn, True) and not next(ijdyd__jcztn, False):
...
def get_nullable_and_non_nullable_types(array_of_types):
    ...
    for typ in array_of_types:
        if typ == bodo.libs.bool_arr_ext.boolean_array:
            nqux__kufy.append(types.Array(types.bool_, 1, 'C'))
        elif isinstance(typ, bodo.libs.int_arr_ext.IntegerArrayType):
            nqux__kufy.append(types.Array(typ.dtype, 1, 'C'))
        elif isinstance(typ, types.Array):
            if typ.dtype == types.bool_:
                nqux__kufy.append(bodo.libs.bool_arr_ext.boolean_array)
            if isinstance(typ.dtype, types.Integer):
                nqux__kufy.append(bodo.libs.int_arr_ext.IntegerArrayType(
                    typ.dtype))
        nqux__kufy.append(typ)
    return nqux__kufy
...
def _gen_objmode_overload(func, output_type, attr_name=None, is_function=
    ...
    if is_function:
        yde__auab = getfullargspec(func)
        assert yde__auab.varargs is None, 'varargs not supported'
        assert yde__auab.varkw is None, 'varkw not supported'
        defaults = [] if yde__auab.defaults is None else yde__auab.defaults
        ghp__mwthf = len(yde__auab.args) - len(defaults)
        args = yde__auab.args[1:] if attr_name else yde__auab.args[:]
        vpwv__czcc = []
        for i, eyo__bazr in enumerate(yde__auab.args):
            if i < ghp__mwthf:
                vpwv__czcc.append(eyo__bazr)
            elif str(defaults[i - ghp__mwthf]) != '<deprecated parameter>':
                vpwv__czcc.append(eyo__bazr + '=' + str(defaults[i -
                    ghp__mwthf]))
            else:
                args.remove(eyo__bazr)
        if yde__auab.kwonlyargs is not None:
            for eyo__bazr in yde__auab.kwonlyargs:
                args.append(f'{eyo__bazr}={eyo__bazr}')
                vpwv__czcc.append(
                    f'{eyo__bazr}={str(yde__auab.kwonlydefaults[eyo__bazr])}')
        sig = ', '.join(vpwv__czcc)
        args = ', '.join(args)
    else:
        sig = 'self'
    ...
    sxh__pqmk = f'self.{attr_name}' if attr_name else f'{func_name}'
    xjuy__mxtvn = f'def overload_impl({sig}):\n'
    xjuy__mxtvn += f'    def impl({sig}):\n'
    if single_rank:
        xjuy__mxtvn += f'        if bodo.get_rank() == 0:\n'
        uwnlj__fxog = '    '
    else:
        uwnlj__fxog = ''
    xjuy__mxtvn += (
        f"        {uwnlj__fxog}with numba.objmode(res='{type_name}'):\n")
    if is_function:
        xjuy__mxtvn += f'            {uwnlj__fxog}res = {sxh__pqmk}({args})\n'
    else:
        xjuy__mxtvn += f'            {uwnlj__fxog}res = {sxh__pqmk}\n'
    xjuy__mxtvn += f'        return res\n'
    xjuy__mxtvn += f'    return impl\n'
    nmfh__qjb = {}
    mmy__sywhx = globals()
    if not attr_name:
        mmy__sywhx[func_name] = func
    exec(xjuy__mxtvn, mmy__sywhx, nmfh__qjb)
    qujmx__izdz = nmfh__qjb['overload_impl']
    return qujmx__izdz


def gen_objmode_func_overload(func, output_type=None, single_rank=False):
    try:
        qujmx__izdz = _gen_objmode_overload(func, output_type, is_function=
            True, single_rank=single_rank)
        overload(func, no_unliteral=True)(qujmx__izdz)
    except Exception as gaih__pvaep:
        pass
...
def gen_objmode_method_overload(obj_type, method_name, method, output_type=
    ...
    try:
        qujmx__izdz = _gen_objmode_overload(method, output_type,
            method_name, True, single_rank)
        overload_method(obj_type, method_name, no_unliteral=True)(qujmx__izdz)
    except Exception as gaih__pvaep:
        pass
...
def gen_objmode_attr_overload(obj_type, attr_name, attr, output_type=None,
    single_rank=False):
    try:
        qujmx__izdz = _gen_objmode_overload(attr, output_type, attr_name, 
            False, single_rank)
        overload_attribute(obj_type, attr_name, no_unliteral=True)(qujmx__izdz)
    except Exception as gaih__pvaep:
        pass
...
class NumTypeStaticGetItem(AbstractTemplate):
    ...
    def generic(self, args, kws):
        val, njtna__pfe = args
        if isinstance(njtna__pfe, slice) and (isinstance(val, types.
            NumberClass) or isinstance(val, types.TypeRef) and isinstance(
            val.instance_type, (types.NPDatetime, types.NPTimedelta))):
            return signature(types.TypeRef(val.instance_type[njtna__pfe]),
                *args)
...
def construct_pysig(arg_names, defaults):
    xjuy__mxtvn = f'def stub('
    for eyo__bazr in arg_names:
        xjuy__mxtvn += eyo__bazr
        if eyo__bazr in defaults:
            if isinstance(defaults[eyo__bazr], str):
                xjuy__mxtvn += f"='{defaults[eyo__bazr]}'"
            else:
                xjuy__mxtvn += f'={defaults[eyo__bazr]}'
        xjuy__mxtvn += ', '
    xjuy__mxtvn += '):\n'
    xjuy__mxtvn += '    pass\n'
    nmfh__qjb = {}
    exec(xjuy__mxtvn, {}, nmfh__qjb)
    znfcu__sadt = nmfh__qjb['stub']
    return numba.core.utils.pysignature(znfcu__sadt)


def fold_typing_args(func_name, args, kws, arg_names, defaults,
    unsupported_arg_names=()):
    kws = dict(kws)
    iszh__hyjjd = len(arg_names)
    osly__nkfhi = len(args) + len(kws)
    if osly__nkfhi > iszh__hyjjd:
        fkuj__vcqv = 'argument' if iszh__hyjjd == 1 else 'arguments'
        occhx__hskae = 'was' if osly__nkfhi == 1 else 'were'
        raise BodoError(
            f'{func_name}(): Too many arguments specified. Function takes {iszh__hyjjd} {fkuj__vcqv}, but {osly__nkfhi} {occhx__hskae} provided.'
            )
    dgviz__hetj = bodo.utils.typing.construct_pysig(arg_names, defaults)
    try:
        bbtpq__hhcu = bodo.utils.transform.fold_argument_types(dgviz__hetj,
            args, kws)
    except Exception as ljrv__xbkd:
        raise_bodo_error(f'{func_name}(): {ljrv__xbkd}')
    if unsupported_arg_names:
        ...
        for i, arg_name in enumerate(arg_names):
            if arg_name in unsupported_arg_names:
                ...
                meo__ycb[arg_name] = bbtpq__hhcu[i]
                todjs__drlh[arg_name] = defaults[arg_name]
        check_unsupported_args(func_name, meo__ycb, todjs__drlh)
    return dgviz__hetj, bbtpq__hhcu
...
class BodoArrayIterator(types.SimpleIteratorType):
    ...
    def __init__(self, arr_type):
        ...
        jzn__qzvlc = arr_type.dtype
        super(BodoArrayIterator, self).__init__(name, jzn__qzvlc)
...
class BodoArrayIteratorModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        qitb__dkhu = [('index', types.EphemeralPointer(types.uintp)), (
            'array', fe_type.arr_type)]
        super(BodoArrayIteratorModel, self).__init__(dmm, fe_type, qitb__dkhu)
...
def iternext_bodo_array(context, builder, sig, args, result):
    [jxf__oafue] = sig.args
    [axzpc__grff] = args
    ncz__wkwdb = context.make_helper(builder, jxf__oafue, value=axzpc__grff)
    pyy__ehjjj = signature(types.intp, jxf__oafue.arr_type)
    yts__tad = context.compile_internal(builder, lambda a: len(a),
        pyy__ehjjj, [ncz__wkwdb.array])
    gqqm__yuxcu = builder.load(ncz__wkwdb.index)
    xhjd__uvud = builder.icmp_signed('<', gqqm__yuxcu, yts__tad)
    result.set_valid(xhjd__uvud)
    with builder.if_then(xhjd__uvud):
        kcrig__apaae = signature(jxf__oafue.arr_type.dtype, jxf__oafue.
            arr_type, types.intp)
        value = context.compile_internal(builder, lambda a, i: a[i],
            kcrig__apaae, [ncz__wkwdb.array, gqqm__yuxcu])
        result.yield_(value)
        owqc__shu = cgutils.increment_index(builder, gqqm__yuxcu)
        builder.store(owqc__shu, ncz__wkwdb.index)


def index_typ_from_dtype_name(elem_dtype, name):
    dhbf__owflz = type(get_index_type_from_dtype(elem_dtype))
    if name is None:
        qhxlv__arbdm = None
    else:
        qhxlv__arbdm = types.StringLiteral(name)
    if dhbf__owflz == bodo.hiframes.pd_index_ext.NumericIndexType:
        rkx__gpt = dhbf__owflz(elem_dtype, qhxlv__arbdm)
    elif dhbf__owflz == bodo.hiframes.pd_index_ext.CategoricalIndexType:
        rkx__gpt = dhbf__owflz(bodo.CategoricalArrayType(elem_dtype),
            qhxlv__arbdm)
    else:
        rkx__gpt = dhbf__owflz(qhxlv__arbdm)
    return rkx__gpt
...
def check_objmode_output_type(ret_tup, ret_type):
    return tuple(_check_objmode_type(wqjw__ypy, t) for wqjw__ypy, t in zip(
...
def _fix_objmode_df_type(val, val_typ, typ):
    ...
    if val_typ != typ:
        rjos__pnhnw = pd.Index(val_typ.columns)
        vwf__dxjn = pd.Index(typ.columns)
        quu__ntovz = rjos__pnhnw.argsort()
        olov__yeyt = vwf__dxjn.argsort()
        uiraj__vjrx = val_typ.copy(data=tuple(np.array(val_typ.data)[
            quu__ntovz]), columns=tuple(rjos__pnhnw[quu__ntovz]))
        cafm__xhfa = typ.copy(data=tuple(np.array(typ.data)[olov__yeyt]),
            columns=tuple(vwf__dxjn[olov__yeyt]))
        if uiraj__vjrx == cafm__xhfa:
            val_typ = typ
