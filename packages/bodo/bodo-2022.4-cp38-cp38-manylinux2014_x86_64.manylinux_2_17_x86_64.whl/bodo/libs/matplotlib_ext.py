...
def install_mpl_class(types_name, python_type):
    zfaso__laea = ''.join(map(str.title, types_name.split('_')))
    mkb__xbwt = f'class {zfaso__laea}(types.Opaque):\n'
    mkb__xbwt += '    def __init__(self):\n'
    mkb__xbwt += f"       types.Opaque.__init__(self, name='{zfaso__laea}')\n"
    mkb__xbwt += '    def __reduce__(self):\n'
    mkb__xbwt += (
        f"        return (types.Opaque, ('{zfaso__laea}',), self.__dict__)\n")
    pfz__wwdn = {}
    exec(mkb__xbwt, {'types': types, 'bodo': bodo}, pfz__wwdn)
    moqa__jmcl = pfz__wwdn[zfaso__laea]
    rgr__lojq = sys.modules[__name__]
    setattr(rgr__lojq, zfaso__laea, moqa__jmcl)
    class_instance = moqa__jmcl()
    setattr(types, types_name, class_instance)
    register_model(moqa__jmcl)(models.OpaqueModel)
    typeof_impl.register(python_type)(lambda val, c: class_instance)
    unbox(moqa__jmcl)(unbox_mpl_obj)
    box(moqa__jmcl)(box_mpl_obj)
...
def _install_mpl_types():
    nksnw__jhq = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for mhulk__dfnba, irdg__fme in nksnw__jhq:
        install_mpl_class(mhulk__dfnba, irdg__fme)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    uaay__nqu = ', '.join(f'e{kmfv__ybkr}' for kmfv__ybkr in range(len(args)))
    if uaay__nqu:
        uaay__nqu += ', '
    nxw__tjzfh = ', '.join(f"{hvtsj__bihbd} = ''" for hvtsj__bihbd in kws.
        keys())
    qjakf__rhcp = 'matplotlib_obj, ' if obj_typ is not None else ''
    znuhw__fxhvr = f'def mpl_stub({qjakf__rhcp} {uaay__nqu} {nxw__tjzfh}):\n'
    znuhw__fxhvr += '    pass\n'
    gqnr__uvf = {}
    exec(znuhw__fxhvr, {}, gqnr__uvf)
    znk__oip = gqnr__uvf['mpl_stub']
    purx__irrlq = numba.core.utils.pysignature(znk__oip)
    ihzde__zzi = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(ihzde__zzi)).replace(pysig=
        purx__irrlq)


def generate_axes_typing(mod_name, nrows, ncols):
    dyqxv__rccuu = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(dyqxv__rccuu.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(dyqxv__rccuu.format(mod_name, 'ncols'))
    stol__pfnhl = get_overload_const_int(nrows)
    vtfh__auzng = get_overload_const_int(ncols)
    if stol__pfnhl < 1:
        raise BodoError(dyqxv__rccuu.format(mod_name, 'nrows'))
    if vtfh__auzng < 1:
        raise BodoError(dyqxv__rccuu.format(mod_name, 'ncols'))
    if stol__pfnhl == 1 and vtfh__auzng == 1:
        tqo__lfu = types.mpl_axes_type
    else:
        if vtfh__auzng == 1:
            xij__dptgi = types.mpl_axes_type
        else:
            xij__dptgi = types.Tuple([types.mpl_axes_type] * vtfh__auzng)
        tqo__lfu = types.Tuple([xij__dptgi] * stol__pfnhl)
    return tqo__lfu


def generate_pie_return_type(args, kws):
    koug__qiyfc = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if koug__qiyfc == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    ocd__jmdia = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(ocd__jmdia):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(ocd__jmdia):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        pmdup__mys = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, pmdup__mys]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    pmdup__mys = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    mhulk__dfnba = str(pmdup__mys)
    if not hasattr(types, mhulk__dfnba):
        mhulk__dfnba = f'objmode_type{ir_utils.next_label()}'
        setattr(types, mhulk__dfnba, pmdup__mys)
    znuhw__fxhvr = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{mhulk__dfnba}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    gqnr__uvf = {}
    exec(znuhw__fxhvr, {'numba': numba, 'np': np}, gqnr__uvf)
    impl = gqnr__uvf['impl']
    return impl
