...
def box_regex_flag(typ, val, c):
    hnd__mcmbd = c.box(types.int64, val)
    bltkm__zgbw = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(bltkm__zgbw, (hnd__mcmbd,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    hnd__mcmbd = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, hnd__mcmbd)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    cchyw__gadd = context.get_python_api(builder)
    vzzs__wibgy = builder.icmp_signed('==', val, cchyw__gadd.borrow_none())
    with builder.if_then(vzzs__wibgy):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    hjaf__dws = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(hjaf__dws)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        cifo__obq = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if cifo__obq > 1:
            typ = types.List(types.Tuple([string_type] * cifo__obq))
...
def overload_match_group(m, *args):
    ...
    vxql__hxra = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            bsnb__wwfed = args[0]
            with numba.objmode(out=vxql__hxra):
                out = m.group(bsnb__wwfed)
            return out
        return _match_group_impl_one
    qfi__hzlt = 'tuple_str_{}'.format(len(args))
    setattr(types, qfi__hzlt, types.Tuple([vxql__hxra] * len(args)))
    rgv__dnv = ', '.join('group{}'.format(twqb__mcda + 1) for twqb__mcda in
        range(len(args)))
    ggnie__okjev = 'def _match_group_impl(m, *args):\n'
    ggnie__okjev += '  ({}) = args\n'.format(rgv__dnv)
    ggnie__okjev += "  with numba.objmode(out='{}'):\n".format(qfi__hzlt)
    ggnie__okjev += '    out = m.group({})\n'.format(rgv__dnv)
    ggnie__okjev += '  return out\n'
    vak__cst = {}
    exec(ggnie__okjev, globals(), vak__cst)
    yvz__nius = vak__cst['_match_group_impl']
    return yvz__nius
...
def overload_match_groups(m, default=None):
    zyzbf__auvhe = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=zyzbf__auvhe):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(assyl__gzk is None for assyl__gzk in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=vxql__hxra):
