...
def _oidc_login(
    ...
    if browser_available:
        djsciops_utils.log(
            f"Browser available. Launching...", pause_duration=1, gui=gui
        )
    ...
    if cancelled:
        ...
        djsciops_utils.log("User successfully authenticated.", gui=gui)
        # generate user info
...
class Session:
    def __init__(
        ...
        djsciops_utils.log("authentication", message_type="header", gui=gui)
        # OAuth2.0 authorization
        if not bearer_token:
            self.bearer_token, self.user = _oidc_login(
                auth_client_id=auth_client_id, gui=gui
            )
