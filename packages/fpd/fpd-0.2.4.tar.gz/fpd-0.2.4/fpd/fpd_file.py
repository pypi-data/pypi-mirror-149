...
class CubicImageInterpolator:
    ...
    def interpolate_image(self, image, clip_min=None, clip_max=None, in_place=True):
        ...
        self.ip.values[:] = image[self.keep_inds].astype(self.ip.values.dtype, copy=False)[:, None]
                                ...
                                if self._row_end_skip and scanXind > self._scanX-1:
                                    ...
                                    if not memmap_possible:
                                        ...
                                        if func is not None:
                                            ...
                                            im_array = im_array.astype(self._det_dtype, copy=False)
                                        if repack:
        ...
        if nav_im is None:
            if not treat_as_array:
                ...
                if self.ncolours == 0:
                    ...
                    nav_im = fpd.fpd_processing.sum_im(self.h5f_ds, 16, 16, progress_bar=progress_bar)
                else:
                    #nav_im = self.h5f_ds[:, :, self.colour_index].sum((-2, -1))
                    nav_im = fpd.fpd_processing.sum_im(self.h5f_ds, 16, 16, progress_bar=progress_bar)[..., self.colour_index]
        self.nav_im = nav_im
