...
"""
# paths
from rohan.lib.dfs import *
from rohan.lib.dict import read_dict,to_dict # overwrite to_dict from df
from rohan.lib.sys import * #is_interactive_notebook,basenamenoext,makedirs,get_all_subpaths
from rohan.lib.str import replacemany
...
def read_ps(ps,test=True):
    ...
    if test:
        ...
        if len(ds1)>1:
            from rohan.lib.str import get_suffix
...
def to_outp(ps,outd=None,outp=None,suffix=''):
    ...
    if not outp is None:
        return outp
    from rohan.lib.str import get_prefix
    makedirs(outd)
    ps=read_ps(ps)
...
def backup(p,outd,
    ...
    from rohan.lib.set import unique
    ps=read_ps(p)
    assert(len(ps)!=0)
#     if test:print(ps)
    ps=[ p+'/' if isdir(p) else p for p in ps]
#     if test:print(ps)
    from rohan.lib.sys import get_time
#     if timed:
...
from rohan.lib.text import get_header
def read_table(p,
               ext=None,
               clean=True,
               filterby_time=None,
    ...
    if isinstance(p,list) or (isinstance(p,str) and ('*' in p)):
        ...
        if check_paths and isdir(splitext(p)[0]):
            ...
            from rohan.lib.google import download_file
...
#                 else:
    ...
    if save_table: 
        if len(df2)!=0 and not test1:
            # save log file
            from rohan.lib.set import to_list
...
def read_tables(ps,
    ...
    if not filterby_time is None:
        from rohan.lib.sys import ps2time
        df_=ps2time(ps)
