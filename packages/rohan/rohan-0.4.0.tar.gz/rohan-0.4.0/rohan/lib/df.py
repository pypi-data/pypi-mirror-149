...
def get_mappings(df1,
    ...
    if len(cols)==2:
        from rohan.lib.set import get_alt
        df2=df1.copy()
        cols2=[]
        for c in cols:
...
#             if isinstance(colgroupby,str):
    ...
    if plot:
        from rohan.viz.bar import plot_intersections
...
def get_chunks(df1,colindex,colvalue,bins=None,value='right'):
    ...
    Returns: 
        ds (Series): output series.
    """
    from rohan.lib.set import unique,nunique
    if bins==0:
        df1['chunk']=bins
    ...
    from rohan.stat.transform import get_bins
...
def coltuples2str(cols,sep=' '):
    ...
    """
    from rohan.lib.str import tuple2str
    cols_str=[]
    for col in cols:
...
class log:
    ...
    def __call__(self,col=None):
        ...
        return self._obj
    def dropna(self,**kws):
        from rohan.lib.df import log_apply
