...
def get_subset2metrics(df: pd.DataFrame,colvalue: str,colsubset: str,colindex: str,outstr=False,subset_control=None) -> dict:
    ...
    if outstr:
        from rohan.viz.annot import pval2annot
...
#               debug=False,
    ...
    from rohan.lib.dfs import merge_paired
    from rohan.lib.str import get_prefix,get_suffix
...
def get_stats(df1: pd.DataFrame,
    ...
    if subsets is None:
        subsets=sorted(df1[colsubset].unique())
    from rohan.stat.diff import get_stat ## remove?
...
def get_q(ds1,col=None,verb=True,test_coff=0.1):
    ...
    if verb:
        from rohan.viz.annot import perc_label        
...
def binby_pvalue_coffs(df1: pd.DataFrame,
    ...
    #        right=False,
    #       ).fillna(1)
    from rohan.viz.colors import saturate_color
    d1={}
    ...
    if color:
        import itertools
        from rohan.stat.transform import rescale,log_pval
        d3={}
...
def plot_model_sanity(model: object):
    ...
    Args:
        model (object): model.
    """
    from rohan.viz.scatter import plot_qq 
    from rohan.viz.dist import plot_normal 
