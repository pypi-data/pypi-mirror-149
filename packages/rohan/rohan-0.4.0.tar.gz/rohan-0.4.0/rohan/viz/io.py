...
# from rohan.lib.io import read_ps,to_outp
...
def savefig(
    ...
    **kws,
    ...
    Returns:
        str: output path.
    """
#         from rohan.viz.ax_ import to_plotp
...
def get_lines(
    ...
    if not is_interactive_notebook():
        from rohan.lib.sys import get_excecution_location
    ...
    **kws
    ...
    if not safe:
        if not p.endswith('.py'):
            ...
            if test:info(p)
            # p=f"{splitext(p)[0]}.py"
        from rohan.workflow.io import import_from_file
        return import_from_file(p).plot_(**kws)
    else:
        from rohan.lib.plot.schem import plot_image
        if p.endswith('.py'):
...
# from rohan.lib.sys import runbash
import subprocess
from rohan.lib.io import makedirs
