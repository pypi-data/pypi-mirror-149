from rohan.lib.sys import logging,exists,dirname,basename,makedirs,basenamenoext,info,abspath
from rohan.lib.io import read_ps
import pandas as pd
import numpy as np
...
def get_scripts(
    ...
    **kws,
    ...
    # print(df1.shape)
    from rohan.workflow.function import to_task
    if not fast or df1['notebook path'].nunique()<5:
        ...
        from rohan.lib.df import get_name
    ...
    from rohan.lib.set import flatten
    # if test:print(d1)
    ...
    **kws,
    ...
    if scripts:
        ...
        if exists(f'{notebooksdp}/.workflow/config.yaml'):
            from rohan.lib.dict import read_dict
