from rohan.lib.sys import isdir,exists,dirname,basename,makedirs,basenamenoext,info,logging
from rohan.lib.str import replace_many,make_pathable_string
...
#         outputs_str=''
    ...
    f"",
    "    Parameters:",
    "        metadata (dict): Dictionary containing information required for the analysis. Metadata files are located in `metadata` folder are read using `rohan.workflow.io.read_metadata` function.",
    f"",
    f"    Snakemake rule:",
...
def to_task(
    ...
    # from rohan.lib.str import removesuffix
    if notebook_suffix!='':
    ...
    from rohan.workflow.io import to_py, get_lines
    if not test:
        to_py(notebookp,
