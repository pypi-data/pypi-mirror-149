...
decider_str_lookup.update({k: k for k in ['MD5', 'content', 'MD5-timestamp', 'content-timestamp', 'timestamp-newer',  # type: ignore
...
def hash_file_signature_new(fname, chunksize=65536, hash_format=None):
    ...
    if fname_ext in special_sig_fns.keys():
        try:
            ...
            if GetOption("show_specialsig"):
                print_during_build('Computed ' + fname_ext[1:] + '-signature: ' + fname + "\n")
            return sig
        except Exception as e:
            print_during_build('Error when computing ' + fname_ext[1:] + '-signature: ' + fname + "\n")
            raise e
