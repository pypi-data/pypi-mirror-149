...
class TestSolverService(unittest.TestCase):
    ...
    def test_SolveUsingLevenbergMarquardt_3D(self):
        coefficientsShouldBe = numpy.array(
            [0.28658387, -0.90215776, 1.15483863])
    ...
    def test_SolveUsingSimplex_3D(self):
        coefficientsShouldBe = numpy.array(
            [0.28658383, -0.90215775, 1.15483864])
        model = pyeq3.Models_3D.Polynomial.Linear('SSQABS')
        # starting values for the simplex solver
    ...
    def test_SolveUsingSpline_3D(self):
        ...
        unFittedSpline = scipy.interpolate.SmoothBivariateSpline(model.dataCache.allDataCacheDictionary['X'], model.dataCache.allDataCacheDictionary[
                                                                 'Y'], model.dataCache.allDataCacheDictionary['DependentData'], s=model.smoothingFactor, kx=model.xOrder, ky=model.yOrder)
        ...
        self.assertTrue(numpy.equal(
            fittedParameters[0], xKnotPointsShouldBe).all())
        self.assertTrue(numpy.equal(
            fittedParameters[1], yKnotPointsShouldBe).all())
        self.assertTrue(numpy.allclose(
    ...
    def test_SolveUsingSpline_2D(self):
        ...
        model = pyeq3.Models_2D.Spline.Spline(
            inSmoothingFactor=1.0, inXOrder=3)
        pyeq3.dataConvertorService().ConvertAndSortColumnarASCII(
            DataForUnitTests.asciiDataInColumns_2D, model, False)
        fittedParameters = pyeq3.solverService().SolveUsingSpline(model)
        ...
        unFittedSpline = scipy.interpolate.UnivariateSpline(
            model.dataCache.allDataCacheDictionary['X'], model.dataCache.allDataCacheDictionary['DependentData'], s=model.smoothingFactor, k=model.xOrder)
        ...
        self.assertTrue(numpy.allclose(
            testEvaluation, testEvaluationShouldBe, rtol=1.0E-10, atol=1.0E-300))
        self.assertTrue(numpy.equal(
            fittedParameters[0], knotPointsShouldBe).all())
        self.assertTrue(numpy.allclose(
    ...
    def test_SolveUsingLinear_3D(self):
        coefficientsShouldBe = numpy.array(
            [2.8658381589774945E-01, -9.0215775175410395E-01, 1.1548386445491325E+00])
        model = pyeq3.Models_3D.Polynomial.Linear('SSQABS')
        pyeq3.dataConvertorService().ConvertAndSortColumnarASCII(
            DataForUnitTests.asciiDataInColumns_3D, model, False)
        coefficients = pyeq3.solverService().SolveUsingLinear(model)
    ...
    def test_ExponentialSensitivity_2D(self):
        ...
        model.Solve()
        self.assertTrue(numpy.allclose(model.solvedCoefficients,
