...
class mockModel_3D(object):
    ...
    def GetDataCacheFunctions(self):
        ...
        functionList.append(
            [pyeq3.DataCache.DataCacheFunctions.X(NameOrValueFlag=1), []])
        functionList.append([pyeq3.DataCache.DataCacheFunctions.Y(
...
class TestDataCache(unittest.TestCase):
    ...
    def test_ReducedDataSize_2D(self):
        ...
        self.assertEqual(
            len(model.dataCache.reducedDataCacheDictionary['X']), 13)

    def test_DataCache_2D(self):
        cached_X_ShouldBe = numpy.array([3.017, 0.742, 0.607, 2.822])
        cached_ExpX_ShouldBe = numpy.exp(
            numpy.array([3.017, 0.742, 0.607, 2.822]))
        ...
        self.assertTrue(numpy.equal(
            model.dataCache.allDataCacheDictionary['X'], cached_X_ShouldBe).all())
        self.assertTrue(numpy.equal(
            model.dataCache.allDataCacheDictionary['ExpX'], cached_ExpX_ShouldBe).all())

        self.assertTrue(numpy.equal(
            model.dataCache.reducedDataCacheDictionary['X'], cached_X_ShouldBe).all())
        self.assertTrue(numpy.equal(
            model.dataCache.reducedDataCacheDictionary['ExpX'], cached_ExpX_ShouldBe).all())
    ...
    def test_DataCache_3D(self):
        ...
        self.assertTrue(numpy.equal(
            model.dataCache.allDataCacheDictionary['X'], cached_X_ShouldBe).all())
        self.assertTrue(numpy.equal(
            model.dataCache.allDataCacheDictionary['Y'], cached_Y_ShouldBe).all())

        self.assertTrue(numpy.equal(
            model.dataCache.reducedDataCacheDictionary['X'], cached_X_ShouldBe).all())
        self.assertTrue(numpy.equal(
            model.dataCache.reducedDataCacheDictionary['Y'], cached_Y_ShouldBe).all())
