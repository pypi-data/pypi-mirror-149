...
class SolverService(object):
    ...
    ListOfNonLinearSolverAlgorithmNames = ['Levenberg-Marquardt',
                                           'NelderMead',
                                           'Powell',
                                           'CG',
                                           'BFGS',
    ...
    def SolveUsingDE(self, inModel):  # adapts to number of coefficients
        ...
        de = diffev.DiffEvolver(inModel.CalculateReducedDataFittingTarget,
                                pop0, crossover_rate=crossoverProbabilityForGA,
        ...
        try:
            if inAlgorithmName == 'Levenberg-Marquardt':
                coeffs, unused = scipy.optimize.curve_fit(inModel.WrapperForScipyCurveFit, None,
                                                          ...
                                                          maxfev=1000000)  # initial coefficients are all equal to 1
            else:
                coeffs = scipy.optimize.minimize(inModel.CalculateAllDataFittingTarget,
                                                 inModel.deEstimatedCoefficients,
                                                 method=inAlgorithmName)
    ...
    def SolveUsingODR(self, inModel):
        ...
        if len(inModel.dataCache.allDataCacheDictionary['Weights']):
            dataObject = scipy.odr.odrpack.Data(inModel.dataCache.allDataCacheDictionary['IndependentData'],
                                                inModel.dataCache.allDataCacheDictionary['DependentData'],
    ...
    def SolveStatisticalDistribution(self, distributionName, data,
        ...
        if distribution.name in ['beta']:
            try:
                ...
                par_a = data_mean * ((data_mean *
                                      (1.0 - data_mean) / data_var) - 1.0)
                par_b = ((1.0 - data_mean) *
                         ((data_mean * (1.0 - data_mean) / data_var) - 1.0))
                par0 = (par_a, par_b, data_min - 0.001*data_min,
                        data_range * 1.001)
                par_est = tuple(distribution.fit(data, *(par0[:-2]),
