...
pyeq3.dataConvertorService().ConvertAndSortColumnarASCII(equation.exampleData,
                                                         equation, False)
...
for i in range(len(equation.solvedCoefficients)):
    print(f"    {equation.GetCoefficientDesignators()[i]} = "
          f"{equation.solvedCoefficients[i]:.16E}")
print(f"{equation.fittingTargetDictionary[equation.fittingTarget]} = "
