...
smallAnimationDelayBetweenFrames = int(
    ((smallAnimationModulus / largeAnimationModulus) * largeAnimationDelayBetweenFrames) / 2)
...
rawData = numpy.array([
    [
        numpy.array([5.357, 5.797, 5.936, 6.161, 6.697,
                    6.731, 6.775, 8.442, 9.861]),
        []
    ],
    numpy.array([0.376, 0.874, 1.049, 1.327,
                2.054, 2.077, 2.138, 4.744, 7.104])
])
...
def SaveModelScatterConfidence(in_fileName, in_equation, in_Ymax, in_Ymin):
    ...
    if -1 != in_fileName.find('_large'):  # large animation frame
        fig = plt.figure(figsize=(float(largeAnimationGraphWidth) /
                         100.0, float(largeAnimationGraphHeight) / 100.0), dpi=100)
    else:  # small animation frame
        fig = plt.figure(figsize=(float(smallAnimationGraphWidth) /
                         100.0, float(smallAnimationGraphHeight) / 100.0), dpi=100)
    ...
    t_value = scipy.stats.t.ppf(0.975, in_equation.df_e)
    ...
    ax.plot(xModel[booleanMask], lower[booleanMask],
            linestyle='solid', color='white')
    ax.plot(xModel[booleanMask], upper[booleanMask],
            linestyle='solid', color='white')
    ax.plot(xModel[booleanMask], lower[booleanMask],
            linestyle='dashed', color='blue')
    ax.plot(xModel[booleanMask], upper[booleanMask], linestyle='dashed',
            color='blue', label='95% Confidence Intervals')
...
errors in recording experimental data.'''
    ...
    # do not need relative error calculations, this flag turns them off
    equation.dataCache.DependentDataContainsZeroFlag = True
    ...
    for i in range(0, 360):
        ...
        equation.dataCache.allDataCacheDictionary['IndependentData'] = copy.copy(
            rawData[0])
        IndependentDataArray = equation.dataCache.allDataCacheDictionary['IndependentData']
        equation.dataCache.allDataCacheDictionary['DependentData'] = copy.copy(
            rawData[1])
        ...
        if not i % largeAnimationModulus:
            print(i,)
            sys.stdout.flush()
            # sine wave multiplier per animation frame
            offset = numpy.sin(numpy.radians(i)) * 14.0
            ...
            fileName = specificBaseName + '_ci' + \
                extraZerosString + str(i) + '_large.png'
            ...
            if not (i % smallAnimationModulus):
                ...
                fileName = specificBaseName + '_ci' + \
                    extraZerosString + str(i) + '_small.png'
                SaveModelScatterConfidence(fileName, equation, Ymax, Ymin)
    ...
    commandString = 'gifsicle --loop --colors 256 --delay ' + \
        str(smallAnimationDelayBetweenFrames) + " " + specificBaseName + \
        '_ci*small.gif > ' + specificBaseName + '_small.gif'
    print("Calling", commandString)
    p = p = os.popen(commandString)
    p.close()

    # make large GIF animation
    commandString = 'gifsicle --loop --colors 256 --delay ' + \
        str(largeAnimationDelayBetweenFrames) + " " + specificBaseName + \
        '_ci*large.gif > ' + specificBaseName + '_large.gif'
    print("Calling", commandString)
    p = os.popen(commandString)
    p.close()
...
by increasing the total number of data points.'''
    ...
    for i in range(0, 360):
        ...
        if not i % largeAnimationModulus:
            ...
            sys.stdout.flush()
            # sine wave multiplier per animation frame
            offsetArray = numpy.sin(numpy.radians(i)) * noiseArray
...
when making multiple data collection runs.'''
    ...
    for i in range(0, 360):
        ...
        equation.dataCache.allDataCacheDictionary['IndependentData'] = [
            numpy.append(xdata, xdata), numpy.ones(2 * len(xdata))]
        IndependentDataArray = equation.dataCache.allDataCacheDictionary['IndependentData']
        equation.dataCache.allDataCacheDictionary['DependentData'] = numpy.append(
            ydata, ydata)
        equation.dataCache.allDataCacheDictionary['Weights'] = []
        ...
        if not i % largeAnimationModulus:
            ...
            sys.stdout.flush()
            equation.dataCache.allDataCacheDictionary['DependentData'] = numpy.append(
                ydata + numpy.sin(numpy.radians(i)) * 4.9, ydata - numpy.sin(numpy.radians(i)) * 4.9)
            DependentDataArray = equation.dataCache.allDataCacheDictionary['DependentData']
            equation.dataCache.allDataCacheDictionary['Weights'] = []
            equation.Solve()
            equation.CalculateModelErrors(
...
temperature change during a lunch break.'''
    ...
    for i in range(0, 360):
        ...
        if not i % largeAnimationModulus:
            ...
            sys.stdout.flush()
            # sine wave multiplier per animation frame
            offsetArray = numpy.sin(numpy.radians(i)) * fixedArray
            equation.dataCache.allDataCacheDictionary['DependentData'] = rawData[0][0] + offsetArray
            DependentDataArray = equation.dataCache.allDataCacheDictionary['DependentData']
            equation.dataCache.allDataCacheDictionary['Weights'] = []
            equation.Solve()
...
equation with an offset will reveal the bias.'''
    ...
    for i in range(0, 360):
        ...
        if not i % largeAnimationModulus:
            ...
            equation.dataCache.allDataCacheDictionary['IndependentData'] = numpy.array(
                [fixedArray, numpy.ones(len(fixedArray))])
            IndependentDataArray = equation.dataCache.allDataCacheDictionary['IndependentData']
            equation.dataCache.allDataCacheDictionary['DependentData'] = numpy.power(
                2.0, fixedArray + offset)
            DependentDataArray = equation.dataCache.allDataCacheDictionary['DependentData']
            equation.dataCache.allDataCacheDictionary['Weights'] = []
            equation.Solve()
            equation.CalculateModelErrors(
...
for i in htmlInfo:
    ...
    individualFile.write(
        '<img src="' + i[2] + '_large.gif"><br><br><br>---- <b>Still Images</b> -----<br>')
    for fileName in i[3]:
...
for i in htmlInfo:
    ...
    htmlIndexFile.write('<br><br><b>' + i[0] + '</b><br><br><a href="' +
                        i[2] + '.html"><img src="' + i[2] + '_small.gif">')
    htmlIndexFile.write(
        '<br><br>High resolution animation<br>and still images</a><br><br></td>\n<td><large>' + i[1] + '</large></td></tr>\n')
