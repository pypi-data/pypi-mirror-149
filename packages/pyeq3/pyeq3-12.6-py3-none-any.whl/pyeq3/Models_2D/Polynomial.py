...
class UserSelectablePolynomial(pyeq3.Model_2D_BaseClass.Model_2D_BaseClass):
    ...
    def GetDataCacheFunctions(self):
        ...
        for i in range(self.xPolynomialOrder+1):  # 0 to xPolynomialOrder
            functionList.append([pyeq3.DataCache.DataCacheFunctions.PowX(
                NameOrValueFlag=1, args=[float(i)]), [float(i)]])
        return self.extendedVersionHandler.GetAdditionalDataCacheFunctions(self, functionList)
        ...
        try:
            for i in range(self.xPolynomialOrder+1):  # 0 to xPolynomialOrder
                temp += inCoeffs[coeffCount] * \
                    eval("inDataCacheDictionary['PowX_" + str(i) + ".0']")
                coeffCount += 1
            return self.extendedVersionHandler.GetAdditionalModelPredictions(temp, inCoeffs, inDataCacheDictionary, self)
        except:
            return numpy.ones(len(inDataCacheDictionary['DependentData'])) * 1.0E300
...
class UserCustomizablePolynomial(pyeq3.Model_2D_BaseClass.Model_2D_BaseClass):
    ...
    def __init__(self, inFittingTarget=None, inExtendedVersionName='Default', inPolynomial2DFlags=[], inPolynomialEquationList_X=[]):
        if not inPolynomialEquationList_X:
            self.polynomialEquationList = pyeq3.PolyFunctions.GenerateListForCustomPolynomials_2D()
        else:
        ...
        if not self.polynomial2DFlags:
            ...
            for index in range(len(self.polynomial2DFlags)):
                self._HTML += '<b>' + \
                    cd[index] + \
                    '(</b> ' + \
                    self.polynomialEquationList[self.polynomial2DFlags[index]
                                                ].HTML + ' <b>)</b>'
                # not the last one
                if (self.polynomial2DFlags[index] != self.polynomial2DFlags[len(self.polynomial2DFlags)-1]) or (0 in self.polynomial2DFlags):
                    self._HTML += " + "
    ...
    def GetDataCacheFunctions(self):
        ...
        for i in self.polynomial2DFlags:
            functionList.append(
                [pyeq3.DataCache.DataCacheFunctions.Polynomial2D(NameOrValueFlag=1, args=i), i])
        return self.extendedVersionHandler.GetAdditionalDataCacheFunctions(self, functionList)
    ...
    def ShouldDataBeRejected(self, unused):
        for i in self.polynomial2DFlags:
            ...
            self.independentData1CannotContainPositiveFlag |= self.polynomialEquationList[
                i].cannotAcceptDataWith_Positive
            self.independentData1CannotContainNegativeFlag |= self.polynomialEquationList[
    ...
    def SpecificCodeCPP(self):
        s = ""
        EquationListForPolynomial = pyeq3.PolyFunctions.GenerateListForCustomPolynomials_2D()
        coeffCount = 0
        for i in self.polynomial2DFlags:
            s += "\ttemp += " + \
