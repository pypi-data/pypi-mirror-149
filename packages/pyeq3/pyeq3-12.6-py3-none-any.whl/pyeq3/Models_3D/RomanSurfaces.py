...
class RomanSurfaceMinus(pyeq3.Model_3D_BaseClass.Model_3D_BaseClass):
    ...
    def CalculateModelPredictions(self, inCoeffs, inDataCacheDictionary):
        ...
        try:
            temp = (k * YSQMINUSXSQ - XSQMINUSYSQ *
                    numpy.sqrt(k * k - XSQMINUSYSQ)) / (2.0 * XSQPLUSYSQ)
...
class RomanSurfaceMinus_OffsetXY(pyeq3.Model_3D_BaseClass.Model_3D_BaseClass):
    ...
    def CalculateModelPredictions(self, inCoeffs, inDataCacheDictionary):
        ...
        try:
            ...
            temp = (k * (temp_y_sq - temp_x_sq) - (temp_x_sq - temp_y_sq) *
                    numpy.sqrt(k * k - temp_x_sq - temp_y_sq)) / (2.0 * (temp_x_sq + temp_y_sq))
            return self.extendedVersionHandler.GetAdditionalModelPredictions(temp, inCoeffs, inDataCacheDictionary, self)
        except:
            return numpy.ones(len(inDataCacheDictionary['DependentData'])) * 1.0E300
...
class RomanSurfacePlus_scaled(pyeq3.Model_3D_BaseClass.Model_3D_BaseClass):
    ...
    def CalculateModelPredictions(self, inCoeffs, inDataCacheDictionary):
        ...
        try:
            temp = scale * (k * YSQMINUSXSQ + XSQMINUSYSQ *
                            numpy.sqrt(k * k - XSQMINUSYSQ)) / (2.0 * XSQPLUSYSQ)
            return self.extendedVersionHandler.GetAdditionalModelPredictions(temp, inCoeffs, inDataCacheDictionary, self)
        except:
            return numpy.ones(len(inDataCacheDictionary['DependentData'])) * 1.0E300
