...
class LogNormalA(pyeq3.Model_3D_BaseClass.Model_3D_BaseClass):
    ...
    def CalculateModelPredictions(self, inCoeffs, inDataCacheDictionary):
        ...
        try:
            temp = a * \
                numpy.exp(-0.5 * (numpy.square((LogX-b)/c) +
                          numpy.square((LogY-d)/f)))
            return self.extendedVersionHandler.GetAdditionalModelPredictions(temp, inCoeffs, inDataCacheDictionary, self)
        except:
            return numpy.ones(len(inDataCacheDictionary['DependentData'])) * 1.0E300
...
class LogisticA(pyeq3.Model_3D_BaseClass.Model_3D_BaseClass):
    ...
    def CalculateModelPredictions(self, inCoeffs, inDataCacheDictionary):
        ...
        try:
            temp = 4.0 * a * numpy.exp(-((x_in-b)/c)) / \
                numpy.square(1.0 + numpy.exp(-((x_in-b)/c)))
            temp += 4.0 * d * numpy.exp(-((y_in-f)/g)) / \
...
class LogisticB(pyeq3.Model_3D_BaseClass.Model_3D_BaseClass):
    ...
    def CalculateModelPredictions(self, inCoeffs, inDataCacheDictionary):
        ...
        try:
            temp = 16.0 * a * numpy.exp(-((x_in-b)/c)-((y_in-d)/f)) / (numpy.square(
                1.0 + numpy.exp(-((x_in-b)/c))) * numpy.square(1.0 + numpy.exp(-((y_in-d)/f))))
            return self.extendedVersionHandler.GetAdditionalModelPredictions(temp, inCoeffs, inDataCacheDictionary, self)
        except:
            return numpy.ones(len(inDataCacheDictionary['DependentData'])) * 1.0E300
