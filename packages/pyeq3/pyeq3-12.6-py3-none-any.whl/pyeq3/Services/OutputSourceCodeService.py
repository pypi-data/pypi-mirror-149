...
class OutputSourceCodeService(object):
    ...
    def GetOutputSourceCodeCPP(
        self, inEquation, inDigitsOfPrecisionString="16", note=""
    ):
        if inEquation.splineFlag is True:
...
"""
    ...
    def GetOutputSourceCodeCSHARP(
        ...
        s += (
            "class "
            + inEquation.extendedVersionHandler.AssembleSourceCodeName(inEquation)
            + "\n{\n"
        )
        if inEquation.GetDimensionality() == 2:
            s += (
            ...
            s += (
                "\tdouble "
                + inEquation.extendedVersionHandler.AssembleSourceCodeName(inEquation)
                + "_model(double x_in, double y_in)\n\t{\n"
            )
        s += """\t\tdouble temp;
\t\ttemp = 0.0;
...
"""
    ...
    def GetOutputSourceCodeJAVASCRIPT(
        self, inEquation, inDigitsOfPrecisionString="16", note=""
    ):
        if inEquation.splineFlag is True:
            if inEquation.GetDimensionality() == 2:
...
"""
    ...
    def GetOutputSourceCodeFORTRAN90(
        self, inEquation, inDigitsOfPrecisionString="16", note=""
    ):
        if inEquation.splineFlag is True:
            raise NotImplementedError("Not implemented for splines")
...
"""
        ...
        s += "{\n"
        s += (
            "    double " + inEquation.__class__.__name__ + "_evaluation(double x_in)\n"
        )
        s += "    {\n"
        s += "        double t [] = {"
        for i in range(len(inEquation.scipySpline._eval_args[0])):
