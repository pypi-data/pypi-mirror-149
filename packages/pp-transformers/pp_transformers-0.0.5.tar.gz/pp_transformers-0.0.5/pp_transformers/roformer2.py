...
class MultiHeadAttentionWithRotary(Layer):
    def __init__(self,
        ...
        self.q_proj = nn.Linear(embed_dim, embed_dim, bias_attr=not v2)
        self.k_proj = nn.Linear(self.kdim, embed_dim, bias_attr=not v2)
        self.v_proj = nn.Linear(self.vdim, embed_dim, bias_attr=not v2)
        self.out_proj = nn.Linear(embed_dim, embed_dim, bias_attr=not v2)
...
class TransformerEncoderLayerWithRotary(nn.TransformerEncoderLayer):
    def __init__(self,
        ...
        if v2:
            self.norm1 = Norm()
            self.norm2 = Norm()
            self.linear1 = nn.Linear(d_model, dim_feedforward, bias_attr=not v2)
            self.linear2 = nn.Linear(dim_feedforward, d_model, bias_attr=not v2)
...
class RoFormerEmbeddings(Layer):
    ...
    def __init__(
        ...
        self.token_type_embeddings = nn.Embedding(type_vocab_size,
                                                  embedding_size)
        self.layer_norm = Norm() if v2 else nn.LayerNorm(embedding_size)
...
class RoFormerPooler(Layer):
    ...
    def __init__(self, hidden_size, pool_act="tanh"):
        ...
        self.dense = nn.Linear(hidden_size, hidden_size, bias_attr=not v2)
    ...
    def forward(
        ...
        for mod in self.encoder.layers:
            if self.recompute and self.training:
                output = recompute(mod, output, attention_mask, None)
            else:
