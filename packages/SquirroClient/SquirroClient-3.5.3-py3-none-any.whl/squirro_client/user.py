...
class UserApiMixin:
    ...
    def change_user_password(
        self, user_id, current_password, new_password, new_password_confirm,
    ):
        """
    ...
    def get_users_password_policy(self):
        ...
        url = "{endpoint}/{version}/{tenant}/users/password_policy".format(
            endpoint=self.user_api_url, version=self.version, tenant=self.tenant,
        )
        res = self._perform_request("get", url)
        return self._process_response(res)
    ...
    def get_users(self, include_config=None):
        ...
        url = "{ep}/{version}/{tenant}/users".format(
            ep=self.user_api_url, version=self.version, tenant=self.tenant,
        )
    ...
    def get_token_from_msexchuid(self, msexchuid):
        ...
        res = self._perform_request("get", url, params)
        return self._process_response(res, [200])
