...
def calculate_asset_statistics(
    assetSearchRequest: dict = None,
    assetFieldAggregation: dict = None,
    assetRangeMetric: dict = None,
    assetStatisticsMetric: dict = None,
    json: bool = True,
    verify: bool = None,
    proxies: dict = None,
    apiKey: str = None,
    ...
    """Generate the requested statistics for Assets matching the provided search criteria.If groupBys are not explicitly requested Customer will be used by default. (PUBLIC)
    
    :param dict assetSearchRequest: 
    :param list assetFieldAggregation: The Asset fields by which to group statistics
    :param list assetRangeMetric: The Asset timestamp fields for which to generate statistics.
    :param list assetStatisticsMetric: The Asset metric fields for which to generate metrics.
    :param json: return the response's body as a ``dict`` parsed from json. ``True`` by
      default. If set to false, the raw ``requests.Response`` object will be returned.
    :param verify: path to a certificate bundle or boolean indicating whether SSL
      verification should be performed.
    ...
    body = body or {}
    # Only send assetSearchRequest if the argument was provided, dont send null values
    if assetSearchRequest is not None:
        body.update({"assetSearchRequest": assetSearchRequest})
    # Only send assetFieldAggregation if the argument was provided, dont send null values
    if assetFieldAggregation is not None:
        body.update({"assetFieldAggregation": assetFieldAggregation})
    # Only send assetRangeMetric if the argument was provided, dont send null values
    if assetRangeMetric is not None:
        body.update({"assetRangeMetric": assetRangeMetric})
    # Only send assetStatisticsMetric if the argument was provided, dont send null values
    if assetStatisticsMetric is not None:
        body.update({"assetStatisticsMetric": assetStatisticsMetric})
...
def search_assets(
    ...
    :param int startTimestamp: Restrict to a time frame based on the set timeFieldStrategy \(start timestamp\). If searching for Assets by Vulnerabilities this must be set. \(default 0\)
    :param int endTimestamp: Restrict to a time frame based on the set timeFieldStrategy \(end timestamp\). If searching for Assets by Vulnerabilities this must be set. \(default 0\)
