...
def lgb_init(x_train, x_val, y_train, y_val, num_leaves, weight, epoch,
    ...
    if is_evaluate:
        ...
        if version >= '3.8':
            callbacks = [lgb.early_stopping(early_stopping), lgb.log_evaluation(1, show_stdv=False)]
...
class ModelBuild:
    ...
    def save_model(self, path):
        if self.rfc_model is not None:
            joblib.dump(self.rfc_model, path)
        if self.lgb_model is not None:
            joblib.dump(self.lgb_model, path)
