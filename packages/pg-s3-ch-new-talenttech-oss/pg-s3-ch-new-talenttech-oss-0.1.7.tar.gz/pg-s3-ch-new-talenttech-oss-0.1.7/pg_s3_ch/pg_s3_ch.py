...
class PGS3CH:
    ...
    def __init__(self, config, s3_config, ch_config, pg_config):
        ...
        self.key_field = (
            self.config["key_field"] if "key_field" in self.config else "id"
        )
        self.pg_history_step = (
            self.config["pg_history_step"]
            if "pg_history_step" in self.config
            else PG_HISTORY_STEP_DEFAULT
        )
    ...
    def get_table_schema_ch(self):
        ...
        if len(self.schema_ch_insert_fields) < 1:
            logger.info(
                "CH schema error: %s. Have you already created the table?", self.config["name"])

            sys.exit(1)

        logger.info("CH schema for %s.%s - OK", self.ch_config["database"], self.config["name"])
    ...
    def extract_to_s3(self):
        ...
        for _id in range(0, end):
            ...
            data = fa.getvalue()
            file_name = "{entity_name}_{range_from}.tsv".format(
    ...
    def get_partion_sorting_key(self):
        sql = """select distinct sorting_key, partition_key
            ...
            database=self.ch_config["database"], name=self.config["name"]
        )
    ...
    def create_temporary_table(self):
        ...
        sql = """CREATE TABLE {staging_database}.{temp_table_prefix}{table} ON CLUSTER '{{cluster}}' AS {database}.{table} \
         ENGINE=ReplicatedMergeTree() PARTITION BY {temp_table_partition_by} ORDER BY ({temp_table_order_by})""".format(
            staging_database=self.ch_config["staging_database"],
            temp_table_prefix=self.temp_table_prefix,
            database=self.ch_config["database"],
            table=self.entity_name,
            temp_table_order_by=temp_table_order_by if temp_table_order_by != '' else 'tuple()',
    ...
    def optimize(self):
        ...
        sql = """SELECT partition, count() cnt from system.parts where database='{database}' and table='{table}' and active
                GROUP BY partition 
                having cnt > 1""".format(
            database=self.ch_config["database"], table=self.config["name"]
        )
        result = self.ch_client.execute(sql)

        for row in result:
            logger.info(
                "Partition %s has %s parts", row[0], row[1]
            )
