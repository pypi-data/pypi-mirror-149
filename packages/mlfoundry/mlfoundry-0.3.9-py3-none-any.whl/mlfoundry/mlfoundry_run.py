...
class MlFoundryRun:
    ...
    def _does_model_exist(self) -> bool:
        # this is bad
        # use run logs
        # also we should refactor this whole model module out
        artifacts = self.mlflow_client.list_artifacts(self.run_id)
        for artifact in artifacts:
            if MlFoundryRun.S3_MODELS_PATH in artifact.path:
                return True
    ...
    def get_model(self, dest_path: str = None, **kwargs):
        ...
        model_exists = self._does_model_exist()
        if not model_exists:
        ...
        if framework == enums.ModelFramework.TENSORFLOW:
            ...
            # we do not have any tests for tensorflow model
            self.model_uri = MlFoundryRun.S3_MODELS_PATH
    ...
    def _download_model(self, path: str):
        ...
        with tempfile.TemporaryDirectory() as local_dir:
            self.download_artifact(MlFoundryRun.S3_MODELS_PATH, local_dir)
            # atleast we should use run logs for this.
            # relying on just a single pattern (model will be always at
            # artifact_root / MlFoundryRun.S3_MODELS_PATH)
            # is not good.
            model_dir = os.path.join(local_dir, MlFoundryRun.S3_MODELS_PATH)
            copy_tree(model_dir, path)
