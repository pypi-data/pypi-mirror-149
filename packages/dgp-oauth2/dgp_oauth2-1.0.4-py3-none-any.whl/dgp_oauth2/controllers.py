...
class Controllers:
    ...
    def authenticate(self, token, next, callback_url, private_key):
        ...
        """
        logging.info('AUTH CONTROLLERS: authenticate %s %s %s' % (token is not None, next, callback_url))
        if token is not None:
            try:
                token = jwt.decode(token, private_key)
            except jwt.InvalidTokenError:
                logging.info('AUTH CONTROLLERS: invalid token %r' % token)
                token = None

            if token is not None:
                userid = token['userid']
                user = self.models.get_user(userid)
                logging.info('AUTH CONTROLLERS: userid %s found %s' % (userid, user is not None))
                if user is not None:
                    ret = {
                        'authenticated': True,
                        'profile': user
    ...
    def oauth_callback(self, state, callback_url, private_key,
        ...
        """Callback from OAuth
        """
        logging.info('AUTH CONTROLLERS: oauth_callback %s' % state)
        try:
            state = jwt.decode(state, private_key)
        except jwt.InvalidTokenError:
            state = {}
        logging.info('AUTH CONTROLLERS: oauth_callback decoded %r' % state)
        ...
        logging.info('AUTH CONTROLLERS: oauth_callback next_url %s %s -> %s' % (state, access_token, next_url))
        return next_url
