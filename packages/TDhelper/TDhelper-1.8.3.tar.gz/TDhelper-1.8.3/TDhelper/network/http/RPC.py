...
class RPC:
    ...
    def RPC_ACCESS(self, entryPoint, access_token, method,protcol='http://', **kwargs):
        ...
        for uri in entryPoint:
            status, body= GET(uri=protcol+uri+"/"+method+"/"+params, time_out=15, http_headers= headers)
            if status == 200:
                if body['state']==200:
                    result['state']=True
                    result['state']= body['msg']
                break
