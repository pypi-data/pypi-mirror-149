...
class RPCRegister:
    ...
    def _registerService(self):
        ...
        for k, v in self._serviceConfig.items():
            ...
            if m_count < len(self._serviceConfig):
                m_service_post_data += "&"
        # 注册服务基本信息.
        state, ret = POST(self._sc_uri+"services/".strip('/')+"/", post_data=bytes(
    ...
    def _registerHost(self, pk, hosts):
        ...
        for i in range(0, len(hosts)):
            ...
            state, ret = POST(self._sc_uri+"hosts/".strip('/')+"/", post_data=bytes(
                                    ...
                                    if k.lower() != 'params':
                                        ...
                                        if 'params' in methods[i]:
                                            ...
                                            if m_count < len(methods[i]):
                                                m_service_uri_post_data += "&"
                                state, ret = POST(self._sc_uri+"uri/".strip('/')+"/", post_data=bytes(
                                                    ...
                                                    for param_offset in range(0, len(params)):
                                                        ...
                                                        for k, v in params[param_offset].items():
                                                            ...
                                                            try:
                                                                ...
                                                                raise e
                                                        state, ret = POST(
                                                            self._sc_uri+"params/".strip('/')+"/", post_data=bytes(m_service_method_params_post_data, encoding='utf-8'), http_headers=self._m_heads, time_out=15)
                                            # 注册返回值
                                                        ...
                                                        if k.lower() != 'descriptions':
                                                            ...
                                                            if 'descriptions' in m_returns:
                                                                ...
                                                                if m_count < len(m_returns):
                                                                    reg_returns_params += "&"
                                                    state, ret = POST(self._sc_uri+"returns/".strip('/')+"/", post_data=bytes(
                                                            ...
                                                            if 'descriptions' in m_returns:
                                                                ...
                                                                for item in m_returns['descriptions']:
                                                                    ...
                                                                    m_returns_description += '&valueDescription=' + \
                                                                        parse.quote(
                                                                            m_description)
                                                                    state, ret = POST(self._sc_uri + "returnDescriptons/".strip('/')+"/", post_data=bytes(
...
class RPC:
    ...
    def RPC_ACCESS(self, entryPoint, access_token, method, **kwargs):
    ...
    def register(self, service: dict, hosts: list, methods: list):
        '''
            注册服务
            - params:
    ...
    def call(self, method, **kwargs):
        ...
        state, ret = self._getApi(self._current_service, self._current_service+"."+method)
        if state:
            try:
                m_uri = ret['uri']
                m_params = ret['params']
