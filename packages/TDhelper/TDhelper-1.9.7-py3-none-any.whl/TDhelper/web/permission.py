...
class register:
    ...
    def _register_permission(self, config, **kwargs):
        ...
        post_data = {
            ...
            else kwargs["permission_parent"] if 'permission_parent' in kwargs else 0,
        }
        state, body = POST(
        ...
        if state == 200:
            ...
            if m_ret_json["state"] == 200:
                ...
                logging.info(
                    "create permission '%s' success."
                    % config["permission_name"]
                )
            else:
                logging.info(
            ...
            logging.info(
                "create permission '%s' failed." % config["permission_name"]
            )
        if "children" not in config:
            ...
            for item in config["children"]:
                ...
                recursionCall(self._register_permission,kwargs['limit'],item, **kwargs)
            kwargs["break"] = False
