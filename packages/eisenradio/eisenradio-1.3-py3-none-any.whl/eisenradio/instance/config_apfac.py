...
def write_config(conf, empty_db=None):
    """
    this is not a prod like config, just playing, testing diff. OS support
    better write all values in .env or an encrypted config file
    """
    ...
    # SECRET_KEY not really useful without a user session cookie
    secret_num = '9fc2e5bd8372430fb6a1012af0b51f37'
    # path must match in list and conf
    # PRODUCTION
    db_path_prod = path.join(radio_db_dir, 'database.db')
    # DEV
    db_path_dev = path.join(radio_db_dir, 'database_dev.db')
    # TEST
